// ink-frame.js — die KFB-Kartenkante fuer DOM-Elemente.
// Zwei Dinge pro Blatt: eine clip-path-Silhouette (das Papier hoert an der
// gezeichneten Kante auf) und ein PNG-Band (die Tusche selbst, ueber die Kante gelegt).
// Kanon: skills/SSOT_Card_Ink_Outline_v2.md, Preset 'card' — Band-Familie, bow 0,196.
// Der Aufrufer dreht KEINE Parameter, er nennt einen Seed.
// --- kfb-ink-canon.js eingelegt (Export-Fassung, kein relativer Import) ---
// ============================================================================
...
const INK_CANON_VERSION = 2;
const INK_COLOR = '#1f1a14';
const CHIP_COLOR = '#191410';
...
function brushLoop(x0, y0, x1, y1, seed, p) {
...
function inkHalfWidth(pts, W, H, seed, p, gain) {
...
function inkRibbon2D(g, pts, halfFn, color) {
  const N = pts.length;
  const off = (j, s) => {
...
function chipLoop(x0, y0, x1, y1, seed, p) {
...
function normContour(presetName, seed, aspect) {
...
function contour(presetName, seed, W, H) {
...
function drawInk(presetName, g, pts, W, H, seed, gain) {
...
function maskGrow(presetName, pts, W, H, seed) {
  const p = P(presetName);
...
function measureInk(pts, W, H, presetName, seed) {
  const p = P(presetName);
...
// --- Ende kfb-ink-canon.js ---

const frames = new Map();
let measured = false;

function build(seed, ar, W, gain) {
  const H = Math.round(W / ar);
  const pts = contour('card', seed, W, H);

  const c = document.createElement('canvas');
  c.width = W; c.height = H;
  drawInk('card', c.getContext('2d'), pts, W, H, seed, gain);

  if (!measured) {
    measured = true;
    const m = measureInk(pts, W, H, 'card', seed);
    console.info('[kfb-ink] Bauchung ' + m.bowPct + ' % · Feder ' + m.featherPct +
      ' % · ' + m.kind + ' · ' + (m.ok ? 'ok' : m.why));
  }

  const npts = normContour('card', seed, ar);
  const clip = 'polygon(' + npts
    .map((p) => (p[0] * 100).toFixed(3) + '% ' + (p[1] * 100).toFixed(3) + '%')
    .join(',') + ')';

  return { url: c.toDataURL('image/png'), clip, safe: safeOf(npts) };
}

// Wie weit beisst die Kante je Seite nach innen (0..1 der Kantenlaenge)?
// Der Inset des Baurahmens ist in X und Y derselbe PIXELWERT, also in Prozent
// senkrecht fast doppelt so gross — gemessen 1,37 % quer gegen 2,45 % hoch.
// Wer ein Seitenbild auf 100 % legt, verliert damit Kopfzeile und Signatur.
function safeOf(npts) {
  let l = 0, t = 0, r = 0, b = 0;
  for (const p of npts) {
    const dl = p[0], dr = 1 - p[0], dt = p[1], db = 1 - p[1];
    if (Math.min(dl, dr) <= Math.min(dt, db)) {
      if (dl < dr) l = Math.max(l, dl); else r = Math.max(r, dr);
    } else {
      if (dt < db) t = Math.max(t, dt); else b = Math.max(b, db);
    }
  }
  return { l, t, r, b };
}

// seed: eine Zahl je Blatt (Seitennummer reicht) — gleiche Zahl, gleiche Kante.
// ar:   Seitenverhaeltnis des Blatts. w: Zeichenbreite in Pixel (Groessen-Eimer).
// gain: Gegenfaktor fuer die Feder. Die gezeichnete Linie ist immer derselbe ANTEIL der
//       Blattbreite — ein aufgezogenes Blatt bekaeme damit einen entsprechend fetteren
//       Strich (bei 1,8x gemessen 1,8x). Wer gleiches Strichgewicht auf dem Schirm will,
//       gibt hier 1/Vergroesserung.
function frame(seed, ar, w, gain) {
  const g = gain == null ? 1 : gain;
  const key = seed + '|' + ar.toFixed(4) + '|' + w + '|' + g.toFixed(3);
  if (!frames.has(key)) frames.set(key, build(seed, ar, w, g));
  return frames.get(key);
}

window.KFBInk = { frame };
window.dispatchEvent(new CustomEvent('kfb-ink-ready'));
