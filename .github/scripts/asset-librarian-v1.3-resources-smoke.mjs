...
async function clickTab(cdp,name){
  await cdp.eval(`document.querySelector('[data-library-tab="${name}"]').click();true`);
  await ev(cdp,`!document.getElementById('resourceWorkspace').hidden`,'workspace');
  await ev(cdp,`document.getElementById('resourceEyebrow')?.textContent===${JSON.stringify(name)}`,`${name} active`);
  await cdp.eval(`(()=>{const q=document.getElementById('resourceSearch');q.value='';q.dispatchEvent(new Event('input',{bubbles:true}));const a=document.getElementById('resourceActorFilter');a.value='';a.dispatchEvent(new Event('change',{bubbles:true}));const s=document.getElementById('resourceStatusFilter');s.value='';s.dispatchEvent(new Event('change',{bubbles:true}));return true;})()`);
  await ev(cdp,`document.querySelectorAll('#resourceList .resource-card').length>0`,`${name} rows`);
}
