...
const PDFJS = PDFJS_BASE + 'build/pdf.min.mjs';
...
export function createCardBuilder(opts = {}) {
  ...
  async function ensurePdfjs() {
    ...
    /* WORKER ALS BLOB. Ein Modul-Worker von fremdem Ursprung wird im Vorschau-iframe blockiert, und
       `getDocument()` hängt dann OHNE Fehler — gemessen 06.09. (Combat Arena v2, Feld-Modus): pdf.js
       geladen (4.7.76), PDF per Range-Request erreichbar (HTTP 206), Seite kam nach 20 s nicht, keine
       Konsolenzeile. Ein Blob-Worker ist same-origin und darf. Fällt der Abruf aus, bleibt die
       CDN-URL stehen und pdf.js nimmt seinen eigenen Fake-Worker-Rückfall. */
    try {
  ...
  /* PDF ALS PUFFER, NICHT ALS URL. `getDocument({ url })` hängt in der Vorschau ohne Fehler und ohne
     Ende — gemessen 06.09.: pdf.js 4.7.76 geladen, Worker startbar, PDF per Range erreichbar (HTTP 206,
     `accept-ranges` fehlt aber im Kopf), `getDocument({url})` nach 25 s ohne Ergebnis. Derselbe Aufruf
     mit `{ data: ArrayBuffer }` liefert in unter zwei Sekunden 14 Seiten (745 × 416 pt). Also holen wir
     die Datei selbst; `slice(0)` gibt pdf.js eine KOPIE, weil es den übergebenen Puffer entkoppelt. */
  const pdfBufs = new Map();
  /* Wo eine Seite hängt, muss ablesbar sein. Ein stiller Ausfall sieht aus wie Absicht. */
  const pageTrace = [];
  const spur = (s) => { pageTrace.push(Math.round(performance.now()) + ' ' + s); if (pageTrace.length > 40) pageTrace.shift(); };
  function pdfData(url) {
    if (!pdfBufs.has(url)) pdfBufs.set(url, fetch(url).then((r) => { if (!r.ok) throw new Error('PDF ' + r.status); spur('fetch ok'); return r.arrayBuffer(); }));
    return pdfBufs.get(url);
  }
  /* Laufende Aufträge werden GETEILT, aber nichts wird aufgereiht. Zwei Befunde vom 06.09.:
       (1) manche Seiten rendern nie — Seite 14 des einen Decks in 61 ms, Seite 10 des nächsten
           bleibt für immer bei »vp 3000×1673« stehen, gleiche Auflösung, gleiche Flaggen. Es ist
           die SEITE, nicht die Größe.
       (2) eine Warteschlange macht das schlimmer: der erste Hänger nahm alle Folgeaufträge mit.
     Also: Frist je Seite, Eintrag danach verworfen (der nächste Aufruf darf neu fragen), und der
     Aufrufer nimmt eine andere Karte. Ein Hänger kostet dann acht Sekunden, nicht das Spielfeld. */
  const RENDER_FRIST = 8000;
  function renderPage(url, pageNum, targetW) {
    const key = url + '#' + pageNum + '@' + targetW;   // Auflösung GEHÖRT in den Schlüssel: sonst bekommt der Zweite die Pixel des Ersten
    if (pages.has(key)) return pages.get(key);
    const p = Promise.race([
      rendereSeite(url, pageNum, targetW, key),
      new Promise((_, no) => setTimeout(() => no(new Error('Seite ' + pageNum + ' rendert nicht (' + (RENDER_FRIST / 1000) + ' s)')), RENDER_FRIST))
    ]).catch((e) => { pages.delete(key); spur('AUSFALL ' + pageNum + ': ' + e.message); throw e; });
    pages.set(key, p);
    return p;
  }
  async function rendereSeite(url, pageNum, targetW, key) {
    spur('renderPage ' + pageNum + ' @' + targetW);
    const lib = await ensurePdfjs();
    spur('pdfjs bereit');
    let doc = docs.get(url);
    if (!doc) { const buf = await pdfData(url); spur('puffer ' + buf.byteLength + ' B'); doc = await lib.getDocument({ data: buf.slice(0), cMapUrl: PDFJS_BASE + 'cmaps/', cMapPacked: true, standardFontDataUrl: PDFJS_BASE + 'standard_fonts/', disableFontFace: true, isEvalSupported: false }).promise; spur('doc ' + doc.numPages + ' Seiten'); docs.set(url, doc); }
    /* `disableFontFace: true` ist kein Geschmack: pdf.js bindet eingebettete Schriften sonst über
       `document.fonts` — und dieser Ladeweg löst im verdeckten Vorschaufenster nie auf. Gemessen
       06.09.: Spur bis »vp 3000×1673«, dann Stille (Puffer, Worker, cMaps, Schriftdaten in Ordnung).
       Mit Pfad-Rendering statt CSS-Schriften bleibt das Bild identisch, nur die Naht fällt weg. */
    const page = await doc.getPage(pageNum);
    spur('seite ' + pageNum + ' geholt');
    const scale = targetW / page.getViewport({ scale: 1 }).width;
    const vp = page.getViewport({ scale });
    spur('vp ' + Math.round(vp.width) + '×' + Math.round(vp.height));
    const cv = document.createElement('canvas'); cv.width = vp.width; cv.height = vp.height;
    /* `onContinue` STATT requestAnimationFrame. pdf.js rendert in Häppchen und plant das nächste per
       rAF ein — im verdeckten Vorschaufenster feuert rAF nicht (ENVIRONMENT CA-4), und dann hängt
       `render().promise` OHNE Fehler und ohne Ende. Gemessen 06.09.: Spur bis »doc 19 Seiten«, dann
       Stille; Puffer, Worker, cMaps, Schriftdaten alle in Ordnung. Mit diesem Haken treibt der
       Aufrufer die Häppchen selbst weiter — kein Warten auf ein Bild, das nicht kommt. */
    const task = page.render({ canvasContext: cv.getContext('2d'), viewport: vp });
    task.onContinue = (weiter) => weiter();
    await task.promise;
    spur('gerendert ' + cv.width + '×' + cv.height);
    pages.set(key, cv);
    return cv;
  }
  ...
  async function cropCard(card) {
    /* AUFLÖSUNG IM SCHLÜSSEL, nicht bloss zeitliches Leeren. `setParams` leert den Cache beim Wechsel,
       aber ein noch fliegender Auftrag der alten Grösse füllt ihn danach WIEDER — und der neue Bau
       konsumiert den zu kleinen Schnitt. Gemessen 06.09. (Kritiker): Startzustand 18 u zeigte 85 px/u
       (Schnitt 1530 px aus einer 3060-px-Seite), obwohl `pdfRes` 4096 war. Mit der Auflösung im
       Schlüssel kann ein Schnitt von 3060 nie als 4096 durchgehen. */
    const ck = card.packId + '#' + card.n + '@' + P.pdfRes;
  ...
  return {
    ...
    /** Wo die letzte Seite stand, mit Zeitmarken — für genau die Hänger, die keine Fehlerzeile werfen. */
    get pageTrace() { return pageTrace.slice(); },
    ...
    // ---- Bauen
    make,
    /* GANZE SEITE als Bild — der Bogen selbst, 2×2 Karten, ohne Schnitt.
       Gebraucht von Combat Arena v2 M1: die Seite wird zum Spielfeld, die vier Viertel zu Zonen.
       Additiv: derselbe Seiten-Cache wie der Schnittweg, kein zweiter Renderpfad.
       `targetW` in Pixeln (Standard `P.pdfRes` 1500; sinnvolles Dach 4096 = WebGL-Grenze). */
    async pageOf(card, targetW) {
      await loadRegistry();
      const rd = pmap[card.packId];
      if (!rd || !rd.pdf) return null;
      const off = rd.coverOffset != null ? rd.coverOffset : 1;
      const cell = card.cell != null ? card.cell : card.n - 1;
      const pageNum = off + 1 + Math.floor(cell / 4);
      const cv = await renderPage(fileUrl(rd.pdf), pageNum, Math.min(4096, targetW || P.pdfRes));
      return cv ? { canvas: cv, seite: pageNum, deck: rd.packId || card.packId, w: cv.width, h: cv.height, ar: cv.width / cv.height } : null;
    },
    ...
    setParams(p) {
      ...
      /* `pdfRes` GEHÖRT in diese Liste: die geschnittenen Zellen im `artCache` tragen die Auflösung,
         mit der sie geschnitten wurden. Ohne Leeren blieb eine Karte auf größerem Feld bei 85 px/u
         stehen, obwohl 4096 px angefordert waren (gemessen 06.09., Fläche/Motiv-Umschalter). */
      if (p && (p.quadrant !== undefined || p.cardGrid !== undefined || p.gridOverrides !== undefined || (p.pdfRes !== undefined && p.pdfRes !== P.pdfRes))) artCache.clear();
      Object.assign(P, p || {});
