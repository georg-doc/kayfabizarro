...
/* **Die Synthese** (v10-S23, Georgs Korrektur 10.8.). Therefore/But läuft **nicht im Satz**, sondern
   **zwischen Sprechern** — South-Park-Closure über drei Panels:

       Goblin A: These      »Kayfabe-Kant!«
       Goblin B: Antithese  »Blödsinn!«
       Goblin C: Synthese   »Then let us get a snack.«

   Der Dritte macht aus zwei Sätzen ein Gespräch, und die Kausalität entsteht **im Kopf des
   Zuhörers**, nicht in einem `therefore`. Improv-Ping-Pong statt Dialogbaum.

   Die stärkste Synthese ist eine, die in eine **Handlung** kippt: nicht »interessant«, sondern
   »dann lass uns«. Deshalb tragen die meisten hier ein Vorhaben statt eines Urteils. */
const SYNTHESE={
  '*':       ['Then let us not.','So we agree. Badly.','Fine. Same as yesterday.',
              'Then somebody else does it.'],
  kingCourt: ['Then it goes in the file.','So we form a committee of two.','Noted. Both of you.'],
  townsfolk: ['Then it can wait.','So nobody carries it. Good.','Fine. I am going home.'],
  camp:      ['Then we take the other road.','So we both keep watch. Neither sleeps.',
              'Then it was never here.'],
  wilds:     ['Then we leave it standing.','So the wood decides.'],
  cave:      ['Then say it once more.','So it comes back wrong twice.'],
  frost:     ['Then it keeps.','So we count it in spring.'],
  shore:     ['Then it sails without us.','So we sell it twice.'],
  dungeon:   ['Then we wait. We are good at that.','So it ends the same. Slower.'],
};
...
window.OW_PHRASES={
  ...
  SYNTHESE,
  /* Der dritte Zug. Ohne eigenen Vorrat greift der allgemeine — eine Synthese fehlt nie. */
  synthese(key,rnd){
    const l=SYNTHESE[key]||SYNTHESE['*'];
