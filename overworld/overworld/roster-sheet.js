...
/* Der Runner hat im selben Shadow-DOM eine generische ¦canvas{position:absolute;inset:0;
   ...
   liegt 850 × 454 groß über dem Blatt (gemessen, alle 28). Deshalb hier explizit zurück in den
...
function mount(game){
  ...
  el.innerHTML='<h4>PLAY AS <b>every unit with an idle is a hero</b>'
    ...
    +'<div class="foot"><span>C opens · click to play</span><span class="st"></span></div>';
  sh.appendChild(el);
  game.rosterEl=el;
  ...
  scroll.innerHTML=groups.map(g=>
    ...
    +g.items.map(r=>{
      ...
      return `<div class="u" data-id="${r.id}" title="${r.label}">`
        ...
        +(r.canAttack?'':'<div class="no">no attack</div>')
        +'</div>';
    }).join('')+'</div>').join('');
  el.querySelector('.foot .st').textContent=list.length+' units';
  paint(game,el,list);
}

/* Vorschau: derselbe Loader, dieselbe Bezugsgröße wie im Spiel. Drei gleichzeitig — dreißig
   Blätter auf einmal blockieren den Frame und die Kacheln erscheinen trotzdem nacheinander. */
async function paint(game,el,list){
  const CAT=window.OW_UNITS,OWL=window.OW_LOADER;
  if(!OWL)return;
  const cvs=el.querySelectorAll('.u canvas');
  let i=0;
  const next=async()=>{
    while(i<list.length){
      const k=i++,r=list[k],cv=cvs[k];
      if(!cv)continue;
      try{
...
window.OW_ROSTER={version:'roster-v1.0',mount,
  note:'Liest OW_UNITS.roster() · Vorschau über OW_LOADER mit derselben refBody wie das Spiel'};
})();
