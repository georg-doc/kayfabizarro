...
/* Die Farbe der drei Stats gehört dem HUD, nicht dem Runner: sie kommt als CSS-Variable aus dem
   ...
   **10.8. · `short` ist jetzt ein eigenes Feld (Georgs Drei-Ebenen-Vertrag, Masterplan K7).**
   Das Kürzel war vorher `name.slice(0,3)` im HUD — abgeleitet, nicht gesagt. Mit den kanonischen
   Etiketten hätte diese Ableitung **Kay · Kay · Kay** geliefert: KayfaBingo, KayfaBongo und
   KayfaBoggle fangen alle mit denselben drei Buchstaben an, und Kayfabe auch. Vier gleiche Kürzel
   in einem Panel mit sechs Zahlen. Also steht das Kürzel da, wo es hingehört: neben dem Namen.
   `Blö` kommt aus demselben Feld — es war nie eine Kennung, sondern der Anfang eines Etiketts. */
const STAT_INFO={
  bizarro:  {name:'Bizarro',    short:'Biz', color:'var(--stat-bizarro,#e8b45c)', line:'The blow lands. Enemies get quiet.'},
  kayfabe:  {name:'Kayfabe',    short:'Kay', color:'var(--stat-kayfabe,#7fb0ff)', line:'One more Kayfabe charge. One more argument.'},
  bingo:    {name:'KayfaBingo', short:'Bin', color:'var(--stat-bingo,#7fd6a2)',   line:'You spot the pattern before it closes.'},
  bongo:    {name:'KayfaBongo', short:'Bon', color:'var(--stat-bongo,#d99a5c)',   line:'Rhythm. You keep going when others stop.'},
  boggle:   {name:'KayfaBoggle',short:'Bog', color:'var(--stat-boggle,#c58ce0)',  line:'The world stops making sense in your favour.'},
  bloedsinn:{name:'BLÖDSINN!',  short:'Blö', color:'var(--stat-bloedsinn,#e8756a)',line:'Nonsense as a method. It should not work.'},
  // Kein Wert, sondern die Folge aller sechs — steht hier nur für Anzeigen, die einen Namen brauchen
  fluff:    {name:'Fluff',      short:'Flu', color:'var(--stat-fluff,#e8756a)',   line:'Your standing. It follows from the six.'},
};
...
class OverworldGame extends HTMLElement{
  ...
  static get observedAttributes(){return['seed','hero-color','herocolor','zoom','zones','sound','rss','back-set','backset','layout','hero','loot','town-color','towncolor','world-size','worldsize','ground','life','terrain','coast-ink','coastink','relief','relief-tex','relieftex','hud-skin','shadow','puddles','prop-sheet','propsheet','prop-grid','propgrid','hudskin','waber','waber-props','waberprops','shade','fluid','render-scale','renderscale','bubble-font','bubblefont','chatter'];}
  constructor(){super();
...
/* v10-S6d · **P gehört ins Bild.** Die Tastenzeile des Runners (`.hint`) ist beim v7-Skin
  ...
  stepReader(dt){
    ...
    if(onSheet!==!!R.onSheet){
      R.onSheet=onSheet;
      if(onSheet){R.hudWas=!!this.minimal;this.minimal=true;}
      else this.minimal=!!R.hudWas;
      this.syncHudMode();
    }
  ...
  drawZoneCards(ctx,x0,y0,x1,y1){
    ...
    for(const z of this.zones){
      ...
      /* v10-S24 · **Der Nachlader ist hier raus** (WS0-Befund 10.8., dieselbe Klasse zum zweiten
         Mal). Er stand im Zeichenpfad: drosselt der Browser die Bilder (Hintergrundtab, Vorschau
         ohne Fokus, Screenshot-Lauf), heilt eine geräumte Zone ihre fehlende Kartenkunst nie und
         zeigt dauerhaft die Rückseite. Jetzt in `step()`.
         *Was einen Zustand zeigt oder heilt, gehört nicht ans Bild — Animation ja, Zustand nein.* */
  ...
  step(dt){
    ...
    if(this.overview){this.cam.x=this.W*TILE/2;this.cam.y=this.H*TILE/2;}
      ...
      let tx=h.x,ty=h.y;
      if(Rd&&Rd.onSheet){tx=(Rd.x+Rd.w/2)*TILE;ty=(Rd.y+Rd.h/2)*TILE;}
    ...
    /* v10-S24: der Heilpfad für fehlende Kartenkunst — im Takt der Welt, nicht im Takt der Bilder.
       Höchstens eine je Sekunde, damit ein Rundgang über sechs Zonen nicht sechs Ladungen anstößt. */
    this._artT=(this._artT||0)+dt;
    if(this._artT>1){
      this._artT=0;
      for(const z of this.zones)if(z.cleared&&!z.art&&!z._artLaeuft){this.loadZoneCard(z);break;}
    }
  ...
  /* v10-S5 · **BLÖDSINN!-Regie** (Georg 9.8.). Der Tod war ein Teleport: ein Satz im Log, und der
     Held stand am Friedhof. Ein Teleport ist ein Menü ohne Fenster — man erlebt nichts.
     Jetzt drei Takte, alle in der Welt, keiner in einem Overlay:
       1. **Blinken** (0,6 s) — der Held flackert auf der Stelle und ruft BLÖDSINN!.
          Der Ausruf ist der Kanon-Name, nicht »you died«.
       2. **Zackiger Flug** (1,15 s) — er fliegt in fünf Zacken quer zur Strecke zum Friedhof.
          Comic-Grammatik: die Strecke ist eine Zickzacklinie, keine Kurve. Amplitude fällt zum
          Ziel hin ab, sonst überschießt der letzte Zack die Landung.
       3. **Landung** (0,4 s) — Stauchen, Staub, dann steht er.
  ...
  heroDown(){
    ...
    this._blod={t:0,phase:'blink',hide:false,base:h.sizeMul||1,
      from:{x:h.x,y:h.y},to:{x:this.spawn.x,y:this.spawn.y},trail:[],dust:[]};
  ...
  stepBlodsinn(dt){
    ...
    const BLINK=0.6,FLY=1.15,LAND=0.4;
    if(b.phase==='blink'){
      b.hide=Math.floor(b.t*12)%2===1;
      h.sizeMul=b.base*(1+0.06*Math.sin(b.t*26));
      h.state='idle';
      if(b.t>=BLINK){b.phase='fly';b.t=0;b.hide=false;}
      return true;
    }
    if(b.phase==='fly'){
      const p=Math.min(1,b.t/FLY);
      const e=p<0.5?2*p*p:1-Math.pow(-2*p+2,2)/2;      // schnell los, weich rein
      const dx=b.to.x-b.from.x,dy=b.to.y-b.from.y,L=Math.max(1,Math.hypot(dx,dy));
      const nx=-dy/L,ny=dx/L;                           // quer zur Strecke
      // Fünf Zacken, Amplitude fällt linear ab: der letzte Zack darf die Landung nicht überschießen
      const zack=Math.sin(e*Math.PI*5)*Math.sign(Math.cos(e*Math.PI*5)||1);
      const amp=Math.min(150,L*0.22)*(1-e);
      h.x=b.from.x+dx*e+nx*zack*amp;
      h.y=b.from.y+dy*e+ny*zack*amp-Math.sin(e*Math.PI)*46;   // Bogen: er fliegt, er rutscht nicht
      h.sizeMul=b.base*(0.86+0.24*Math.sin(e*Math.PI));
      h.state='idle';h.face=dx<0?-1:1;
      b.trail.push({x:h.x,y:h.y});if(b.trail.length>26)b.trail.shift();
      if(p>=1){
        b.phase='land';b.t=0;h.x=b.to.x;h.y=b.to.y;
        for(let i=0;i<10;i++){
          const a=Math.PI*(0.15+0.7*(i/9)),v=70+40*rand2(i,3,7);
          b.dust.push({x:h.x,y:h.y,vx:Math.cos(a)*v*(i%2?1:-1),vy:-Math.sin(a)*v*0.5,t:0});
        }
        if(this.audio)this.audio.sfx('hit',{gain:0.4});
        if(this.hudShake)this.hudShake('panel',6);
      }
      return true;
    }
    // Landung: stauchen, Staub treiben lassen
    const p=Math.min(1,b.t/LAND);
    h.sizeMul=b.base*(1+0.3*(1-p)*Math.cos(p*Math.PI*2.2));
    for(const d of b.dust){d.t+=dt;d.x+=d.vx*dt;d.y+=d.vy*dt;d.vy+=180*dt;}
    if(p>=1){
      h.sizeMul=b.base;this._blod=null;this.keys={};
      this.cam={x:h.x,y:h.y};
      console.log('[blödsinn] gelandet · Feld',Math.round(h.x/TILE)+','+Math.round(h.y/TILE));
    }
    return true;
  }
  /* Die Tempolinien sind der Flug — der Sprite allein sieht aus, als würde er geschoben. */
  drawBlodsinn(){
    const b=this._blod;if(!b)return;
    const ctx=this.ctx;
    if(b.trail.length>2){
      ctx.save();
      ctx.strokeStyle='rgba(31,26,20,.55)';ctx.lineWidth=3;ctx.lineJoin='round';ctx.lineCap='round';
      ctx.beginPath();ctx.moveTo(b.trail[0].x,b.trail[0].y);
      for(let i=1;i<b.trail.length;i++)ctx.lineTo(b.trail[i].x,b.trail[i].y);
      ctx.stroke();
      ctx.strokeStyle='rgba(232,211,138,.5)';ctx.lineWidth=1.4;ctx.stroke();
      ctx.restore();
    }
    if(b.dust.length){
      ctx.save();
  ...
  updateHud(){
    ...
    /* 10.8. · **Auch das Notpanel ist eine kompakte Fläche.** Es setzte die vollen Namen in eine
       Reihe — mit `KayfaBingo/KayfaBongo/KayfaBoggle` reicht die Breite nicht mehr, und die letzten
       zwei Werte standen außerhalb. Also dieselbe Regel wie im v7-Panel: kompakt zeigt `short`, der
       volle Name steht im Tooltip. Ein Etikett zu ändern heißt, jede Fläche zu prüfen, die es setzt. */
    this.panel.querySelector('.stats').innerHTML=STAT_KEYS
      .map(k=>`<span style="color:${STAT_INFO[k].color}" title="${STAT_INFO[k].name}"><b>${h.stats[k]}</b> ${STAT_INFO[k].short||STAT_INFO[k].name}</span>`).join('');
    let kf='';
    for(let i=0;i<3;i++){
  ...
  draw(){
    ...
    if(paint){
      ...
      this._t('wasser');
      /* Die Wassertextur fällt bei kleinem Zoom weg (v10-S1f). Georg in der Übersicht: »die
         Wasser-Textur geht so nicht, weil buggy/streifig« — das ist kein Fehler im Muster, sondern
         Aliasing: eine 512er Rauschkachel, mit Nearest auf ein Zehntel gezogen, greift je Zeile
         andere Quellpixel und wird zum Streifen. Unter derselben Schwelle wie Relief und Bodenkorn
         (V9-B6) bleibt die Fläche glatt — auf dieser Entfernung ist Wasser eine Farbe. */
      if(this.dbg.wasser&&nah)OW_TERRAIN.drawWater(ctx,this.paintOpt,this.time);
      this._t('boden');
  ...
  drawUnitTag(ctx,u,bh){
    ...
    // Neutral, unversehrt, unbeteiligt: nichts. Das ist der häufigste Fall auf der Insel.
    if(!verwundet&&!kampf&&!gemerkt&&!(u.guard||u.elite))return;
    const y=u.y-bh-LUFT-BLOCK;
    if(verwundet||kampf){
      const w=u.elite?56:u.guard?50:42;
      const t=Math.max(0,Math.min(1,u.hp/u.maxhp));
      ctx.fillStyle='rgba(14,20,17,.82)';
      ctx.fillRect(u.x-w/2,y,w,7);
      ctx.fillStyle=freund?'#5fbf7a':kampf?'#d8452f':gemerkt?'#e0a12c':
    ...
    /* Unversehrt, aber nicht harmlos: die Stufe als Einschätzung. Klein, in der Farbe der Haltung —
       dieselbe Auskunft wie die Leiste, nur ruhiger. */
    const txt='Lv '+(u.lv||1);
    ctx.font='600 12px "Courier New",ui-monospace,monospace';
    ctx.textAlign='center';ctx.textBaseline='alphabetic';
    const tw=ctx.measureText(txt).width;
    ctx.fillStyle='rgba(14,20,17,.72)';
    ctx.fillRect(u.x-tw/2-4,y-2,tw+8,14);
    ctx.fillStyle=freund?'#8fd8a2':gemerkt?'#e0a12c':(u.critter&&!feind)?'#c9dcb4':'#e2c98a';
    ctx.fillText(txt,u.x,y+9);
    ctx.textAlign='left';
  }
  ...
  drawUnit(u){
    ...
    if(!isHero)this.drawUnitTag(ctx,u,bh);
