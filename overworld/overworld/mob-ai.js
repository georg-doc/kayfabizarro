...
const RUN_HOLD=0.18;        // s Haltezeit idle⇄run
...
function speak(m,text,life,frei){
  ...
  m.ai.bubble={text:t.length>44?t.slice(0,43)+'…':t,potty,typ:denkt?'thought':'speech',
    eskal:eskalation,nackt,chatId:m._chatId||null,
    t:0,life:nackt?0.9:(life||3.2),w:0,dx:0,dy:0};
...
/* v10-S23b · **Ein Wortwechsel ist EIN Eintrag, keine drei.** Die Synthese fiel unters gewöhnliche
   Budget — ausgerechnet dann, wenn der Wortwechsel es selbst gefüllt hatte (These + Antithese = 2,
   Deckel erreicht, dritter Zug abgelehnt). Gemessen brach die Kette in 7 von 8 Läufen genau dort;
   sie schloss sich nur, wenn die These zufällig vorher auslief — ein Rennen, kein Verhalten.
   Dieselbe Klasse wie beim Schmähruf (S19b/c), und Georgs Regel nennt »kurze social calls«
   ausdrücklich als Ausnahme. Blasen mit derselben `chatId` zählen deshalb zusammen als eine:
   *ein Gespräch ist ein Angebot, nicht drei.* */
function weltVoices(g){
  if(!g||!g.mobs)return 0;
  let n=0;const ketten=new Set();
  for(const o of g.mobs){
    const b=o.ai&&o.ai.bubble;if(!b)continue;
    if(b.chatId){if(ketten.has(b.chatId))continue;ketten.add(b.chatId);}
    n++;
  }
  return n;
}
...
function chatLine(g,m,kind,rng){
  ...
  return (kind==='reply'||kind==='curse'||kind==='synthese')?null:cardLine(g,m,'spin');
}
...
function think(g,m,dt,heroSpd){
  ...
  if(a.replyIn>0){
    a.replyIn-=dt;
    /* v10-S23 · **Der dritte Sprecher.** Bisher war eine Plauderei ein Satz und eine Antwort —
       These und Antithese, und dann stand es im Raum. Georgs Korrektur (10.8.): Therefore/But läuft
       **zwischen Sprechern**, nicht im Satz, und erst der Dritte schließt den Gedanken.
       Wer antwortet, sucht deshalb einen Dritten in der Nähe und reicht weiter. Die Synthese kommt
       ohne Umweg über die vier Quellen — sie ist kein neuer Einfall, sondern ein Abschluss. */
    if(a.replyIn<=0&&m.hp>0){
      if(speak(m,chatLine(g,m,'reply',rng),2.1,true)&&!a.warDritter){
        const kette=m._chatId;
        let dritter=null,bd=9*64;
        for(const o of g.mobs){
          if(o===m||o===a.chatPartner||o.hp<=0||!o.ai||o.ai.bubble||o.critter)continue;
          const d=Math.hypot(o.x-m.x,o.y-m.y);
          if(d<bd){bd=d;dritter=o;}
        }
        if(dritter&&rng()<0.7){dritter.ai.syntheseIn=0.8+rng()*0.6;dritter.ai.warDritter=true;
          dritter._chatId=kette||(m.ai.bubble&&m.ai.bubble.chatId)||null;}
      }
    }
  }
  if(a.syntheseIn>0){
    a.syntheseIn-=dt;
    if(a.syntheseIn<=0&&m.hp>0){
      speak(m,chatLine(g,m,'synthese',rng),2.2,true);
      a.warDritter=false;m._chatId=null;
    }
  }
  ...
  if(m.state==='attack'){
    ...
    if(!m.didHit&&m.atkT>dur*0.5){
      ...
      if(dh<(a.strike||68)+12){g.damage(h,m.dmg,false);if(h.hp<=0)g.heroDown();}
    }
  ...
  switch(a.st){
    ...
    case 'mingle': {
      ...
      if(d<74||a.t>6){
        ...
        m._chatId='c'+(g.time*1000|0)+'_'+(rng()*999|0);
        if(speak(m,chatLine(g,m,'spin',rng),2.8)&&o.ai){
          o.ai.replyIn=0.9+rng()*0.7;
          o.ai.chatPartner=m;         // damit der Dritte nicht der Erste ist
          o._chatId=m._chatId;        // dieselbe Kette, ein Eintrag im Budget
        }
        m._chatId=null;
      }
      break;
    ...
    case 'chase': {
      ...
      const ring=Math.max(ringOf(g,m),a.tp.standoff*(bodyOf(m)+bodyOf(h))/2),strike=ring+12;
...
function present(m,dt,sp){
  ...
  if(wantRun!==(m.state==='run')&&a.runHold<=0){
    m.state=wantRun?'run':'idle';a.runHold=RUN_HOLD;m.anim=0; // Frame nur beim echten Bildwechsel neu setzen
  }
