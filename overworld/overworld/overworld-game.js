...
const BIOMES=['camp','camp','wilds','wilds','cave'];const BESTIARY={
  ...
  dungeon:{melee:['skull','thief'],elite:['skull']},
};
...
const FLUFF_UNIT=20, LEVEL_CAP=8, SLOT_LEVELS=[3,6];
const XP_NEED=lv=>10+5*(lv-1);
const XP_SRC={mob:1,elite:3,secret:2,zone:8,boss:12};
const START_STATS={fluff:5,kayfabe:2,bizarro:3};
...
const STAT_INFO={
  fluff:{name:'Fluff',color:'var(--stat-fluff,#e8756a)',line:'You last longer — even when nobody is listening.'},
...
const TOWN=[
  ...
  {id:'archive',  label:'the archive',              hint:'turn the page',      a:-2.55,r:11, asset:'b_monastery'},
  {id:'arena',    label:'the arena',                hint:'look at the ring',   a:-0.35,r:11, asset:'b_tower'},
  {id:'graveyard',label:'the graveyard',            hint:'set your respawn',   a:-3.00,r:13, asset:null},
...
class OverworldGame extends HTMLElement{
  ...
  static get observedAttributes(){return['seed','hero-color','herocolor','zoom','zones','sound','layout','hero','loot','town-color','towncolor','world-size','worldsize','ground','life','terrain','coast-ink','coastink','relief','relief-tex','relieftex','hud-skin','shadow','puddles','prop-sheet','propsheet','prop-grid','propgrid','hudskin'];}
  constructor(){super();
    this.att={seed:7,color:'Blue',zoom:1,zones:6,sound:true,layout:'utopia',hero:'warrior',loot:'pen',townColor:'Blue',worldSize:'large',ground:true,life:1,terrain:'tiles',propSheet:'mix',propGrid:'smooth',shadow:'contact',puddles:'on',coastInk:'feather',relief:'bevel',reliefTex:0.55,hudSkin:'v7'};
    this.keys={};this.stats={fps:0,tiles:0,sprites:0};
    this.time=0;this.freeze=0;this.ready=false;
    this.mobs=[];this.corpses=[];this.floaters=[];this.msgs=[];
  }
  attributeChangedCallback(n,_o,v){
    ...
    if(n==='terrain'){this.att.terrain=(v==='paint')?'paint':'tiles';}
  ...
  connectedCallback(){
    ...
    sh.innerHTML=`<style>
      ...
      .lvup,.cap{position:absolute;inset:0;display:none;align-items:center;justify-content:center;
        background:rgba(12,18,15,.72)}
      .lvup .box,.cap .box{background:#1a2420;border:2px solid #e8d38a;border-radius:8px;padding:20px 22px;
        color:#e8e4d8;max-width:580px;text-align:center}
      .lvup h3,.cap h3{margin:0 0 4px;font-size:19px;color:#e8d38a;letter-spacing:1px}
      .lvup p,.cap p{margin:0 0 16px;font-size:13px;color:#a9b3a8}
    ...
    <div class="zone"></div>
    <div class="hint" style="display:none">WASD / arrows move · <b>right-click</b> walk / attack · space attack · <b>1 2 3</b> Kayfabe · <b>J</b> diary · <b>C</b> play as · <b>V</b> shapeshift · <b>M</b> overview · <b>F</b> audit · <b>H</b> help · <b>+ −</b> zoom · <b>Tab</b> hide HUD</div>
    ...
    this.cv.addEventListener('pointerdown',e=>{
      ...
      if(e.button!==2||!this.ready||this.paused)return;
      e.preventDefault();
      ...
      if(this.overview){ // God-Mode: Shift setzt den Helden, sonst reist er — die Karte ist der Hub
        ...
        if(this.tavern&&Math.hypot(wx-this.tavern.x,wy-this.tavern.y)<200){
          this.travelPoint(this.tavern.x,this.tavern.y+70,'the tavern');return;
        }
        if(this.places){
          const p=this.places.find(p=>Math.hypot(wx-p.x,wy-p.y)<150);
          if(p){this.travelPoint(p.x,p.y+70,p.label);return;}
        }
        if(this.spawn&&Math.hypot(wx-this.spawn.x,wy-this.spawn.y)<200){
          this.travelPoint(this.spawn.x,this.spawn.y,'camp');return;
        }
        this.setMoveTarget(wx,wy);
        return;
      }
  ...
  async waitForDeps(){
    ...
    this.OWA=aiUp()?window.OW_AI:null;
  ...
  heroDamage(){return 6+4*this.hero.stats.bizarro;}
  ...
  async spawnCritters(){
    ...
    for(const [zi,z] of this.zones.entries()){
      ...
      for(let i=0;i<k;i++){
        ...
        if(!this.passable(px,py))continue;
  ...
  // XP kommt aus Taten, nie aus Kayfabe (kein Snowball) — Spec §2
  gainXp(n,x,y){
    const h=this.hero;
    if(h.lv>=LEVEL_CAP)return;
    h.xp+=n;
    this.floaters.push({x:x!=null?x:h.x,y:(y!=null?y:h.y)-96,txt:'+'+n+' XP',t:0,c:'#9fd1ff'});
    while(h.lv<LEVEL_CAP&&h.xp>=XP_NEED(h.lv)){h.xp-=XP_NEED(h.lv);h.lv++;this.levelUp();}
    if(h.lv>=LEVEL_CAP)h.xp=0;
  }
  // Abwechselnd: Stat-Wahl — auf Level 3 und 6 stattdessen ein Kayfabe-Slot (Spec §2)
  levelUp(){
    const h=this.hero;
    this.paused=true;
  ...
  say(text,ttl,style){this.bubble={text,t:0,ttl:ttl||2.4,style:style||'talk'};}
  ...
  captureJourney(){
    ...
    s.hero={color:this.att.color,lv:h.lv,xp:h.xp,slots:h.slots,charges:h.charges,
  ...
  applyJourney(s){
    ...
    Object.assign(h.stats,s.hero.stats||{});
    h.lv=s.hero.lv||1;h.xp=s.hero.xp||0;h.slots=s.hero.slots||1;
  ...
  auditActions(){
    ...
    return[
      ...
      {k:'Level 6',go:mark(()=>{
        ...
        while(h.lv<6){
          h.lv++;
          if(SLOT_LEVELS.includes(h.lv)&&h.slots<3)h.slots++;
          else h.stats[['fluff','bizarro','kayfabe'][h.lv%3]]++;
        }
        h.xp=0;
        h.maxhp=h.stats.fluff*FLUFF_UNIT;h.hp=h.maxhp;
        h.charges=h.stats.kayfabe;
        this.updateHud();
        this.msg('Level '+h.lv+' · '+h.slots+' Kayfabe slots. Set, not played.');
      })},
  ...
  /* **Jedes** Reiseziel wird geprüft, nicht nur die Kampfzonen (V5-S10e). Der Schutz saß in
     ...
     der zweiten Tür. */
  travelPoint(x,y,label){
    let tx=Math.floor(x/TILE),ty=Math.floor(y/TILE);
    if(!this.walk(tx,ty)||!this.sameLand(tx,ty)){
      /* Ausweichen, nicht ablehnen: das Klickziel liegt 70 px unter dem Ortsanker, also fast zwei
         Felder tiefer — dort kann Wasser oder ein Baum stehen, obwohl der Ort selbst erreichbar ist.
         Erst wenn im Umkreis von drei Feldern nichts erreichbar ist, gibt es keinen Weg. */
      let found=null;
      for(let r=1;r<=3&&!found;r++)for(let k=0;k<=r*8;k++){
  ...
  drawOverview(ox,oy,z){
    ...
    // Beschriftungen sind breiter als die Zonen, die sie meinen — also erst prüfen, dann setzen.
    const boxes=[];
    const hits=r=>boxes.some(b=>!(r.x+r.w<b.x-3||b.x+b.w<r.x-3||r.y+r.h<b.y-2||b.y+b.h<r.y-2));
    const put=(cx,baseY,dir,w,h)=>{ // dir -1 = über der Zone, +1 = darunter
      let r=null;
      for(let k=0;k<9;k++){
        const y=baseY+dir*k*16;
        r={x:cx-w/2,y:dir<0?y-h:y,w,h};
        if(!hits(r))break;
      }
      boxes.push(r);return r;
    };
    const clip=t=>t.length>16?t.slice(0,15)+'…':t;
    for(const zo of this.zones){
      const x=toX(zo.x*TILE),y=toY(zo.y*TILE),w=zo.w*TILE*s,h=zo.h*TILE*s,cx=x+w/2;
      ctx.strokeStyle=zo.cleared?'rgba(232,211,138,.95)':'rgba(255,255,255,.8)';
      ctx.lineWidth=2;ctx.strokeRect(x,y,w,h);
      const label='»'+clip(zo.card.t)+'«';
      const lb=put(cx,y-4,-1,ctx.measureText(label).width+10,15);
      ctx.fillStyle='rgba(20,26,24,.9)';ctx.fillRect(lb.x,lb.y,lb.w,lb.h);
      ctx.fillStyle=zo.cleared?'#e8d38a':'#e8e4d8';ctx.fillText(label,cx,lb.y+11);
      const sub=zo.biome+(zo.cleared?' · secured':' · '+zo.alive+' guards');
      const sb=put(cx,y+h+3,1,ctx.measureText(sub).width+10,14);
      ctx.fillStyle='rgba(20,26,24,.8)';ctx.fillRect(sb.x,sb.y,sb.w,sb.h);
      ctx.fillStyle='#9aa79c';ctx.fillText(sub,cx,sb.y+10);
      // Verbindungsstrich, wenn die Beschriftung ausweichen musste
      if(Math.abs(lb.y+lb.h-(y-4))>2){
        ctx.strokeStyle='rgba(232,211,138,.35)';ctx.lineWidth=1;
        ctx.beginPath();ctx.moveTo(cx,lb.y+lb.h);ctx.lineTo(cx,y);ctx.stroke();
      }
    }
    const pin=(wx,wy,txt,col,dir)=>{
      ...
      const b=put(x,y+(dir<0?-8:8),dir,ctx.measureText(txt).width+8,14);
      ctx.fillStyle='rgba(20,26,24,.9)';ctx.fillRect(b.x,b.y,b.w,b.h);
      ctx.fillStyle=col;ctx.fillText(txt,x,b.y+10);
    };
    // Wirtshaus, Lagerplatz und Held liegen dicht beieinander — auch sie weichen einander aus
    if(this.places)for(const p of this.places)
      pin(p.x,p.y,p.id,p.id==='tavern'?'#e8d38a':'#cfc9b8',-1);
    if(this.spawn)pin(this.spawn.x,this.spawn.y,'camp','#9fd1ff',1);
    if(this.hero)pin(this.hero.x,this.hero.y,'you','#6fc48a',1);
    const head='OVERVIEW · seed '+this.att.seed+' · click a place to travel · shift+click to drop yourself';
  ...
  async loadDeck(){
    ...
    try{
      ...
      const pick=decks[Math.floor(rand2(this.att.seed,17,3)*decks.length)]||decks[0];
      if(pick){
  ...
  buildWorld(){
    ...
    this.zones=[];this.bridge=new Map();this.mobs=[];this.corpses=[];this.collected=[];
    this.gutter=new Set();this.causeway=new Set();
    this.connectLand();   // erst prüfen, ob die Insel eine ist — dann darauf bauen
    const IW=10,IH=8,PAD=1; // Innenmaß + Rand um den Graben
    const fits=(zx,zy)=>{
      let landN=0,tot=0;
    ...
    for(let y=0;y<H;y++)for(let x=0;x<W;x++){
      ...
      if(land[i]===2){
        ...
        if(t>0.46&&r<(t-0.46)*1.25&&this.img.tree1&&!onEdge(x,y)){
          if(useSheet(x,y)){
          ...
          reserve(x,y,1);
        }else if(r>0.62&&r<0.70&&!onEdge(x,y)){
          if(useSheet(x,y)){
            const p=sheetProp(x,y,px,py-4,'deco',rand2(x*3+9,y*5+2,s+77));
            if(p){sprites.push(p);small++;reserve(x,y,1);continue;}
          }
          const img=this.img['deco'+(1+Math.floor(rand2(x*3+9,y*5+2,s+77)*18))];
          if(img){sprites.push({img,fw:img.width,frames:1,x:px,y:py-4,
            def:deform(x,y,0.35)});small++;reserve(x,y,1);}
        }
    ...
    this.hero={unit:this.heroUnit,x:(best.x+0.5)*TILE,y:(best.y+0.5)*TILE,face:1,dir:'side',
      ...
      stats:Object.assign({},START_STATS),lv:1,xp:0,slots:1,charges:START_STATS.kayfabe,
      unlocked:['monologue'],equipped:['monologue',null,null],buffs:{},
      hp:START_STATS.fluff*FLUFF_UNIT,maxhp:START_STATS.fluff*FLUFF_UNIT,lastHit:-99};
  ...
  buildTown(best,sprites,inZoneArea){
    ...
    for(const def of TOWN){
      const K=this.worldK||1;
      const spot=snap(Math.round(mid.x+Math.cos(def.a)*def.r*K),
                      Math.round(mid.y+Math.sin(def.a)*def.r*K));
      if(!spot){
    ...
    /* Zweiter Durchgang: erst JETZT wird gesperrt. Vorher setzte jeder Ort seinen Fußabdruck und
       ...
       Der Fußabdruck ist zwei Felder tief: in der Draufsicht steht das Gebäude auf seiner
       Grundfläche, der Rest des Blattes ist Höhe. */
    let solid=0;
    for(const p of this.places){
      const img=p.img;
      const tw=img?Math.max(1,Math.round(img.width/TILE)-1):2;
      const th=img?2:1;
  ...
  async openLoot(p){
    ...
    this.paused=true;this.keys={};
    await OW_CARD.ready();
    const sheet=OW_CARD.draw(p.card,{size:900,deck:this.deck&&this.deck.title,seed:(p.card.n|0)+11});
  ...
  enterInterior(int,place){
    ...
    this.zones=[];this.decos=[];this.places=[];this.corpses=[];
  ...
  addZone(zx,zy,IW,IH,s){
    ...
    for(let y=zy-1;y<=zy+IH;y++)for(let x=zx-1;x<=zx+IW;x++){
      ...
      land[y*W+x]=inner?2:0;
      if(!inner)gut.add(y*W+x);
    }
    ...
    const biome=zi===0?'camp':BIOMES[Math.floor(rand2(zi,23,zseed)*BIOMES.length)];
    ...
    this.zones.push({x:zx,y:zy,w:IW,h:IH,card,zseed,biome,roster:[],eliteId:null,
      gate:{x:gx,y:gy},
  ...
  tryAttack(u){
    ...
    // Wer zuschlägt, verliert die Fahrt — das Ereignis bricht sie, nicht der Bewegungsframe (S12c)
    if(u===this.hero&&window.OW_FEEL)OW_FEEL.breakFlow(this);
  ...
  moveEntity(u,dx,dy,speed,dt){
    ...
    if(stuck){u.x=nx;u.y=ny;moved=true;}
      ...
      if(this.passable(nx+Math.sign(dx)*R,u.y-8)&&this.passable(nx+Math.sign(dx)*R,u.y+8)){u.x=nx;moved=moved||!!dx;}
      if(this.passable(u.x-8,ny+Math.sign(dy)*R)&&this.passable(u.x+8,ny+Math.sign(dy)*R)){u.y=ny;moved=moved||!!dy;}
    }
    if(Math.abs(dx)>6&&Math.abs(dx)>Math.abs(dy)*0.3)u.face=Math.sign(dx); // Hysterese: sonst klappt der Sprite jeden Frame um
    u.dir=Math.abs(dx)>=Math.abs(dy)?'side':(dy>0?'down':'up');
    return moved;
  }
  ...
  killMob(m){
    ...
    if(z.alive<=0&&!z.cleared&&!z.noProgress){
      z.cleared=true;this.collected.push(z.card);
  ...
  step(dt){
    ...
    if(h.state==='attack'){
      ...
      if(!h.didHit&&h.atkT>dur*0.45){
        ...
        for(const m of this.mobs){
          ...
          if(d<62&&(mx*h.face>=-14||Math.abs(my)>Math.abs(mx))){
    ...
    if(this.overview){this.cam.x=this.W*TILE/2;this.cam.y=this.H*TILE/2;}
      ...
      this.cam.x+=(h.x-this.cam.x)*Math.min(1,dt*5);
      this.cam.y+=(h.y-this.cam.y)*Math.min(1,dt*5);
    ...
    // HUD
    this.stats.fps=this.stats.fps*0.94+(dt>0?1/dt:120)*0.06;
    this._hud=(this._hud||0)+ (dt||0.008);
    if(this._hud>0.2){this._hud=0;this.updateHud();}
  }
  heroDown(){
    const h=this.hero;
    this.msg('Out of Fluff — back at camp. The island remembers nothing.');
    if(this.audio)this.audio.voice('heroDown',1,0);
    h.hp=h.maxhp;h.x=this.spawn.x;h.y=this.spawn.y;h.busy=0;
  ...
  updateHud(){
    ...
    this.pill.textContent=`${Math.round(this.stats.fps)} fps · ${this.stats.tiles} tiles · ${this.stats.sprites+this.mobs.length} sprites · seed ${this.att.seed}`;
    ...
    const cap=h.lv>=LEVEL_CAP,need=XP_NEED(h.lv);
    this.panel.querySelector('.xp i').style.transform=`scaleX(${cap?1:Math.min(1,h.xp/need)})`;
    this.panel.querySelector('.xp span').textContent=cap?'MAX':h.xp+' / '+need;
    this.panel.querySelector('.lv').textContent=`LV ${h.lv} · evidence ${this.collected.length}/${this.zones.length}`;
    this.panel.querySelector('.stats').innerHTML=['fluff','kayfabe','bizarro']
  ...
  draw(){
    ...
    const paint=this.att.terrain==='paint'&&window.OW_TERRAIN&&window.OW_GROUND;
    ...
    if(paint){
      ...
      this.paintOpt={land,W:Wd,H:Hd,TILE,x0,y0,x1,y1,key:'w'+this.att.seed+'_'+Wd+'x'+Hd,
        inkStyle:this.att.coastInk||'feather',relief:this.att.relief||'bevel'};
      ...
      // Wasser-Spiegel vor dem Land: sie liegen auf dem Meer, nicht auf dem Strand
      OW_TERRAIN.drawWater(ctx,this.paintOpt,this.time);
      this.stats.tiles=OW_TERRAIN.draw(ctx,Object.assign({},this.paintOpt,{
        sand:OW_GROUND.tileFor('__sand',4,false,true),
        grass:OW_GROUND.tileFor(this.baseGround||'grass',11,false,true)}));
    }else{
    for(let y=y0;y<=y1;y++)for(let x=x0;x<=x1;x++){
      ...
      if(pathSet&&v===2&&pathSet.has(y*Wd+x)){this.drawAuto(this.img.flat,this.sandBX,x,y,pathM);tiles++;}
    }
    ...
    if(this.att.ground&&window.OW_GROUND&&!paint){
    ...
    if(this.att.puddles!=='off'&&window.OW_PUDDLES){
    ...
    // Brücken
    if(this.img.bridge)for(const[idx,horiz]of this.bridge){
    ...
    for(const m of this.mobs)if(m.hp>0)view.push({ent:m,x:m.x,y:m.y});
    ...
    if(this.OWA&&this.OWA.drawBubbles)this.OWA.drawBubbles(this);
    // Schadenszahlen
  ...
  drawUnit(u){
    ...
    const key=u.state==='attack'?(u.atkKey||'idle'):un.pick(u.state,u.dir,0);
    const a=un.anim(key);
    ...
    if(un.shadow==='ellipse'&&this.att.shadow!=='off'){
    ...
    if(!isHero&&u.hp<u.maxhp){
      const w=u.elite?54:44;
      ctx.fillStyle='rgba(14,20,17,.8)';ctx.fillRect(u.x-w/2,u.y-bh-16,w,6);
      ctx.fillStyle=u.elite?'#e8a33c':'#c43c3c';
      ctx.fillRect(u.x-w/2+1,u.y-bh-15,(w-2)*Math.max(0,u.hp/u.maxhp),4);
    }
