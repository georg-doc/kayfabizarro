...
class OverworldGame extends HTMLElement{
  ...
  static get observedAttributes(){return['seed','hero-color','herocolor','zoom','zones','sound','layout','hero','loot','town-color','towncolor','world-size','worldsize','ground','life','terrain','coast-ink','coastink','relief','relief-tex','relieftex','hud-skin','shadow','puddles','prop-sheet','propsheet','prop-grid','propgrid','hudskin','waber','waber-props','waberprops','shade','fluid','render-scale','renderscale'];}
  constructor(){super();
    this.att={seed:7,color:'Blue',zoom:1,zones:6,sound:true,layout:'utopia',hero:'warrior',loot:'pen',townColor:'Blue',worldSize:'large',ground:true,life:1,terrain:'tiles',propSheet:'mix',propGrid:'smooth',shadow:'contact',puddles:'on',coastInk:'feather',relief:'bevel',reliefTex:0.55,hudSkin:'v7',renderScale:1,fightTalk:'off'};
  ...
  connectedCallback(){
    ...
    this.cv.addEventListener('pointerdown',e=>{
      ...
      if(e.button!==2&&!this.overview)return;
  ...
  async loadReaderCard(){
    const R=this.reader;if(!R)return;
    if(!R.back)this.OWL.loadImg(window.OW_SRC?OW_SRC.kfb('KayfaBizarro_Card_Backside_01.png')
        :'overworld/card-backside.png')
      .then(i=>{if(this.reader===R)R.back=i;})
      /* Rückweg über die Projektdatei — jeder Auftrag braucht einen (Fehlerklasse v8). */
      .catch(()=>this.OWL.loadImg('overworld/card-backside.png')
  ...
  drawCardPlate(ctx,o){
    ...
    const w=o.w, h=Math.round(o.w/AR), dy=Math.round((o.h-h)/2);
    const x=o.x, y=o.y+dy;
    const seed=o.seed||7;
  ...
  /* Die geteilte Rückseite. Eine Datei für alle Zonen und den Reader — sie ist dieselbe Zeichnung. */
  loadCardBack(){
    if(this._back||this._backLaeuft)return;
    this._backLaeuft=true;
    const nimm=i=>{this._back=i;this._backLaeuft=false;
      console.log('[zone] Rückseite',i.naturalWidth+'×'+i.naturalHeight);};
    this.OWL.loadImg(window.OW_SRC?OW_SRC.kfb('KayfaBizarro_Card_Backside_01.png')
      :'overworld/card-backside.png').then(nimm)
      .catch(()=>this.OWL.loadImg('overworld/card-backside.png').then(nimm)
  ...
  drawZoneCards(ctx,x0,y0,x1,y1){
    ...
    for(const z of this.zones){
      ...
      this.drawCardPlate(ctx,{x:z.x*TILE,y:z.y*TILE,w:z.w*TILE,h:z.h*TILE,
        seed:(z.card&&z.card.n|0)+z.zseed,img:z.art,back:this._back,reveal:rev,
  ...
  addZone(zx,zy,IW,IH,s){
    ...
    // Der Zeichner braucht das Tor, um sein Loch in den Strich zu schneiden
    const zoNeu=this.zones[this.zones.length-1];
    if(zoNeu)zoNeu.gate={x:bx,y:by};
    ...
    /* Das Biom hängt am Zonenindex, nicht am Würfel: sechs Zonen sind sechs Welten (V9-B1).
       Die Startzone bleibt `camp` — der erste Ort soll lesbar sein, nicht überraschend. */
    const biome=BIOMES[zi%BIOMES.length];
