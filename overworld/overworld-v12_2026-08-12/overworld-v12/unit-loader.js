...
function makeUnit(id,def,anims,meta){
  return {
    ...
    kind:meta.kind,anims,scale:meta.scale,bodyH:meta.bodyH,srcBodyH:meta.srcBodyH,bump:!!meta.bump,
    ...
    pick(state,dir,combo){
      if(state==='attack'){
        ...
        /* v11-U1 · **Rempler.** Zwei der 30 Einheiten haben kein Angriffsblatt (Schwein, Schaf).
           Vorher schlugen sie in die RUHEANIMATION — sichtbar also gar nicht, und das las sich
           als kaputt, nicht als friedlich. Jetzt nehmen sie ihre Laufbilder: ein Anrennen. Nichts
           erfunden, nur anders benutzt; den Schub gibt der Runner, das Bild kommt vom Blatt. */
        if(this.bump&&this.has('run'))return 'run';
        return 'idle';
      }
...
async function build(id,def,opts){
  ...
  /* Hat dieses Blatt überhaupt einen Hieb? Gemessen an den GELADENEN Animationen, nicht am
     Katalog: ein Eintrag kann `attack` nennen und die Datei fehlt (dann steht die Warnung oben in
     der Konsole). Wer keinen hat, bekommt den Rempler in `pick`. */
  const hasAtk=Object.keys(anims).some(k=>k==='attack'||k==='attack2'||k.indexOf('attack:')===0);
  ...
  const u=makeUnit(id,def,anims,{kind,scale,bodyH:srcBodyH*scale,srcBodyH,bump:!hasAtk,
  ...
  console.log('[unit-loader]',id,kind,
    ...
    '· Anims',Object.keys(anims).map(k=>k+':'+anims[k].frames).join(' ')+(hasAtk?'':' · KEIN HIEB → Rempler'));
  return u;
}
