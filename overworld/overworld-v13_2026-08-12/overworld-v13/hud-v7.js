...
/* WS0-Eingriff 10.8. (Re-Home S22): `kfb-paper-atlas.js` ist im HUD-DC ausgehaengt — der
   Baukasten (`OW_UIKIT`) ist die eine Wahrheit ueber die Tiny-Swords-Blaetter. Das Zahnrad war
   die letzte Stelle, die den Atlas noch brauchte; es holt seine Adresse jetzt synchron aus
   `iconSrc()`. Synchron ist die Bedingung, nicht der Zufall: das Chrome wird beim Anhaengen
   gebaut und kann nicht auf ein Versprechen warten (§4h Naht 2). Reihenfolge: Atlas, wenn er
   geladen ist (Runner-DC) — sonst Baukasten — sonst der Glyph. */
const PA=()=>{
  if(window.KFB_PAPER)return window.KFB_PAPER;
  const U=window.OW_UIKIT;
  if(U&&typeof U.iconSrc==='function')return {icon:k=>U.iconSrc(k)};
  return null;
};
...
function loadInk(){
  ...
  inkTry=(async()=>{
    /* **Eine relative Adresse gegen eine `blob:`-Basis wirft** (Standalone-Befund 12.8.) — und
       zwar synchron, also erreicht der Fehler den vorhandenen Rückweg NICHT, sondern landet als
       unbehandelte Rejection im Log. Deshalb jeden Kandidaten einzeln bauen.
       *Ein Rückweg, der erst nach dem Wurf greift, ist keiner.* */
    const tries=[];
    try{tries.push(new URL('../cardbuilder/kfb-ink-canon.js',SRC).href);}catch(e){}
    try{tries.push(new URL('./cardbuilder/kfb-ink-canon.js',location.href).href);}catch(e){}
    tries.push(RAWROOT+'skills/kfb-ink-canon.js');
    tries.push('https://cdn.jsdelivr.net/gh/georg-doc/kayfabizarro@main/skills/kfb-ink-canon.js');
...
async function loadCfg(){
  ...
  try{
    /* Dieselbe Klasse: gegen eine `blob:`-Basis gibt es keine relative Adresse. Dann über den
       Kanal des Spiels, der im Bündel genauso funktioniert wie im Projekt. */
    let ziel=null;
    try{ziel=new URL('./hud-slots.json',SRC).href;}catch(e){
      ziel=window.OW_SRC?OW_SRC.ow('hud-slots.json'):null;}
    if(!ziel)throw new Error('keine Adresse für hud-slots.json');
    const r=await fetch(ziel,{cache:'no-cache'});
    if(r.ok){const j=await r.json();delete j.skin;delete j.colors;cfg=Object.assign(cfg,j);}
    /* WS0-Eingriff 9.8., beim Re-Home S22 wieder aufgetragen: ein 404 war hier still. Das HUD
       fiel auf FALLBACK zurueck und sah dabei heil aus — gefunden wurde es nur ueber den
       Netzwerkweg (ein 404 von 104 Ressourcen). Wer eine Konfiguration laedt, muss ihren
       Fehlschlag melden. */
    else console.warn('[hud-v7] hud-slots.json: Status '+r.status+' — FALLBACK-Slots.');
  }catch(e){console.warn('[hud-v7] hud-slots.json nicht lesbar — FALLBACK-Slots.',e);}
...
/* **Oben ausblenden statt abschneiden** (Georg, 12.8.: »chat ist abgeschnitten«). Das Logbuch ist
   eine rollende Liste: die älteste Zeile wird von der festen Höhe beschnitten. Eine halb sichtbare
   Zeile mit harter Kante liest sich als Fehler, dieselbe Zeile mit weichem Auslauf als Vergangenheit.
   *Was verschwindet, muss verschwinden dürfen — abgeschnitten sieht nach Defekt aus.* */
.v7-log .lines{height:min(19vh,108px);overflow:hidden;display:flex;flex-direction:column;
  justify-content:flex-end;
  -webkit-mask-image:linear-gradient(to bottom,transparent 0,rgba(0,0,0,.35) 9px,#000 22px);
  mask-image:linear-gradient(to bottom,transparent 0,rgba(0,0,0,.35) 9px,#000 22px)}
...
function inkKit(){
  ...
  function draw(el){
    ...
    /* **Wer ohne Kanon gezeichnet hat, muss noch einmal drankommen** (Georgs Befund 12.8.:
       »die Action Cards haben abgerundete Ecken und keinen KFB-Ink-Style mehr«). Der Kanon kommt
       asynchron; Flächen, die vorher fertig wurden, behielten still den Rückfall — eine runde Ecke
       sieht nicht nach Fehler aus, sondern nach Entwurf. Also merken und nachziehen.
       *Ein Rückfall ohne Wiedervorlage wird zum Zustand.* */
    if(!INK)OHNE_INK.add(el); else OHNE_INK.delete(el);
...
function install(game,sh){
  ...
  loadInk().then(()=>{
    const nachziehen=()=>{
      for(const cv of sh.querySelectorAll('canvas.ow7ink'))
        if(cv.parentElement&&cv.parentElement._ink)ik.mark(cv.parentElement);
      /* Die Karten der Hand entstehen später als dieser Aufruf — deshalb zusätzlich die Liste
         derer, die ohne Kanon gezeichnet haben, und ein zweiter Blick nach 1,2 s. */
      for(const el of OHNE_INK)ik.mark(el);
      OHNE_INK.clear();
    };
    nachziehen();
  ...
  function paintHero(){
    ...
    const kurz=k=>((info[k]&&info[k].name)||k).slice(0,3);
