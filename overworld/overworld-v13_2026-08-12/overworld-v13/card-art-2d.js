...
async function ensureGrids(){
  ...
  /* **Eine relative Adresse gegen eine `blob:`-Basis wirft** (Standalone-Befund 12.8.).
   `new URL('./x.json', 'blob:…')` ist kein gültiger Aufruf; der Fehler fliegt synchron und
   erreicht den vorhandenen Rückweg nicht. Deshalb: versuchen, und bei Fehlschlag den Kanal des
   Spiels nehmen. *Ein Rückweg, der erst nach dem Wurf greift, ist keiner.* */
  const kandidaten=[];
  try{kandidaten.push(new URL('./card-grids.json',SELF).href);}catch(e){}
  if(window.OW_SRC)kandidaten.push(OW_SRC.ow('card-grids.json'));
  for(const u of kandidaten){
    try{const j=await(await fetch(u)).json();grids=j.grids||{};break;}
    catch(e){console.warn('[card-art] card-grids.json:',u,e.message);}
  }
  if(!grids)grids={};
  return grids;
}
