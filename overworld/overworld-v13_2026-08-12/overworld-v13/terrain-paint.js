...
/* v12-W1 · Das Glitzern gehört seit dem 12.8. `water-kiss.js` (OW_WATER): eine Streufarbe je
   Fluid-Körper, Alpha aus der NEIGUNG statt aus der Höhe, Schelf am Ufer aus derselben Farbe.
   Hier bleiben nur Clip und Ausschnitt — die Küstenkontur ist dieselbe wie für Fläche und Feder,
   und genau deshalb steht sie hier und nicht im neuen Modul (eine Kontur, mehrere Leser).
   Fällt `OW_WATER` aus, zeichnet der alte Rauschweg weiter (weiß, ohne Schelf): das Spiel läuft
   mit einem schlechteren Wasser weiter statt gar nicht. */
function drawWater(ctx,o,t){
  ...
  if(w<=0||h<=0)return;
  const P2=ringsPath2D(m.coast.land,T);
  ctx.save();
  ...
  ctx.clip(P2.inv,'evenodd');
  const rect={x:px0*T,y:py0*T,w,h};
  if(window.OW_WATER){
    OW_WATER.schelf(ctx,P2.solid,{fluid:o.fluid||'wasser',TILE:T});
    OW_WATER.glitzer(ctx,{fluid:o.fluid||'wasser',TILE:T,time:t,rect});
    ctx.restore();
    return;
  }
...
window.OW_TERRAIN={version:'tp-v4.6',S,P,LAYERS,masks,draw,drawInk,drawWater,fillZone,INK_STYLES,
