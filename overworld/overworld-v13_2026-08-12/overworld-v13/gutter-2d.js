...
/* v12-B1: `torLoch()` stand hier — sie schnitt ein Feld aus dem Graben, damit die Brücke
   darauf liegen konnte. Sie ist RAUS, nicht auskommentiert: die Brücke ist seit v12-B1 zwei Felder
   lang und überdeckt beide Linien, das Wasser läuft durch. Eine Funktion ohne Leser ist die nächste
   falsche Fundstelle — der Rückweg steht im Changelog, nicht als toter Code im Modul. */
function draw(ctx,o){
  ...
  for(const z of zones){
    ...
    n++;
    ctx.save();
    /* v12-B1 · DAS TOR IST KEIN LOCH MEHR. Hier wurde ein Feld aus dem Graben ausgeschnitten, damit
       die Brücke daraufliegen kann — und darunter kam Gras zum Vorschein (Georgs Bild 12.8.:
       »natürlich kein grünes Gras darunter!«). Das Wasser läuft jetzt durch, die Planke liegt
       darüber und ist zwei Felder lang. Damit gilt hier dieselbe Regel wie im Ink-Slice:
       **eine schwarze Linie wird nie unterbrochen, nur überdeckt — die Holzbrücke ist die Tür.**
       (`torLoch` und `ringPfadInvers` sind mit dem Loch gefallen.) */
    ...
    ctx.save();
    /* v12-B1: hier stand derselbe Ausschnitt noch einmal, für die Linie statt für die Füllung.
       Er ist mit dem Loch gefallen — die Linie läuft jetzt durch und wird von der Planke überdeckt. */
