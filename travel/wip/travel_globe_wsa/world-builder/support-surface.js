...
export function createSupportSurfaceResolver({ terrainRadiusAt, bodyHeight = 0.022 } = {}) {
  ...
  const params = {
    probeHeight: bodyHeight * 14,
    belowTolerance: bodyHeight * 0.08,
    minUpDot: 0.24,
  };
  ...
  function registerObject(id, object, options = {}) {
    ...
    const entry = {
      ...
      // A0 semantics are metadata/tie-break input only. Geometric height still wins.
      ...
      normalPolicy: options.normalPolicy ?? null,
  ...
  function terrainRadius(direction) {
    const value = typeof terrainRadiusAt === 'function' ? terrainRadiusAt(direction) : 5;
  ...
  function resolve(direction, baseRadius = null) {
    stats.queries++;
    n.copy(direction).normalize();
    const terrain = Number.isFinite(baseRadius) ? baseRadius : terrainRadius(n);
    const top = terrain + params.probeHeight;
    origin.copy(n).multiplyScalar(top);
    inward.copy(n).multiplyScalar(-1);
    ray.set(origin, inward);
    ray.near = 0;
    ray.far = params.probeHeight + params.belowTolerance;
    ...
    let best = {
      ...
      kind: 'terrain',
      id: 'terrain',
      point: n.clone().multiplyScalar(terrain),
      priority: Number.NEGATIVE_INFINITY,
      roles: ['support', 'walkable'],
      normalPolicy: 'baked-terrain',
      surfaceId: 'terrain',
      recipeId: null,
    };
    ...
    for (const entry of entries.values()) {
      ...
      const hits = ray.intersectObject(entry.object, true);
      ...
      for (const hit of hits) {
        ...
        // A registered support must actually raise the feet above baked terrain. A0 priority only
        // resolves near-coincident supports; it never allows a lower surface to beat a higher one.
        ...
        best = {
          ...
          roles: [...entry.roles],
          normalPolicy: entry.normalPolicy,
  ...
  return {
    name: 'wb0-support-surface-resolver',
    params,
    registerObject,
    unregister,
    resolve,
