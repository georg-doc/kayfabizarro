...
export function createGroundController({ THREE, camera, globe, renderer, bodyHeight = 0.022 }) {
  ...
  const cameraForward = new THREE.Vector3(), cameraSurfaceDir = new THREE.Vector3();
  ...
  const params = {
    ...
    cameraPitchMin: -0.35,
    cameraPitchMax: 0.55,
    cameraTerrainClearance: bodyHeight * 1.35,
  ...
  function resolveSupport(n) {
    ...
    if (typeof supportResolver !== 'function') {
      return { radius: terrain, terrainRadius: terrain, delta: 0, source: 'terrain', kind: 'terrain', id: 'terrain' };
    }
    try {
      ...
      if (Number.isFinite(candidate)) {
        ...
        return {
          radius, terrainRadius: terrain, delta: radius - terrain,
  ...
  function supportRadiusAt(n) { return resolveSupport(n).radius; }
  ...
  function clampCameraAboveTerrain(position) {
    const length = position.length();
    if (length < 1e-8) return position;
    cameraSurfaceDir.copy(position).multiplyScalar(1 / length);
    const minimum = terrainRadiusAt(cameraSurfaceDir) + params.cameraTerrainClearance;
    if (length < minimum) position.multiplyScalar(minimum / length);
    return position;
  }
  ...
  function syncCamera(dt, snap = false) {
    ...
    clampCameraAboveTerrain(desiredCam);
    ...
    if (snap || !snappedCamera) {
      ...
      clampCameraAboveTerrain(camera.position);
    }
    camera.up.copy(up);
  ...
  function stateSnapshot() {
    return {
      ...
      camera: {
        ...
        terrainClearance: params.cameraTerrainClearance,
      },
    };
  }
  ...
  function update(dt) {
    ...
    // Human browser correction: Q must strafe left and E right in the actual tangent/camera frame.
