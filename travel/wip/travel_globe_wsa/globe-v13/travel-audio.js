...
// ⚠ **Eine relative Adresse gegen eine `blob:`-Basis WIRFT** — und zwar synchron, hier beim Bauen
// der Versuchsliste, also noch vor dem `try`. Im Standalone-Bündel ist jedes Modul eine Blob-URL;
// ohne diesen Fang stirbt der ganze Jukebox-Ladeversuch, RAW-Zeile inklusive, an der LOKALEN
// Adresse. Dieselbe Naht wurde in diesem Projekt am 12.8. schon zweimal gezogen (hud-v7.js,
// card-ink-2d.js): ein Rückweg, der erst NACH dem Wurf greift, ist keiner.
const LOCAL = (p) => {
  try { return new URL(p, import.meta.url).href; }
  catch (e) { return (window.__KFB_BASE || '') + String(p).replace(/^\.\//, ''); }
};
