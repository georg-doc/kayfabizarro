...
export function createRadialPost(opts = {}) {
  ...
  return {
    ...
    /**
     * v5 · Slice C · **Vorwärmen.** Dieser Pass rendert nur, wenn `strength > eps` — also wird
     * sein Shader **beim ersten Kartendurchflug** kompiliert, mitten im Moment, den er schmücken
     * soll. tinyskies wärmt genau deshalb seinen Collect-Burst beim Laden vor (`Game.ts:1527`).
     * Ein Aufruf, ein Compile, danach kostet der erste Treffer nichts mehr.
     * Gibt die Zeit in Millisekunden zurück — eine Vorwärmung ohne Zahl ist eine Behauptung.
     */
    praeludium() {
      const t = performance.now();
      try { renderer.compile(qScene, qCam); } catch (e) { return -1; }
