...
/* Slice D · v5 · **Die Impulssprache des Pets als Tabelle.**
 *
 * Was hier freigelegt ist: die Kick-Beträge (das Vokabular, das `fx-script.js` seit Slice B von
 * außen benutzt), die Sprungphysik, die Squash-Klemmen, die Barrel-Roll-Dauer, das Atmen und die
 * Schrittlängen. Ihr gemeinsames Merkmal: **eine Kaskade kann sie nennen.** `kick(-0.14)` in der
 * Partitur und `kick(-0.18)` beim Boost sind dieselbe Sprache — sie muss an EINER Stelle stehen,
 * sonst driften Partitur und Modul auseinander.
 *
 * ⚠ **Was hier NICHT freigelegt ist, und das gehört gesagt:** die Federkonstanten der
 * Trägheitsneigung (`sp(roll, …, 120, 0.72)` und ihre drei Geschwister) und die Zahlen im
 * Lauf-Modus. Das sind Dutzende Werte in zwei langen Pfaden, und sie einzeln zu benennen wäre in
 * diesem Slice eine Umschreibung mit Änderungsrisiko und ohne Nachfrage. Ein Slice, der behauptet
 * „acht von acht" und dabei die Hälfte still auslässt, ist schlimmer als einer, der seine Grenze
 * nennt. Vorgemerkt für den Tag, an dem jemand daran drehen WILL.
 */
export const PETKIN_QUELLE = Object.freeze({
  // Kick-Beträge · + streckt, − duckt
  kickGain: 10,          // war ein Literal: `flSq.v += a * 10`
  kickBoostAn: -0.18,
  kickBoostAus: 0.08,
  kickSteigen: 0.09,
  kickSprung: 0.16,
  kickLandungMin: 0.14,  // Flug-Landung: 0,14 + Aufprall·0,030, geklemmt auf 0,14…0,38
  kickLandungMax: 0.38,
  kickLandungGain: 0.030,
  // Sprungphysik in kartenlokalen Einheiten
  schwerkraft: 26,
  absprung: 5.0,
  ausholen: 0.10,        // Anticipation-Fenster — das Ducken IST das Signal
  // Squash
  // ⚠ **A13 (D-08) ist geprüft und ABSICHT — 1.9., nichts geändert.** Die Notiz fragte, ob die
  // zwei Klemmen übereinander ein Versehen sind. Sie klemmen **zwei verschiedene Größen**:
  //   `federMin/federMax` klemmt die FEDER allein (`flSq.s`),
  //   `squashMin/squashMax` klemmt das PRODUKT in `applySquash` — Feder × Ruheatem (`motionSy`,
  //   von `PetMotion` geschrieben) × Flugphase × Kurvenabzug.
  // Das Produkt kann die Federgrenzen verlassen, obwohl die Feder in ihnen liegt: 1,28 × Atem ×
  // Phase steht über 1,40, sobald zwei Faktoren gleichzeitig nach oben zeigen. Die äußere Klemme
  // ist also nicht die innere zweimal, sondern die einzige, die das Ergebnis kennt.
  // **Eine doppelte Klemme auf DERSELBEN Größe wäre Redundanz; auf einer Größe und ihrem Produkt
  // ist es die einzige Stelle, an der Volumen garantiert werden kann.** Bleibt.
  squashMin: 0.66, squashMax: 1.40,     // harte Volumenklemme in `applySquash`
  federMin: 0.78, federMax: 1.28,       // Klemme auf die Squash-Feder selbst
  federStiff: 80, federDamp: 11,
  // Atmen im Reiseflug
  bobHz: 2.4, bobAmp: 0.035,
  // Barrel-Roll
  rollDauer: 0.85,
  // Lauf-Modus: Schrittlängen (Weltmaß je Zyklus)
  strideWalk: 2.7, strideRun: 4.7,
  kickWalkSprung: 0.30, kickWalkPrell: 0.11,
  kickWalkLandMin: 0.20, kickWalkLandMax: 0.52,
});

export function createPetKinetics(opts = {}) {
  /** Standard = die gemessenen Werte dieser Baureihe. `abweichungen()` sagt, was davon abweicht. */
  const P = Object.assign({}, PETKIN_QUELLE, opts.params || {});
  ...
  // Schwerkraft und Absprung stehen in `P` (Slice D) — hier bleibt nur der Verweis.
  ...
  const br = { active: false, t: 0, dur: P.rollDauer, angle: 0, on: true };
  function rollOnce(dur) {
    if (!br.on || br.active) return false;
    br.active = true; br.t = 0; br.dur = dur || P.rollDauer;
    return true;
  }
  const kick = (a) => { flSq.v += a * P.kickGain; };    // + stretch, − duck
  ...
  const applySquash = (ch, bs, sY) => { const s = clampf(sY, P.squashMin, P.squashMax), xz = 1 / Math.sqrt(s); ch.inner.scale.set(bs * xz, bs * s, bs * xz); };
  ...
  function trigger() {
    ...
    jump.wind = P.ausholen;                     // anticipation window; the duck IS the cue
  }

  function update(pet, st, dt) {
    if (!pet || !pet.character || !pet.character.inner) return;
    const ch = pet.character, bs = ch._baseS || 1;
    bobT += dt; const bob = Math.sin(bobT * P.bobHz) * P.bobAmp;

    // --- squash impulse kicks (SOFTENED so climb/dive no longer flattens the pet) ---
    if (st.boosting && !prevBoost) { kick(P.kickBoostAn); rollOnce(); }
    if (!st.boosting && prevBoost) kick(P.kickBoostAus);
    prevBoost = st.boosting;
    const cs = st.climbIn > 0.10 ? 1 : st.climbIn < -0.10 ? -1 : 0;
    if (cs !== prevClimb) { if (cs > 0) kick(P.kickSteigen); else if (cs < 0) kick(-P.kickSteigen); }
    prevClimb = cs;
    ...
    if (jump.wind > 0) {
      ...
      if (jump.wind <= 0) {
        jump.wind = 0; flyCrouch = 0; jump.active = true; jump.vy = P.absprung; kick(P.kickSprung);
        if (pet.rig && pet.rig.blinkNow) pet.rig.blinkNow();
      }
    } else if (flyCrouch > 0) flyCrouch = Math.max(0, flyCrouch - dt * 8);
    if (jump.active) {
      jump.y += jump.vy * dt; jump.vy -= P.schwerkraft * dt;
      if (jump.y <= 0) {
        const hit = Math.abs(jump.vy);
        jump.y = 0; jump.vy = 0; jump.active = false;
        kick(-clampf(P.kickLandungMin + hit * P.kickLandungGain, P.kickLandungMin, P.kickLandungMax));
    ...
    flSq.v += (1 - flSq.s) * P.federStiff * dt - flSq.v * P.federDamp * dt; flSq.s += flSq.v * dt;
    const motionSy = ch.inner.scale.y / bs;
    applySquash(ch, bs, motionSy * clampf(flSq.s, P.federMin, P.federMax) * flyPhase * (1 - turnMag * 0.06));
  ...
  const FEET = { walk: P.strideWalk, run: P.strideRun };
  ...
  const kickW = (a) => { wSq.v += a * P.kickGain; };
  ...
  function updateWalk(pet, ws, dt) {
    ...
    if (air && !wasAir) {
      kickW(ws.bouncing ? P.kickWalkPrell : P.kickWalkSprung);   // ein Prellsprung holt nicht neu aus
      elastic = 1;
      if (!ws.bouncing && pet.rig && pet.rig.blinkNow) pet.rig.blinkNow();
    }
    if (!air && wasAir) {
      const hit = ws.impact || 0;
      kickW(-clampf(P.kickWalkLandMin + hit * P.kickLandungGain, P.kickWalkLandMin, P.kickWalkLandMax));
      elastic = 1;
  ...
  return {
    ...
    params: P, quelle: PETKIN_QUELLE,
    abweichungen() {
      const a = [];
      for (const k in PETKIN_QUELLE) if (P[k] !== PETKIN_QUELLE[k]) a.push(k + ' ' + PETKIN_QUELLE[k] + '→' + P[k]);
      return a;
    },
    report() {
      const a = this.abweichungen();
      return { parameter: Object.keys(PETKIN_QUELLE).length, abweichungen: a.length, abweichend: a,
               squash: +flSq.s.toFixed(3), sprung: jump.active, rolle: br.active,
               nichtFreigelegt: 'Federkonstanten der Trägheitsneigung + Lauf-Modus (bewusst, siehe Modulkopf)' };
    },
    zeile() {
      const a = this.abweichungen();
      return Object.keys(PETKIN_QUELLE).length + ' params · '
        + (a.length ? '⚠ ' + a.length + ' off default: ' + a.join(', ') : 'all at default')
        + ' · squash ' + flSq.s.toFixed(3) + (jump.active ? ' · JUMPING' : '')
        + (br.active ? ' · ROLLING' : '')
        + ' · lean springs still hard-coded (by decision)';
    },
    /** v4 · Slice B · **Der Squash-Impuls, von außen.** `kick` war intern und wurde nur von den
     *  eigenen Zustandswechseln (Boost, Steigen, Landen) gerufen — kein WELT-Ereignis erreichte
     *  das Pet. D-08 §11.1: „ein gebautes Vokabular, das kein Ereignis aufruft, ist teurer als
     *  ein fehlendes, weil es in der Codebasis wie erledigt aussieht."
     *  Vorzeichen wie intern: **+ streckt, − duckt.** Beträge um 0,06…0,24 (die eigenen
     *  Landungswerte liegen bei 0,14…0,38, das ist die Obergrenze der Sprache). */
    kick(a) { kick(Math.max(-0.5, Math.min(0.5, +a || 0))); },
