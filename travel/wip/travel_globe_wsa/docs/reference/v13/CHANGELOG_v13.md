...
## 10.9.2026 · Der Standalone, geöffnet und repariert

**Kein Code an v13 geändert** — 87 von 87 Modulen sind Zeichen für Zeichen der Stand vom 3.9.
Geändert wurde das **Bündel**, und zwar wegen eines Fundes, den nur das Öffnen liefert.

### Befund · `rift.png` fehlte im Standalone

`portal.js` lädt die Textur an zwei Stellen relativ: `window.__KF`RIFT_URL || "./globe-v13/rift.png"`
und `ldr.load("./rift.png")`. Beide lösen gegen die **Dokument**-Adresse auf, nicht gegen das Modul —
im Bündel gibt es dort nichts. `__KF`RIFT_URL` wurde nie gesetzt. Die Portale liefen also seit dem
3.9. ohne ihre Textur, **ohne dass etwas im Log stand**: ein Texturfehler ist kein Ladefehler.

Warum ausgerechnet diese Datei: `rift.png` stammt aus tinyskies (`client/public/2D/rift.png`, 581×680,
70 % transparent) und hat als einzige kein kanonisches KFB-Gegenstück im Repo. Sie kann nicht per
RAW-URL laufen wie alles andere Schwere — also gehört sie eingebettet.

**Behoben:** `rift.png` (120 kB) als data-URL in `window.__KF`RIFT_DATA`, beide Ladestellen lesen sie
zuerst. Im Projekt bleibt die Datei unverändert; die Umschreibung passiert beim Bündeln.

### Gemessen am neuen Bündel

`export/kfb-travel-globe-v13_2026-09-10/` · 2698 kB (vorher 2537) · `__KF`BUNDLE` meldet
**87 Module, ok: true, fehler: null** · Portal sichtbar im Bild, Contrails laufen.

### Was die Blob-Kette richtig macht

Die Globe-Linie trägt die Quellen als **base64 in einem JS-String**, nicht als JSON in einem
`<script type="application/json">`. Damit kann eine Modulquelle, die in einem Kommentar ein
schließendes Script-Tag enthält, das Bündel nicht kippen — genau daran ist die terrain-Linie am
10.9. einmal gescheitert (Naht 180 dort). Für künftige Bündel ist das Globe-Verfahren das robustere.
