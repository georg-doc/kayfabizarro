...
// ⚠ **Pfad-Hygiene, 30.8. (Standalone-Befund).** Hier stand nur
// `new URL(p, import.meta.url).href` — und das WIRFT, sobald das Modul aus einer Blob-URL läuft:
// `blob:https://…` ist keine hierarchische Basis. Gemessen im Standalone-Bündel als
// „Failed to construct 'URL': Invalid URL", mit der Folge „Welt-Deck nicht geladen" — die Welt
// startete, aber ohne Karten.
// Der Rückfall ist `window.__KFB_BASE`, das die Standalone-Hülle setzt. Additiv: im normalen
// Betrieb ändert sich nichts, weil `import.meta.url` dort eine gültige Basis ist.
// Das ist genau der „stille Killer" aus `session-export_v1` §6: ein relativer Pfad, der in der
// Vorschau funktioniert und im Export nicht — und der Fehlschlag betrifft nicht das Laden, sondern
// erst das, was danach fehlt.
const LOCAL = (p) => {
  try { return new URL(p, import.meta.url).href; }
  catch (e) {
    const basis = (typeof window !== 'undefined' && window.__KFB_BASE) || './';
    try { return new URL(p, basis).href; } catch (e2) { return String(p); }
  }
};
...
export function createCardRegistry(opts = {}) {
  ...
  const leerDecks = [];             // v10 · Decks mit Einträgen, aber ohne verwertbare Karte
  ...
  async function loadDeck(packId) {
    ...
    // ⚠ **DRITTE Schreibweise, und sie hat zwölf Decks unsichtbar gemacht** (Abnahme 2.9.).
    // Der Kopf dieser Datei nennt zwei Formen — lang (`cardNumber`/`cardName`) und kompakt
    // (`n`/`t`). Im Repo liegt eine dritte: `num`/`name`. Gemessen: die Werkbank listet 129
    // Decks, das Spiel konnte 117 bauen. Georgs ausdrücklich gewähltes Testwelt-Deck
    // (`anti_rules_toolkit`) war eines der zwölf, und die Datei lädt mit HTTP 200 und 57 Karten —
    // sie fielen nur alle durch `filter(c.n && c.title)`.
    // **Der eigentliche Fehler ist nicht die fehlende Variante, sondern die Stille.** Ein Deck,
    // das null Karten ergibt, sah hier genauso aus wie ein Deck, das gar nicht existiert. Deshalb
    // kommt mit der Variante eine Zählung: die nächste Schreibweise soll sich MELDEN.
    const list0 = list.length;
    // ⚠ **Vierte Variante, und diesmal fehlt das Feld GANZ** (Abnahme 2.9., zweite Runde).
    // Vier Dateien schreiben nur `name, power, lore, grade` — keine Nummer, in keiner Schreibweise.
    // Bei ihnen IST die Array-Position die Kartennummer, und genau das erwartet `cardMapping`
    // ohnehin: `Seite = coverOffset + 1 + floor((cardNumber − 1) / 4)`. Die Anzahlen stimmen mit
    // der Registry (47 bzw. 52 = `cardCount`), die Reihenfolge ist also die kanonische.
    // **Deshalb ist der Index der letzte Rückfall und nicht die erste Wahl:** wo eine Nummer
    // steht, gewinnt sie — eine gemischte oder lückenhafte Reihe darf nicht heimlich neu
    // durchnumeriert werden, sonst zeigt die Karte einen fremden Ausschnitt.
    const cards = list.map((c, i) => ({
      n: c.cardNumber != null ? c.cardNumber
         : (c.num != null ? c.num : (c.n != null ? c.n : i + 1)),
      title: c.cardName || c.name || c.t || '',
      power: c.power || c.p || '',
      lore: c.lore || c.l || '',
      deck: rd.title, packId, role: rd.role, gameMode: rd.gameMode,
    })).filter((c) => c.n && c.title);
    if (list0 && !cards.length) {
      leerDecks.push(packId);
      console.warn('[cards] „' + packId + '": ' + list0 + ' Einträge, 0 verwertbar — unbekannte '
        + 'Schlüssel: ' + Object.keys(list[0] || {}).join(', '));
    }
  ...
  function pump() {
    ...
    // pdf.js rendert intern per rAF — im versteckten Tab kommt nichts. Der Wecker dazu steht
    // weiter unten (`visibilitychange` → `pump()`), er war immer da.
    // ⚠ Folge fürs MESSEN: in einer verdeckten Seite bleibt `pending` bei 1, und es feuert weder
    // Erfolgs- noch Fehler-Callback. Das ist erwartetes Verhalten dieses Guards, KEIN Hänger — am
    // 27.8. habe ich genau das als Defekt gemeldet und eine falsche Ursache daraus gebaut.
  ...
  return {
    ...
    /** Welche Decks mit Einträgen kamen, aber ohne verwertbare Karte — für die Abnahme. */
    get leerDecks() { return leerDecks.slice(); },
