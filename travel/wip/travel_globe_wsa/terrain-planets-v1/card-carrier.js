...
export function createCardCarrier(opts = {}) {
  ...
  const _ws = new THREE.Vector3();   // Weltskalierung — siehe Versatz der Clip-Ebene in `sync`
  ...
  function sync(st, dt, petChar) {
    ...
    if (clipEnabled) {
      ...
      // ⚠ **Der Versatz entsteht im WELTRAUM und muss deshalb mit der Weltskalierung gehen.**
      // Feste −0,05 gelten für eine 3,0 breite Karte (1,7 % der Breite). Wird die ganze Gruppe
      // uniform verkleinert — so setzt Travel Globe v1 das Rig auf einen Planeten mit Radius 5 ein,
      // Kartenbreite 0,075 —, dann sind dieselben 0,05 plötzlich **67 %** der Kartenbreite: die
      // Ebene liegt weit unter dem Pet und schneidet nie etwas. Gemessen am 27.8.: Ebene 0,0506
      // unter dem Sitz, Pet 0,0236 hoch, Feature inert, und `localClippingEnabled` kostete eine
      // Shader-Variante für nichts. Bei Skalierung 1 (Travel) ändert sich durch `_ws` nichts.
      seat.getWorldPosition(_pt).addScaledVector(_up, -0.05 * lean.getWorldScale(_ws).x);
