...
export function createSettingsOverlay(opts = {}) {
  ...
  // v25.2e · Ein Name, damit die Fokusring-Regel des Runners dieses Overlay treffen kann (sie gilt
  // ausdrücklich nur für die eigenen Bühnen, nicht global — sonst wäre es eine Barriere).
  root.id = 'kfb-settings';
  ...
  // ---------------------------------------------------------------- Sektionen
  const cards = {};
  /* ═══ v25.1 · LIVE-SUCHE ÜBER ALLEM ══════════════════════════════════════════
     Georg, 05.09.: „das ist ja eine komplette Website mit Scrollen und extrem vielen Parametern —
     ich komme da überhaupt nicht mehr durch und wüsste auch gar nicht, wo ich irgendwas einstelle."
     Gemessen: dieses Panel trägt rund 200 Regler in 20 Sektionen. Das ist kein Gestaltungsfehler
     an einer Stelle, sondern eine Menge, die man nicht mehr durch Scrollen erreicht.
     Damit eine Suche überhaupt filtern KANN, bekommt jeder Regler eine eigene ZEILE (vorher hingen
     Beschriftung und Eingabe als Geschwister direkt in der Karte — es gab kein Element, das „ein
     Regler" bedeutet, und ohne das kann man nichts ein- oder ausblenden). Das ist der eigentliche
     Umbau; das Eingabefeld ist nur die Oberfläche darauf. */
  const zeilen = [];   // { row, sid, text }
  for (const s of sections) {
    ...
    for (const c of (s.controls || [])) {
      ...
      // Der Suchtext ist Beschriftung + Sektionstitel: wer „Größe" sucht, findet sie auch über
      // „Kampf", und wer den Abschnitt kennt, findet alles darin.
      zeilen.push({ row: row, sid: s.id, text: ((c.label || '') + ' ' + (c.hint || '') + ' ' + s.title + ' ' + (s.hint || '')).toLowerCase() });
    }
  ...
  function highlight(id) {
    if (filter) return;   // gefiltert gibt es keine „aktuelle Sektion" — es gibt Treffer
  ...
  // ── Das Suchfeld. Es sitzt als ERSTES Kind im Scroll-Container und bleibt beim Scrollen oben:
  // eine Suche, die man wegscrollen kann, findet man beim zweiten Mal nicht wieder.
  let filter = '';
  const suchZeile = el('div', 'grid-column:1/-1;position:sticky;top:0;z-index:3;padding:2px 0 8px;'
    + 'background:linear-gradient(' + CARD + ' 62%, rgba(0,0,0,0));');
  const suche = el('input', 'width:100%;box-sizing:border-box;font-family:' + MONO + ';font-size:12px;color:' + INK + ';'
    + 'background:#fff;border:1px solid ' + LINE + ';border-radius:2px;padding:10px 11px;min-height:38px;');
  suche.type = 'search'; suche.placeholder = 'suchen — z. B. größe, augapfel, schritt, karten';
  suche.setAttribute('autocomplete', 'off');
  const trefferZeile = el('div', 'font-family:' + MONO + ';font-size:10px;letter-spacing:.12em;color:' + MUTED + ';padding:5px 2px 0;');
  suchZeile.appendChild(suche); suchZeile.appendChild(trefferZeile);
  panel.insertBefore(suchZeile, panel.firstChild);

  function anwenden() {
    const q = filter.trim().toLowerCase();
    if (!q) {
      for (const z of zeilen) z.row.style.display = '';
      for (const k in cards) cards[k].style.display = '';
      trefferZeile.textContent = '';
      highlight(current);
      return;
    }
    // Alle Begriffe müssen vorkommen (UND, nicht ODER): „gegner größe" soll EINEN Regler finden.
    const teile = q.split(/\s+/);
    const proSektion = {};
    let n = 0;
    for (const z of zeilen) {
      const hit = teile.every((t) => z.text.indexOf(t) >= 0);
      z.row.style.display = hit ? '' : 'none';
      if (hit) { n++; proSektion[z.sid] = (proSektion[z.sid] || 0) + 1; }
    }
    // Eine Sektion ohne Treffer verschwindet ganz — sonst bleibt ihr Titel als leere Hülse stehen
    // und man zählt zwanzig Überschriften, um drei Regler zu finden.
    for (const k in cards) {
      const an = !!proSektion[k];
      cards[k].style.display = an ? '' : 'none';
      cards[k].style.borderLeftColor = an ? AC : LINE;
      cards[k].style.borderColor = an ? AC : LINE;
    }
    trefferZeile.textContent = n === 0 ? 'KEIN TREFFER'
      : n + ' REGLER IN ' + Object.keys(proSektion).length + ' ABSCHNITT' + (Object.keys(proSektion).length === 1 ? '' : 'EN');
    panel.scrollTop = 0;
  }
  suche.addEventListener('input', () => { filter = suche.value; anwenden(); });
  // Escape leert erst die Suche, dann schließt es — zwei Bedeutungen, in der Reihenfolge, die man
  // erwartet. Ohne das wäre das Panel nach jeder Suche zu und die Filterung verloren.
  suche.addEventListener('keydown', (e) => {
    if (e.key === 'Escape' && suche.value) { e.stopPropagation(); suche.value = ''; filter = ''; anwenden(); }
  });
