...
export function createSkyDice(opts = {}) {
  ...
  const P = Object.assign({
    ...
    azOffset: Math.PI,
    /* v25.2c · Georg: „die sky-dice scheinen etwas zu groß und wirken näher als die sky-cards."
       Zwei Ursachen, beide gemessen:
         GRÖSSE · Der Würfel spannt 32 u auf 360 u Abstand = 5,1° Bildwinkel. Eine Himmelskarte
           spannt 11 u auf 150 u Ring = 4,2°. Das FERNERE Objekt war also das größere im Bild —
           damit ist der Abstand nicht mehr lesbar. 26 u ergeben 4,1°: knapp unter der Karte, und
           die 2,4-fache Entfernung liest sich wieder als Entfernung.
         NÄHE · Die Würfelmaterialien standen auf `fog: false`, die Karten auf `fog: true`. Damit
           fehlte die Luftperspektive — das eine Bildmerkmal, an dem man ohne Bewegung Tiefe liest.
           Nicht three's Nebelschalter (der ist an/aus und würde bei Fern 520 rund 63 % der Farbe
           fressen — der Kanon rot/gelb/blau wäre weg), sondern ein eigener Anteil: die Grundfarbe
           wandert um `nebel` zur Horizontfarbe, das Leuchten wird im selben Maß gedämpft. */
    size: 26,           // scheinbare Kantenlänge in Weltunits (Georg, 26.7. · v25.2c 32 → 26)
    nebel: 0.34,        // Anteil Luftperspektive (0 = wie v25.2b, 1 = ganz im Dunst)
    nebelGlow: 0.7,     // wieviel davon auch das Leuchten dämpft
  ...
  /* v25.2c · Die Luftperspektive. Sie braucht die UNVERFÄLSCHTE Grundfarbe je Material, sonst
     mischt der zweite Aufruf auf das Ergebnis des ersten und die Würfel bleichen mit jedem
     Palettenwechsel weiter aus. Also wird die Grundfarbe beim Bauen weggelegt. */
  const nebelFarbe = new THREE.Color(0x1f232e);
  let nebelDaempfer = 1;   // Faktor auf emissiveIntensity, im Frame-Pfad angewandt
  function merken(m) { m.userData.grund = m.color.clone(); if (m.emissive) m.userData.grundEmi = m.emissive.clone(); return m; }
  function nebelAnwenden() {
    const k = Math.max(0, Math.min(1, P.nebel));
    nebelDaempfer = 1 - k * P.nebelGlow;
    for (const m of allMats) {
      if (!m.userData.grund) continue;
      m.color.copy(m.userData.grund).lerp(nebelFarbe, k);
      if (m.emissive && m.userData.grundEmi) m.emissive.copy(m.userData.grundEmi).lerp(nebelFarbe, k * 0.6);
    }
  }
  ...
  (async () => {
    try {
      ...
      nebelAnwenden();   // die neuen Materialien kennen den Dunst noch nicht
      fromGlb = true;
    } catch (e) {
  ...
  function update(dt, camera) {
    ...
    for (const d of dice) {
      ...
      const gi = P.glow * breathe * flash * nebelDaempfer;
  ...
  return {
    ...
    setParams(o = {}) { Object.assign(P, o); if (o.nebel != null || o.nebelGlow != null) nebelAnwenden(); },
    setPalette() {},   // die Würfel tragen ihre eigenen Farben — rot/gelb/blau sind Kanon, keine Laune
    /** v25.2c · Horizontfarbe und Anteil der Luftperspektive. Der Wirt kennt seinen Nebel, dieses
     *  Modul nicht — also sagt er es, statt daß hier eine Farbe geraten wird. */
    setNebel(farbe, anteil) {
      if (farbe != null) { if (Array.isArray(farbe)) nebelFarbe.setRGB(farbe[0], farbe[1], farbe[2]); else nebelFarbe.set(farbe); }
      if (anteil != null) P.nebel = anteil;
      nebelAnwenden();
    },
