...
const PAPER = '#efe6d0';   // v24: `INK` ist mit dem Textblatt gegangen — eine Rückseite hat keine Schrift
...
// v24 · 5.9. · Das Laden und Malen ist nach `card-backside.js` GEZOGEN. Grund: es gibt mehr als
// einen Kartenmaler in dieser Welt, und beim ersten Anlauf habe ich nur diesen hier umgestellt —
// `academy-deck.fieldTexture` trug seinen Textsteckbrief weiter. Eine Regel an einer von zwei
// Stellen ist keine Regel. Der Kopf von card-backside.js erklärt auch, warum es ein eigenes Modul
// sein MUSS und kein Import von hier: diese Datei hängt an academy-deck.js, das wäre ein Zyklus.
export function backsideBereit() { return rueckseiteBereit(); }

// Kartenblatt = **Rückseite**. Das ist der Normalzustand einer nicht aufgedeckten Karte, kein
// Notbehelf. Auf die gejitterte Silhouette geclippt — dieselbe Kontur wie beim Artwork, damit die
// Kante beim Aufdecken nicht springt. `card` wird nicht mehr gelesen; der Parameter bleibt, weil
// vier Aufrufstellen ihn übergeben und eine stumme Signaturänderung schlimmer ist als ein
// unbenutztes Argument.
function cardTexture(THREE, card, seed) {
  ...
  const malen = () => {
    ...
    malRueckseite(g, W, H);
    g.restore();
    // Band ZULETZT: es liegt mittig auf der Kontur, überdeckt also die halbe Papierkante.
    inkTail(g, pts, W, H, seed);
    t.needsUpdate = true;
  };

  malen();
  if (!rueckseiteBereit()) beiRueckseite(malen);
  return t;
}
...
export function createSkyCards(opts = {}) {
  ...
  // v24 · 5.9. · Der `fonts.ready`-Nachschlag ist WEG. Er malte die Blätter neu, sobald der Webfont
  // da war — richtig, solange sie Schrift trugen. Sie tragen jetzt die Rückseite, und die kennt
  // keine Typografie; das Nachmalen hätte drei Dutzend Texturen ohne jeden Unterschied ersetzt.
  // Das Nachreichen des BILDES macht `cardTexture` selbst (`_backWarter`).
