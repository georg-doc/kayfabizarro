...
export function buildSections(ctx) {
  ...
  return [
      // v25 · **Kampf.** Avatar (Pet | 4 Mechs), Mobs, Sekundärwaffe, Kapselprobe, Tor 2 als Knopf.
      { id: 'kampf', title: 'Kampf (v25)', hint: 'Linksklick = Signaturwaffe (halten = Dauerfeuer) · Rechtsklick = Sekundär (Rakete oder Augapfel) · Klick auf Gegner wählt · Tab = Ziel-Cycle. Gegner greifen erst an, wenn du einen von ihnen triffst. ?combat=0 schaltet alles ab.',
        controls: [
          { kind: 'select', label: 'Avatar', options: () => (ctx.combat ? ctx.combat.AVATARE : []).map((a) => ({ v: a.id, l: a.name })),
            get: () => (ctx.combat ? ctx.combat.avatarId : 'pet'), set: (v) => { if (ctx.combat) ctx.combat.mountAvatar(v); } },
          { kind: 'info', label: 'Geladen', get: () => (ctx.combat ? (ctx.combat.mech ? ctx.combat.mech.name + ' · ' + ctx.combat.primary : 'Cube-Pet · stinger') : 'Kampf aus (?combat=0)') },
          { kind: 'slider', label: 'Mech-Größe (lädt neu)', min: 1, max: 4, step: 0.05, get: () => (ctx.combat ? ctx.combat.avatar.params.hoehe : 3.2),
            set: (v) => { if (ctx.combat) { ctx.combat.avatar.params.hoehe = v; if (ctx.combat.mech) ctx.combat.mountAvatar(ctx.combat.mech.id); } }, fmt: (v) => v.toFixed(2) + ' u' },
          // v25/S3 · Die Antwort auf „wirkt zu klein" steht als ZAHL im Panel, nicht als Eindruck:
          // Spielerhöhe gegen den größten geladenen Gegner. Unter ×1,05 fällt `tor()` durch.
          { kind: 'info', label: 'Größenverhältnis', get: () => { if (!ctx.combat) return '—'; const G = ctx.combat.groessen(); return G.gegner ? 'Mech ' + G.mech + ' u : ' + G.wer + ' ' + G.gegner + ' u = ×' + G.faktor : 'kein Gegner geladen'; } },
          { kind: 'slider', label: 'Mobs gleichzeitig', min: 0, max: 4, step: 1, get: () => (ctx.combat ? ctx.combat.mobs.params.mobs : 0), set: (v) => { if (ctx.combat) ctx.combat.mobs.params.mobs = v; } },
          { kind: 'slider', label: 'Gegner-Größe (nächster Spawn)', min: 0.6, max: 2.2, step: 0.05, get: () => (ctx.combat ? ctx.combat.mobs.params.scale : 1.05),
            set: (v) => { if (ctx.combat) { ctx.combat.mobs.params.scale = v; for (const k of ctx.combat.mobs.kinds) { k.hWant = k.h * v; k.sc = k.hRaw > 0.01 ? k.hWant / k.hRaw : 1; } } }, fmt: (v) => v.toFixed(2) + '×' },
          // v25 · 5.9. · Wie nah ein Gegner an die KAMERA darf. Darunter wird seine Ruhelage radial
          // nach außen geschoben — die Antwort auf „man steckt im Modell". Die Zeile „Stand" zählt,
          // wie oft es nötig war; steigt die Zahl in die Tausende, ist der Spawn falsch, nicht der Radius.
          { kind: 'slider', label: 'Gegner halten Kamera frei', min: 0, max: 20, step: 0.5, get: () => (ctx.combat ? ctx.combat.mobs.params.kameraMin : 8),
            set: (v) => { if (ctx.combat) ctx.combat.mobs.params.kameraMin = v; }, fmt: (v) => (v === 0 ? 'aus (sie fliegen durch)' : v.toFixed(1) + ' u Abstand') },
          { kind: 'toggle', label: 'Schwerer Treffer drückt die Musik', get: () => !!(ctx.combat && ctx.combat.seam.params.duckSchwer), set: (on) => { if (ctx.combat) ctx.combat.seam.params.duckSchwer = on; } },
          // ── v25/S5 · Augapfel (Rechtsklick). Die Spur ist der Engpaß, also steht ihr Regler hier
          // und die Pool-Spitze daneben: 64 Plätze, Kreis je 0,9 u (gerechnet auf 19 gleichzeitig
          // lebende Kreise je Wurf, gemessen Spitze 53/64). Wer 0,35 wählt — den Wert aus dem Plan
          // — sieht in „Stand" sofort `Pool voll ×n`: am 05.09. waren es 905 bei zehn Würfen. Das
          // ist die Messung, kein Verbot.
          { kind: 'select', label: 'Sekundär (Rechtsklick)', options: () => (ctx.combat ? ctx.combat.SEKUNDAER : []).map((a) => ({ v: a.id, l: a.name })),
            get: () => (ctx.combat ? ctx.combat.params.secondary : 'rocket'), set: (v) => { if (ctx.combat) ctx.combat.setSekundaer(v); } },
          { kind: 'slider', label: 'Augapfel-Spur: Kreis je', min: 0.35, max: 1.8, step: 0.05, get: () => (ctx.combat && ctx.combat.eye ? ctx.combat.eye.params.kreisAbstand : 0.9),
            set: (v) => { if (ctx.combat && ctx.combat.eye) ctx.combat.eye.params.kreisAbstand = v; }, fmt: (v) => v.toFixed(2) + ' u  ·  ' + Math.round(0.36 * 34 / v * 1.33) + ' Kreise gleichzeitig' },
          { kind: 'button', label: 'Kapselprobe: 10 Würfe zählen', onClick: () => { if (!ctx.combat) return; const r = ctx.combat.kapselprobe(10); if (r) ctx.note('Kapselprobe läuft: ' + r.laeuft + ' × ' + r.waffe + ' auf ' + r.ziel, 5); } },
          // v25.1d · Selbstheilung. Vorgabe AN und ausdrücklich als TEST-Modus (Herleitung an
          // `regen` in combat-host): ein Prototyp, der den Prüfer tötet, wird seltener geprüft.
          // Die Verzögerung ist der Teil, der es trotzdem zu einem Spielwert macht — niemand heilt
          // im Feuer, die Erholung liegt in der Pause danach.
          { kind: 'slider', label: 'Selbstheilung', min: 0, max: 40, step: 1, get: () => (ctx.combat ? ctx.combat.params.regen : 0),
            set: (v) => { if (ctx.combat) ctx.combat.params.regen = v; }, fmt: (v) => (v === 0 ? 'aus — Burnout ist endgültig' : v + ' HP/s  ·  100 HP in ' + (100 / v).toFixed(1) + ' s') },
          { kind: 'slider', label: 'Heilung erst nach Ruhe', min: 0, max: 6, step: 0.2, get: () => (ctx.combat ? ctx.combat.params.regenDelay : 0),
            set: (v) => { if (ctx.combat) ctx.combat.params.regenDelay = v; }, fmt: (v) => (v === 0 ? 'sofort — heilt im Feuer' : v.toFixed(1) + ' s ohne Treffer') },
          { kind: 'info', label: 'Stand', get: () => (ctx.combat ? ctx.combat.zeile() : '—') },
          { kind: 'button', label: 'Tor 2 messen (Konsole)', onClick: () => { if (!ctx.combat) return; const t = ctx.combat.tor(); console.info(t.text + '\n' + t.zeilen.join('\n')); ctx.note(t.text, 5); } },
        ] },
      ...
      { id: 'travel', title: 'Reise',
        ...
        controls: [
          ...
          { kind: 'info', label: 'Drift jetzt', get: () => {
            ...
            // v17/F4 · Der Rutschwinkel in GRAD, nicht nur als Prozent eines unsichtbaren Maßstabs.
            // Das ist die Zahl, die man neben dem Bild nachlesen will: „die Streifen laufen schräg —
            // wie schräg?" Prozent beantwortet das nicht, Grad schon.
            // `driftAmt` ist bereits wrap-sicher gerechnet (voll bei 45°) — daraus die Grad zu
            // holen ist billiger und richtiger, als `heading - vHeading` hier nochmal zu wrappen.
            const deg = Math.round(s.driftAmt * 45 * 10) / 10;
          ...
          // ── v17/F4 · Den Drift SEHEN ─────────────────────────────────────────────────
          // Gefahren wird seit v15 entlang `vDir` — nur zeigte es nichts an. Zwei Regler, weil
          // beide Antworten vertretbar sind und keine davon ein Beschluss sein soll (Regel 4).
          { kind: 'slider', label: 'Streifen folgen dem Kurs', min: 0, max: 1, step: 0.05,
            get: () => ctx.lines.params.driftFollow, set: (v) => ctx.lines.setParams({ driftFollow: v }),
            fmt: (v) => (v === 0 ? 'aus — Ring sitzt mittig (v16)' : Math.round(v * 100) + ' %  ·  Fluchtpunkt wandert mit') },
          { kind: 'slider', label: 'Blick in den Kurs', min: 0, max: 1, step: 0.05,
            get: () => ctx.camRig.params.lookDrift, set: (v) => ctx.camRig.setParams({ lookDrift: v }),
            fmt: (v) => (v === 0 ? 'aus — Blick am Facing (v16)' : v >= 0.95 ? 'ganz am Kurs — die Kurve läuft aus dem Bild' : Math.round(v * 100) + ' % zum Kurs') },
          { kind: 'info', label: 'Fluchtpunkt (Abnahme)', get: () => {
            const o = ctx.lines.driftOffset;
            return 'x ' + o.x.toFixed(3) + '  ·  y ' + o.y.toFixed(3)
              + (Math.abs(o.x) < 0.005 && Math.abs(o.y) < 0.005 ? '  ·  mittig' : '  ·  wandert');
          } },
          ...
          /* v25.2d · Die Größe der Figur am Boden — der Regler, der Georgs „läuft zu schnell für
             seine Größe" beantwortet. Er ändert NICHT das Tempo, sondern die Körperlänge, an der das
             Tempo gemessen wird: bei 1,0 u sind 5,4 u/s fünf Körperlängen je Sekunde (Strampeln),
             bei 3,2 u (= Mech-Höhe) sind es 1,7. Die Zeile „Was das heißt" darunter zeigt die Folge
             sofort als Kadenz und Rutsch. */
          { kind: 'toggle', label: 'Pet-Größe messen (statt Maßstab 1,25)',
            get: () => S.petGroesseMessen, set: (on) => { S.petGroesseMessen = on; } },
          { kind: 'slider', label: 'Pet-Höhe am Boden', min: 0.8, max: 4.5, step: 0.1,
            get: () => S.petBodenH, set: (v) => { S.petBodenH = v; },
            fmt: (v) => v.toFixed(1) + ' u  ·  ' + (v === 3.2 ? 'wie der Mech' : (v / 3.2).toFixed(2) + '× Mech')
              + '  ·  ' + (ctx.walk.params.speed / Math.max(0.1, v)).toFixed(1) + ' Körperlängen/s' },
          /* v25.2f · Die beiden Zahlen, zwischen denen das Gehen entschieden wird (Georg: „eher ein
             Trippeln als Laufen"). Der Clip trägt 1,34 u je Zyklus bei 3,2 u Figur — 0,42
             Körperlängen, wo ein Gehzyklus etwa eine trägt. Der Faktor STRECKT den Schritt (größere
             Schritte, dafür genau dieser Rutsch), die Eichung setzt das Tempo auf Schritt × Clip-Takt
             (2 Hz gehen / 4 Hz rennen). Wer selbst dreht, schaltet die Eichung ab — sonst schreibt
             sie beim nächsten Betreten des Bodenmodus zurück. */
          { kind: 'slider', label: 'Schrittfaktor (Rutsch)', min: 0.6, max: 2.2, step: 0.05,
            get: () => S.schrittFaktor, set: (v) => { S.schrittFaktor = v; },
            fmt: (v) => v.toFixed(2) + '×  ·  ' + (v === 1 ? 'kein Rutsch, kleinste Schritte' : Math.round((v - 1) * 100) + ' % Rutsch für größere Schritte') },
          { kind: 'toggle', label: 'Gangart eichen (Tempo = Schritt × Clip-Takt)',
            get: () => S.gangEichung, set: (on) => { S.gangEichung = on; } },
          { kind: 'slider', label: 'Lauftempo', min: 2, max: 12, step: 0.2, get: () => ctx.walk.params.speed,
            set: (v) => ctx.walk.setParams({ speed: v }), fmt: (v) => v.toFixed(1) + ' u/s  ·  '
              + (v / Math.max(0.05, ctx.petKin.feetReport().stride || 1)).toFixed(1) + ' Zyklen/s' },
          ...
          // v25/S7 · Die gemessene Schrittlänge (`schrittmass.json`, 23 Pets) neben der eingebauten.
          // Umstellen ist ausdrücklich EINE Geste, keine Voreinstellung: 0,37 u gegen 2,70 u ist
          // Faktor 7, und welche der beiden Zahlen bleibt, ist eine Entscheidung über das
          // Laufgefühl — entweder rutschen die Füße oder sie strampeln. Erst ansehen, dann wählen.
          { kind: 'toggle', label: 'Gemessene Schrittlänge benutzen', get: () => ctx.petKin.strideQuelle === 'gemessen',
            set: (on) => { if (!ctx.petKin.setStrideQuelle(on ? 'gemessen' : 'gekoppelt')) ctx.note('Schrittmaß noch nicht geladen', 4); } },
          { kind: 'info', label: 'Schrittmaß', get: () => {
            const r = ctx.petKin.schrittReport();
            if (!r.gemessen) return 'nicht geladen — eingebaut ' + r.eingebaut.walk + ' u/Zyklus';
            return 'gemessen ' + r.gemessen.walk + ' / eingebaut ' + r.eingebaut.walk + ' u  ·  Faktor ×' + r.faktor + '  ·  aktiv: ' + r.quelle;
          } },
          /* v25/S7 · **Was das Schrittmaß fürs Gameplay heißt, als zwei Zahlen** (Georg, 5.9.:
             „ich weiß nicht, was das KONKRET für die Geschwindigkeit und das GAMEPLAY heißt").
             Der Clip-Zyklus hängt an der STRECKE, nicht an der Uhr — deshalb gibt es genau zwei
             Größen, und sie sind zwei Seiten derselben Münze:
               Kadenz  = Tempo / Schrittlänge   → wie oft die Beine je Sekunde durchlaufen
               Rutsch  = Schrittlänge / gemessene Schrittlänge → um wieviel der Fuß über den
                         Boden schleift, weil der Clip weniger Weg macht als der Körper
             Sauber wäre Rutsch ×1,0. Lesbar ist eine Kadenz von etwa 2–4 Zyklen/s. Beides
             gleichzeitig geht nur, wenn Tempo und Körpergröße dazu passen — genau das ist die
             offene Entscheidung, und hier steht sie live an der laufenden Figur. */
          { kind: 'info', label: 'Was das heißt', get: () => {
            const r = ctx.petKin.schrittReport(), f = ctx.petKin.feetReport();
            if (!r.gemessen) return '—';
            const aktiv = r.quelle === 'gemessen' ? r.gemessen.walk : r.eingebaut.walk;
            const stride = Math.max(0.05, aktiv * (f.sizeK || 1));
            const v = ctx.isWalk() ? (f.tempo || 0) : (ctx.walk.params.speed || 5.4);
            const kad = v / stride, rutsch = aktiv / r.gemessen.walk;
            return 'bei ' + v.toFixed(1) + ' u/s: ' + kad.toFixed(1) + ' Zyklen/s'
              + (kad > 4.5 ? ' (strampelt)' : kad < 1.2 ? ' (schleicht)' : ' (lesbar)')
              + '  ·  Fußrutsch ×' + rutsch.toFixed(1) + (rutsch > 1.4 ? ' (sichtbar)' : ' (sauber)');
          } },
      ...
      { id: 'world', title: 'Welt', hint: 'Alles hier baut das Terrain neu — die Änderung greift kurz nach dem Loslassen.',
        controls: [
          ...
          /* v25.2h · WELCHER Schatten gerade liegt. Zwei Systeme können denselben Körper werfen —
             die echte Schattenkarte und die Werfer-Scheibe aus S15 —, und genau das war Georgs
             Doppelschatten. Eine Zeile, die es sagt, macht die Verwechslung unmöglich. */
          /* v25.2j · Die Auflösung, in Zahlen. „Verpixelt" ist damit eine Messung: u je Texel,
             und wie viele Texel der Körper des Läufers bekommt. */
          { kind: 'info', label: 'Schattenschärfe', get: () => {
            const q = ctx.schattenMass();
            if (!q) return 'keine Karte';
            const ab = q.halb * 0.88;
            return '±' + q.halb.toFixed(0) + ' u Box · ' + q.perTexel.toFixed(4) + ' u/Texel · Pet ≈ '
              + Math.round(q.petTexel) + ' Texel · Verlauf ab ' + ab.toFixed(1) + ' u · Kamera '
              + q.camD.toFixed(1) + ' u ' + (ab > q.camD ? '· gedeckt' : '· NICHT GEDECKT');
          } },
          { kind: 'info', label: 'Schattenquelle', get: () => {
            if (!(S.shadowGain > 0.01)) return 'aus';
            const karte = ctx.hatSonnenkarte();
            if (!ctx.isWalk()) return 'Flug: Werfer-Scheibe (Karte)' + (karte ? ' + echter Wurf' : '');
            return karte ? 'Boden: echter Wurf (Scheibe aus)' : 'Boden: Werfer-Scheibe (noch keine Schattenkarte)';
          } },
      ...
      // v20/K1 · **Die Kartencollage.** Zwei Sorten Regler, und die Trennung ist der ganze Slice:
      // die KARTENHAUT liegt auf den Würfeln (Höhenfeld unangetastet, `voxel-terrain.js`), die
      // PAPPAUFSTELLER stehen davor (`card-collage.js`). Die drei Abnahme-Schalter aus dem Handover
      // §7 bleiben ausdrücklich im Panel, weil eine Abnahme ohne Gegenprobe keine ist: Reisskante
      // aus muss nach aufgeklebten Fotos aussehen, Drehung aus muss die Aufsteller von der Seite
      // verschwinden lassen, und ohne ganze Karten darf nichts mehr herausstechen.
      { id: 'collage', title: 'Kartencollage (v20)', wide: true,
        hint: 'Die Landschaft besteht aus Karten: die Würfel tragen Ausschnitte als Haut, davor stehen gerissene Pappaufsteller, und ganz sind nur die echten Reiseziele. Quelle sind dieselben Deck-Seiten, aus denen die Flug-Karten geschnitten werden — sind sie nicht erreichbar, laufen Ersatzmotive, und der Bericht sagt es.',
        controls: [
          { kind: 'info', label: 'Quelle', get: () => {
            const r = ctx.collage.report();
            return (r.quelle === 'registry' ? 'echte Decks' : r.quelle === 'ersatz' ? 'Ersatzmotive' : 'noch nichts geladen')
              + '  ·  ' + r.felder + ' Atlasfelder  ·  ' + r.ausschnitte + ' Ausschnitte  ·  ' + r.orte + ' Orte';
          } },
          { kind: 'info', label: 'Palette aus den Karten', get: () => (ctx.collage.report().palette || []).join(' ') || '–' },
          { kind: 'toggle', label: 'Palette aus den Karten abtasten', get: () => S.cardPalOn, set: (on) => { S.cardPalOn = on; } },
          { kind: 'toggle', label: 'Reisskante (Abnahme: aus = aufgeklebte Fotos)',
            get: () => ctx.collage.params.torn, set: (on) => ctx.collage.setTorn(on) },
          { kind: 'toggle', label: 'Drehung um die Hochachse (Abnahme: aus = von der Seite weg)',
            get: () => ctx.collage.params.turn, set: (on) => ctx.collage.setTurn(on) },
          { kind: 'toggle', label: 'Ganze Karten als Orte',
            get: () => ctx.collage.params.whole, set: (on) => ctx.collage.setWhole(on) },
          { kind: 'slider', label: 'Ausschnitte im Feld', min: 0, max: 220, step: 4,
            get: () => ctx.collage.params.count, set: (v) => ctx.collage.setCount(v) },
          { kind: 'slider', label: 'Tiefenbänder', min: 1, max: 3, step: 1,
            get: () => ctx.collage.params.bands, set: (v) => ctx.collage.setBands(v),
            fmt: (v) => (v === 1 ? 'eines (flach)' : v + ' Bänder') },
          { kind: 'slider', label: 'Belegte Zellen', min: 0.05, max: 0.8, step: 0.01,
            get: () => ctx.collage.params.density, set: (v) => ctx.collage.setDensity(v),
            fmt: (v) => Math.round(v * 100) + ' %' },
          { kind: 'slider', label: 'Kartenhaut auf den Würfeln', min: 0, max: 1, step: 0.02,
            get: () => ctx.terrain.cardParams.mix, set: (v) => ctx.terrain.setCardSurface({ mix: v }),
            fmt: (v) => (v === 0 ? 'aus (v17-Gelände)' : Math.round(v * 100) + ' %') },
          { kind: 'slider', label: 'nur auf den Oberseiten', min: 0, max: 1, step: 0.05,
            get: () => ctx.terrain.cardParams.top, set: (v) => ctx.terrain.setCardSurface({ top: v }),
            fmt: (v) => (v > 0.9 ? 'nur oben' : v < 0.1 ? 'alle Flächen' : Math.round(v * 100) + ' %') },
          { kind: 'slider', label: 'Tönung durch die Weltfarbe', min: 0, max: 1, step: 0.02,
            get: () => ctx.terrain.cardParams.tint, set: (v) => ctx.terrain.setCardSurface({ tint: v }),
            fmt: (v) => Math.round(v * 100) + ' %' },
          { kind: 'slider', label: 'Würfel mit Papier', min: 0.1, max: 1, step: 0.02,
            get: () => ctx.terrain.cardParams.dense, set: (v) => ctx.terrain.setCardSurface({ dense: v }),
            fmt: (v) => Math.round(v * 100) + ' %' },
          { kind: 'slider', label: 'Streifenhöhe an den Wänden', min: 2, max: 24, step: 0.5,
            get: () => ctx.terrain.cardParams.sideRep, set: (v) => ctx.terrain.setCardSurface({ sideRep: v }),
            fmt: (v) => v.toFixed(1) + ' u' },
          { kind: 'button', label: 'Bericht in die Konsole',
            onClick: () => { console.info('[collage] ' + JSON.stringify(ctx.collage.report())); ctx.note('Collage-Bericht steht in der Konsole.'); } },
        ] },
      ...
      { id: 'wellen', title: 'Ringwellen (v16)', wide: true, hint: 'Farbkreise laufen über das Terrain, breiten sich aus und verlaufen — Variante A: kein Gedächtnis, danach ist alles wie vorher. Ausgelöst durch EREIGNISSE, nie durch den Beat: eine Welle beim Aufsetzen liest sich als Ursache, eine Welle im Takt als Deko.',
        controls: [
          ...
          { kind: 'toggle', label: 'Auslöser: Schuss ohne Ziel', get: () => S.rippleShot, set: (on) => { S.rippleShot = on; } },
        ] },
      ...
      { id: 'props', title: 'Props (v16)', wide: true, hint: 'Die Standorte gehören dem Terrain (dieselben wie die grauen Blöcke), die Modelle diesem Abschnitt. Laden die Assets nicht, streuen die Blöcke weiter — ein 404 macht die Landschaft nicht leer, nur ärmer.',
        controls: [
          ...
          /* v25/A5 · Gruppen (rule of three) als Regler statt als Entscheidung (Georg, 5.9.).
             Sie kosten KEINE Draw-Calls — dieselben InstancedMeshes, nur mehr Matrizen. Was sie
             kosten, sind INSTANZEN: bei 1 stehen ~2200 Props auf ~1400 Standorten, also rund 56 %
             mehr Vertex-Arbeit, und zwar zweimal (Haupt- und Schattenpass). Die Zahl daneben ist
             die Messung; ob es fühlbar ist, sagt der Regler in einer Sekunde. */
          { kind: 'slider', label: 'Gruppen (rule of three)', min: 0, max: 1, step: 0.1, get: () => ctx.props.params.cluster,
            set: (v) => { ctx.props.setParams({ cluster: v }); ctx.props.rebuild(ctx.terrain); },
            fmt: (v) => (v === 0 ? 'aus — jeder Standort trägt eins' : Math.round(v * 100) + ' %  ·  ' + ctx.props.report().gesetzt + ' Props auf ' + ctx.props.report().standorte + ' Standorten') },
          ...
          /* v25 · 5.9. · Antwort auf „warum färben die KayKit-Assets anders?" — gemessen: 44 der 57
             Prop-Materialien sind Kenney OHNE Textur (Farbe im Material), 13 sind KayKit mit weißer
             Grundfarbe und einem geteilten 1024²-Atlas. Der Weltton multipliziert bei beiden; nur
             was er multipliziert, ist verschieden. Dieser Regler zieht NUR die texturierten Teile
             auf ihren Helligkeitswert, damit die Weltfarbe bei ihnen dasselbe tun kann. */
          { kind: 'slider', label: 'KayKit-Textur entfärben', min: 0, max: 1, step: 0.05, get: () => ctx.props.params.texFlat,
            set: (v) => ctx.props.setParams({ texFlat: v }),
            fmt: (v) => (v === 0 ? 'aus — Atlas bleibt, wie geliefert (13 von 57 Teilen)' : Math.round(v * 100) + ' % → Weltfarbe färbt auch KayKit') },
          // ── Der Verbieger ─────────────────────────────────────────────────────────────
      ...
      { id: 'controls', title: 'Steuerung', wide: true, hint: 'Beide Travel-Modi in einer Tabelle — dafür ist die Legende aus dem Bild verschwunden. v15: Landen und Starten gehören der Hand (F), Bodenkontakt fliegt weiter.',
        controls: [{ kind: 'keys', rows: [
          ...
          ['J', 'Flug: Pet hüpft'],
          ['Linksklick (halten)', 'v24: Signaturwaffe · Dauerfeuer'], ['Rechtsklick', 'v24: Rakete'],
          ['Klick auf Gegner', 'v24: Ziel wählen'], ['Tab', 'v24: Ziel-Cycle · ohne Gegner: Einstellungen'], ['Esc', 'Einstellungen schließen'],
