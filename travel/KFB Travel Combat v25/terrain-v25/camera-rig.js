...
export function createCameraRig(opts = {}) {
  ...
  const P = Object.assign({
    ...
    // v17/F4 · Wie weit der Vorausblick dem KURS folgt statt dem Facing. 0 = v16 (reines
    // Facing), 1 = reine Fahrtrichtung. Reines `vDir` zeigt im Drift an der Kurve vorbei —
    // man sieht, wo man landet, aber nicht, wo man hinlenkt. Deshalb eine Mischung und ein
    // Regler: was Geschmack ist, wird ein Regler (Workspace-Regel 4).
    lookDrift: 0.45,
  ...
  // v17/F4 · Die Richtung, in die der Vorausblick zeigt: Facing und Fahrtrichtung gemischt.
  // Eigener Vektor, weil `st.forward` dem Controller gehört und hier nichts geschrieben wird.
  const _course = new THREE.Vector3();
  ...
  function updateFly(dt, st, env) {
    ...
    // v17/F4 · Der Vorausblick schaut in den KURS, die Kamera steht weiter hinter dem FACING.
    // Beides zu drehen wäre falsch: dann fährt das Bild im Drift seitlich weg und die Kurve
    // verschwindet aus dem Blick. Das Pad rutscht — die Kamera schaut ihm nach, sie rutscht nicht mit.
    _course.copy(st.forward);
    if (P.lookDrift > 0 && st.vDir) _course.lerp(st.vDir, P.lookDrift).normalize();
    _back.copy(st.forward).negate(); if (yaw) _back.applyAxisAngle(st.up, yaw);
    const camH = 2.6 + h * 1.4 + dist * 0.18 + bp * 0.15;
    _look.copy(st.position).addScaledVector(_course, 2 + h * 3.4).addScaledVector(st.up, 0.8);
