...
const _edgeCanon = 'https://raw.githubusercontent.com/georg-doc/kayfabizarro/main/travel/travel-v16/terrain-v16/edge3.jpg';
...
function edgeTexture(T) {
  if (!_edgeTex) {
    ...
    // bei jedem Laden, für nichts. Die Datei liegt neben dem Modul und im Repo
    // (travel-v16, seit 09.09. bestätigt). Der kanonische Pfad bleibt als Fallback (Standalone-Export hat kein `./` neben sich).
...
export const MOTION_GLSL = /* glsl */`
  ...
  // v24 · 5.9. · WIE STARK TANZT DIESER WUERFEL GERADE? 0..1, ohne Welteinheiten.
  // Georg: 'die props scheinen leicht deformiert zu werden - aber nicht passend zu den
  // dance/bounce-bewegungen'. Die Ursache war eine halbe Kopplung: die Stauchung der Props hing an
  // kfbBobAt, also an der PHASE des Bobs — aber nicht an seiner AMPLITUDE und nicht an den
  // Ruhezonen. In einer Ruhezone (und im Flug ist der Boden unter dem Fahrzeug eine) stand der
  // Boden still, und die Baeume verformten sich trotzdem: eine Verformung ohne Anlass.
  // Die 2,8 sind kein gewaehlter Wert, sondern die Amplitude bei Standardeinstellung
  // (uAmp 0,35 × uMotionGain 1,2 × 8 × voller Energie/Beat) — also 'ein Wuerfel, der voll tanzt'.
  float kfbBeweg(vec2 wxz) {
    // v24 · 5.9. · WURZEL statt linear. Georg: 'die bäume sollen tanzen bzw. die voxel-bewegungen
    // plausibel mit deformer mitmachen'. Linear war die Kopplung korrekt und unsichtbar: bei einem
    // ruhigen Boden (kleine Energie, kein Beat) blieb sowohl der Auftrieb klein ALS AUCH die
    // Stauchung — zwei kleine Zahlen multipliziert sind gar nichts. Die Wurzel hebt den unteren
    // Bereich: bei einem Viertel Tanz gibt es die halbe Stauchung. Null bleibt null, und das ist
    // der Punkt, der erhalten bleiben musste (Ruhezone = keine Verformung).
    float k = clamp(kfbAmp() / 2.8, 0.0, 1.0);
    return sqrt(k) * kfbCalm(wxz);
  }
`;
...
const VERT = /* glsl */`
  ...
  attribute vec3 aCard;    // v20 · x,y = Feld im Kartenatlas · z = Kartenlos (0..1)
  varying vec3 vCard;
  varying vec2 vUv;
...
// v24/S4 · Der Sonnenschatten als GLSL-Block. Die Begründung steht EINMAL, unten im Kopf von
// `FRAG`, wo ein Shader-Leser sie sucht. Hier steht nur, warum es ein geteilter Block ist: beide
// Terrain-Materialien binden ihn ein (`FRAG` und `SCAT_FRAG`), und die Streu-Geometrie steht auf
// DEMSELBEN Boden — sie muss dieselbe Karte mit derselben Vorspannung lesen, sonst liegt ein Stein
// hell in einem dunklen Feld. Zwei Kopien wären zwei Wahrheiten.
const SUN_SHADOW_GLSL = /* glsl */`
  #include <packing>
  uniform sampler2D uSunShadow;
  uniform mat4 uSunShadowMat;
  uniform vec2 uSunShadowTexel;
  uniform float uSunShadowHas, uSunShadowAmt, uSunShadowBias, uSunShadowNOff;
  /** 0 = volle Sonne, 1 = im Schatten. Ortho-Projektion, also ist w = 1 — die Division bleibt
   *  trotzdem stehen, damit die Funktion auch unter einer perspektivischen Schattenkamera stimmt.
   *  Außerhalb der Karte gibt es KEINEN Schatten (0, nicht 1): die Schattenbox ist klein und fährt
   *  mit dem Fahrzeug — würde sie außerhalb 1 liefern, läge die halbe Welt im Dunkeln. */
  float sonnenSchatten(vec3 welt, vec3 N) {
    if (uSunShadowHas < 0.5) return 0.0;
    /* ═══ v25.2p · DER VERSATZ GEHÖRT AN DIE NORMALE, NICHT AN DIE TIEFE ═══════════════
       Georgs Bild zeigt den Schatten unter dem Pet und am Pfeiler **abgetrennt** — mit einer
       harten Stufe an der Kachelkante. Das ist kein Kartenrand (der läuft seit v25.1d aus) und
       kein Auflösungsproblem, sondern der Bias: 0,0016 klingt nach nichts, ist aber ein Anteil
       des TIEFENBEREICHS der Schattenkamera. Gemessen in seiner Sitzung: near 20, far 150, also
       130 u Bereich — 0,0016 × 130 = **0,208 u**. Das Pet ist 1 u hoch; ein Fuß, dessen
       Kontaktpunkt um ein Fünftel der Körperhöhe verschoben wird, steht neben seinem Schatten.
       Und weil der Versatz in der TIEFE liegt, wächst sein sichtbarer Betrag mit der Neigung der
       Fläche: an einer Stufe kippt er auf einmal, das ist die Kante im Bild.
       Richtig ist der Versatz entlang der NORMALEN, in Weltmaß, bemessen an der Texelgröße der
       Karte (der Wirt kennt beide und schickt sie: nOff). Er verschiebt den Abfragepunkt von
       der Fläche weg, nicht den Körper von seinem Schatten — der Kontakt bleibt am Fuß. Der
       Tiefenbias bleibt als Rest gegen Selbstschattierung stehen, aber klein und **mit der
       Neigung skaliert**: wo die Sonne streift, ist ein Texel weiter gestreckt, und genau dort
       (und nur dort) braucht der Vergleich Luft. */
    float sl = clamp(1.0 - dot(normalize(N), normalize(uLightDir)), 0.0, 1.0);
    vec3 wp = welt + normalize(N) * uSunShadowNOff;
    vec4 sc = uSunShadowMat * vec4(wp, 1.0);
    vec3 p = sc.xyz / sc.w;
    if (p.z > 1.0 || p.z < 0.0) return 0.0;
    // v25.1d · **DIE KANTE DER SCHATTENKARTE WAR EINE LINIE IM BILD.** Georgs Befund („Clipping
    // beim Asset-Schatten"), und die Ursache ist gemessen, nicht vermutet: die Schattenkamera ist
    // ein Ortho-Kasten von 80 × 80 u (±40), die Kamera sieht aber bis ~520 u. Wo der Kasten endet,
    // sprang dieser Shader von „voller Schatten" auf „gar keiner" — eine gerade Kante quer über den
    // Boden, die keiner Geometrie entspricht.
    // Die Reparatur ist NICHT ein größerer Kasten: 2048 Pixel auf 80 u sind 25,6 px/u; auf 160 u
    // wären es 12,8, und der Preis wäre ein grober Schatten ÜBERALL, um eine Kante am Rand zu
    // verstecken. Stattdessen läuft der Schatten in den letzten 20 % des Kastens AUS. Dann gibt es
    // keine Kante mehr, sondern einen Verlauf — und ein Schatten, der in der Ferne verschwindet,
    // ist ohnehin das, was Dunst und Nebel dort tun.
    vec2 q = abs(p.xy * 2.0 - 1.0);         // 0 = Mitte des Kastens, 1 = Rand
    float rand = max(q.x, q.y);
    // v25.2l · Band von 0,80 auf 0,88 nach AUSSEN gerückt. Bei ±24 u beginnt der Verlauf damit
    // erst bei 21,1 u statt bei 19,2 u, und nutzbar sind 88 % der Box statt 80 %. Der Verlauf
    // wird dadurch steiler (2,8 u statt 4,7 u bei dieser Box), aber er liegt jenseits des
    // Nahfeldes — und ein Verlauf, der im Nahfeld liegt, ist ein wandernder Ring (Verifier-Fund
    // 06.09.: bei ±12 u begann er 9,6 u vom Läufer, also genau am Kameraabstand).
    float kante = 1.0 - smoothstep(0.88, 0.995, rand);
    if (kante <= 0.001) return 0.0;
    float d = p.z - uSunShadowBias * (1.0 + 3.0 * sl);
    /* ═══ v25.2q · DIE TREPPE IST DAS TEXELRASTER, ALSO WIRD ZWISCHEN TEXELN GELESEN ═════
       Georgs Bild (07.26): eine Diagonale zwischen hell und dunkel, in Stufen von rund vier
       Bildpunkten, mit Einzelpixel-Rauschen entlang der Kante — und sie flackert. Beides ist
       DIESE Funktion, nicht die Geometrie: der Renderer läuft mit antialias und der
       Post-Pass zeichnet direkt auf den Schirm, echte Silhouetten sind also geglättet. Was hier
       treppt, ist eine im Shader BERECHNETE Grenze.
       Die vorige Fassung summierte 3×3 step-Vergleiche. Ein step ist eine Entscheidung je
       Texel — das Ergebnis kann nur zehn Werte annehmen (0/9 … 9/9) und springt an jeder
       Texelgrenze. Bei 0,0234 u je Texel und einer Kamera 9 u davor ist ein Texel etwa sechs
       Bildpunkte groß: genau die Stufenhöhe im Bild. Mehr Taps machen die Treppe breiter, nicht
       weicher.
       Weich wird sie nur, wenn ZWISCHEN den Texeln gelesen wird. Jeder Block unten vergleicht vier
       Nachbartexel und mischt sie mit dem Bruchteil der Texelkoordinate — das ergibt einen
       stetigen Verlauf über ein Texel statt einer Stufe. Vier solcher Blöcke im Abstand von je
       einem Texel geben die Weichheit des alten 3×3 mit stetigem Übergang (16 Lesevorgänge statt
       9 — der Aufpreis für eine Kante, die nicht treppt).
       Das FLACKERN behebt der Shader nicht: es kommt davon, daß die Schattenkamera dem Läufer
       stufenlos folgt und ihr Texelraster damit jedes Bild unter der Welt durchrutscht. Das
       gehört dem Wirt (§ Texelraster in travel-poc.js), der die Kamera auf ganze Texel
       einrastet. Ohne diese Hälfte bleibt eine weiche Kante, die kriecht. */
    vec2 kg = vec2(1.0) / uSunShadowTexel;      // Kartengröße in Texeln
    vec2 uvT = p.xy * kg;
    float s = 0.0;
    for (int y = -1; y <= 1; y += 2) {
      for (int x = -1; x <= 1; x += 2) {
        vec2 c = uvT + vec2(float(x), float(y)) * 0.5;
        vec2 b = floor(c - 0.5) + 0.5;
        vec2 f = c - b;
        float s00 = step(unpackRGBAToDepth(texture2D(uSunShadow, (b + vec2(0.0, 0.0)) / kg)), d);
        float s10 = step(unpackRGBAToDepth(texture2D(uSunShadow, (b + vec2(1.0, 0.0)) / kg)), d);
        float s01 = step(unpackRGBAToDepth(texture2D(uSunShadow, (b + vec2(0.0, 1.0)) / kg)), d);
        float s11 = step(unpackRGBAToDepth(texture2D(uSunShadow, (b + vec2(1.0, 1.0)) / kg)), d);
        s += mix(mix(s00, s10, f.x), mix(s01, s11, f.x), f.y);
      }
    }
    return (s / 4.0) * uSunShadowAmt * kante;
  }
`;

const FRAG = /* glsl */`
  uniform sampler2D uEdge;
  // v20/K1 · die Kartenhaut. EIN Atlas für die ganze Welt (4×4 Felder), das Feld kommt je Würfel
  // aus dem Attribut aCard — also kein Material je Karte und kein zusätzlicher Draw Call.
  // ⚠ Keine Backticks in diesem Block (Naht 111): der GLSL steht in einem Template-Literal.
  uniform sampler2D uCards;
  uniform float uCardHas, uCardMix, uCardTop, uCardTint, uCardStep, uCardDense, uCardSideRep;
  uniform vec3 uPA0, uPA1, uPA2, uPB0, uPB1, uPB2;
  ...
  // ── v24/S4 · ECHTER SONNENSCHATTEN ────────────────────────────────────────────
  // Georg, 5.9.: „eine base-betonten schatten wie bei KFB Travel Globe v13 sehe ich noch nicht"
  // — und auf die Frage nach den Optionen: „echter Shadowmap-Wurf … die eigene shadow-map-slice
  // wäre perspektivisch für uns sowieso gut".
  //
  // Warum das bis heute NICHT ging, stand seit S15 im Kopf von ground-shadow.js: das Pet wirft
  // längst (castShadow steht, die Schattenkarte der Sonne läuft) — aber nichts EMPFÄNGT.
  // ⚠ KEINE Backticks in diesem Block (Naht 111) — daran ist die erste Fassung dieses Kommentars
  // gescheitert: drei Wortmarkierungen beendeten das Template-Literal, in dem der GLSL steht.
  // Dieses Material ist ein rohes ShaderMaterial, und in ein rohes ShaderMaterial baut three seine
  // Shadow-Chunks nicht ein.
  //
  // Der Weg hier ist deshalb NICHT 'lights: true' (das kippt den ganzen Licht-Uniform-Satz in ein
  // Material, das kein einziges davon benutzt), sondern die Karte SELBST lesen: der Wirt gibt uns
  // sun.shadow.map.texture und sun.shadow.matrix als eigene Uniforms, wir projizieren die
  // Weltposition, die dieses Fragment schon kennt, und tasten 3×3 ab. Vier Uniforms statt eines
  // Chunk-Baums — und die Matrix wird als OBJEKT geteilt, nicht kopiert (three schreibt sie im
  // Schattenpass jedes Bild neu; eine Kopie wäre einen Frame alt und der Schatten würde nachziehen).
  ${SUN_SHADOW_GLSL}
  // ── v16/L2d · RINGWELLEN ────────────────────────────────────────────────────────────────────────────────────
  ...
  float castShadow(vec4 c) {
    ...
    return core * (1.0 - smoothstep(8.0, 46.0, dy));   // mit der Höhe weich raus
  }
  void main() {
    ...
    // v20/K1 · **Karten AUF den Würfeln.** Georgs Wahl vom 20.8.: das Höhenfeld bleibt, die
    // Oberfläche wechselt. Die Mischung sitzt HIER, und die Stelle ist dieselbe Überlegung wie bei
    // den Ringwellen: NACH der Palette (die Streuung uSatBase/uSatRange würde sie sonst
    // entsättigen), VOR dem Licht (danach addiert wäre sie ein Aufkleber, der auf Nordhängen so
    // leuchtet wie in der Sonne).
    float kTop = step(0.55, N.y);
    // Oben das Feld ganz, an den Wänden ein senkrechter Streifen aus derselben Kachel: eine Säule
    // ist bis vierzig Einheiten hoch, ein einmal gestrecktes Feld wäre ein Schmierstreifen.
    vec2 kUv = mix(vec2(vUv.x, fract(vWorld.y / max(uCardSideRep, 0.5))), vUv, kTop);
    vec3 kCol = texture2D(uCards, (vCard.xy + clamp(kUv, 0.004, 0.996)) * uCardStep).rgb;
    float kW = uCardHas * uCardMix * mix(1.0 - uCardTop, 1.0, kTop) * step(1.0 - uCardDense, vCard.z);
    // **Die Palette bleibt im Bild**: die Karte wird mit der Weltfarbe getönt, statt sie zu
    // ersetzen. Sonst wäre der Farbwelten-Sprint aus v16 tot — eine Front, die über eine
    // Fototapete läuft, sieht niemand.
    col = mix(col, kCol * mix(vec3(1.0), col * 2.2, uCardTint), clamp(kW, 0.0, 1.0));
    ...
    // v24 · Stärker (0,52 → 0,62) und höher tragend (5–34 → 8–46 u): der Mech fliegt, sein Schatten
    // war bei Reisehöhe schon halb ausgefadet und lag auf hellen Kartenfeldern — also unsichtbar,
    // genau da, wo Georg ihn vermisst hat (4.9., „ground shadow wäre gut").
    float sh = max(castShadow(uCast0), castShadow(uCast1)) * uCastGain;
    lit *= 1.0 - 0.62 * sh;
    // Der echte Wurf multipliziert NACH dem Werfer-Fleck und VOR Glut und Nebel: er ist Licht, das
    // fehlt, keine Farbe, die dazukommt. Beides zusammen kann nicht über 1 gehen, weil beide als
    // Faktor unter 1 wirken — ein addiertes Paar hätte an der Überschneidung schwarze Flecken.
    lit *= 1.0 - sonnenSchatten(vWorld, N);
...
export function createVoxelTerrain(opts = {}) {
  ...
  // Ein weisses Pixel als Platzhalter: `null` in einem Sampler-Uniform bindet three eine
  // Standard-Textur unter, das Bild wäre aber ein Zufall des Renderers. `uCardHas` bleibt 0,
  // solange kein Atlas da ist — dann sieht die Welt aus wie v17, und das ist der Rückweg.
  const blankTex = new T.DataTexture(new Uint8Array([255, 255, 255, 255]), 1, 1);
  ...
  const U = {
    uEdge: { value: edgeTex },
    uCards: { value: blankTex },
    uCardHas: { value: 0 }, uCardMix: { value: 0.72 }, uCardTop: { value: 0.85 },
    uCardTint: { value: 0.6 }, uCardStep: { value: 1 / 6 }, uCardDense: { value: 0.62 },
    uCardSideRep: { value: 7.5 },
    ...
    uLightDir: { value: new T.Vector3(0.4, 1.0, 0.35).normalize() }, uAmbient: { value: 0.40 }, uLightInt: { value: 0.55 },
    uCast0: { value: new T.Vector4(0, 0, 0, 0) }, uCast1: { value: new T.Vector4(0, 0, 0, 0) }, uCastGain: { value: 1 },
    // v24/S4 · Sonnenschatten. `uSunShadow` bekommt beim Start die Blanko-Textur: `sun.shadow.map`
    // entsteht erst im ERSTEN Schattenpass, und `null` in einem Sampler lässt three eine beliebige
    // Standardtextur unterschieben — das Bild wäre ein Zufall des Renderers. `uSunShadowHas` bleibt
    // 0, bis der Wirt eine echte Karte meldet.
    uSunShadow: { value: blankTex }, uSunShadowMat: { value: new T.Matrix4() },
    uSunShadowTexel: { value: new T.Vector2(1 / 2048, 1 / 2048) },
    uSunShadowHas: { value: 0 }, uSunShadowAmt: { value: 0.5 }, uSunShadowBias: { value: 0.00023 },
    // v25.2p · Normalenversatz in WELTMASS (§ `sonnenSchatten`). Erststand = 1,6 Texel der
    // Flugbox (±40 u auf 2048 = 0,0391 u/Texel); der Wirt schickt den passenden Wert, sobald er
    // die Box ändert — die Zahl gehört zur Box, nicht zum Shader.
    uSunShadowNOff: { value: 0.0625 },
  ...
  function makeChunk() {
    ...
    const aCard = new Float32Array(count * 3);
    ...
    geo.setAttribute('aCard', new T.InstancedBufferAttribute(aCard, 3));
    ...
    const sCard = new Float32Array(SC * 3);
    ...
    // Die Streu-Blöcke fahren denselben Vertex-Shader, brauchen also das Attribut — ihr eigener
    // Fragment-Shader liest es nie. Fehlt es, warnt WebGL bei jedem Draw.
    sgeo.setAttribute('aCard', new T.InstancedBufferAttribute(sCard, 3));
    ...
    return { mesh, scatter, cx: 0, cz: 0, aPack1, aPack2, aCard, sPack1, sPack2 };
  }
  ...
  function bakeChunk(ch, cx, cz) {
    ...
    for (let j = 0; j < C; j++) {
      for (let i = 0; i < C; i++, idx++) {
        ...
        // v20/K1 · Welches Kartenfeld dieser Würfel trägt, ist eine reine Funktion der ZELLE —
        // wie die Höhe. Deshalb trägt derselbe Ort nach dem Rebake wieder denselben Ausschnitt,
        // und die Landschaft ist zweimal dieselbe (Handover §7, Punkt 5).
        const o3 = idx * 3, kt = hash2(cxCell, czCell, seed ^ 0xCA7D);
        const tile = Math.min(cardCols * cardCols - 1, (kt * cardCols * cardCols) | 0);
        ch.aCard[o3]     = tile % cardCols;
        ch.aCard[o3 + 1] = (tile / cardCols) | 0;
        ch.aCard[o3 + 2] = hash2(cxCell, czCell, seed ^ 0x0FA5);   // Los: welche Würfel Papier tragen
    ...
    ch.mesh.geometry.getAttribute('aCard').needsUpdate = true;
  ...
  return {
    ...
    get paletteUniforms() {
      return { uPA0: U.uPA0, uPA1: U.uPA1, uPA2: U.uPA2, uPB0: U.uPB0, uPB1: U.uPB1, uPB2: U.uPB2,
               ...
               // v24/S4 · Nebel geteilt. Er FEHLTE hier — und damit hatte der Prop-Shader keinen
               // Nebel-Term: die Würfel verschwanden im Dunst, die Bäume darauf nicht. Georg, 5.9.:
               // „sie sind bis zum horizont farbig sichtbar, was die illusion zerstört". Das war ein
               // fehlender Term, kein Regler — und die Zahl muss DIESELBE sein, sonst enden Boden
               // und Bewuchs an zwei verschiedenen Horizonten.
    ...
    // ---------------------------------------------------------------- v20/K1 · Kartenhaut
    // Der Atlas kommt von aussen (`card-collage.js`), weil die Karten dort geladen und geschnitten
    // werden. Das Terrain besitzt nur die FLÄCHE — es soll nicht wissen, was ein Deck ist.
    setCardAtlas(tex, cols) {
      const c = Math.max(1, Math.round(cols || 4));
      U.uCards.value = tex || blankTex;
      U.uCardHas.value = tex ? 1 : 0;
      U.uCardStep.value = 1 / c;
      if (c !== cardCols) { cardCols = c; if (built) rebakeAll(); }
    },
    setCardSurface(p = {}) {
      if (p.mix != null) U.uCardMix.value = p.mix;
      if (p.top != null) U.uCardTop.value = p.top;
      if (p.tint != null) U.uCardTint.value = p.tint;
      if (p.sideRep != null) U.uCardSideRep.value = p.sideRep;
      // Die Dichte entscheidet, WELCHE Würfel Papier tragen — das Los steht schon im Attribut,
      // also braucht sie kein Rebake.
      if (p.dense != null) U.uCardDense.value = Math.max(0, Math.min(1, p.dense));
    },
    get cardParams() {
      return { has: !!U.uCardHas.value, mix: U.uCardMix.value, top: U.uCardTop.value,
               tint: U.uCardTint.value, dense: U.uCardDense.value, sideRep: U.uCardSideRep.value,
               cols: cardCols };
    },
    ...
    /** v24/S4 · Die Schattenkarte der Sonne annehmen. Der Wirt ruft das JEDEN Frame — `sun.shadow.map`
     *  entsteht erst im ersten Schattenpass, und `sun.shadow.matrix` wird dort jedes Bild neu
     *  geschrieben. Die Matrix wird als OBJEKT übernommen (keine Kopie): eine Kopie wäre einen Frame
     *  alt, und ein Schatten, der einen Frame nachzieht, läuft beim Fliegen sichtbar hinterher. */
    setSunShadow(o) {
      if (!o || !o.map) { U.uSunShadowHas.value = 0; return; }
      U.uSunShadow.value = o.map;
      if (o.matrix) U.uSunShadowMat.value = o.matrix;
      if (o.size) U.uSunShadowTexel.value.set(1 / Math.max(1, o.size.x), 1 / Math.max(1, o.size.y));
      if (o.amount != null) U.uSunShadowAmt.value = Math.max(0, o.amount);
      if (o.bias != null) U.uSunShadowBias.value = o.bias;
      if (o.nOff != null) U.uSunShadowNOff.value = o.nOff;
      U.uSunShadowHas.value = 1;
    },
    get sunShadow() {
      return { an: U.uSunShadowHas.value > 0.5, amt: U.uSunShadowAmt.value, bias: U.uSunShadowBias.value,
               nOff: U.uSunShadowNOff.value, texel: U.uSunShadowTexel.value.x };
    },
    // Damit light-budget.js den Terrain-Pfad MESSEN kann, statt ihn zu erraten: dieser Shader
    // schreibt Display-Raum und hat sein EIGENES Licht — die Szenen-Lampen wissen nichts davon.
    lichtWerte: () => ({ ambient: U.uAmbient.value, lightInt: U.uLightInt.value }),
