...
// v24 · 5.9. · Die Rückseite ist der Platzhalter JEDER Karte — hier wie in sky-cards.js. Das Modul
// hängt an nichts, damit es beide Kartenmaler bedienen kann, ohne einen Import-Zyklus zu bauen.
...
export function fieldTexture(THREE, d, W) {
  ...
  const t = new THREE.CanvasTexture(c);
  t.colorSpace = THREE.SRGBColorSpace; t.anisotropy = 8;
  // v24 · 5.9. · **Hier stand der zweite Textsteckbrief.** Schraffur, Kapitelnummer als
  // Wasserzeichen, Kurzwort, Beispiel-Id, Tag — und der eigene Kommentar sagte: „das Feld ist
  // erkennbar ein Platzhalter". Genau das hat Georg noch im Bild gesehen (5.9.), nachdem
  // `sky-cards.js` schon umgestellt war: „es werden noch texte/platzhalter bei karten angezeigt,
  // statt der KFB backside." Es gibt in dieser Welt mehr als EINEN Kartenmaler — eine Regel an
  // einer von zwei Stellen ist keine Regel.
  //
  // Jetzt: die kanonische Rückseite, auf DIESELBE Kontur geclippt wie das Artwork (`seedFor`
  // unverändert) — die Kante springt beim Aufdecken also nicht. Der Text ist nicht verschoben, er
  // ist WEG: eine Karte, die ihr Kurzwort zeigt, bevor man sie besucht hat, verschenkt das
  // Ereignis, um das es geht. Die Zonenfarbe trägt die Karte weiter über die Voxel-Glyphen und die
  // Tusche — nicht über ihr Blatt.
  const malen = () => {
    g.clearRect(0, 0, W, H);
