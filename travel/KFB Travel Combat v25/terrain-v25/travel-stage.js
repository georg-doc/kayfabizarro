...
// v24 · 4.9. · 2,6 + 1,0 waren (2,6+1,0)/π = **1,146** — eine weiße Fläche im Sonnenlicht war schon
// über dem Anschlag, bevor Prop-Tint und Rim dazukamen. ACES nimmt das nicht als Helligkeit weg,
// sondern als SÄTTIGUNG: genau Georgs „überstrahlt". Jetzt (1,9+0,7)/π = 0,827, gemessen und mit
// Namen in light-budget.js — wer hier dreht, dreht an einer Zahl, die ein Tor prüft.
const sun = new THREE.DirectionalLight(0xfff2e0, 1.9); sun.position.set(30, 60, 20); sun.castShadow = true;
sun.shadow.mapSize.set(2048, 2048); sun.shadow.camera.near = 1; sun.shadow.camera.far = 200;
sun.shadow.camera.left = -40; sun.shadow.camera.right = 40; sun.shadow.camera.top = 40; sun.shadow.camera.bottom = -40;
/* ═══ v25.2p · DER NORMALBIAS WAR ZEHNMAL EIN BAUM ══════════════════════════════════════
   Georg: „Schatten-Clipping für Pet & 3D Props". Für die Props ist DAS die Ursache, gemessen:
   `normalBias = 0.6` verschiebt den Nachschlagepunkt jedes Empfängers um **0,6 Weltmeter** entlang
   seiner Normalen. Ein Baumstamm hat rund 0,2 u Durchmesser — die Abfrage landet also vollständig
   NEBEN dem Stamm, und was zurückkommt, ist der Schatten eines anderen Ortes. An jeder Kante
   wechselt die Normale, und mit ihr springt der Ort: das sind die harten Masken in seinem Bild,
   und sie folgen der Geometrie, nicht dem Licht. Die Props sind seit v24 Empfänger
   (`prop-scatter.js`: castShadow + receiveShadow), also traf es sie voll.
   0,04 u ist der richtige Betrag: knapp über einem Texel der Bodenbox (0,0234 u), weit unter
   jeder Prop-Dicke. Und `bias` geht auf 0 — ein negativer Tiefenbias zieht in die andere Richtung
   (mehr Schatten, also Akne) und arbeitete gegen den Normalversatz, statt mit ihm. */
sun.shadow.bias = 0; sun.shadow.normalBias = 0.04;
sun.name = 'sun';
scene.add(sun, sun.target);
const hemi = new THREE.HemisphereLight(0xe6f0ff, 0x6a6250, 0.7); hemi.name = 'hemi';
scene.add(hemi);
