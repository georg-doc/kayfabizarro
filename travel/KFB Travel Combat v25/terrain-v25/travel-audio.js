...
export function createTravelAudio(opts = {}) {
  ...
  // v24 · **Ton-Namen als Datenzeilen nachreichen** (EINBAU §4). Der Kampf bringt ~50 Schlüssel
  // (`impact.kinetic.metal` …) mit; sie kommen hier ins Manifest, nicht in einen zweiten Mischer.
  // `hasSfx` ist die Gegenprobe für das Tor: welcher Name klingt als Datei, welcher als Synthese.
  async function addSfx(rows) {
    const m = await loadSfxManifest();
    let n = 0;
    for (const id of Object.keys(rows || {})) { if (!m[id]) { m[id] = rows[id]; n++; } }
    if (ctx) for (const id of Object.keys(rows || {})) for (const f of (rows[id].variants || (rows[id].file ? [rows[id].file] : []))) loadSfxBuffer(f);
    return n;
  }
