...
//
// **v17/F4 · DER FLUCHTPUNKT WANDERT.** Bis v16 saß der Streifenring fest auf der Bildmitte —
// die Streifen sagten „schnell", aber nicht „wohin". Im Drift laufen Facing (`forward`) und
// Fahrtrichtung (`vDir`) bis 40° auseinander (`driftMaxGap`), und der Fluchtpunkt der Bewegung
// liegt dann eben NICHT in der Bildmitte. Jetzt verschiebt der Aufrufer den Ringmittelpunkt auf
// die Bildposition von `vDir` — der Ring wandert als Ganzes, jeder Streifen bleibt radial zum
// neuen Mittelpunkt. Das ist der eigentliche Trick: es wird keine Bewegung ERFUNDEN, es wird
// eine gezeigt, die seit v15 gefahren wird und die niemand sehen konnte.
//
//   const lines = createSpeedLines({ THREE, ink: 0xb5642a });
//   lines.setAspect(w / h);
//   lines.update(dt, { heat, pulse, driftX, driftY });   // driftX/Y = NDC des Fluchtpunkts
...
export function createSpeedLines(opts = {}) {
  ...
  const P = Object.assign({
    ...
    look: 'light',
    // v17/F4 · Wie weit der Ring dem Kurs folgt. 0 = Bildmitte wie in v16 (der Rückweg).
    // Der Deckel ist kein Geschmack, sondern Optik: über ~0,5 verlässt der Ring das Bild auf
    // der einen Seite, und die Streifen kommen auf der anderen als Wand herein.
    driftFollow: 0.6, driftMax: 0.42,
    driftDamp: 3.5,      // 1/s — der Fluchtpunkt zieht nach, er springt nicht
  }, opts.params || {});
  ...
  let ofsX = 0, ofsY = 0;   // v17/F4 · geglätteter Ringmittelpunkt im aspekt-korrigierten Raum
  ...
  function update(dt, src) {
    ...
    // v17/F4 · Der Fluchtpunkt. Er kommt in NDC herein (x schon aspekt-behaftet), gerechnet wird
    // im quadratischen Raum — deshalb `* aspect`, dieselbe Umrechnung wie beim Schreiben, nur
    // rückwärts. Bei f = 0 zieht er zur Mitte zurück: ein stehender Ring gehört mittig.
    const cl = (v, m) => Math.max(-m, Math.min(m, v));
    const wantX = f > 0.001 ? cl((src.driftX || 0) * aspect * P.driftFollow, P.driftMax) : 0;
    const wantY = f > 0.001 ? cl((src.driftY || 0) * P.driftFollow, P.driftMax) : 0;
    const k = Math.min(1, dt * P.driftDamp);
    ofsX += (wantX - ofsX) * k; ofsY += (wantY - ofsY) * k;
    ...
    for (let i = 0; i < N; i++) {
      ...
      const cx = ca * r + ofsX, cy = sa * r + ofsY;
  ...
  return {
    ...
    get params() { return P; },
    // v17/F4 · Abnahme-Ausgang: wo der Ring wirklich sitzt, nicht wo er sitzen sollte.
    get driftOffset() { return { x: ofsX, y: ofsY }; },
    ...
    reset() { for (let i = 0; i < N; i++) { active[i] = 0; hide(i * 4); } gOpacity = 0; spawnAcc = 0; ofsX = ofsY = 0; lifeBA.needsUpdate = true; },
