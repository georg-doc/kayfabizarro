...
addEventListener('keydown', (e) => {
  ...
  // v24 · 5.9. · N = **neue Welt**, sofort, ohne Neuladen. Georg: „die möglichen kombos beim testen
  // sehen zu können, ohne das händisch in den settings einzustellen". Ein Panel-Besuch je Kombination
  // ist kein Testen, sondern Buchhaltung — und der Weltwürfel lag bisher nur beim Start.
  if (e.code === 'KeyN' && !e.repeat && ctx.rollWorld) ctx.rollWorld();
  ...
  // v24 · Tab = Ziel-Cycle (WoW), solange der Kampf läuft und Gegner im Blick sind; sonst wie v20.
  if (e.code === 'Tab' && !e.repeat) { e.preventDefault(); if (ctx.combat && ctx.combat.cycleTarget()) return; ctx.settings.toggle('travel'); }
});
