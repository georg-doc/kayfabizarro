...
export function createGearIcon(o = {}) {
  ...
  /* v25.2d · **Wieviel der Zelle der Glyph füllt — als Zahl, nicht als Annahme.** Die Würfel der
     Action-Leiste richten ihre Füllung danach (sie sollen gleich groß aussehen wie das Zahnrad).
     Bisher stand dort 1,5/1,895 fest verdrahtet, und das gilt NUR mit geladenem GLB: fällt das
     entfernte GEAR_ICON.glb aus, steht hier das gebaute Zahnrad mit 1,74 Spannweite (Zahnspitze
     0,87) — die Würfel wären dann 14 % zu klein statt 20 % zu groß. Also melden. */
  let glyphSpan = 1.74;   // gebautes Zahnrad: Zahnspitze 0,87 im Radius
  ...
  (async () => {
    try {
      ...
      gear.add(root);
      source = 'GLB';
      if (o.onGlyph) { try { o.onGlyph(glyphSpan / bildHoehe()); } catch (e2) {} }
      console.info('[gear] GEAR_ICON.glb geladen · Glyph füllt ' + (glyphSpan / bildHoehe()).toFixed(3) + ' der Zelle');
    } catch (e) {
      console.info('[gear] GEAR_ICON.glb nicht geladen (' + ((e && e.message) || e) + ') — gebautes Zahnrad bleibt');
    }
  })();

  // Bildhöhe in Einheiten an der Ebene z = 0: gerechnet aus fov und Abstand, nicht abgeschrieben.
  const bildHoehe = () => 2 * camera.position.z * Math.tan((camera.fov * Math.PI / 180) / 2);
  ...
  /** Anteil der Zelle, den der Glyph füllt (GLB 1,5/1,895 = 0,792 · gebaut 1,74/1,895 = 0,918). */
  const glyphAnteil = () => glyphSpan / bildHoehe();
  return {
    get glyphAnteil() { return glyphAnteil(); },
    name: 'gear-icon', el,
