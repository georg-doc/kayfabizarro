...
// Pfad-Hygiene (v25.2s): dieselbe Kette wie in `voxel-terrain.js` und `hud-cube.js` — lokaler
// Spiegel zuerst, kanonische RAW-URL als Rückfall. Vorher stand hier NUR der lokale Pfad, und das
// ist der Unterschied zwischen Chat-Vorschau und Standalone: neben der einen exportierten Datei
// liegt kein `terrain-v25/`, also lud die Kachel dort nicht und die Glyphen standen texturlos.
// **Die Reihenfolge ist Absicht und kein Versehen:** die RAW-URL antwortet seit 09.09. (die
// Textur lag unter `media/3D_Assets/Textures/` nie im Repo — sie liegt unter
// `travel/travel-v16/terrain-v16/edge3.jpg`, 2543 B, am 09.09. im Baum bestätigt. Genau das war der
// 404. Reihenfolge bleibt lokal-zuerst: in der Vorschau spart das den Netzweg, im Standalone (kein
// `./` daneben) greift der Rückfall und die Kachel steht. Beide Wege enden jetzt auf einem Bild.
