...
export function createPetFacing(opts = {}) {
  ...
  return {
    ...
    /** v25.1c · **Die Zuwendung als DELTA von der Ruhelage** — die eine Zahl, die jeder Körper im
     *  Sitz anwenden kann, egal wie er gebaut ist. `yaw` ist absolut und enthält `P.base` (die
     *  Ruheausrichtung DES PETS); ein Mech hat seine eigene (`vorne`), und wer `yaw` direkt
     *  übernimmt, dreht sich um die Differenz falsch. Das Delta ist neutral: 0 = schaut nach vorn,
     *  ±x = so weit zur Kamera. Damit braucht kein Avatar eine eigene Anpassung. */
    get zuwendung() { return shortAngle(curYaw - P.base); },
    /** v25.1d · **Nur der Anteil, den die REGIE führt.** Georgs Befund: „der Mech dreht sich im
     *  Idle-Flug im Loop rechts/links" — und das war richtig und mein Fehler. Die Zuwendung hat ZWEI
     *  Quellen: `blend` (Neugier, an das Tempo gekoppelt — im langsamen Flug voll offen) und
     *  `arrive` (die Ankunftsregie an einer Karte). Die Kamera schwebt und wippt, also wandert
     *  `yawToCam` dauernd; ein Körper, der der Neugier folgt, pendelt mit.
     *  Für das PET ist genau das gewollt — es schaut zurück, es hat einen Mund. **Ein Fahrzeug
     *  schaut nicht zurück.** Es dreht sich, wenn die Regie es hinstellt, und sonst nach vorn.
     *  Also derselbe Winkel, skaliert mit dem Regie-Anteil an der Mischung: im freien Flug 0, an
     *  der Karte ganz. Immer noch EINE Zahl und kein zweiter Apparat. */
    get zuwendungRegie() {
      const anteil = lastMix > 1e-3 ? Math.max(0, Math.min(1, lastArrive / lastMix)) : 0;
      return shortAngle(curYaw - P.base) * anteil;
    },
