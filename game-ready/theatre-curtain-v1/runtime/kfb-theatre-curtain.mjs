...
class Panel{
  constructor(side,w,h,top,half,nx=26,ny=32){
    ...
    for(let y=0;y<=ny;y++)for(let x=0;x<=nx;x++){
      ...
      if(x<nx&&y<ny){const a=i,b=this.id(x+1,y),c=this.id(x+1,y+1),d=this.id(x,y+1);idx.push(a,b,d,b,c,d)}
    }
    this.geometry=new THREE.BufferGeometry();
    this.geometry.setAttribute('position',new THREE.BufferAttribute(pos,3));
    this.geometry.setAttribute('uv',new THREE.BufferAttribute(uv,2));
    this.geometry.setAttribute('uv1',new THREE.BufferAttribute(uv.slice(),2));
    this.geometry.setIndex(idx); this.geometry.computeVertexNormals();
  }
  ...
  step(dt,t,wind,open){
    ...
    for(let y=0;y<=this.ny;y++)for(let x=0;x<=this.nx;x++){
      ...
      if(this.fixed[i]){q.lerp(this.target(i,open),.88);this.prev[i].copy(q);continue}
      const old=this.prev[i],vx=(q.x-old.x)*.986,vy=(q.y-old.y)*.986,vz=(q.z-old.z)*.978; old.copy(q);
      const v=y/this.ny,phase=q.x*1.8+q.y*.72+t*.86+(this.side==='left'?0.4:1.7),gust=Math.sin(phase)*.64+Math.sin(phase*.43+t*.27)*.36;
    ...
    for(let it=0;it<5;it++){
      ...
      for(const i of this.hooks){this.p[i].lerp(this.target(i,open),.97);this.prev[i].copy(this.p[i])}
    }
    const a=this.geometry.getAttribute('position');
    for(let i=0;i<this.count;i++)a.setXYZ(i,this.p[i].x,this.p[i].y,this.p[i].z);
    a.needsUpdate=true; this.geometry.computeVertexNormals(); this.geometry.getAttribute('normal').needsUpdate=true;
  }
...
export class KFBTheatreCurtain{
  constructor(o={}){
    this.o=Object.assign({texture:'velour_velvet',tint:'#8c3f37',wind:.62,width:8.8,height:5.55,top:2.78,openDuration:2.5},o);
    this.panels=[];this.meshes=[];this.open=0;this.goal=0;this.state='closed-rest';this.time=0;this.acc=0;this.stepDt=1/120;this.last=performance.now();this.stats={impulses:0,backend:'CPU Verlet · WebGL fallback'};
  }
  async mount(el){
    ...
    this.scene.add(new THREE.HemisphereLight(0xe8d0bf,0x211814,.78));
    const key=new THREE.SpotLight(0xffd2b0,82,22,Math.PI*.32,.42,1.65);key.position.set(-3.5,5.8,6.6);key.target.position.set(-.6,.2,0);key.castShadow=true;this.scene.add(key,key.target);
    const fill=new THREE.PointLight(0xb75243,25,14,2);fill.position.set(4.2,1.5,4.5);this.scene.add(fill);
  ...
  async setMaterial(id){
    ...
    maps.forEach((t,i)=>{if(!t)return;t.wrapS=t.wrapT=THREE.RepeatWrapping;t.repeat.set(2.55,2.1);if(i===0)t.colorSpace=THREE.SRGBColorSpace});
    const mat=new THREE.MeshPhysicalMaterial({color:this.o.tint,map:maps[0],normalMap:maps[1],roughnessMap:maps[2],aoMap:maps[3],roughness:.82,metalness:0,sheen:.92,sheenColor:new THREE.Color('#d88c75'),sheenRoughness:.78,side:THREE.DoubleSide});
  ...
  syncRings(){for(const p of this.panels)for(const r of p.rings){const q=p.target(r.i,this.open);r.m.position.set(q.x,this.o.top+.075,q.z*.18+.02)}}
  setState(s){if(s==='open'){this.goal=1;this.state='opening'}else if(s==='close'||s==='idle'){this.goal=0;this.state=this.open>.02?'closing':'closed-wind'}else if(s==='impact')this.impulse({x:0,y:-.2,strength:2.1,radius:1.25});else if(s==='reset')this.reset();return this.state}
