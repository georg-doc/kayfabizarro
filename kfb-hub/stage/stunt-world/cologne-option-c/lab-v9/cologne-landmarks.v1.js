  ...
  for (let s = 0; s < SPANS; s++) {
    ...
    for (const side of [-1, 1]) {
      ...
      for (let i = 0; i < SEG; i++) {
        ...
        const y0 = DECK_Y + RISE * 4 * u0 * (1 - u0);
        const y1 = DECK_Y + RISE * 4 * u1 * (1 - u1);
        const ax = x0 + span * u0, bx = x0 + span * u1;
        const dx = bx - ax, dy = y1 - y0;
        const len = Math.hypot(dx, dy);
        const m2 = new THREE.Mesh(new THREE.BoxGeometry(len, 1.6, 1.6), steel);
        m2.position.set((ax + bx) / 2, (y0 + y1) / 2, side * (W / 2 - 1.5));
    ...
    // Querverband über dem Scheitel
    box(THREE, g, 'brace', span * 0.5, 0.9, W - 2, steel, x0 + span * 0.5, DECK_Y + RISE, 0);
  }
  return g;
}
...
function hauptbahnhof(THREE) {
  ...
  const rib = mat(THREE, 0x6e5a52, { roughness: 0.85 });
  const base = mat(THREE, C.shoulder, { roughness: 0.95 });
  const LEN = 255, W = 64, H = 24;

  box(THREE, g, 'plinth', LEN, 5, W, base, 0, 2.5, 0);

  // Tonnendach: Rippen über einen Halbkreis.
  // Gemessener Fehler der ersten Fassung: die Segmente wurden über eine
  // Euler-Drehung um X ausgerichtet (rotation.x = -atan2(dy,dz) + PI/2). Im Bild
  // ergab das einen aufgefächerten Haufen schräger Platten quer über der Strecke.
  // Ein Balken wird über seine ENDPUNKTE ausgerichtet, nicht über einen Winkel —
  // dieselbe Regel, die der Dom-Spender in seiner beam()-Hilfe schon anwendet.
  const beam = (a, b, thick) => {
    const va = new THREE.Vector3(a[0], a[1], a[2]);
    const vb = new THREE.Vector3(b[0], b[1], b[2]);
    const dlt = vb.clone().sub(va);
    const m2 = new THREE.Mesh(new THREE.BoxGeometry(thick, dlt.length(), thick), rib);
    m2.position.copy(va).add(vb).multiplyScalar(0.5);
    m2.quaternion.setFromUnitVectors(new THREE.Vector3(0, 1, 0), dlt.clone().normalize());
    m2.castShadow = true;
    g.add(m2);
  };
  const RIBS = 14, SEG = 12;
  for (let i = 0; i <= RIBS; i++) {
    const x = -LEN / 2 + (LEN / RIBS) * i;
    for (let k = 0; k < SEG; k++) {
      const a0 = Math.PI * (k / SEG), a1 = Math.PI * ((k + 1) / SEG);
      beam([x, 5 + Math.sin(a0) * H, Math.cos(a0) * (W / 2)],
           [x, 5 + Math.sin(a1) * H, Math.cos(a1) * (W / 2)], 1.1);
    }
  }
...
export function buildGeniusLoci(THREE, anchors, route) {
  ...
  const place = (mesh, anchor, ry, marginM, note) => {
    const holder = new THREE.Group();
    holder.name = mesh.name;
    holder.rotation.y = ry;
    holder.add(mesh);
    const off = clearOfRoute(THREE, mesh, anchor, route, marginM, 30);
  ...
  place(hohenzollern(THREE), anchors.hohenzollern, 0, 14,
    'öffentliche Hauptmaße 409 m / 3 Bogenfelder / Stich 25 m');
