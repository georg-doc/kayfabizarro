  ...
  return {
    ...
    async addPlanets(GLTFLoader, count = 7) {
      ...
      for (let i = 0; i < picks.length; i++) {
        ...
        const el = 0.09 + r2() * 0.30 + (i % 3) * 0.035;
        ...
        for (let attempt = 0; attempt < 12; attempt++) {
          ...
          const v = new THREE.Vector3(
            Math.cos(a2) * Math.cos(e2), Math.sin(Math.max(0.06, e2)), Math.sin(a2) * Math.cos(e2)).normalize();
          if (v.dot(sunDir) <= 0.78) { dirv = v; if (attempt) retries++; break; }
        }
        if (!dirv) continue;
        const scale = (46 + r2() * 74) * (0.75 + r2() * 0.6);
    ...
    async addDice(GLTFLoader, count = 4) {
      ...
      // 62 Einheiten waren im Bild ein Wuerfel, der halb den Himmel fuellte.
      // 22 liest als Himmelskoerper neben den Planeten, nicht als Vordergrund.
      const base = 22 / Math.max(0.001, Math.max(sz2.x, sz2.y, sz2.z));
      for (let i = 0; i < count; i++) {
        const m = proto.clone(true);
        const s = base * (0.6 + r4() * 0.9);
        m.scale.setScalar(s);
        m.traverse(o => { if (o.isMesh) { o.material = o.material.clone(); o.material.fog = false; } });
        const az = i * GOLD * 3.3 + r4() * 0.8;
        const el = 0.20 + r4() * 0.34;
