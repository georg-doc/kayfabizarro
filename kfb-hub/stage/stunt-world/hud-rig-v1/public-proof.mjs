...
const URL='https://kayfabizarro.pages.dev/kfb-hub/stage/stunt-world/hud-rig-v1/';
...
pass('Cloudflare deployed HUD marker',marker,'status='+lastStatus);
let browser;
try{
  ...
  pass('public Stage HTTP OK',response?.ok()===true,'status='+response?.status());
  ...
  await fs.writeFile('hud-stage-evidence/public-browser.json',JSON.stringify({url:URL,checks,pageErrors,consoleErrors,failed,snapshot:s},null,2));
