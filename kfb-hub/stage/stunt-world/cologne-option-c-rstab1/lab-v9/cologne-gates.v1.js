...
function archGate(THREE, p, opts) {
  ...
  const steel = mat(THREE, opts.body, { emissive: opts.body, emissiveIntensity: 0.1, flatShading: false });
  ...
  // gerader Zylinder-Segmente. An jedem Knick trifft eine flache Stirnfläche auf
  // die Seite des nächsten Segments — sichtbare Kerbe statt Kurve, genau wie am
  // Hohenzollern-Bogen. Ein durchgehendes Rohr hat diese Naht nicht.
  const tube = (fn, radius, seg2, mat2) => {
    const pts = [];
    for (let i = 0; i <= SEG; i++) pts.push(new THREE.Vector3().copy(fn(i / SEG)));
    const geo = new THREE.TubeGeometry(new THREE.CatmullRomCurve3(pts), SEG * 2, radius, seg2 || 10, false);
    const m = new THREE.Mesh(geo, mat2);
    m.castShadow = true;
    return m;
  };
  g.add(tube(u => { const lat = -U + 2 * U * u; return crossPoint(p, lat, RISE * 4 * u * (1 - u)); }, opts.thick, 10, steel));
  ...
  g.add(tube(u => crossPoint(p, -U * 0.94 + 2 * U * 0.94 * u, RISE * 4 * u * (1 - u) - 0.75), opts.thick * 0.26, 8, light));
  return g;
}
