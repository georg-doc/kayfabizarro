...
import { buildCity, buildRhine, buildDom, buildGround, setFormLanguage, getFormLanguage, softenDonor, unsoftenDonor, FORM, RAW, PIN } from './cologne-world.v1.js';
import { OrbitControls } from 'three/addons/controls/OrbitControls.js';
import {
  loadVehicle, createDriver, stepDriver,
  applyCamera, VEHICLES, FLOW, REVIEW_MODES, vehicleLift
} from './cologne-play.v1.js';
import { createDrive, createTrails, createDice } from './cologne-drive.v1.js';
import { createCountdown3D } from './cologne-countdown3d.v1.js';
import { buildBillboard, buildBillboardFamily, buildCCTV, renderCardQuarter, CARD_POOL } from './cologne-props.v1.js';
...
export async function boot(canvas, report, opts = {}) {
  ...
  // Formsprache VOR dem Bau setzen — die Gebaeudegeometrie entsteht beim Bauen.
  // §8: Geometrie und Oberflaeche muessen unabhaengig schaltbar sein, darum ein
  // eigener Schalter und keine Kopplung an die Palette.
  setFormLanguage(opts.formLanguage !== 'CURRENT');
  if (opts.formRoundness !== undefined) FORM.K = Math.max(0, Math.min(0.26, Number(opts.formRoundness)));
  if (opts.formMaxRadius !== undefined) FORM.MAX_M = Math.max(0.2, Math.min(6, Number(opts.formMaxRadius)));
  ...
  // GEMELDETER BEFUND (Georg): streifige, flackernde Schatten auf der Fahrbahn,
  // besonders neben der Bande. Gemessen: 2048 Texel auf 840 m Schattenbox sind
  // 0,41 m JE TEXEL — auf einer fast flachen Fahrbahn im streifenden Licht ist
  // das klassische Schatten-Akne, und ein reiner Tiefen-Bias kann sie bei dieser
  // Texelgroesse nicht wegdruecken, ohne Schatten von ihren Objekten zu loesen.
  // normalBias verschiebt die Abfrage entlang der FLAECHENNORMALEN, also genau
  // quer zum Fehler; ein Texel breit reicht.
  ...
  const ground = buildGround(THREE, ctx, route);
  scene.add(ground);
  if (ground.userData.cut) {
    const c = ground.userData.cut;
    say('boden-einschnitt', `Platte aufgeschnitten Stuetzpunkt ${c.fromIndex}–${c.toIndex} · ${c.lengthM} m · Sohle bis ${c.deepestM} m · Bankett ${c.marginM} m · Wand max ${c.maxWallM} m, an den Muendern ${c.mouthWallM.join(' / ')} m`);
  }
  ...
  // Strahler: standardmaessig AN, weil Georg sie ausdruecklich zum Beurteilen
  // von Modell und Farbe wollte. Ueber die Tweaks abschaltbar.
  if (dom.setFloodlights) {
    const fl = dom.setFloodlights(opts.domFloodlights !== false, opts.domFloodLevel);
    say('dom-strahler', `${fl.lamps} Bodenstrahler · ${fl.on ? 'an' : 'aus'} · Pegel ${fl.level}`);
  }
  ...
  // Spender-Weichzeichnung. KEINE neue Topologie — das Netz gehoert KayKit.
  // Was hier weich wird, ist die BELEUCHTUNG der Kante.
  let vehSoft = null;
  // Standard AUS. Georgs Befund: die Weichzeichnung nimmt dem Auto die Lesbarkeit
  // (Fenster/Lichter), also faehrt das Spendermodell in Originalform, bis die
  // Rundung am Modell selbst gemacht ist. Der Schalter bleibt, die Gruppen sind
  // repariert — er luegt jetzt nicht mehr.
  if (opts.softenVehicle === true) {
    vehSoft = softenDonor(THREE, veh.holder);
    say('form-spender', `Auto weichgezeichnet · ${vehSoft.meshes} Netze · Aufblaehung ${vehSoft.inflateM} m · Topologie und Material-Gruppen unveraendert`);
  }
  ...
  // B1 der Addendum: kleine deterministische Tafelfamilie am Streckenrand,
  // drei Kenney-Varianten, beide Seiten, kein Tunnel, keine kritische Kurve
  // (Sperrzone an Start/Ziel bereits im Streuungsfenster ausgespart).
  let billboardFamily = null;
  try {
    billboardFamily = await buildBillboardFamily(THREE, GLTFLoader, route, { seed: 1337, count: 8 });
    scene.add(billboardFamily.group);
    billboardFamily.tintFrames(THREE, C.hot, 0.22);
    say('billboard-family', `${billboardFamily.count}/8 gestellt · Varianten ${billboardFamily.variants.join(', ')}`);
  } catch (e) { say('billboard-family-FAIL', e.message); }

  // Kartentexturen NICHT den Bootvorgang blockieren lassen — derselbe Kniff
  // wie beim Heldenschild (Zeile mit sky.addCards oben): fire-and-forget,
  // parallel statt seriell, das Ladefenster schliesst sofort.
  if (billboardFamily && billboardFamily.boards.length) {
    Promise.all(billboardFamily.boards.map((b, i) =>
      renderCardQuarter(CARD_POOL[i % CARD_POOL.length])
        .then(card => b.setCard(card, 'COVER_CROP'))
        .catch(() => { /* einzelne Karte fehlt -> Tafel bleibt auf dem Anthrazit-Grund */ })
    )).then(() => say('billboard-family-cards', `${billboardFamily.count} Kartentexturen gesetzt`));
  }
  ...
  // Zweiter Versuch fuer 3D-Ziffern: klein (2,4 m), nah am Start, Kanten per
  // mergeVertices+Normal-Offset abgerundet. Default AUS, per setCountdown3D
  // zuschaltbar — das 2D-HUD (Bangers, farbig) bleibt der Normalfall.
  let cd3d = null, cd3dOn = opts.countdown3D === true;
  try {
    cd3d = await createCountdown3D(THREE, GLTFLoader, scene, route.sampleAt(0));
    say('countdown', cd3d.ready
      ? `3D-Ziffern optional (aus) · ${cd3d.builtCount}/3 geladen, Kanten abgerundet · Rueckfall HUD`
      : '2D im HUD, Bangers, farbig animiert — 3D-Ziffern nicht geladen');
  } catch (e) { say('countdown-FAIL', e.message); }
  ...
  // GEMELDETER BEFUND: kurz aus dem Fenster geklickt und zurueck — das Fahrzeug
  // dreht sich endlos weiter und laesst sich nicht mehr einfangen. Ursache: geht
  // der Fokus verloren, WAEHREND eine Taste gedrueckt ist, liefert der Browser
  // nie ein keyup. Die Taste bleibt im Set und lenkt fuer immer. Beim Verlassen
  // des Fensters wird deshalb alles losgelassen — Standardfall jeder Tastatur-
  // steuerung, hier vorher nicht behandelt.
  ...
  // --- Freiflug: Orbit ueber der ganzen Szene ------------------------------
  // Eigener Modus neben den Review-Kameras. Er uebernimmt die Kamera KOMPLETT:
  // solange er laeuft, laeuft applyCamera() nicht. Zwei Besitzer auf derselben
  // Kamera waeren genau der Ruckel-Fehler aus dem Postmortem, nur andersherum.
  const FREE = 'FREE ORBIT';
  const orbit = new OrbitControls(camera, renderer.domElement);
  orbit.enabled = false;
  orbit.enableDamping = true;
  orbit.dampingFactor = 0.08;
  orbit.screenSpacePanning = true;        // Panning in der Blickebene
  orbit.minDistance = 6;
  orbit.maxDistance = 2600;
  orbit.maxPolarAngle = Math.PI * 0.495;  // nicht unter den Horizont kippen
  orbit.zoomSpeed = 0.9; orbit.rotateSpeed = 0.8; orbit.panSpeed = 0.9;
  orbit.touches = { ONE: THREE.TOUCH.ROTATE, TWO: THREE.TOUCH.DOLLY_PAN };
  const enterFree = () => {
    orbit.target.set(driver.x, driver.y + 1.2, driver.z);
    const bx = Math.sin(driver.yaw), bz = Math.cos(driver.yaw);
    camera.position.set(driver.x - bx * 34, driver.y + 16, driver.z - bz * 34);
  ...
  // GEMELDETER BEFUND (Georg): direkt nach dem Start steht der Wagen kurz, dann
  // springt er vor. Ursache ist nicht die Physik — dt ist auf 0,05 s gedeckelt —
  // sondern der erste Bildaufbau: three uebersetzt Shader ERST, wenn ein Material
  // zum ersten Mal gezeichnet wird. Bei 850 Gebaeuden, Himmel, Wasser und Spuren
  // faellt das komplett in die ersten Bilder, die dadurch sekundenlang dauern:
  // gezeichnet wird fast nichts, gerechnet schon — das Bild steht und laeuft dann
  // nach. renderer.compile() holt die Uebersetzung VOR die Schleife, und die Uhr
  // wird danach neu gestellt, damit die Ladezeit nicht als erster Zeitschritt in
  // die Physik faellt.
  try { renderer.compile(scene, camera); } catch (e) { say('compile-FAIL', e.message); }
  ...
  function frame() {
    ...
    if (mode === FREE) orbit.update();
    else applyCamera(THREE, camera, mode, driver, route, ANCHORS.dom, t, cctvMeta);

    if (cd3d) {
      const cdVal = Math.max(0, 3 - (t - raceStart));
      if (cd3dOn && cdVal > 0.001) {
        const n = Math.min(3, Math.ceil(cdVal - 1e-6));
        const frac = Math.max(0, Math.min(1, cdVal - (n - 1)));
        // Gleiche Farblogik wie das 2D-HUD: kuehl (line) bei "3", ueber den
        // Palettenakzent (hot) laufend bis warm-hell bei "1".
        const nrm = 1 - (n - 1) / 3;
        const col = new THREE.Color(C.line).lerp(new THREE.Color(C.hot), Math.min(1, nrm / 0.7))
          .lerp(new THREE.Color(C.hot).lerp(new THREE.Color(0xffffff), 0.4), Math.max(0, (nrm - 0.7) / 0.3));
        if (cd3d.currentDigit !== n) cd3d.show(n, col.getHex());
        cd3d.update(camera, frac);
      } else if (cd3d.currentDigit != null) {
        cd3d.hide();
      }
    }
  ...
  const api = {
    ...
    formLanguage: getFormLanguage() ? 'CARTOON_FORM' : 'CURRENT',
    formRule: `r = ${FORM.K} x kleinste Dimension, ${FORM.MIN_M}\u2013${FORM.MAX_M} m, nur Silhouettenkanten`,
    vehicleSoften: vehSoft,
    scrubCorridor,
    corridorScrub: scrub,
    ...
    setMode(m) {
      mode = m;
      if (m === FREE) enterFree();
      else { orbit.enabled = false; seatCamera(); }
    },
    getMode() { return mode; },
    modes: REVIEW_MODES.concat(cctvMeta.map((c, i) => 'CCTV' + (i + 1))).concat([FREE]),
    // Freiflug programmatisch auf ein Objekt setzen (spaeter: Klick-Fokus)
    focusFree(x, y, z, dist) {
      if (mode !== FREE) return false;
      orbit.target.set(x, y, z);
      const d = dist || 40;
      camera.position.set(x + d * 0.6, y + d * 0.55, z + d * 0.6);
      orbit.update();
      return true;
    },
    cctvMeta,
    setOrb(v) { drive.setEnabled(v); },
    // Domstrahler: Schalter und Pegel. Ereignis, Uhrzeit oder Taste koennen ihn
    // bedienen — Besitzer bleibt das Bauwerk.
    setDomFloodlights(on, level) { return dom.setFloodlights ? dom.setFloodlights(on, level) : null; },
    getDomFloodlights() { return dom.getFloodlights ? dom.getFloodlights() : null; },
    ...
    setCountdown3D(v) { cd3dOn = !!v; if (!cd3dOn && cd3d) cd3d.hide(); },
    getCountdown3D() { return cd3dOn; },
    async setVehicle(id) {
      ...
      scene.add(veh.holder);
      if (opts.softenVehicle === true) softenDonor(THREE, veh.holder);
    ...
    auditRoute(opts) {
      ...
      // DER BODEN, ausdruecklich und gezaehlt ausgenommen — nicht stillschweigend.
      // Die skip-Liste oben ist auf ^ verankert und prueft die Ahnenkette vom Netz
      // nach oben; 'ground-plate' steht dort, greift fuer die KINDER der Gruppe
      // aber nie. Gemessener Grund fuer die Ausnahme (2026-09-22): die Sohle des
      // Einschnitts liegt konstruktiv 3 m UNTER der Fahrbahn und die Waende
      // stehen w/2 + 7 m seitlich — ein seitlicher Strahlentest auf der Fahrraum-
      // Halbbreite (w/2 + 1,5 m) ueber die Stuetzpunkte 75–134 gibt beidseitig
      // NULL Treffer. Was der Scan trotzdem melden wuerde, ist die Huelle: sie
      // umspannt den gebogenen Einschnitt und damit rechnerisch die Fahrbahn —
      // dieselbe Huellen-Luege, die fuer die verschmolzenen oms-* Netze schon
      // dokumentiert ist. Ein Dauer-Fehlalarm auf DIESEM Werkzeug waere teuer:
      // §3 laesst es als ERSTES laufen, wenn jemand "ich fahre in etwas hinein"
      // meldet. Der belastbare Test fuer den Boden ist der Hoehenvergleich in
      // groundCutSpan() plus der Senkrecht-Strahl von der Fahrbahn nach oben.
      const groundByConstruction = /(ground-plate|ground-cut-)/;
      ...
      scene.traverse(o => {
        ...
        if (groundByConstruction.test(all)) { groundExcluded++; return; }
      ...
      return {
        ...
        groundExcluded,
        note: 'oms-* ausgenommen (verschmolzene Netze, siehe city.portals/skipped); Startlinie, Tore und der eigene Antrieb stehen absichtlich im Fahrraum und zaehlen nicht; ' + groundExcluded + ' Bodennetze (ground-plate/ground-cut-*) ausgenommen — Sohle liegt konstruktiv 3 m unter der Fahrbahn, Waende w/2+7 m seitlich, seitlicher Strahlentest 0 Treffer. DER BELASTBARE BODENTEST IST DER STRAHL VON OBEN NACH UNTEN, nicht von der Fahrbahn nach oben: die Platte ist FrontSide und three beachtet material.side beim Raycast — ein Aufwaertsstrahl kann sie GAR NICHT treffen und meldet immer 0 (gemessen 2026-09-22, Stuetzpunkt 110: aufwaerts 0 Treffer, abwaerts 1). Zweite belastbare Probe: der Strahlenfaecher vom Fahrersitz, also sieht man von der Strasse aus Boden.',
    ...
    evidence() {
      return {
        ...
        billboardFamily: billboardFamily ? { count: billboardFamily.count, variants: billboardFamily.variants, source: billboardFamily.source } : null,
        cctv: cctvMeta,
        startGate: startInfo,
