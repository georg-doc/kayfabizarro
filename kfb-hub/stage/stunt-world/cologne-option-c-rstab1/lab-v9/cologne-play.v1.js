...
export async function loadVehicle(THREE, GLTFLoader, spec) {
  ...
  // WIDERLEGTE VERMUTUNG, hier notiert damit sie nicht zurueckkommt: als Georg
  // meldete, der Antriebs-Donut sitze nicht mittig hinter dem Wagen, habe ich
  // zuerst das Spendermodell verdaechtigt und es in seinem Halter zentriert.
  // Die Messung hat das widerlegt — der Hatchback ist seitlich exakt zentriert
  // (Versatz 0,000 m). Die Zentrierung ist deshalb wieder raus: eine Korrektur
  // ohne Ursache verschiebt den Spender nur grundlos.
  //
  // Die echte Ursache lag woanders und ist in cologne-drive.v1.js behoben: die
  // Karosserie wird mit (pitch, yaw, roll) gedreht und um vehicleLift()
  // angehoben, Donut und Spuransaetze standen in einem Rahmen, der nur die
  // Gierung kennt. Bei 14 Grad Ueberhoehung sind das 0,25 m Versatz.
  ...
  return {
    ...
    measured: {
      ...
      recenteredM: null,   // widerlegt, siehe oben
      forwardAxis: '+Z'
    }
  };
}

// Aufbauhoehe, die Nick und Wank mittragen. Der Fahrzeugursprung liegt auf der
// Radunterkante — wird der Wagen gedreht, taucht seine Kante sonst unter die
// Fahrbahn. EINE Stelle, weil Buehne UND Antrieb dieselbe Zahl brauchen:
// stuenden Karosserie und Donut in verschieden hohen Rahmen, waeren sie
// sichtbar versetzt (gemeldeter Befund 2026-09-22).
export function vehicleLift(d) {
  return 0.04 + 2.08 * Math.abs(d.pitch) + 1.08 * Math.abs(d.roll);
}
...
export function stepDriver(d, route, keys, dt, t) {
  ...
  //
  // GEMELDETER BEFUND (Georg): nach einem Bandenkontakt steckt der Wagen in der
  // Fahrbahn. Ursache: `lateral` wurde VOR der weichen Bande und der harten
  // Kante gelesen, die Bezugshoehe also fuer die Lage BERECHNET, die das
  // Fahrzeug gerade verlassen hat. Die harte Kante versetzt es um bis zu
  // mehrere Meter quer; bei 24 Grad Ueberhoehung sind das 0,4 m Hoehenfehler je
  // Meter Versatz, und die Hoehenfeder zieht das nur langsam nach. Also: nach
  // den Korrekturen neu lesen.
  const lateralNow = (d.x - p.x) * p.nx + (d.z - p.z) * p.nz;
  const surfaceY = p.y + Math.sin(p.bank) * lateralNow;
  ...
  // GEMESSEN: die Feder hat eine Zeitkonstante von rund 0,1 s. Auf der
  // Tunnelrampe faellt die Bezugsflaeche bei 41 m/s um etwa 0,3 m je Bild —
  // nach 0,1 s sind das 0,83 m Rueckstand, und genau so weit steckt der Wagen
  // dann in der Fahrbahn. Eine Feder kann einem bewegten Ziel nicht ohne
  // Verzug folgen, wenn sie nur die LAGE sieht.
  //
  // Also: die Aenderung der Bezugsflaeche wird direkt mitgenommen, die Feder
  // glaettet nur noch den Rest. Der Charakter bleibt, der Rueckstand faellt weg.
  if (d.lastSurfaceY !== undefined && d.onGround) d.y += surfaceY - d.lastSurfaceY;
  d.lastSurfaceY = surfaceY;
  if (Math.abs(targetY - d.y) > 2.5) d.y = targetY;   // Versetzung, keine Bewegung
  else d.y += (targetY - d.y) * Math.min(1, FEEL.surfaceSpring * dt / FEEL.surfaceDamping * 2.4);
  ...
  // Die Karosserie liegt auf der Fahrbahn, nicht daneben: der Bankanteil folgt
  // der Ueberhoehung GANZ. GEMESSEN: mit dem alten Faktor 0,75 blieb bei 20,2
  // Grad Ueberhoehung eine Differenz von 10,2 Grad — das hebt bzw. versenkt eine
  // Wagenkante um 19 cm bei 37 cm Radradius. Das Hineinlehnen bleibt als
  // eigener Anteil erhalten (lateralRollGain), es ueberlagert die Lage jetzt nur,
  // statt sie zu ersetzen.
  d.roll += (-d.lateralV * FEEL.lateralRollGain * 6 + p.bank - d.roll) * Math.min(1, dt * 5);
...
// KAMERA AUF DER STRECKENSCHIENE.
//
// Verfahren uebernommen aus FILAMENT #02 — KilledByAPixel/SP13KTRA @166ad838,
// code/game.js updateCamera(), im Original kommentiert mit:
//   "keep the boom above the nearby road without a general scenery collider:
//    project the target onto the route, clamp it inside the walls and never let
//    it sink under the road"
// Dort wird die Wunschlage der Kamera auf die Route projiziert, seitlich in den
// Korridor geklemmt und in der Hoehe auf die Fahrbahn gehoben. Es gibt KEINEN
// Hindernis-Strahl und kein Hindernis-Register.
//
// Das ist der Grund, warum es dort sauber aussieht und bei uns nicht: unsere
// Verfolgerkamera durfte frei im Raum haengen und wurde nachtraeglich per Strahl
// aus Fassaden gezogen — das ergibt genau das Rucken, das Georg meldet (die Kamera
// springt auf 3,5 m heran, zeigt Innenseiten, klemmt unter dem Deck). Eine Kamera,
// die die Roehre gar nicht verlaesst, braucht keine Kollision: der Fahrkorridor ist
// von Bau aus frei, und in Tunneln ist er genau der Raum, den auch das Fahrzeug hat.
//
// Kein Code, keine Zahl aus dem Spender uebernommen — nur das Verfahren.
export function railClamp(THREE, pos, route, driver, opts = {}) {
  const near = route.nearest(pos.x, pos.z, driver.lastIndex || 0);
  const p = near.point;
  const tunnel = p.kind === 'TUNNEL';
  // Seitlich: in Tunneln bis knapp an die Bandkante, im Freien ein Stueck darueber
  // hinaus (dort steht nichts, das Bild wird weiter).
  const side = opts.sideMargin != null ? opts.sideMargin : (tunnel ? 0.6 : 2.4);
  const limit = p.w * 0.5 + side;
  const lateral = (pos.x - p.x) * p.nx + (pos.z - p.z) * p.nz;
  const corr = Math.max(-limit, Math.min(limit, lateral)) - lateral;
  pos.x += p.nx * corr;
  pos.z += p.nz * corr;
  // Hoehe: nie unter die Fahrbahn, in Tunneln nie in die Schale
  // (die Roehre traegt ihren Scheitel 11,5 m ueber dem Band, cologne-track).
  const floorY = p.y + (opts.minHeight != null ? opts.minHeight : 2.0);
  const ceilY = p.y + (opts.maxHeight != null ? opts.maxHeight : (tunnel ? 6.8 : 400));
  pos.y = Math.min(Math.max(pos.y, floorY), ceilY);
  return p;
}
...
export function applyCamera(THREE, cam, mode, d, route, dom, t, cctv) {
  ...
  if (mode === 'CHASE') {
    ...
    const boom = 8.6 + sp * 3.4;
    const want = new THREE.Vector3(d.x - fx * boom, d.y + 3.5 + sp * 0.9, d.z - fz * boom);
    railClamp(THREE, want, route, d);
    // Weit weg heisst neu gesetzt (Start, Reset, Palettenwechsel): dann wird
    // gesprungen, nicht nachgezogen. Sonst kriecht die Kamera ueber die halbe
    // Karte und zeigt unterwegs das Innere der Strecke.
    if (cam.position.distanceTo(want) > 55) cam.position.copy(want);
    else cam.position.lerp(want, 0.14);
    // Auch die geglaettete Lage wird geklemmt — sonst schneidet der Nachlauf in
    // Kurven die Ecke und verlaesst die Roehre wieder.
    railClamp(THREE, cam.position, route, d);
    cam.lookAt(look.x + fx * 11, look.y - 0.2, look.z + fz * 11);
  } else if (mode === 'CLOSE ORBIT') {
    cam.fov = 38;
    const a = t * 0.45;
    const want = new THREE.Vector3(d.x + Math.sin(a) * 14.5, d.y + 4.4, d.z + Math.cos(a) * 14.5);
    railClamp(THREE, want, route, d, { sideMargin: 6 });
    ...
    cam.fov = 40;
    const want = new THREE.Vector3(d.x + fz * 13 - fx * 7.5, d.y + 1.35, d.z - fx * 13 - fz * 7.5);
    // Diese Ansicht steht absichtlich neben dem Band, aber nicht in einem Haus:
    // knapp hinter der Randmauer ist die Grenze.
    railClamp(THREE, want, route, d, { sideMargin: 4.5, minHeight: 0.9 });
