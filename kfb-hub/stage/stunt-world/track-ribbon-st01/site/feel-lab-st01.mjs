...
const ST01=await fetch(new URL('./RACE_TRACK_ST01_CONFIG.json',import.meta.url),{cache:'no-store'}).then(r=>{if(!r.ok)throw new Error(`S-T01 config HTTP ${r.status}`);return r.json()});
...
const UI={
  ...
  hash:$('#trackHash'),surface:$('#surfaceStatus'),width:$('#trackWidth'),widthClass:$('#widthClass'),zone:$('#zoneName'),grid:$('#gridStatus'),cue:$('#cueStatus'),test:$('#selfTest'),
...
function bell01(t,a,b){
  if(t<=a||t>=b)return 0;
  const m=(a+b)/2;
  return t<m?smooth((t-a)/(m-a)):smooth((b-t)/(b-m));
}
function verticalOffsetAtT(t){
  t=mod(t,1);
  const V=ST01.verticalGestures||{},c=V.crest,d=V.dip,r=V.ramp;
  let y=0;
  if(c)y+=(c.amplitude||0)*bell01(t,c.start,c.end);
  if(d)y+=(d.amplitude||0)*bell01(t,d.start,d.end);
  if(r){
    const A=r.amplitude||0;
    if(t>=r.riseStart&&t<r.riseEnd)y+=A*smooth((t-r.riseStart)/(r.riseEnd-r.riseStart));
    else if(t>=r.riseEnd&&t<r.deckEnd)y+=A;
    else if(t>=r.deckEnd&&t<r.landEnd)y+=A*(1-smooth((t-r.deckEnd)/(r.landEnd-r.deckEnd)));
  }
  return y;
}
function zoneAtT(t){
  t=mod(t,1);
  const hits=(ST01.zones||[]).filter(z=>t>=z.start&&t<=z.end);
  return hits.length?hits[hits.length-1]:{id:'open-ribbon',label:'OPEN RIBBON'};
}
...
function flowRoute(){
  ...
  for(let i=0;i<r.points.length;i++)r.points[i].y+=verticalOffsetAtT(i/r.points.length);
  return r;
}
...
// S-T01 absolute width ladder. v0.8 feel stays unchanged; only track geometry/contact width reads this function.
const WIDTH_KEYS=(ST01.widthProfile||[[0,14.4],[1,14.4]]).map(([t,w])=>[+t,+w]);
function fullWidthAtT(t){
  t=mod(t,1);
  ...
  return ST01.widthClasses?.STANDARD||14.4;
}
function halfWidthAtS(s,total){return fullWidthAtT(mod(s,total)/total)/2}
function widthClassFor(w){
  const C=ST01.widthClasses||{};
  let best='STANDARD',d=Infinity;
  for(const [k,v] of Object.entries(C)){const q=Math.abs(w-v);if(q<d){d=q;best=k}}
  return best;
}
...
const MAT={
  road:new THREE.ShaderMaterial({
    uniforms:{
      uRoad:{value:new THREE.Color(0x15171d)},
      uGrid:{value:new THREE.Color(0x6f402b)},
      uMajor:{value:new THREE.Color(0xd8a04a)}
    },
    vertexShader:`varying vec2 vGrid;
void main(){vGrid=uv;gl_Position=projectionMatrix*modelViewMatrix*vec4(position,1.0);}`,
    fragmentShader:`precision highp float;
varying vec2 vGrid;
uniform vec3 uRoad;
uniform vec3 uGrid;
uniform vec3 uMajor;
float lineAA(float x,float w){float d=abs(fract(x+0.5)-0.5);float a=max(fwidth(x)*1.25,0.0008);return 1.0-smoothstep(w-a,w+a,d);}
void main(){
  float longitudinal=lineAA(vGrid.x*6.0,0.045);
  float minor=lineAA(vGrid.y/4.5,0.040)*0.72;
  float major=lineAA(vGrid.y/18.0,0.055);
  float g=max(longitudinal,minor);
  vec3 c=mix(uRoad,uGrid,g);
  c=mix(c,uMajor,major);
...
function quadUV(A,U,a,b,c,d,ua,ub,uc,ud){quad(A,a,b,c,d);U.push(...ua,...ub,...uc,...ua,...uc,...ud)}
...
function meshUV(A,U,m){const g=new THREE.BufferGeometry();g.setAttribute('position',new THREE.Float32BufferAttribute(A,3));g.setAttribute('uv',new THREE.Float32BufferAttribute(U,2));g.computeVertexNormals();return new THREE.Mesh(g,m)}
function buildWorld(){
  core=buildCore();if(trackGroup)world.remove(trackGroup);trackGroup=new THREE.Group();world.add(trackGroup);
  const N=core.route.length,top=[],topUV=[],side=[],lp=[],rp=[],lines=[];let minY=Infinity,maxR=120;
  for(let i=0;i<N;i++){
    ...
    quadUV(top,topUV,al,ar,br,bl,[0,i*core.ds],[1,i*core.ds],[1,(i+1)*core.ds],[0,(i+1)*core.ds]);
  ...
  trackGroup.add(meshUV(top,topUV,MAT.road),mesh(side,MAT.side));trackGroup.userData.gridMode='SINGLE_SURFACE_SHADER';
  ...
  const t=selfTest(core);UI.hash.textContent=`${FLOW_RECIPE.id} · ${FLOW_RECIPE.revision} · ${core.hash}`;UI.surface.textContent='KFB TOY GRID · single surface';if(UI.grid)UI.grid.textContent='shader · 0 overlay meshes';UI.test.textContent=`${t.ok?'PASS':'CHECK'} · continuity ${t.cont.toFixed(3)}`;
}

const loader=new GLTFLoader();
const cueReport={loaded:0,failed:[],placements:[],triangleAreaXZ:0,sourceCommit:ST01.cueCluster?.sourceCommit||null};
function fitCue(o,target){
  o.updateMatrixWorld(true);let b=new THREE.Box3().setFromObject(o),z=v3();b.getSize(z);
  o.scale.multiplyScalar(target/(Math.max(z.x,z.y,z.z)||1));
  o.updateMatrixWorld(true);b=new THREE.Box3().setFromObject(o);const c=v3();b.getCenter(c);
  o.position.x-=c.x;o.position.z-=c.z;o.position.y-=b.min.y;
}
async function buildCueCluster(){
  const C=ST01.cueCluster;if(!C)return;
  const baseS=C.routeT*core.total,base=core.sample(baseS),side=base.curvature>=0?-1:1;
  const root='https://raw.githubusercontent.com/georg-doc/kayfabizarro/'+C.sourceCommit+'/'+C.packRoot.split('/').map(encodeURIComponent).join('/')+'/';
  for(const item of C.items||[]){
    try{
      const o=(await loader.loadAsync(root+encodeURIComponent(item.asset))).scene;
      fitCue(o,item.targetSize);
      const s=baseS+(item.forwardOffset||0),r=core.sample(s),half=halfWidthAtS(s,core.total);
      const p=r.pos.clone().addScaledVector(r.right,side*(half+(item.lateralExtra||4))).addScaledVector(r.up,.06);
      o.position.add(p);
      o.quaternion.premultiply(new THREE.Quaternion().setFromRotationMatrix(new THREE.Matrix4().makeBasis(r.right,r.up,r.forward)));
      trackGroup.add(o);cueReport.loaded++;
      cueReport.placements.push({role:item.role,asset:item.asset,position:{x:p.x,y:p.y,z:p.z}});
    }catch(e){cueReport.failed.push({role:item.role,asset:item.asset,error:String(e)})}
  }
  if(cueReport.placements.length===3){
    const [a,b,c]=cueReport.placements.map(q=>q.position);
    cueReport.triangleAreaXZ=Math.abs((b.x-a.x)*(c.z-a.z)-(b.z-a.z)*(c.x-a.x))/2;
  }
  if(UI.cue)UI.cue.textContent=cueReport.loaded+'/'+(C.items?.length||0)+' loaded';
}
...
const state={
  ...
  jumpY:0,jumpV:0,airborne:false,landingImpulse:0,landingCount:0,lastLandingPeak:0
};
const keys={},camLook=v3();let auto=false,mapView=false,physicsView=false,qaView='chase';

function reset(){
  Object.assign(state,{s:2.5,speed:0,x:0,vx:0,railHits:0,wallCooldown:0,impact:0,impactSide:0,driftActive:false,driftDir:0,regrip:0,longAccel:0,heave:FEEL.hoverBase,heaveV:0,jumpY:0,jumpV:0,airborne:false,landingImpulse:0,landingCount:0,lastLandingPeak:0});
...
function updateJump(dt){
  ...
  if(state.jumpY<=0){const impact=Math.abs(state.jumpV);state.jumpY=0;state.jumpV=0;state.airborne=false;state.landingImpulse=clamp(impact/10,0,1);state.landingCount++;state.lastLandingPeak=Math.max(state.lastLandingPeak,state.landingImpulse);state.heaveV+=.035+.055*state.landingImpulse}
}
...
function step(dt){
  ...
  window.__KFB_RACE_TELEMETRY__={
    ...
    railImpact:state.impact,railHits:state.railHits,steerInput:steer,trackWidth:currentHalf*2,widthClass:currentClass,zoneId:currentZone.id,landingCount:state.landingCount,lastLandingPeak:state.lastLandingPeak
  };
  return n;
}
function cameraStep(dt,r){
  const car=vehicleRoot.position;if(qaView==='oblique'){const ctr=core.route.reduce((a,p)=>a.add(p),v3()).multiplyScalar(1/core.route.length),p=ctr.clone().add(v3(92,108,84));camera.position.lerp(p,expS(5,dt));camera.up.set(0,1,0);camera.lookAt(ctr);return}if(mapView){const ctr=core.route.reduce((a,p)=>a.add(p),v3()).multiplyScalar(1/core.route.length),p=ctr.clone().add(v3(0,190,0));camera.position.lerp(p,expS(4,dt));camera.up.set(0,0,-1);camera.lookAt(ctr);return}
...
function placeAtRouteT(t){
  state.s=mod(t,1)*core.total;state.speed=0;state.x=0;state.vx=0;state.jumpY=0;state.jumpV=0;state.airborne=false;
  const r=core.sample(state.s),p=r.pos.clone().addScaledVector(r.up,.08);vehicleRoot.position.copy(p);
  vehicleRoot.quaternion.setFromRotationMatrix(new THREE.Matrix4().makeBasis(r.right,r.up,r.forward));
  camera.position.copy(p).addScaledVector(r.forward,-11.5).addScaledVector(r.up,4.7);camera.up.copy(r.up);camLook.copy(p.clone().addScaledVector(r.forward,9));camera.lookAt(camLook);
}
function st01Diagnostics(){
  let maxBank=0,maxBankT=0,minY=Infinity,maxY=-Infinity;
  for(let i=0;i<core.frames.length;i++){const f=core.frames[i],b=Math.abs(f.bank);if(b>maxBank){maxBank=b;maxBankT=i/core.frames.length}minY=Math.min(minY,f.pos.y);maxY=Math.max(maxY,f.pos.y)}
  const V=ST01.verticalGestures||{},samples=ST01.samples||{};
  return {maxBankDeg:maxBank*180/Math.PI,maxBankT,minY,maxY,verticalRange:maxY-minY,
    crestOffset:verticalOffsetAtT((V.crest?.start+V.crest?.end)/2||0),
    dipOffset:verticalOffsetAtT((V.dip?.start+V.dip?.end)/2||0),
    rampOffset:verticalOffsetAtT(samples.ramp||.79)};
}
function st01Snapshot(){
  const t=mod(state.s,core.total)/core.total,r=core.sample(state.s),w=halfWidthAtS(state.s,core.total)*2,c=core.recipe.flow;
  return {
    id:'kfb-track-ribbon-st01',revision:ST01.revision,physicalOwner:'TRACK_LAB_V08_RACE',
    routeT:t,trackWidth:w,widthClass:widthClassFor(w),zone:zoneAtT(t),bankDeg:r.bank*180/Math.PI,
    position:{x:r.pos.x,y:r.pos.y,z:r.pos.z},verticalOffset:verticalOffsetAtT(t),
    meshHalfWidth:w/2,contactLimit:Math.max(1.8,w/2-c.proxyHalfWidth),proxyHalfWidth:c.proxyHalfWidth,
    grid:{preset:ST01.grid?.preset,mode:trackGroup?.userData?.gridMode,overlayMeshes:0,minorSpacingM:4.5,majorSpacingM:18},
    cueCluster:JSON.parse(JSON.stringify(cueReport)),telemetry:{...(window.__KFB_RACE_TELEMETRY__||{})},
    diagnostics:st01Diagnostics()
  };
}
window.__KFB_ST01__={
  snapshot:st01Snapshot,
  teleport:t=>placeAtRouteT(t),
  setView:v=>{qaView=v==='oblique'?'oblique':'chase';mapView=false;return qaView},
  setLateral:x=>{state.x=+x||0;return state.x},
  samples:ST01.samples,
  config:ST01
};
buildWorld();fallback();await Promise.all([loadCar(UI.vehicle.value),buildCueCluster()]);setLab(false);resize();reset();UI.loading.style.display='none';if(UI.build)UI.build.textContent='S-T01 · TOY GRID + WIDTH + RAMP';window.__KFB_ST01_READY__=true;
