...
import { buildStyledBuildingMesh, buildStyledBuildingContours } from './city-style.mjs';
import { createInkWorldAdapter, createOsmInkContours } from './ink-world.mjs';
import {
  createDeformer,
  FALLBACK_PROFILE,
...
const lookName = ['cartoon', 'grotesque', 'stacked'].includes(query.get('look')) ? query.get('look') : recipe.defaultLook;
const outlineMode = ['off', 'ink', 'bend', 'torn'].includes(query.get('outline')) ? query.get('outline') : 'off';
const VISUAL_DEPTH = Object.freeze({
  groundY: -0.14,
  landuseY: -0.075,
  sidewalkY: -0.035,
  roadY: 0,
  landuseOffsetFactor: -1,
  landuseOffsetUnits: -1
});
...
const lookLabel = lookName === 'grotesque' ? 'groteske' : lookName === 'stacked' ? 'gestapelte' : 'cartoon';
$('city-title-long').textContent = `${cityLabel} · ${lookLabel} Fahrprobe`;
...
const preserveLayerCake = lookName === 'stacked';
renderer.toneMapping = preserveLayerCake ? THREE.ACESFilmicToneMapping : THREE.NoToneMapping;
renderer.toneMappingExposure = preserveLayerCake ? 1.08 : 1;

const scene = new THREE.Scene();
if (preserveLayerCake) {
  ...
  // Match the accepted City Lab presentation instead of washing the palette through
  // the warmer Race-only lighting/tone-mapping stack.
  scene.background = new THREE.Color(0xc6d7dc);
  scene.fog = new THREE.Fog(0xc6d7dc, 450, 1200);
...
ground.position.set(groundSupport.centerX, VISUAL_DEPTH.groundY, groundSupport.centerZ);
scene.add(ground);
const land = new THREE.Mesh(
  geometryFrom(landData),
  new THREE.MeshStandardMaterial({
    color: style.palette.greenDark,
    roughness: 1,
    polygonOffset: true,
    polygonOffsetFactor: VISUAL_DEPTH.landuseOffsetFactor,
    polygonOffsetUnits: VISUAL_DEPTH.landuseOffsetUnits
  })
);
scene.add(land);
...
const layerState = { ground: true, landuse: true, roads: true, buildings: true };
const layerObjects = {
  ground: [ground],
  landuse: [land],
  roads: [sidewalks, roads],
  buildings: [buildings]
};
function setLayerVisibility(name, visible) {
  if (!(name in layerObjects)) return false;
  layerState[name] = Boolean(visible);
  for (const object of layerObjects[name]) object.visible = layerState[name];
  const input = document.querySelector(`[data-layer="${name}"]`);
  if (input) input.checked = layerState[name];
  return true;
}

const buildingInk = buildStyledBuildingContours(city, style, lookName, {
  focus: { x: start.p[0], z: start.p[2] },
  maxCount: 72,
  lift: 0.025
});
const inkContours = createOsmInkContours({ city, buildingContours: buildingInk.contours, depth: VISUAL_DEPTH });
const inkWorld = createInkWorldAdapter(THREE, { scene, renderer, contours: inkContours, initialMode: outlineMode });
function setOutlineMode(next) {
  const mode = inkWorld.setMode(next);
  $('outline-select').value = mode;
  $('ink-status').textContent = `OSM semantic contours · ${mode.toUpperCase()} · ${inkWorld.snapshot().contourCount} contours`;
  return mode;
}
...
const snapshot = () => ({
  ...
  presentation: {
    ...
    worldLook: {
      visualDepth: { ...VISUAL_DEPTH, physicalGroundTopY: groundSupport.topY },
      layers: { ...layerState },
      ink: inkWorld.snapshot(),
      buildingInk: buildingInk.stats
    }
  },
  city: {
    id: city.id,
...
$('outline-select').onchange = () => {
  const mode = setOutlineMode($('outline-select').value);
  const next = new URL(location.href);
  if (mode === 'off') next.searchParams.delete('outline'); else next.searchParams.set('outline', mode);
...
window.__OSM_CITY_DRIVE__ = { snapshot, dispose, setOutlineMode, setLayerVisibility };
