...
/* FACE_NORMALS rule (WORLD-INTEGRATION-01 r2, global presenter rule): the shell's lids SHARE the top and
   bottom ring vertices with the walls. A whole-geometry computeVertexNormals() averages each ring vertex
   over wall AND lid faces → the top wall row gets normals tilted ~45° up (bright band directly under the
   roof) and the bottom row ~45° down, weighted by the uneven lid triangulation (irregular dark strip at
   the wall/ground edge). `wallNormals` recomputes normals from the wall triangles only; the top lid sits
   under the roof and the bottom lid is sunk below the ground plate, so neither is seen. */
function orientEG(g, L, { wallNormals = false } = {}) {
  ...
  if (wallNormals) {
    const full = g.index, wall = new THREE.BufferAttribute(full.array.slice(0, nWall), 1);
    g.computeVertexNormals();
    const N = g.attributes.normal, t0 = (rows - 1) * L; let fy = 0; for (let k = t0; k < t0 + L; k++) fy += N.getY(k);
    g.setIndex(wall); g.computeVertexNormals(); g.setIndex(full);
    const N2 = g.attributes.normal; let wy = 0; for (let k = t0; k < t0 + L; k++) wy += N2.getY(k);
    orientEG.lastTop = { full: fy / L, wall: wy / L };   // top-ring mean n.y with and without the lid (evidence)
...
/* HOST SUPPORT (WORLD-INTEGRATION-01 r2): OSM carries no elevation, the host terrain does. Each building
   sits on the LOWEST host height under its footprint (vertices + edge midpoints + centroid): no side floats,
   the up-slope side is buried like a hillside house. Walls, roof and facade details move together (ranges
   recorded while merging). No plate, no hidden support mesh. The sunk wall foot (0.6 m) stays below. */
function makeSupport(out, recs) {
  for (const r of recs) {
    let x0 = Infinity, x1 = -Infinity, z0 = Infinity, z1 = -Infinity, cx = 0, cz = 0;
    for (const p of r.fp) { x0 = Math.min(x0, p.x); x1 = Math.max(x1, p.x); z0 = Math.min(z0, p.z); z1 = Math.max(z1, p.z); cx += p.x; cz += p.z; }
    const n = r.fp.length, S = [[cx / n, cz / n]];
    for (let i = 0; i < n; i++) { const a = r.fp[i], b = r.fp[(i + 1) % n]; S.push([a.x, a.z], [(a.x + b.x) / 2, (a.z + b.z) / 2]); }
    Object.assign(r, { x0, x1, z0, z1, samples: S });
  }
  const byId = new Map(recs.map((r) => [r.id, r]));
  const shift = (mesh, start, count, dy) => { if (!mesh || !count) return; const p = mesh.geometry.attributes.position; for (let i = start; i < start + count; i++) p.setY(i, p.getY(i) + dy); };
  return {
    count: recs.length,
    offsetOf(id) { const r = byId.get(id); return r ? r.y : 0; },
    records: recs,
    apply(heightAt, rect = null, margin = 6) {
      const touched = new Set(); let moved = 0, maxAbs = 0, maxSpan = 0;
      for (const r of recs) {
        if (rect && (r.x1 < rect.minX - margin || r.x0 > rect.maxX + margin || r.z1 < rect.minZ - margin || r.z0 > rect.maxZ + margin)) continue;
        let lo = Infinity, hi = -Infinity; for (const [x, z] of r.samples) { const h = heightAt(x, z); lo = Math.min(lo, h); hi = Math.max(hi, h); }
      ...
      return { moved, maxOffsetM: +maxAbs.toFixed(3), maxFootprintSpanM: +maxSpan.toFixed(3) };
    }
  };
}
export function buildCityLayer(zone, { mode = 'elastic', style, CC, EG, ghosts = true, renderer = null, extraBase = new Set(), facade = 'rule-v1' }) {
  ...
  const supportRecs = [], vc = { walls: 0, roofs: 0 };   // blockPull zur Landmarke, ohne Landmarke Blockmittel wie im Viewer — Host-Setzung
  for (let b of zone.buildings) {
    ...
    if (clean || role === 'ghost') {
      ...
      stats.wallNormalsOnly = (stats.wallNormalsOnly || 0) + 1;
      if (orientEG.lastTop) { const q = stats.topRing || (stats.topRing = { n: 0, full: 0, wall: 0 }); q.n++; q.full += orientEG.lastTop.full; q.wall += orientEG.lastTop.wall; orientEG.lastTop = null; }
