...
const legacyBrush=createRgbBrushTexture(THREE,{seed:0x4b4642,size:256});
const tileableBrush=createTileableRgbBrushTexture(THREE,{seed:0x4b4642,size:256});
let brush=tileableBrush;
let macroMode='tileable';
legacyBrush.edgeMetrics=measureTileEdges(legacyBrush.canvas);
tileableBrush.canvas.id='seamLabMaskCanvas';
document.getElementById('maskMount').appendChild(tileableBrush.canvas);
...
const macroButtons=()=>[...document.querySelectorAll('[data-macro-mode]')];
...
function replaceMacroTexture(texture){
  for(const p of [...prepared.environment,...prepared.actors]){
    for(const rec of p.records||[]){
      for(const material of (Array.isArray(rec.hybrid)?rec.hybrid:[rec.hybrid])){
        const h=material?.userData?.kfbHybridV2;
        if(!h)continue;
...
function setMacroMode(mode){
  macroMode=mode==='legacy'?'legacy':'tileable';
  brush=macroMode==='legacy'?legacyBrush:tileableBrush;
  replaceMacroTexture(brush.texture);
  macroButtons().forEach(b=>b.classList.toggle('active',b.dataset.macroMode===macroMode));
  document.getElementById('edgeOut').textContent=macroMode==='legacy'
    ? 'legacy edge Δmax '+legacyBrush.edgeMetrics.maxAbs
    : 'tileable edge Δmax '+tileableBrush.edgeMetrics.maxAbs;
  updateStatus();
}
...
function updateStatus(){
  ...
  statusEl.innerHTML='<strong>'+currentLook.toUpperCase()+' · '+currentView.toUpperCase()+'</strong>'+
    '<br>'+macroMode+' macro · edge Δmax '+brush.edgeMetrics.maxAbs+' · one texture '+(e.textureUuids[0]||a.textureUuids[0]||'—').slice(0,8)+
...
window.__KFB_SEAM_LAB__={
  version:'0.2-seam-lab',
  build:'KFB-HYBRID-SEAM-LAB-01',
  ready:false,error:null,
  sourcePins:{world:WORLD_PIN,actors:ACTOR_PIN,brushDonor:V1_RUNTIME_PIN},
  brushDonorUrl:BRUSH_DONOR_URL,
  sources:SOURCES,
  setLook,setView,setIsolatedActor,setMacroMode,
  setStrength:v=>{
  ...
  snapshot:()=>({
    ready:window.__KFB_SEAM_LAB__.ready,error,
    sourcePins:{world:WORLD_PIN,actors:ACTOR_PIN,brushDonor:V1_RUNTIME_PIN},
    look:currentLook,view:currentView,isolatedActor,
    strength,macroFactor,grainStrength,roughnessStrength,macroMode,
    edgeMetrics:{legacy:legacyBrush.edgeMetrics,tileable:tileableBrush.edgeMetrics},
...
for(const b of macroButtons())b.onclick=()=>setMacroMode(b.dataset.macroMode);
...
try{
  ...
  setView('integrated');
  setLook('hybrid');
  setMacroMode('tileable');
  await new Promise(r=>requestAnimationFrame(()=>requestAnimationFrame(r)));
  window.__KFB_SEAM_LAB__.ready=true;
  updateStatus();
}catch(e){
  error=String(e?.stack||e);
  window.__KFB_SEAM_LAB__.error=error;
