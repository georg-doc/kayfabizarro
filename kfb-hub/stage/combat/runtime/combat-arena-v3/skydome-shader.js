// ============================================================================
// PROJEKTKOPIE aus georg-doc/kayfabizarro@main · travel/travel-v16/terrain-v16/
// ----------------------------------------------------------------------------
// EINE Änderung gegenüber dem Original, und sie ist der Grund für die Kopie:
// die Zeile `import * as THREE from 'three';` ist ENTFERNT. Ein nackter
// Specifier lädt ohne Import-Map nicht, und dieses Projekt hat keine — die
// Falle steht seit dem 01.09. in `github.md` ("ein Copy von examples/jsm/**
// bringt den nackten from 'three'-Import mit und legt v1 still").
// Das Modul braucht ihn auch nicht: `createSkydome({ THREE })` nimmt die
// Instanz des Wirts, und genau so wird es hier gerufen. Wer neu synct, muss
// diese eine Zeile wieder entfernen.
// Sonst UNVERÄNDERT: kein Nachbau, keine Politur. Änderungen gehören ins Repo.
// ============================================================================
// ============================================================================
...
/* import * as THREE from 'three';  ← siehe Kopf: absichtlich entfernt */
...
const PROC_FRAG = /* glsl */`
  ...
  uniform float uScale, uTwist, uFoldSoft, uRampGain, uSweep; uniform vec3 uFoldOff;
  varying vec3 vDir;
  ${GLSL_NOISE}
  vec3 rotY(vec3 q, float a){ float c=cos(a), s=sin(a); return vec3(q.x*c - q.z*s, q.y, q.x*s + q.z*c); }
  // ── ZWEITE ABWEICHUNG VOM ORIGINAL (Georg 08.09.: "der Waber zeigt noch zu deutliche
  // Spiegel-Kanten"). Eine Faltung IST ein abs(), und ein abs() hat einen Knick — genau dort
  // entsteht die Spiegelkante. sabs rundet den Knick (sqrt(x²+k) statt |x|): bei k = 0 ist es
  // bitgenau das Original, darüber wird die Kante zu einem Grat. Der Wert kommt als Uniform, damit
  // die Kopie ohne ihn dasselbe tut wie das Repo.
  vec3 sabs(vec3 v, float k){ return k > 0.0 ? sqrt(v*v + k) : abs(v); }
  ...
  vec3 skyA(vec3 dir){
    ...
    q = sabs(q, uFoldSoft) - uFoldOff;          q = rotY(q, uTwist);
    q = sabs(q, uFoldSoft*0.7) - uFoldOff*0.7;  q = rotY(q, uTwist*-1.3);
    q = sabs(q, uFoldSoft*0.5) - uFoldOff*0.5;
    float n = fbm(q + ph);
    float rmp = clamp(n*0.5+0.5 + uEnergy*0.25, 0.0, 1.0);
    // ── DRITTE ABWEICHUNG VOM ORIGINAL (Georg 08.09.: "leicht psychedelische Farbpaletten").
    // Gemessen am Bild: nach den drei Faltungen liegt rmp geballt bei 0,55 bis 0,68 — damit
    // saettigt der erste smoothstep gegen 1 (voll colB), der zweite bleibt bei 0 (kein colC), und
    // colA ist gar nicht erreichbar. Ergebnis: 93,6 Prozent aller Himmelspunkte in EINEM
    // 60-Grad-Farbeimer. Die Palette bestimmte also nur, WELCHE eine Farbe der Himmel hat.
    // uRampGain spreizt die Rampe um ihre Mitte. Bei 1 ist es bitgenau das Original.
    rmp = clamp((rmp - 0.5) * uRampGain + 0.5, 0.0, 1.0);
    // ── VIERTE ABWEICHUNG (Georg 08.09.: der Himmel blieb einfarbig, und die Kreisstatistik gab ihm
    // recht: Farbton-Streuung 19,6 Grad bei einer Palette, die 248 Grad spannt).
    // Ursache, gemessen: bei 37 Grad Neigung sieht man nur einen schmalen ELEVATIONS-Streifen der
    // Kugel, und n = fbm(...) ist darüber fast konstant — die beiden smoothstep-Fenster sind für
    // die Verteilung über die GANZE Kugel gebaut. Spreizen um 0,5 verschiebt deshalb nur, WELCHE
    // eine Farbe gewinnt (Gain 1 zu 100 Prozent Eimer 0, Gain 5 zu 81 Prozent Eimer 1).
    // Also wird ein Anteil an den AZIMUT gehängt, der über den sichtbaren Streifen um volle 360 Grad
    // läuft — und langsam wandert, damit es keine feste Farbtapete ist.
    // uSweep = 0 ist bitgenau das Original.
    // ── FUENFTE ABWEICHUNG, UND SIE NIMMT DIE VIERTE ZURUECK (Georg 08.09.: "da sieht man nur
    // 2-3 riesige Farb-Blobs"). Er hat recht, und die Ursache ist genau die Zeile von vorhin:
    // sweep war ein GLATTER Sinus ueber den Azimut, und mix(rmp, sweep, 0.8) hat die Rampe zu
    // 80 Prozent DURCH IHN ERSETZT. Damit war nicht mehr das gefaltete Rauschen der Bildinhalt,
    // sondern eine einzige Sinuswelle: zwei Baeuche im sichtbaren Ausschnitt, durch die beiden
    // smoothstep-Fenster zu zwei bis drei Flaechen gehaertet. Ich hatte die Farbstreuung repariert
    // und dabei die STRUKTUR erschlagen — die Messung sagte 63,7 Grad und war trotzdem falsch,
    // weil sie Farbwinkel zaehlt und nicht Formen.
    // Die Korrektur: der Azimut bleibt, aber als ZYKLISCHE Rampe statt als Ersatzbild, und das
    // Rauschen schreibt in dieselbe Rampe hinein. abs(fract(t)*2-1) laeuft 0..1..0 ohne Sprung,
    // also darf t beliebig gross werden — drei Umlaeufe je Runde, plus zwei Rauschoktaven, die die
    // Bahn verbiegen. Ergebnis: Marmor statt Baender, und die Palette wird trotzdem ganz
    // durchlaufen. Ganzzahlig bleibt es auch: 3 volle Zyklen ueber 2 pi schliessen an der Naht.
    // Merksatz: eine Rampe, die man ERSETZT, zeigt die Rampe. Eine, in die man HINEINSCHREIBT,
    // zeigt weiter den Inhalt.
    if (uSweep > 0.0) {
      float az = atan(dir.z, dir.x);
      float fein = fbm(q*2.9 - ph*0.5);
      float t = az * (3.0/6.2831853) + n*1.05 + fein*0.6 + ph*0.03;
      float tri = abs(fract(t)*2.0 - 1.0);
      rmp = clamp(mix(rmp, tri, uSweep), 0.0, 1.0);
    }
...
export function createSkydome(opts = {}) {
  const T = opts.THREE;
  if (!T) throw new Error('skydome-shader: opts.THREE fehlt (die Projektkopie hat keinen three-Import, siehe Kopf)');
  ...
  const U = {
    ...
    uScale: { value: opts.scale == null ? 0.9 : opts.scale }, uTwist: { value: feel.twist }, uFoldOff: { value: new T.Vector3(0.92, 1.08, 0.92) },
    uFoldSoft: { value: opts.foldSoft == null ? 0 : opts.foldSoft },
    uRampGain: { value: opts.rampGain == null ? 1 : opts.rampGain },
    uSweep: { value: opts.sweep == null ? 0 : opts.sweep },
  };
  ...
  return {
    ...
    setFoldSoft(k) { U.uFoldSoft.value = Math.max(0, k); },
    setScale(s) { U.uScale.value = Math.max(0.05, s); },
    setRampGain(g) { U.uRampGain.value = Math.max(0.1, g); },
    setSweep(v) { U.uSweep.value = Math.max(0, Math.min(1, v)); },
