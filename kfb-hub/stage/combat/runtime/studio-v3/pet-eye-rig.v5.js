...
/* ═══ DAS AUGAPFEL-REZEPT, EINMAL ════════════════════════════════════════
   HERAUSGEZOGEN am 05.09.2026, weil es ein ZWEITER Leser bekommen hat:
   `modules/kfb-weapon-eyeball.js` schiesst mit Augaepfeln und hatte die Zahlen
   abgeschrieben — Augapfel `f3ede2` mit Clearcoat, Pupille `070707`, Kappenwinkel
   `0.12 + pupilSize * 0.62`. Zwei Kopien einer Zahl sind keine Zahl: wer hier den
   Glanz aendert, haette den geschossenen Augapfel stumpf zurueckgelassen, ohne dass
   es irgendwo aufgefallen waere.
   Zurueckgegeben werden GEOMETRIEN UND MATERIALIEN, keine Meshes — der Rig setzt
   sie mit Lidern und Pivot zusammen, das Geschoss ohne. Und je Aufruf FRISCHE
   Materialien, weil ein geteiltes Material alle Augaepfel gleichzeitig verblassen
   liesse (v5-Befund, dort an den Wuerfeln gemessen).
   Die Werte sind die des Rigs, unveraendert — dieser Schnitt darf das Studio nicht
   umfaerben. */
export function eyeballCap(pupilSize) { return 0.12 + clamp(pupilSize, 0, 1) * 0.62; }

export function eyeballParts(THREE, o) {
  o = o || {};
  const R = o.R != null ? o.R : 1;
  const googly = !!o.googly;
  const gloss = o.gloss != null ? o.gloss : 0;
  const cap = o.cap != null ? o.cap : eyeballCap(o.pupilSize != null ? o.pupilSize : 0.5);
  ...
  const matP = new THREE.MeshPhysicalMaterial({ color: 0x070707, metalness: 0, roughness: googly ? 0.28 : 0.45, clearcoat: 1, clearcoatRoughness: googly ? clamp(0.13 - gloss * 0.11, 0.02, 0.13) : 0.11, envMapIntensity: googly ? 1.35 : 1.0 });
  const geoBall = new THREE.SphereGeometry(R, 26, 18);
  const geoPupil = new THREE.SphereGeometry(R * 1.004, 30, 22, 0, Math.PI * 2, 0, cap);
  /* Die Kappe zeigt nach dieser Drehung entlang −Z — die Blickachse des Rigs. */
  geoPupil.rotateX(Math.PI / 2);
  return { matW: matW, matP: matP, geoBall: geoBall, geoPupil: geoPupil, cap: cap };
}
...
export class EyeRig {
  ...
  _capAngle() { return eyeballCap(this.pupilSize); }
  ...
  build() {
    ...
    /* Augapfel und Pupille kommen aus dem EINEN Rezept (`eyeballParts`, oben) —
       dasselbe, aus dem die Augapfel-Munition baut. Die Lider bleiben hier: ein
       Geschoss hat keine. */
    const teile = eyeballParts(THREE, { R: R, cap: this._capAngle(), googly: googly, gloss: this.gloss });
    const matW = teile.matW;
    const matP = teile.matP;
    const cap = this._puAng = teile.cap;
    const puGeo = teile.geoPupil;
    const scGeo = teile.geoBall;
    ...
    const mkEye = (sx) => {
      ...
      const sc = new THREE.Mesh(scGeo, matW);
