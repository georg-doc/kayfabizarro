...
export const FLOORS = [
  ...
  /* ── v3 · S5: der Combat Beat. Die drei Böden von DP1/DP2 (SPRINT §4). ───────────────────── */
  {
    id: 'C13', text: 'Jedes sichtbare Kampfereignis trägt genau eine Beat-Nummer — 0 Waisen (Schichtaufruf ohne Beat)',
    register: REGISTER.block, slice: 'S5', quelle: 'M10 beat.v3 · Schichten melden sich beim Beat',
    /* Eine Waise ist ein Effekt ohne Absender. v2 hatte drei an EINEM Tag (Foundation nie gebootet,
       `col` statt `color`, `charge` nicht im Atlas) — keiner davon war zuzuordnen. */
    mess: (p) => !p.beat ? wait('wartet auf M10 (S5)')
      : p.beat.c13.beats === 0 ? wait('noch kein Beat gefahren — schießen oder Schießprobe')
        : (p.beat.c13.pass ? pass(p.beat.c13.zeile.slice(6)) : fail(p.beat.c13.zeile.slice(6)))
  },
  {
    id: 'C14', text: 'Abstand zwischen Beat-fire und der eingestellten Feuerzeit ≤ 1 Bild (16,7 ms) — die Zeit selbst ist eine Entscheidung am Bild, ihr Einhalten nicht',
    register: REGISTER.block, slice: 'S5', quelle: 'M10 beat.v3 + M11 clipmarks.v3 (Tempo-Maximum der Waffenspitze)',
    /* v2 hatte die Feuerzeit als Bruchteil einer Clipdauer und die Vorhaltezeit aus einer ANDEREN
       Bühne. Zwei geerbte Zahlen für etwas, das man an dieser Figur ablesen kann. */
    mess: (p) => !p.beat ? wait('wartet auf M10 (S5)')
      : p.beat.c14.pass == null ? wait(p.beat.c14.zeile.slice(6))
        : (p.beat.c14.pass ? pass(p.beat.c14.zeile.slice(6)) : fail(p.beat.c14.zeile.slice(6)))
  },
  {
    id: 'C16', text: 'Jede Schicht meldet je Beat, ob sie etwas erzeugt hat — 0 stille Ausfälle über 20 Beats',
    register: REGISTER.warn, slice: 'S5', quelle: 'M10 beat.v3 · stumm im Bild, laut im Prüfblatt',
    mess: (p) => !p.beat ? wait('wartet auf M10 (S5)')
      : p.beat.c16.beats === 0 ? wait('noch kein Beat gefahren')
        : (p.beat.c16.pass ? pass(p.beat.c16.zeile.slice(6)) : fail(p.beat.c16.zeile.slice(6)))
  },

  /* ── v3 · S11: die Zensur-Schicht (M12). Die Karte IST die Uhr (ENTWURF §3). ─────────────── */
  {
    id: 'C22', text: 'Deckungsgrad der Zensur steigt monoton (0 Rückgang über alle Lesungen) und die MESSFLÄCHE (Blatt ohne Beschnittrand) gilt genau bei 100 % als verloren',
    register: REGISTER.block, slice: 'S11', quelle: 'M12 zensur.v3 · 64 × 64 Rücklesung alle 0,5 s',
    /* Warum ausgerechnet Monotonie der Boden ist: die Schicht wird nie gelöscht, nur ergänzt — ein
       fallender Deckungsgrad wäre also kein Balance-Problem, sondern ein Beweis, dass die Zahl aus
       etwas anderem kommt als aus der Textur (Kartenwechsel, zweite Schicht, Rücklesefehler).
       Ein Rückgang ≤ 0,02 % ist die Rasterauflösung, kein Verlust: 64 × 64 = 4096 Felder.
       Und die zweite Hälfte des Bodens ist die WICHTIGE: gemessen 07.09. sättigte die Deckung über
       dem ganzen Blatt bei 99,4 % (14 offene Felder, alle in der äußersten Reihe) — ein Tor bei
       100 %, das sich nicht schließen kann, ist kein Boden, sondern eine Zusage ohne Deckung.
       Gelesen wird deshalb die Messfläche (zensur.v3 · SPEC.leseEinzug). */
    mess: (p) => !p.zensur ? wait('wartet auf M12 (S11)')
      : !p.zensur.steht ? wait('Schicht steht nicht — keine Karte gemessen')
        : p.zensur.messungen < 2 ? part('Schicht steht · ' + p.zensur.messungen + ' Lesung(en), 2 nötig')
          : p.zensur.rueckgang > 0.02 ? fail('Rückgang ' + p.zensur.rueckgang.toFixed(2) + ' % über ' + p.zensur.messungen + ' Lesungen — die Zahl kommt nicht aus der Schicht')
            : pass('Deckung ' + p.zensur.deckung.toFixed(1) + ' % der Messfläche (' + p.zensur.messflaeche + ' u², Einzug ' + p.zensur.einzug + ' u) · Rückgang ' + p.zensur.rueckgang.toFixed(2) + ' % über ' + p.zensur.messungen + ' Lesungen'
              + (p.zensur.verlorenBei != null ? ' · schwarz nach ' + p.zensur.verlorenBei + ' s' : ' · noch nicht schwarz'))
  },
  {
    id: 'C23', text: 'Ein Spucken = genau ein Spritzer (0 verworfen ohne Zähler), die Schicht wird höchstens 2 ×/s zurückgelesen, und der Zulauf ist von den Spritzern unterscheidbar',
    register: REGISTER.warn, slice: 'S11', quelle: 'M12 zensur.v3 · Zeichenweg, Leseweg und Zulauf getrennt gezählt',
    mess: (p) => !p.zensur ? wait('wartet auf M12 (S11)')
      : p.zensur.spuckBeats === 0 && p.zensur.gezeichnet === 0 ? wait('noch nicht gespuckt — Knopf »Censor salvo« oder 3 s zusehen')
        : (p.zensur.gezeichnet + p.zensur.verworfen < p.zensur.spuckBeats
          ? fail(p.zensur.spuckBeats + ' Spucken, aber nur ' + p.zensur.gezeichnet + ' gezeichnet + ' + p.zensur.verworfen + ' verworfen')
          : pass(p.zensur.gezeichnet + ' Spritzer aus ' + p.zensur.spuckBeats + ' Spucken · ' + p.zensur.verworfen + ' neben der Karte · '
            + (1 / p.zensur.leseTakt).toFixed(1) + ' Lesungen/s · ' + p.zensur.formen + ' Vektorformen · Zulauf ' + p.zensur.zulaeufe + ' ×'))
  }
];
