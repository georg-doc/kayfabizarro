...
export default class PlayerController {
  ...
  /** STERBEN. Georg 06.09.: »FB zeigt keine death-animation«. Der Name wird GESUCHT, nicht
      angenommen (Hausregel dieser Datei): KayKit legt den Todesclip in die Kategorie »General«
      (`Death_A`/`Death_B`), das eigene Pack bringt ihn nicht mit. Wer keinen findet, bekommt eine
      benannte Notlage statt einer stillen: die Figur kippt (Rückweg unten), und das Log sagt es.
      Der Clip besitzt die Figur, bis `aufstehen()` kommt — deshalb dieselbe Sperre wie beim
      Schussclip, nur ohne Ablauf. */
  sterben(name) {
    if (!this.fb || !this.fb.play) return null;
    const n = name || (this.fb.findClip && ((this.fb.findClip(/death/i) || {}).name || (this.fb.findClip(/^die|dead/i) || {}).name));
    this._soll = 'tot';
    if (n) {
      this.fb.play(n, { loop: false });
      this.clipName = n;
      this._schussHalt = 1e9;                     // hält, bis aufgestanden wird
      this.log('Todesclip ' + n);
      return n;
    }
    /* Notlage, benannt: ohne Clip kippt die Figur um ihre Längsachse. Das ist keine Animation,
       das ist eine Lage — und sie steht so im Log, damit niemand sie für eine hält. */
    /* Notlage: keine Animation, sondern eine LAGE — und sie liegt auf der Karte, nicht darüber.
       (Vorher blieb der Fußversatz stehen, also kippte FB in der Luft.) */
    this._kipp = 1;
    this._schussHalt = 1e9;
    this.fussOffset = 0;
    this.log('kein Todesclip gefunden — Figur legt sich hin (Notlage)');
    return null;
  }

  aufstehen() {
    this._schussHalt = 0; this._soll = null; this._kipp = 0;
  ...
  interruptHit(){
    if(this._soll==='hit'){this._schussHalt=0;this._soll=null;}
  }

  prepareGun() {
    this.interruptHit();
    const entry=this.fb?.findClip(/^idle_gun$/i);
    if(!entry)return null;
    // A held aim already has the arm up; do not repeatedly replay its blend.
    if(this._soll==='gun-ready' && this.fb.action===this._aimAction)return this._aimAction;
    this._aimAction=this.fb.play(entry.name,{loop:true,fade:.14});
    this.clipName=entry.name;this._soll='gun-ready';this.zielt=Math.max(this.zielt||0,.8);
    return this._aimAction;
  }

  gunReady() {
    return this._soll==='gun-ready' && this.fb?.action===this._aimAction
      && this._aimAction.time>=.15 && this._aimAction.getEffectiveWeight()>=.98;
  }

  gunHeld() {
    return (this.zielt||0)>0 && /^(idle_gun|walk_gun|run_gun|idle_shoot|run_shoot)$/i.test(this.clipName||'');
  }

  schuss() {
    const re=this.tempo>(this.tempoMax||SPEC.tempo).gehen*1.05?/^run_shoot$/i:/^idle_shoot$/i;
    const e=this.fb?.findClip(re);if(!e)return null;
    const action=this.fb.play(e.name,{loop:false,fade:.025,timeScale:1.6});
    this.clipName=e.name;this._soll='schuss';this._shotAction=action;
    this._schussHalt=e.clip.duration/1.6;this.zielt=Math.max(this.zielt||0,e.clip.duration+.2);
    // Authored marker: first animation frame after the prepared gun pose, before the recoil lift.
    // Both supplied clips are sampled at 24 fps. This clock stops with the mixer.
    return {action,marker:1/24,clip:e.name};
  }

  cancelShot() {
    if(this._soll==='gun-ready' || this._soll==='schuss') {
      this._soll=null;this._schussHalt=0;this._aimAction=null;this._shotAction=null;
    }
    this.zielt=0;this.zielRichtung=null;
  }

  /** GETROFFEN WERDEN. Georg 06.09.: »was ist mit der death/hit-Animation? FB hat da doch Clips«.
      Ja — `HitReact` liegt im eigenen Pack, und bis jetzt hat sie niemand gerufen: FB starb
      sichtbar, aber jeder Treffer davor ging spurlos an ihm vorbei.
      Der Clip ist KURZ und darf die Figur nicht besitzen wie der Schuss: wer im Treffer festhängt,
      kann nicht ausweichen. Deshalb ein eigener, kleiner Halt (0,45 der Cliplänge) und keine
      Sperre auf `_soll`, wenn die Figur gerade stirbt oder schießt — ein Treffer unterbricht keinen
      Tod und keinen Abgang.
      Merksatz: eine Reaktion, die die Steuerung blockiert, ist eine Strafe, keine Reaktion. */
  treffer() {
    if (!this.fb || !this.fb.play) return null;
    if (this._soll === 'tot' || this._soll === 'schuss' || this._soll === 'gun-ready') return null;
    const e = this.fb.findClip(/^hit_?react$/i) || this.fb.findClip(/hit/i);
    if (!e) return null;
    this.fb.play(e.name, { loop: false,fade:.04 });
    this.clipName=e.name;this._soll='hit';
    this._schussHalt=Math.min(.18,(e.dur||.5)*.45);
    return e.name;
  }
  ...
  _clip(t, dt = 0) {
    if(this._soll==='gun-ready')return;
    if (!this.fb || !this.fb.play) return;
    /* Der Schussclip gehört sich selbst, solange er läuft. */
    if (this._schussHalt > 0) { this._schussHalt -= dt; if (this._schussHalt > 0) return; this._soll = null; }
  ...
  update(dt) {
    ...
    const stunned=(this.stunRemaining||0)>0;
    this.slamAction?.beforeStep(dt,!!s.jumpPressed,SPEC.sprung.kraft);
    this.stunRemaining=Math.max(0,(this.stunRemaining||0)-dt);
    this.stunImmune=Math.max(0,(this.stunImmune||0)-dt);
    if(s.x || s.z || s.jumpPressed || s.jump)this.interruptHit();
    this.schritt(dt, stunned?0:s.x, stunned?0:s.z, !stunned && (s.jumpPressed || s.jump), laufen);
    this.slamAction?.afterStep();
    ...
    if (this._fussT >= 0.25) {
      ...
      /* ⚠ NICHT WÄHREND DES STERBENS. Der Nachführer hält FÜSSE auf dem Boden — ein Todesclip legt
         den Körper hin, und dann jagt der Nachführer eine Kiste, die in jedem Bild woanders anfängt:
         er hebt die Wurzel mit. Sichtbar war ein FB, der schräg über der Karte SCHWEBT (Georg
         06.09.). Ein liegender Körper hat keine Füße auf dem Boden — also schweigt der Nachführer.
         Merksatz: ein Regler, der eine Annahme durchsetzt, muss aufhören, wenn die Annahme fällt. */
      if (!box.isEmpty() && this._soll !== 'tot' && this.amBoden && this._soll !== 'schuss') {
    ...
    if (ziel != null) {
      ...
      if (this._kipp) { this._kipp = Math.min(1, this._kipp + dt * 3.2); this.root.rotation.z = (Math.PI / 2) * Math.min(1, this._kipp); }
    }
    this._clip(this.tempo, dt);
  }
  /**
