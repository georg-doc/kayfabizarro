...
addEventListener('keydown',e=>{if(window.__KFB_BOX_BLOCK_INPUT__||isEditing(e)){if(e.type==='keydown')window.__KFB_BOX_HANDLE_KEY__?.(e);e.stopImmediatePropagation()}},{capture:true});
addEventListener('keyup',e=>{if(window.__KFB_BOX_BLOCK_INPUT__||isEditing(e)){if(e.type==='keydown')window.__KFB_BOX_HANDLE_KEY__?.(e);e.stopImmediatePropagation()}},{capture:true});
document.addEventListener('focusin',e=>{if(isEditing(e))dispatchEvent(new Event('blur'))});
document.addEventListener('pointerdown',e=>{if(e.target instanceof Element&&e.target.closest('button,input,select,textarea,summary,#boxOverlay'))e.stopPropagation()});
try{
  ...
  function sync(){const d=env.snapshot();$('worldSelect').value=d.recipe;$('seedInput').value=d.seed;document.body.dataset.world=d.recipe;
    ...
    $('buildLabel').textContent=window.__KFB_BOX_READY__?'BOX1 · v0.8 + ENV1':d.build;const url=new URL(location.href);url.searchParams.set('world',d.recipe);url.searchParams.set('seed',d.seed);history.replaceState(null,'',url);
  }
  ...
  announce('Connecting Box Stop · original WS1 vehicle sources…');
  try{await installBoxStop(port,env);status.hidden=true}catch(e){announce('Driving is available; Box Stop could not start: '+e.message);window.__KFB_BOX_ERROR__=String(e.stack||e);console.error(e)}
