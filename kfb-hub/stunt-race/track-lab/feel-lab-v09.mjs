...
const FEEL_RUNTIME=await fetch(new URL('./RACE_FEEL_V09_CONFIG.json',import.meta.url),{cache:'no-store'}).then(r=>{if(!r.ok)throw new Error(`Feel v09 config HTTP ${r.status}`);return r.json()});
const FLOW=adaptA0TrackRecipeForRace(FLOW_RECIPE,RACE_RUNTIME);
const FEEL={...(FEEL_RUNTIME.feel||{})};
const BRANCH={...(FEEL_RUNTIME.branch||{})};
...
const UI={
  ...
  speed:$('#speed'),drive:$('#driveState'),bank:$('#bank'),lat:$('#latLoad'),boost:$('#boostState'),route:$('#routeState'),
  auto:$('#auto'),reset:$('#reset'),map:$('#mapMode'),phys:$('#physMode'),lab:$('#labToggle'),
  panel:$('#labPanel'),miniWrap:$('#miniWrap'),mini:$('#mini'),
  hash:$('#trackHash'),surface:$('#surfaceStatus'),width:$('#trackWidth'),branch:$('#branchStatus'),test:$('#selfTest'),
...
scene.fog=new THREE.Fog(0x86ced1,120,300);
const camera=new THREE.PerspectiveCamera(67,1,.08,560);
...
function resample(points,N,closed=true){
  const M=points.length,cum=new Float64Array(M+(closed?1:0));
  const segments=closed?M:M-1;
  for(let i=0;i<segments;i++)cum[i+1]=cum[i]+points[i].distanceTo(points[(i+1)%M]);
  const total=cum[segments],ds=total/(closed?N:N-1),out=[];let j=0;
  for(let i=0;i<N;i++){
    const s=Math.min(total,i*ds);
    while(j<segments-1&&cum[j+1]<s)j++;
    const a=points[j],b=points[(j+1)%M],span=cum[j+1]-cum[j]||1;
    out.push(a.clone().lerp(b,(s-cum[j])/span));
  }
  return {points:out,total,ds};
}
function flowRoute(){
  const {dense,corners}=buildDenseFlow(FLOW.track),r=resample(dense,FLOW.track.sampleCount,true);
...
function tangents(route,closed=true){
  const N=route.length,T=[];
  for(let i=0;i<N;i++){
    const a=closed?route[mod(i-1,N)]:route[Math.max(0,i-1)],b=closed?route[mod(i+1,N)]:route[Math.min(N-1,i+1)];
...
function transportedFrames(route,rolls,closed=true){
  const N=route.length,T=tangents(route,closed),R=new Array(N),U=new Array(N);let seed=v3(0,1,0);if(Math.abs(seed.dot(T[0]))>.92)seed=v3(1,0,0);
  ...
  let closureTwist=0;
  if(closed){
    const qClose=new THREE.Quaternion().setFromUnitVectors(T[N-1],T[0]),endR=R[N-1].clone().applyQuaternion(qClose).normalize();
    closureTwist=signedAround(endR,R[0],T[0]);
  }
  for(let i=0;i<N;i++){
    const extra=closed?closureTwist*i/N:0,q=new THREE.Quaternion().setFromAxisAngle(T[i],extra+(rolls?rolls[i]:0));
    R[i].applyQuaternion(q).normalize();U[i]=new THREE.Vector3().crossVectors(T[i],R[i]).normalize();R[i]=new THREE.Vector3().crossVectors(U[i],T[i]).normalize();
  }
  return route.map((p,i)=>({pos:p,forward:T[i],right:R[i],up:U[i],bank:rolls?rolls[i]:0}));
}
function sampleFrames(frames,total,ds,s,closed=true){
  if(closed)s=mod(s,total);else s=clamp(s,0,total);
  const N=frames.length,f=s/ds,i=Math.min(N-1,Math.floor(f)),u=clamp(f-Math.floor(f),0,1),j=closed?mod(i+1,N):Math.min(N-1,i+1),a=frames[i],b=frames[j];
  const pos=a.pos.clone().lerp(b.pos,u),forward=a.forward.clone().lerp(b.forward,u).normalize(),right=a.right.clone().lerp(b.right,u).normalize(),up=new THREE.Vector3().crossVectors(forward,right).normalize();
  right.copy(new THREE.Vector3().crossVectors(up,forward).normalize());
  return {i,u,pos,forward,right,up,bank:lerp(a.bank,b.bank,u),curvature:lerp(a.curvature||0,b.curvature||0,u)};
}
function buildCore(){
  ...
  const F=transportedFrames(r.points,rolls,true);for(let i=0;i<N;i++)F[i].curvature=curv[i];
  return {recipe:FLOW,route:r.points,total:r.total,ds:r.ds,frames:F,sample:s=>sampleFrames(F,r.total,r.ds,s,true),hash:hash(stable(FLOW_RECIPE))};
}
function selfTest(c){
...
const WIDTH_KEYS=FEEL_RUNTIME.trackWidth.profile;
function widthScaleAt(t){
  t=mod(t,1);
  for(let i=0;i<WIDTH_KEYS.length-1;i++){const a=WIDTH_KEYS[i],b=WIDTH_KEYS[i+1];if(t>=a[0]&&t<=b[0])return lerp(a[1],b[1],smooth((t-a[0])/(b[0]-a[0]||1)))}
  return 1;
}
const baseHalf=FLOW.track.roadWidth/2;
function mainHalfWidthAtS(s,total){return baseHalf*widthScaleAt(mod(s,total)/total)}
...
const MAT={
  ...
  branchRail:new THREE.MeshStandardMaterial({color:0x3c434b,roughness:.68,metalness:.10}),
  line:new THREE.MeshBasicMaterial({color:0xb9b8b1,polygonOffset:true,polygonOffsetFactor:-2,polygonOffsetUnits:-2}),
  branchLine:new THREE.MeshBasicMaterial({color:0xaedfdc,polygonOffset:true,polygonOffsetFactor:-2,polygonOffsetUnits:-2}),
  ground:new THREE.MeshStandardMaterial({color:0xd8c98d,roughness:1}),
  support:new THREE.MeshStandardMaterial({color:0x55515a,roughness:.95})
};
let core=null,branchCore=null,trackGroup=null;
...
function buildOpenBranch(){
  const startS=core.total*BRANCH.startFrac,endS=core.total*BRANCH.endFrac,raw=[];
  const count=BRANCH.samples||360;
  for(let i=0;i<count;i++){
    const u=i/(count-1),base=core.sample(lerp(startS,endS,u));
    const side=-Math.sin(TAU*u)*BRANCH.lateralOffset;
    const rise=Math.sin(Math.PI*u)*BRANCH.bridgeHeight;
    raw.push(base.pos.clone().addScaledVector(base.right,side).addScaledVector(base.up,rise));
  }
  const r=resample(raw,count,false),rolls=new Float64Array(count);
  for(let i=0;i<count;i++){const u=i/(count-1);rolls[i]=Math.sin(Math.PI*u)*BRANCH.bankPeak*Math.sign(Math.sin(TAU*u)||1)}
  const frames=transportedFrames(r.points,rolls,false);
  for(let i=0;i<count;i++){
    const prev=frames[Math.max(0,i-2)].forward,next=frames[Math.min(count-1,i+2)].forward;
    frames[i].curvature=angleDelta(Math.atan2(prev.x,prev.z),Math.atan2(next.x,next.z))/(Math.max(1,4*r.ds));
  }
  return {startS,endS,route:r.points,total:r.total,ds:r.ds,frames,sample:s=>sampleFrames(frames,r.total,r.ds,s,false)};
}
function addStrip(group,frames,total,ds,halfWidthFn,materials,closed=true){
  const N=frames.length,top=[],side=[],lines=[],lp=[],rp=[];
  const segments=closed?N:N-1;
  for(let i=0;i<segments;i++){
    const j=closed?mod(i+1,N):i+1,a=frames[i],b=frames[j],hwa=halfWidthFn(i*ds,total),hwb=halfWidthFn(j*ds,total);
    ...
    if(i%30<17){
      const frac=0,h=.055,l=.032,latA=hwa*frac,latB=hwb*frac;
      const l0=a.pos.clone().addScaledVector(a.right,latA-h).addScaledVector(a.up,l),r0=a.pos.clone().addScaledVector(a.right,latA+h).addScaledVector(a.up,l),l1=b.pos.clone().addScaledVector(b.right,latB-h).addScaledVector(b.up,l),r1=b.pos.clone().addScaledVector(b.right,latB+h).addScaledVector(b.up,l);
      quad(lines,l0,r0,r1,l1);
    }
  }
  group.add(mesh(top,materials.road||MAT.road),mesh(side,materials.side||MAT.side),mesh(lines,materials.line||MAT.line));
  for(const pts of [lp,rp]){
    const c=new THREE.CatmullRomCurve3(pts,closed,'catmullrom',.5),g=new THREE.TubeGeometry(c,Math.max(24,pts.length),.115,10,closed);
    group.add(new THREE.Mesh(g,materials.rail||MAT.rail));
  }
}

function buildWorld(){
  core=buildCore();branchCore=buildOpenBranch();
  if(trackGroup)world.remove(trackGroup);trackGroup=new THREE.Group();world.add(trackGroup);

  addStrip(trackGroup,core.frames,core.total,core.ds,(s,total)=>mainHalfWidthAtS(s,total),{road:MAT.road,side:MAT.side,line:MAT.line,rail:MAT.rail},true);
  addStrip(trackGroup,branchCore.frames,branchCore.total,branchCore.ds,()=>baseHalf*BRANCH.widthScale,{road:MAT.road,side:MAT.side,line:MAT.branchLine,rail:MAT.branchRail},false);

  for(const u of [.23,.77]){
    const b=branchCore.sample(branchCore.total*u),main=core.sample(lerp(branchCore.startS,branchCore.endS,u));
    const h=Math.max(.8,b.pos.y-main.pos.y-FLOW.track.roadThickness);
    const pillar=new THREE.Mesh(new THREE.BoxGeometry(.75,h,.75),MAT.support);
    pillar.position.copy(main.pos).lerp(b.pos,.5);pillar.position.y=(main.pos.y+b.pos.y)/2-.2;trackGroup.add(pillar);
  }

  let minY=Infinity,maxR=120;
  for(const p of [...core.route,...branchCore.route]){minY=Math.min(minY,p.y);maxR=Math.max(maxR,Math.abs(p.x)+45,Math.abs(p.z)+45)}
  ...
  for(const sign of [-1,1]){
    const e=core.sample(branchCore.startS),marker=new THREE.Mesh(new THREE.CylinderGeometry(.16,.22,1.35,10),new THREE.MeshStandardMaterial({color:sign<0?0x8eeeff:0xffd24d,roughness:.7}));
    marker.position.copy(e.pos).addScaledVector(e.right,sign*2.9).addScaledVector(e.up,.7);trackGroup.add(marker);
  }

  const t=selfTest(core);
  UI.hash.textContent=`${FLOW_RECIPE.id} · ${FLOW_RECIPE.revision} · ${core.hash}`;
  UI.surface.textContent=FLOW.surfaces[0].roles.join(' / ');
  UI.branch.textContent=`ALT BRIDGE ${Math.round(BRANCH.startFrac*100)}–${Math.round(BRANCH.endFrac*100)}% · Race-local`;
  UI.test.textContent=`${t.ok?'PASS':'CHECK'} · continuity ${t.cont.toFixed(3)} · branch ${branchCore.frames.length} samples`;
}
...
const state={
  ...
  branchArmed:false,branchActive:false,branchDist:0,branchRuns:0
};
const keys={},camLook=v3();let auto=false,mapView=false,physicsView=false;

function reset(){
  Object.assign(state,{s:2.5,speed:0,x:0,vx:0,railHits:0,wallCooldown:0,impact:0,impactSide:0,driftActive:false,driftDir:0,regrip:0,longAccel:0,heave:FEEL.hoverBase,heaveV:0,jumpY:0,jumpV:0,airborne:false,landingImpulse:0,branchArmed:false,branchActive:false,branchDist:0});
...
function startJump(){if(state.airborne||state.jumpY>.01)return;state.jumpV=FEEL.jumpImpulse;state.airborne=true}
function updateJump(dt){
  state.landingImpulse=Math.max(0,state.landingImpulse-dt*5);if(!state.airborne)return;
...
function nearForwardSegment(s,start,end){if(start<=end)return s>=start&&s<=end;return s>=start||s<=end}
function activeSample(){return state.branchActive?branchCore.sample(state.branchDist):core.sample(state.s)}
function currentHalfWidth(){return state.branchActive?baseHalf*BRANCH.widthScale:mainHalfWidthAtS(state.s,core.total)}
function maybeArmBranch(){
  if(state.branchActive||state.speed<=0)return;
  const lead=core.total*BRANCH.decisionLeadFrac,start=Math.max(0,branchCore.startS-lead);
  if(nearForwardSegment(state.s,start,branchCore.startS)){
    if(state.x<-BRANCH.armLateral)state.branchArmed=true;
    else if(state.x>BRANCH.cancelLateral)state.branchArmed=false;
  }
}
function advancePath(ds){
  if(state.branchActive){
    state.branchDist+=ds;
    if(state.branchDist>=branchCore.total){state.branchActive=false;state.branchDist=0;state.s=branchCore.endS;state.x=clamp(state.x,-baseHalf*.35,baseHalf*.35);state.branchArmed=false;state.branchRuns++}
    else if(state.branchDist<=0){state.branchActive=false;state.branchDist=0;state.s=branchCore.startS;state.branchArmed=false}
    return;
  }
  const old=state.s;state.s=mod(state.s+ds,core.total);
  if(ds>0&&old<branchCore.startS&&state.s>=branchCore.startS&&state.branchArmed){state.branchActive=true;state.branchDist=0;state.s=branchCore.startS;state.x=clamp(state.x,-baseHalf*.28,baseHalf*.28)}
}

function step(dt){
  const r=activeSample(),c=FLOW.flow;
  ...
  const half=currentHalfWidth(),limit=Math.max(1.8,half-c.proxyHalfWidth);
  ...
  let damping=c.lateralDamping*(drifting?FEEL.driftDampingScale:1);if(state.regrip>0)damping*=1+FEEL.regripDampingScale*(state.regrip/FEEL.regripSeconds);
  ...
  advancePath(state.speed*dt);updateJump(dt);
  const n=activeSample(),p=n.pos.clone().addScaledVector(n.right,state.x).addScaledVector(n.up,.08+state.jumpY);vehicleRoot.position.copy(p);
  ...
  const driftYaw=drifting?driftDir*.18-slip*FEEL.driftYawGain:0,yawTarget=drifting?clamp(driftYaw,-.46,.46):clamp(-slip*.26,-.08,.08);
  ...
  let route='MAIN';const decisionLead=core.total*BRANCH.decisionLeadFrac;
  if(state.branchActive)route='BRIDGE';else if(nearForwardSegment(state.s,Math.max(0,branchCore.startS-decisionLead),branchCore.startS))route=state.branchArmed?'BRIDGE ARMED':'LEFT = BRIDGE';
  UI.speed.textContent=(state.speed<-.1?'R ':'')+Math.round(sa*7.2);UI.bank.textContent=`${(n.bank*180/Math.PI).toFixed(0)}°`;UI.drive.textContent=drive;UI.drive.classList.toggle('hot',drifting);UI.lat.textContent=(Math.abs(rawLat)*.08).toFixed(1);UI.boost.textContent=boost?'BOOST':'—';UI.boost.classList.toggle('hot',boost);UI.width.textContent=`${(half*2).toFixed(1)} m`;UI.route.textContent=route;UI.route.classList.toggle('hot',state.branchActive||state.branchArmed);
  ...
  window.__KFB_RACE_TELEMETRY__={
    ...
    railImpact:state.impact,railHits:state.railHits,steerInput:steer,trackWidth:half*2,
    routeVariant:state.branchActive?'bridge':'main',branchArmed:state.branchArmed,branchRuns:state.branchRuns
  };
  return n;
}

function cameraStep(dt,r){
  const car=vehicleRoot.position;
  if(mapView){const pts=[...core.route,...branchCore.route],ctr=pts.reduce((a,p)=>a.add(p),v3()).multiplyScalar(1/pts.length),p=ctr.clone().add(v3(0,205,0));camera.position.lerp(p,expS(4,dt));camera.up.set(0,0,-1);camera.lookAt(ctr);return}
...
function mini(){
  ...
  for(const p of [...core.route,...branchCore.route]){minX=Math.min(minX,p.x);maxX=Math.max(maxX,p.x);minZ=Math.min(minZ,p.z);maxZ=Math.max(maxZ,p.z)}
  ...
  mctx.lineJoin='round';mctx.strokeStyle='rgba(238,232,216,.78)';mctx.lineWidth=7;mctx.beginPath();for(let i=0;i<core.route.length;i+=6){const p=core.route[i];i?mctx.lineTo(X(p),Z(p)):mctx.moveTo(X(p),Z(p))}mctx.closePath();mctx.stroke();
  mctx.strokeStyle='rgba(142,238,255,.92)';mctx.lineWidth=4;mctx.beginPath();for(let i=0;i<branchCore.route.length;i+=3){const p=branchCore.route[i];i?mctx.lineTo(X(p),Z(p)):mctx.moveTo(X(p),Z(p))}mctx.stroke();
  const r=activeSample(),p=r.pos.clone().addScaledVector(r.right,state.x);mctx.fillStyle='#f05b93';mctx.beginPath();mctx.arc(X(p),Z(p),5,0,TAU);mctx.fill();
}
...
addEventListener('keydown',e=>{
  keys[e.code]=true;if(['ArrowUp','ArrowDown','ArrowLeft','ArrowRight','Space','KeyQ','KeyE'].includes(e.code))e.preventDefault();
  if(e.code==='Space'&&!e.repeat)startJump();if(e.code==='KeyR')reset();
...
UI.lab.onclick=()=>setLab(UI.panel.hidden);UI.auto.onclick=()=>{auto=!auto;UI.auto.textContent='AUTO: '+(auto?'ON':'OFF')};UI.reset.onclick=reset;UI.map.onclick=()=>{mapView=!mapView;UI.map.textContent=mapView?'CHASE':'MAP'};UI.phys.onclick=()=>{physicsView=!physicsView;forceRoot.visible=physicsView;UI.phys.textContent='PHYS: '+(physicsView?'ON':'OFF')};UI.vehicle.onchange=()=>loadCar(UI.vehicle.value);$('#flip').onclick=()=>{userFlip=mod(userFlip+Math.PI,TAU);if(currentVisual)currentVisual.rotation.y=baseYaw+userFlip};

buildWorld();fallback();await loadCar(UI.vehicle.value);setLab(false);resize();reset();UI.loading.style.display='none';if(UI.build)UI.build.textContent='V0.9 BRANCH + BRIDGE';
