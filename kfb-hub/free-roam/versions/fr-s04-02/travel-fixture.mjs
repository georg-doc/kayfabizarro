...
export function createTravelFixture(scene){
 const scale=.01, radius=48, recoveryRadius=56, seed=1842;
 ...
 const waterMargin=.52,waterBins=new Map(),cell=p=>[Math.floor(p.x/waterMargin),Math.floor(p.y/waterMargin),Math.floor(p.z/waterMargin)];
 ...
 for(let i=0;i<index.count;i+=3){const ids=[index.getX(i),index.getX(i+1),index.getX(i+2)];if(ids.some(j=>land.getX(j)<.99))continue;
  ...
  if(v.every(p=>p.x>65)||v.every(p=>p.x< -65)||v.every(p=>p.z>65)||v.every(p=>p.z< -65)||v.every(p=>p.y< -45))continue;
 ...
 for(let x=-radius;x<=radius;x+=8)for(let z=-radius;z<=radius;z+=8)if(Math.hypot(x,z)<=radius)height(x,z);
 const surfaces=[{id:'island',shape:'gltf',kind:'terrain',road:true,p:[0,0,0]}];
 // No circular fence in S04-02. The validity edge is a soft recovery envelope outside the
 // playable area, so high-speed lines and wide turns are testable without wall traps.
 const start={p:[0,1.25,0],yaw:0};
 const inBounds=(p,mode)=>p.every(Number.isFinite)&&Math.hypot(p[0],p[2])<(mode==='safe'?radius-6:recoveryRadius)&&p[1]>-20&&p[1]<55;
 const diagnostics={seed,travelRadius:5,travelUnitsPerPhysicsUnit:scale,bodyHeightReference:.022,localRadius:radius,recoveryRadius,boundaryColliders:0,anchor:origin.toArray(),frameQuaternion:frameQ.toArray(),sourceTriangle:best.triangle,sourceNormalUpDot:bestScore,contactTriangles:triangles,dryAreaSamples:drySamples,waterVertexMargin:waterMargin,sampledHeightRange:maxHeight-minHeight,minSupportUpDot,gravityPolicy:'fixed local -Y; wide bounded probe only; 6-degree diagnostic ceiling',upBudgetDegrees:6.0};
