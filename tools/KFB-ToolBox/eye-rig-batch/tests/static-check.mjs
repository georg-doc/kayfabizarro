...
const files=['index.html','styles.css','app.js','lib/kaykit-eye-adapter.v1.js','lib/source-face-cleanup.v1.js','lib/medium-source-eye-cleanup.v1.js','lib/face-color-sampler.v1.js','data/gothgirl.seed.json','data/rig-medium-default.v0.json','data/rig-medium-actors.v0.json','data/rig-large-default.v0.json','data/rig-large-actors.v0.json','data/rig-large-reviewed.v1.json','docs/SOURCE_AUDIT.md','docs/SOURCE_COMPONENT_IDENTITY_2026-09-19.md','docs/source-components-0-11.html','docs/QA_EYE_CALIBRATION_LOOP_2026-09-19.md','docs/BATCH_FEATURE_BACKLOG_2026-09-19.md','docs/RIG_LARGE_MONSTROSITY_CALIBRATION_2026-09-19.md'];
...
const app=fs.readFileSync(path.join(root,'app.js'),'utf8'); const html=fs.readFileSync(path.join(root,'index.html'),'utf8'); const cleanup=fs.readFileSync(path.join(root,'lib/source-face-cleanup.v1.js'),'utf8'); const mediumCleanup=fs.readFileSync(path.join(root,'lib/medium-source-eye-cleanup.v1.js'),'utf8'); const faceColorSampler=fs.readFileSync(path.join(root,'lib/face-color-sampler.v1.js'),'utf8'); const adapter=fs.readFileSync(path.join(root,'lib/kaykit-eye-adapter.v1.js'),'utf8'); const seed=JSON.parse(fs.readFileSync(path.join(root,'data/gothgirl.seed.json'),'utf8')); const medium=JSON.parse(fs.readFileSync(path.join(root,'data/rig-medium-default.v0.json'),'utf8')); const actors=JSON.parse(fs.readFileSync(path.join(root,'data/rig-medium-actors.v0.json'),'utf8')); const large=JSON.parse(fs.readFileSync(path.join(root,'data/rig-large-default.v0.json'),'utf8')); const largeActors=JSON.parse(fs.readFileSync(path.join(root,'data/rig-large-actors.v0.json'),'utf8')); const largeReviewed=JSON.parse(fs.readFileSync(path.join(root,'data/rig-large-reviewed.v1.json'),'utf8')); const identity=fs.readFileSync(path.join(root,'docs/SOURCE_COMPONENT_IDENTITY_2026-09-19.md'),'utf8'); const componentHtml=fs.readFileSync(path.join(root,'docs/source-components-0-11.html'),'utf8'); const qa=fs.readFileSync(path.join(root,'docs/QA_EYE_CALIBRATION_LOOP_2026-09-19.md'),'utf8'); const backlog=fs.readFileSync(path.join(root,'docs/BATCH_FEATURE_BACKLOG_2026-09-19.md'),'utf8'); const largeDoc=fs.readFileSync(path.join(root,'docs/RIG_LARGE_MONSTROSITY_CALIBRATION_2026-09-19.md'),'utf8');
...
check('large-reviewed-per-actor-no-shared-default',large.status==='REVIEWED_PER_ACTOR_NO_CLASS_DEFAULT'&&large.authoringDefault===null&&large.reviewedBatch?.actorCount===4);
...
check('large-reviewed-batch-accepted',largeReviewed.schema==='kfb.eye-profile-batch/0.2-reviewed'&&largeReviewed.profiles?.length===4&&largeReviewed.profiles.every((p)=>p.reviewState==='ADJUSTED_APPROVED'&&p.eye?.baseColor===null&&p.sourceFace?.faceColorSource==='AUTO_SOURCE_HEAD'));
check('large-reviewed-known-actors',['monstrosity','black-knight','demon-lord','orc-brute'].every((id)=>largeReviewed.profiles.some((p)=>p.actorId===id)));
check('large-reviewed-source-pinned',largeReviewed.source?.revision==='52532aee17f85324a6d04ab53150216f00ac5d89'&&largeReviewed.source?.blob==='f55db82d1a76ba9362df261cf66517859abbf953');
check('large-seed-reviewed-link',large.status==='REVIEWED_PER_ACTOR_NO_CLASS_DEFAULT'&&large.reviewedBatch?.path==='data/rig-large-reviewed.v1.json');
check('face-color-sampler-present',faceColorSampler.includes("export function sampleActorFaceColor")&&faceColorSampler.includes("head-texture-around-eye-host")&&faceColorSampler.includes("pickDominantColor"));
check('face-color-sampler-reuses-texture-pattern',faceColorSampler.includes("document.createElement('canvas')")&&faceColorSampler.includes("getImageData")&&faceColorSampler.includes("flipY"));
check('auto-face-color-runtime',app.includes("sampleActorFaceColor")&&app.includes("runtime-head-texture-sample")&&app.includes("needsAutoFaceColor"));
check('canonical-large-profiles-loaded',app.includes("largeReviewed")&&app.includes("for(const p of largeReviewed?.profiles||[])"));
check('generic-actor-base-not-gothgirl',app.includes("actorBase=profile.sourceFace?.faceColor || actor?.faceColor || null")&&app.includes("baseColor:actor.faceColor||null"));
