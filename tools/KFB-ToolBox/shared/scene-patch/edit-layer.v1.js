/* KFB shared scene editor layer · v1 candidate
   Exact donor: tools/KFB-ToolBox/_inbox/KayKit Resident Atlas vS7/S38-rig-werkstatt/lib/edit-layer.js
   Donor revision: 6c1b02a3338c005127d45bc7bff3ecbb785f1342
   Change from donor: nested record-node collection for hosts whose editable objects sit below
   semantic container groups. No new gizmo, picking grammar or transform owner is introduced. */
/* KFB · Editor-Schicht
...
export function makeEditLayer(viewer, canvas, opts = {}) {
  ...
  /* Shared-host generalisation: Resident Scene keeps actor/habitat/props in nested groups.
     Record-bearing nodes may therefore live below root.children. S38's flat-root behaviour is
     unchanged; this only broadens semantic-group selection to nested host groups. */
  function collectRecordNodes(o, out) {
    if (recordOf(o)) { out.push(o); return out; }
    for (const c of o.children) collectRecordNodes(c, out);
    return out;
  }
  function nodeOf(obj) {
    let o = obj;
  ...
  const E = {
    ...
    follow() {
      ...
      if (_b.isEmpty()) { menu.hidden = true; return; }
      const c = _b.getCenter(_v).clone();
      const p = new THREE.Vector3(c.x, _b.max.y, c.z).project(viewer.camera);
