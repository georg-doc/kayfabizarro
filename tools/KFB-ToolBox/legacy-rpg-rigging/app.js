...
const state={catalog:null,tab:'characters',mode:'source',selected:null,sourceNode:null,actor:null,headPart:null,faceHost:null,eyes:null,mixer:null,scene:null,camera:null,renderer:null,controls:null,clock:new THREE.Clock(),profiles:{},reviews:{},eyeVisible:false,sourceEyesVisible:true,currentMotion:null};
...
async function assemble(){
  clearStageObject();$('#loading').hidden=false;const body=state.catalog.characters.find(c=>c.id===$('#bodySelect').value), head=state.catalog.heads.find(h=>h.id===$('#headSelect').value);try{
    const actor=await assembleLegacy({THREE,loader,rigUrl,partsUrl:sourceUrl(body.path),catalogCharacter:body,log:console.log});state.actor=actor;state.scene.add(actor.root);normalizeAndAddExisting(actor.root);
    ...
    buildFaceAndEyeCandidate(head);await applyWeapons();playMotion('Idle');frame(actor.root);logReport({placed:actor.report.placed,extras:actor.report.extraPlaced,missing:actor.report.missing,clips:actor.animations.length,head:head.label,bodyMaterials:body.bodyMaterials});
  }catch(e){console.error(e);logReport('ASSEMBLY FAILED\n'+e.message);setBadge('FAILED','danger');}$('#loading').hidden=true;
}
