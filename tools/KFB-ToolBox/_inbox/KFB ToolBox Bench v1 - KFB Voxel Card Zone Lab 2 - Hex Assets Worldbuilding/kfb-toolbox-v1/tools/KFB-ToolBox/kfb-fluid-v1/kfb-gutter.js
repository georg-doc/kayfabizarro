...
export function createGutter(cfg) {
  ...
  // ------------------------------------------------------------ Wanne
  const basinMat = cfg.basinMaterial || new T.MeshStandardMaterial({ roughness: 0.95, metalness: 0 });
  const basinGeo = new T.BoxGeometry(cell, 1, cell).translate(0, 0.5, 0);  // Ursprung unten: scale.y = Hoehe
  ...
  // ------------------------------------------------------------ Wasser
  const fluidMat = createFluidMaterial(T, { fluid: cfg.fluid, dudvMap: cfg.dudvMap, map: cfg.map });
