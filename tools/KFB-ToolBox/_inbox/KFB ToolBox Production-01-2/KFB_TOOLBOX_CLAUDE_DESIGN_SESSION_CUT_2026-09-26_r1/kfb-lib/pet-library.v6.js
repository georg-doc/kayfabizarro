/* PetLibrary — SPRINT 10: config-getriebene Character-Library (Baukasten).
   ...
   - MODS (steckbar, PRO AUFRUF bestellbar — seit 17.09.2026 führt kein Set eine Augenquelle als
     Vorgabe, siehe SETS.animals.mods): googly (Sclera+Pupille, geteilter Default-Anchor,
...
export const SETS = {
  animals: {
    ...
    overrides: {},
    /* ⚠ 17.09.2026 · HIER STAND ['googly','emotes'] — UND DAS WAR DER DAUERFEHLER.
       googly ist eine AUGENQUELLE. Als Set-Vorgabe hängte er sich an jede Figur, deren Aufrufer
       `mods: []` vergaß — zusätzlich zum EyeRig der ToolBox. Georg, am Bild gemessen: »kitten hat
       doppeltes augen-rig«, »das darf niemals vermischt werden«. Eine Vorgabe, die man nur durch
       Wissen abwählen kann, ist eine Falle; deshalb führt kein Set mehr eine Augenquelle.
       `emotes` bleibt — Comic-Pips über der Figur sind keine Augen.
       Wer googly will, BESTELLT ihn: `face.pupil.form = 'googly'` (so beschreibt es der Vertrag,
       kfb-pets.json embed.verbote) oder ausdrücklich `opts.mods: ['googly']`.
       Geprüft von `eye-source-guard.v1.js#assertEmbedEyePolicy` — Abnahmebedingung, kein Hinweis. */
    mods: ['emotes']
  }
...
/* Welche Mods hängen? Drei Wege, in dieser Reihenfolge — und keiner davon ist ein Zufall:
   1 · `opts.mods` ausdrücklich übergeben (auch `[]`) → das gilt, vollständig und allein. Der
       Aufrufer, der sich erklärt, wird nicht überstimmt (`eye-source-guard.loadOpts` nutzt das).
   2 · sonst die Set-Vorgabe `set.mods` — darin steht KEINE Augenquelle mehr (§4).
   3 · googly kommt hinzu, wenn der face-Block ihn bestellt: `face.pupil.form === 'googly'`.
   Ausgelagert und exportiert, damit die Regel prüfbar ist statt nur eingehalten. */
export function resolveMods(set, opts = {}) {
  if (Array.isArray(opts.mods)) return opts.mods.slice();
  const base = ((set && set.mods) || []).slice();
  const form = opts.face && opts.face.pupil && opts.face.pupil.form;
  if (form === 'googly' && base.indexOf('googly') < 0) base.push('googly');
  return base;
}
...
export class Character {
  ...
  async load(setId, charId, opts) {
    ...
    /* WS1 24.8. — GEMESSENER BUG: Hosts cachen das GLB (Pet Studio: `modelCache`) und geben bei
       jedem Laden DIESELBE Szene zurueck. Die Bounding-Box unten enthielt dann die Skalierung des
       vorigen Ladens, und der Faktor wurde ein zweites Mal eingerechnet: erst zu gross, beim
       naechsten Mal zu klein. Georgs Befund („Skalierungsproblem, das ich durch Kamera nicht
       beheben kann; bei erneutem Import sehr klein"). Deshalb VOR dem Messen auf 1 zuruecksetzen —
       ein Mass, das die eigene Wirkung mitmisst, ist kein Mass. */
    ...
    this.loaded = true;
    this.play('idle');
    // Mods anstecken (steckbar per Set-Config; googly nur auf Bestellung — siehe resolveMods)
    this._mods = resolveMods(set, opts).filter((m) => MODS[m]);
...
try {
  if (typeof window !== 'undefined') {
    ...
    window.PetLibrary = { Character, SETS, ARCHETYPES, MODS, resolveMods, faceShells, selectEyeShells, buildStripped };
  }
} catch (e) {}
