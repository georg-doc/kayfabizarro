...
import { FLOW } from './cologne-play.v1.js';
...
export async function createDrive(THREE, GLTFLoader, parent, opts = {}) {
  ...
  return {
    ...
    update(d, t) {
      ...
      group.position.set(d.x - fx * -DRIVE_Z, d.y + 0.56, d.z - fz * -DRIVE_Z);
    ...
    emitters(d) {
      ...
      const at = (lat, back, up) => ({
        x: d.x + rx * lat + fx * back,
        y: d.y + up,
        z: d.z + rz * lat + fz * back,
        nx: rx, nz: rz
      });
      return [
        { id: 'drive', width: 0.30, p: at(0, DRIVE_Z, 0.56) },
  ...
  return {
    ...
    update(d) {
      ...
      for (let k = 0; k < rigs.length; k++) {
        ...
        const head = src.p;
        const last = rig.hist[0];
        if (!last) rig.hist.unshift({ x: head.x, y: head.y, z: head.z, nx: head.nx, nz: head.nz });
        else {
          const dist = Math.hypot(head.x - last.x, head.y - last.y, head.z - last.z);
          if (dist >= STEP) {
            const steps = Math.min(6, Math.floor(dist / STEP));
            for (let i2 = steps; i2 >= 1; i2--) {
              const tt = i2 / steps;
              rig.hist.unshift({
                nx: head.nx, nz: head.nz,
                x: last.x + (head.x - last.x) * tt,
                y: last.y + (head.y - last.y) * tt,
                z: last.z + (head.z - last.z) * tt
              });
            }
          }
          // Der Kopf wurde frueher jedes Bild ueberschrieben. Dadurch schwankte die
          // Laenge des ersten Abschnitts zwischen 0 und 0,42 m, die Differenz zum
          // Nachbarn kippte und die Spur facettierte genau am Fahrzeug. Jetzt bleibt
          // die Vorgeschichte stehen; der Kopf wird weiter unten eigens gesetzt.
        }
        if (rig.hist.length > N) rig.hist.length = N;

        // Immer die ganze Vorgeschichte: keine Kante, an der die Spur endet.
        const h = rig.hist;
        const used = h.length;
        const halfW = rig.halfW * (0.4 + 0.6 * sp) * (1 + d.drift * 0.35);

        for (let i = 0; i < N; i++) {
          const o = i * 6, oa = i * 2;
          if (i >= used || used < 2) {
            const q = h[Math.min(i, h.length - 1)] || h[0];
            for (const s of [0, 3]) { rig.pos[o + s] = q.x; rig.pos[o + s + 1] = q.y; rig.pos[o + s + 2] = q.z; }
            rig.alp[oa] = 0; rig.alp[oa + 1] = 0;
            continue;
          }
          // GEMESSENER BEFUND: der Seitenvektor kam aus der Differenz benachbarter
          // Stuetzstellen. Bei 0,42 m Abstand ist diese Differenz in engen Kurven
          // instabil und kippt — daher die eckige Spur. Jetzt traegt JEDE Stuetzstelle
          // den Rechtsvektor ihres Ansatzpunktes aus dem Moment der Entstehung mit.
          // Die Gierung ist stetig, also ist die Spur von Bau aus glatt und faechert
          // im Drift korrekt auf, weil die Gierung dort schnell dreht.
          const q = h[i];
          let nx = q.nx, nz = q.nz;
          if (nx === undefined) {
            const a2 = h[Math.max(0, i - 1)], b2 = h[Math.min(used - 1, i + 1)];
            const tx = b2.x - a2.x, tz = b2.z - a2.z, tl = Math.hypot(tx, tz);
            if (tl < 1e-5) { nx = 1; nz = 0; } else { nx = -tz / tl; nz = tx / tl; }
          }
          const f = i / (used - 1);
          const w = halfW * Math.pow(1 - f, 0.5);
          ...
          // Weicher, laenger auslaufender Schwanz statt eines harten Endes
          const alpha = base * gate * Math.pow(1 - f, 1.9);
          rig.alp[oa] = alpha; rig.alp[oa + 1] = alpha;
        }
        // Kopf exakt auf den Ansatzpunkt setzen: die Spur haengt am Fahrzeug,
        // nicht an der letzten gespeicherten Stuetzstelle.
        if (used >= 2) {
          const w0 = halfW;
          rig.pos[0] = head.x - head.nx * w0; rig.pos[1] = head.y; rig.pos[2] = head.z - head.nz * w0;
          rig.pos[3] = head.x + head.nx * w0; rig.pos[4] = head.y; rig.pos[5] = head.z + head.nz * w0;
        }
