...
function fromFrozenFixture(fx, url) {
  ...
  return {
    ...
    } : null,               // Zone ohne Landmarke (huerth-crop-v0 bis S1.4) → Presenter nimmt das Blockmittel
    railways: (fx.railways || []).map((r) => ({ ...r, line: r.line.map(w) })),
    heroes: fx.heroes,
    home: fx.home ? { ...fx.home, centroid: { x: fx.home.centroid.x, z: -fx.home.centroid.z } } : null,
    hbf: fx.hbf,
    conflicts: new Set((fx.trackCorridorConflicts || []).map((b) => b.id)),
    counts: fx.counts,
    sourceCounts: fx.sourceCounts || null
  };
}
