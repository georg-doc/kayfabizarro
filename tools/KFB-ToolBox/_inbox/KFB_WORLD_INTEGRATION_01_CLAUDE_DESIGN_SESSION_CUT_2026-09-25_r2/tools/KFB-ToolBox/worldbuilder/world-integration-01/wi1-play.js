...
const CYC = new Set(['walk', 'run', 'backward', 'strafe.left', 'strafe.right', 'crouch', 'sneak', 'crawl']);
...
const TIER = {
  ...
  run: { clip: 'run' }, sprint: { clip: 'run', variant: 'sprint' },
...
export const TRANSITIONS = [
  ...
  ['run ↔ sprint', 'same clip · Shift held ≥ paceUp · label band ±4 %'],
...
export async function makePlay({ scene, camera, dom, groundAt, obstacles, hud, log = () => {} }) {
  ...
  function retune() {
    ...
    V.run = nat('run', 2.2);
    V.sprint = TUNE.sprint ? V.run * VARIANTS.sprint.rate : V.run;
