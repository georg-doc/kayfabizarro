...
## S22 · 2026-09-22 · R08 abgebrochen · Editor-Toolbox als Deliverable notiert

**R08 Vorratskeller: drei Fail-Runden, dann Abbruch.** Georgs Befund nach mehreren
Screenshot-Korrekturrunden: Treppe stand wiederholt falsch (nicht begehbar ausgerichtet, nicht in
der Eckzelle), das freistehende Innenteil (`wall_doorway_sides`-Vermutung) und die Bodenplatte
blieben falsch. Ursache über alle Runden: Koordinaten/Rotation für neue, ungemessene Bauteile
wurden aus der Bildkomposition geraten statt gemessen oder von Hand platziert — Postmortem mit
drei benannten Fails und drei neuen Hartregeln (R1 achsparallele Bauteile an der Wand, R2
geprüfter Bodenkontakt, R3 Rotation durchprobieren statt aus der letzten falschen Rotation
weiterrechnen) in `docs/POSTMORTEM_R08_S22.md`. **R08 gilt als nicht abgenommen.**

**Deliverable für die Toolbox (noch nicht gebaut):** der Inline-3D-Editor aus
`KayKit_Room_Study_S21.html` (`TransformControls`, Verschieben/Drehen/Gruppe/Raster/Absetzen,
neu ergänzt: Kollisionsfrei-Button + JSON-Ablage-Export/Import) soll zentral in die Toolbox
wandern statt pro Seite dupliziert zu werden — Georgs Vorgabe für den Weg nach R08. Umfang und
Begründung stehen im Postmortem, damit der Ansatz nicht verloren geht, bis er umgesetzt wird.

**Editor-Ergänzungen in S21 (sofort nutzbar):** „Kollisionsfrei" schiebt das ausgewählte Objekt
aus Überlappung mit Wand/Ecke/Endstück/Requisite (AABB, mehrere Durchgänge, Achse mit kleinerer
Überlappung) — On-Demand wie „Absetzen", kein Live-Constraint gegen das Gizmo. „Ablage
exportieren/importieren" sichert die Handkorrekturen eines Raums als rohes JSON, unabhängig vom
Rezept-Code-Export („Patch kopieren").

**Kleinfix R02:** Goldschein-VFX (`funken`/`glut`) beim Öffnen der Truhe sass 0,4–0,6 Einheiten
über der Boxmitte von `chest_gold` — sichtbar über dem Gold statt darin. Jetzt Boxmitte minus
0,15/0,25, im Goldhaufen.



**Der Vertrag zuerst, die Messung vor dem Bau.** Sprint 22 nennt R07 als ersten Raum und
nennt zugleich das Risiko: „Endcap-Platzierungsregel ist noch nicht gemessen". Genau daran
war R07 in S21 gescheitert. Diesmal stand die Sonde vor dem Rezept:
`tools/probe-wall-nodes.html` misst Endstück, T, Kreuz und Ecke aus der Draufsicht und
beantwortet drei Fragen je Teil — wo liegt das Gelenk, welche Arme hat es, wie weit reichen
sie. Die Rechnung ist als `nodeFrame()` in `lib/dungeon-grid.js` geteilt, damit die Seite
dieselbe Messung macht und keine Zahl abgeschrieben wird.

**Die Regel, die dabei herauskam.**

| Teil | Gelenk | Arme | Weite | Folge fürs Nachbarmodul |
|---|---|---|---|---|
| `wall_endcap` | 0,00 / 0,00 | 1 | **0,00** | bleibt GANZ — das Teil kragt 1,07 nach aussen aus |
| `wall_Tsplit` | 0,00 / 0,00 | 3 | 2,00 | halbieren, wie bei der Ecke |
| `wall_crossing` | Mitte | 4 | 2,00 | halbieren |
| `wall_corner` | 0,00 / 0,00 | 2 | 2,00 | halbieren |
| `wall_corner_small` | −0,24 / 0,24 | 2 | 0,76 | bleibt ganz (R02 unverändert) |

Zwei Grenzen im Klassifikator sind selbst Messwerte: ohne Untergrenze (0,95 × Plattendicke)
zählt beim Endstück die verjüngte Seitenflanke (0,92) als dritter Arm, ohne Obergrenze
(1,15 ×) bei der geraden Wand die 4,00 lange Flanke. `wall_corner_small` ist mit dieser
Messung NICHT zu erkennen (alle vier Randspannen ≈ Plattendicke) — Ecken laufen weiter über
`cornerFrame`, und das steht im Code, statt dass es jemand später herausfindet.

**`buildRoom()` kennt jetzt Knoten statt nur Ecken.** Ecke, T, Kreuz und freies Ende sind
derselbe Fall: ein Gitterpunkt mit n Armen. Welches Teil gesetzt wird, sagt das Rezept
(`ecke`, `enden`, `tTeil`, `kreuzTeil`); ob die anschliessende Fuge halbiert werden muss,
sagt die gemessene Armweite — nicht mehr ein globales Flag der Eckenwahl. Ein freies Ende
ohne Endstück (R02, Bühnendiorama) lässt das letzte Modul ganz.

**R07 · Grosse Halle** (Bild S07) ist damit gebaut: 8 × 5 Module, geschlossener Grundriss,
zwei Stummelwände mit Endstück, zwei T-Knoten in der Nordwand, eine Innenecke, Durchgang mit
Türblatt, Gitterfenster, vier Wandfackeln — und in der Mitte ein einziges Ziel, das Fass mit
der Beute. Prüfblock grün: 14 Requisiten stehen alle auf, 0 von 89 Paaren durchdringen sich,
0 stecken in der Wand, 2 freie Enden mit `wall_endcap` geschlossen, Blickfang 100 %,
Deckungsprobe 0,671 × 0,776 gegen Vorlage 0,67 × 0,76.

**Drei Befunde unterwegs, alle aus der Deckungssicht und nicht aus dem Gefühl.**

1. *Die Vorlage war die falsche Datei.* Die Nummern des Atlas sind NICHT die Nummern des
   Packs: Atlas `s02` ist `Dungeon_sample1.png`, `s07` ist `Dungeon_sample6.png` — um eins
   versetzt, weil s01 das Übersichtsbild ist. R02 hat seit S21 gegen ein fremdes Promobild
   überlagert (zweigeschossiges Lager), ohne dass eine Prüfung anschlug: die Zielwerte der
   Silhouette kommen aus der lokalen Datei, die Überlagerung kam aus dem Netz. Beide URLs
   korrigiert.
2. *Die Brennweite ist ein Messwert, kein Geschmack.* Mit fov 20 liefen die Seitenwände
   sichtbar zusammen, in der Vorlage stehen sie fast senkrecht. fov 12 deckt sich.
3. *Die Grundfläche entscheidet, wo die Trennwände liegen.* Bei 7 × 5 lagen beide
   Stummelwände und der Durchgang messbar zu weit rechts (0,41 statt 0,30 der Bildbreite).
   Bei 8 × 5 fallen Stummelwand, Trennwand und Durchgang auf die Vorlage — und in der
   Nordwand wird eine volle Fuge für das Gitterfenster frei, die vorher fehlte. Das ist pm7
   („erst die Grundfläche, dann möblieren") ein zweites Mal, diesmal vor dem Möblieren
   gemerkt.

**Zwei Requisiten-Lehren.** `rubble_large` misst 8,13 × 3,18 — das ist eine Felsformation
über zwei Module, kein Kiesel; sechs gestreute Brocken waren wieder die Streuregel statt der
Abmessung (pm1). Geblieben sind zwei `rubble_half`-Nester dort, wo im Bild welche liegen.
Und: die halbe Ausdehnung einer GEDREHTEN Box ist grösser als die halbe Kantenlänge — der
Goldstapel neben dem um 20° gedrehten Fass meldete 0,07 Durchdringung, bis er 0,4 weiter
aussen stand.

**Kleinkram, der zur Wahrheit gehört.** Die Prüfung „Durchdringung" zählt Kontakt nicht mehr
als Fehler, wenn ein Teil laut Rezept AUF dem anderen steht. Die Kamerahöhe darf jetzt bis
6 statt 2 steigen (eine Draufsicht braucht mehr). Der Blickfang liest sein Ziel aus dem
Rezept (`blickfang`) statt aus einem fest verdrahteten Truhennamen. `floor_tile_large_rocks`
ist wieder raus: die Kachel legt einen dunklen Felsriegel quer über den Boden, den die
Vorlage nicht hat.

---

## S19 · 2026-09-19 · Topfmuster: Register-Grammatik statt Zellraster

**Der Befund, der den Umbau ausgelöst hat.** Das Muster war ein RASTER. Ein Raster hat
keine Absicht — es füllt eine Fläche gleichmässig, wird bei jeder Dichte über 8 körnig,
liest aus drei Metern als Textur statt als Form, und zwei Töpfe nebeneinander sehen aus
wie dieselbe Tapete in zwei Farben. Die Antwort ist keine zehnte Musterfamilie, sondern
eine Grammatik: waagerechte Register, je eines mit genau einem Motiv, eine führende
Stimme, der Rest begleitet. Älter als Talavera, Oaxaca und Herrimans Coconino — und der
Grund, warum handbemalte Keramik aus einem Guss wirkt, obwohl kein Topf dem anderen
gleicht. Vollständig in `docs/DESIGN_LINE_TOPFMUSTER.md`.

**Vier Regeln, durchgesetzt von `composeBands()`, nicht von der Disziplin des Bauers.**
Leserichtung Fuss → Haupt → Begleitung → Kranz · höchstens EINE laute Stimme (Punkt,
Raute, Zahn, Schuppe, Kreuzblume), nie am Fuss · wenige grosse Formen (laut 3–10, leise
bis 16) · EINE Konturstärke über alle zehn Glyphen, weil alle dasselbe normierte Feld
liefern (`d < 1` innen, `d = 1` Kontur). Sechs Abnahmetore stehen im `patReport`.

**Vier Fehler unterwegs gefunden — jeder war eine Messung, die der Bau nicht gemacht hat.**

1. *Die Kontur frass die Fläche.* Die Kantenglättung aus `fwidth()` war auf 0,6 gedeckelt
   und ging in die Konturbreite ein — auf kleinen Töpfen wurde der Strich breiter als das
   Glyph und der ganze Topf ersäufte in Rahmenfarbe. Deckel auf 0,16, Breite ist jetzt
   eine feste Zahl plus eine halbe Glättung.
2. *Die Paletten hatten keine Rollenregel.* Terrakotta, Kobalt und Tinte trugen einen
   dunklen Grund mit fast ebenso dunklem Motiv — aus drei Metern ein brauner Fleck. Alle
   zwölf sind auf **hellen Grund / gesättigtes Motiv / fast schwarzen Rahmen** umgebaut,
   gemessen statt behauptet: `contrastRatio(Grund, Motiv) >= 2,6` ist ein Abnahmetor.
   Melone, Düne und Kürbis lagen danach noch bei 2,42–2,57 und wurden im Motivton
   abgedunkelt; jetzt liegen alle zwölf zwischen 3,29 und 7,82:1.
3. *Die Begleitung zählte gegen sich selbst.* Ein schmales Kranzband hat eine winzige
   eigene Bandhöhe, also lieferte die Quadrat-Rechnung dort 19–22 Wiederholungen und sass
   dauerhaft am Deckel — der Massstabsregler bewegte nur noch die Führung. Die Teilung der
   lauten Stimme ist jetzt der Taktgeber, die Begleitung ihr 1,9-faches.
4. *Presets wurden still verworfen.* `defaultStyle()` setzt selbst `family: 'bands'`, also
   war nach dem `Object.assign` nicht mehr erkennbar, ob der AUFRUFER eine Familie genannt
   hatte — alle zwölf Presets zogen ein zufälliges Leitmotiv. Beweis 1 hiess „Pack-Optik"
   und zeigte einen vollgemusterten Topf. Die Auflösung hängt jetzt am rohen Argument.
   Dabei aufgedeckt: ein LEISES Leitmotiv (`bands`→Welle, `blocks`→Greca) fiel ganz aus dem
   Stapel, weil der Platz nur bei lauter Stimme besetzt wurde; und `plain` erzeugte
   Ornamentbänder statt der unbemalten Pack-Optik.

**Non-repetitiv ohne Stilbruch.** `seed` wählt Registerstapel, Glyphen und Position der
Führung · `wobble` verzieht Registergrenzen und Glyphenmitten mit einer langsamen
Sinuswelle (die Hand am Pinsel) · `variance` würfelt je Glyph-Instanz Grösse, Höhenversatz
und gelegentlich einen Farbtausch Motiv↔Akzent. 1440 Kompositionen geprüft (4
Topfproportionen × 120 Seeds × 3 Dichten), kein Verstoss gegen die Tore.

**Biome.** `paletteForBiome(biome, seed)` zieht deterministisch aus der Palettenliste der
Zone (frei / Wüste / Dschungel / Küste / Markt / Hochland / Nacht). Ein Level-Bauer sagt
„Wüste" statt zwölf Paletten zu sortieren. Palette von Hand setzt `paletteLocked`.
„Dunkler Grund" (`invert`) tauscht Grund und Motiv, lässt Akzent und Rahmen stehen — eine
Umkehrung der Linie, kein Bruch mit ihr.

**Zwei Reparaturen an der Schale.** Der Bericht beschrieb nach jeder Live-Änderung den
UNTERSETZER statt den Topf: `b.styles` enthält beide, der Bauzeit-Bericht nimmt
`styles[0]`, `rebuildStyle()` nahm den letzten Rückgabewert — das Panel wechselte nach der
ersten Interaktion stillschweigend das Subjekt. Und „Nur Ansicht" liess den Metakasten
unten links stehen; die Legende hat dieselbe Spezifität wie die Beweis-Regel und brauchte
die spätere Quellzeile.

**Vertrag für S20** in `docs/SPRINT_20_LIVING_PLANTS.md`: Muster an die Topfgeometrie
binden (Silhouettenmessung `r(y)` statt drei Zahlen), Bewegungsvorrat als Clip-Leiste
(idle/breathe/wind/shake/interact/startle/wilt), Bühnen TinySkies-Terrain und
OMS-Grotesque. Nichts davon gebaut.

---

## S18.2 · 2026-09-19 · Übergabe an WSA Lead, Backlog, Vertrag für S19

Keine Verhaltensänderung an der Werkbank. Dieser Eintrag schnürt den Stand.

**Drei Dokumente.** `docs/HANDOFF_WSA_S18.md` ist die Prüfvorlage: Vertrag in fünf Sätzen,
die vier tragenden Entscheidungen (Projektion statt Atlas · Erde am Radius · Augen im Netz ·
kein zweites Augensystem), Clean-Run in sechs Schritten, die gemessenen Zahlen mit ihrer
Sonde, vier Risiken zur Entscheidung. `docs/BACKLOG_PLANT_PROP.md` sortiert die offenen
Punkte nach P0–P3 — P0-1 Revisionsbindung an `main` statt Commit-SHA, P0-2 fehlendes
Durchdringungs-Gate für Mischgruppen. `docs/SPRINT_19_CARTOON_DEFORMER.md` ist der Vertrag
vor Code für den Cartoon-Deformer nach `skills/kfb-cartoon-animation_v2.md`.

**Inhaltswünsche gegen den Bestand geprüft, nicht gegen die Erinnerung.** Kakteen
(`cactus_A…D`, 0,33–0,64) und Sukkulenten (`succulent_A…D`) sind **vermessen vorhanden** —
sie fehlen nur in der Grammatik. Bäume ebenso (Quaternius 2,08–5,88; KayKit Forest
3,5–10,8), brauchen aber eine eigene Maßstabsklasse. Früchte liegen in den Restaurant Bits
(`food_ingredient_*`, 144 Teile vermessen), Hamburger-Bauteile auch (`food_burger`,
`food_ingredient_bun_top/bottom`) — ein Hamburger-Baum ist damit **Komposition, kein Asset**.
**Fleischfressende Pflanzen, Pilze, Blumen und Süsswaren fehlen** in allen geprüften Packs;
das Forest Pack hat 105 Teile und darunter keinen Pilz. Als Aufgabe P2-Q notiert:
Contents-API-Sonde + Ladeversuch, Ergebnis nach `docs/PACK_GAPS.md` — Fund oder Lücke,
beides wird eingetragen, nichts wird mit Ersatzgeometrie überspielt.

**Export r2** `export/KFB_Plant_Prop_Lab_v1_EXPORT_2026-09-19/` (25 Dateien, 252 KiB,
Revision `2026-09-19-r1`) mit den S18.1-Modulen und den drei neuen Dokumenten.

**GitHub-Check-in vorbereitet, nicht ausgeführt.** Der Zugang dieses Arbeitsbereichs ist
lesend: Branch-Anlage und Schreiben auf `main` antworten mit `403 Resource not accessible by
integration`. Der Commit liegt deshalb push-fertig in
`export/github-checkin-2026-09-19/` — Repo-Baum `tools/plant-prop-lab/` (der Slot stand auf
„NO RUNTIME YET"), dazu `COMMIT_MESSAGE.txt` und `PUSH_ANLEITUNG.md`. Genau eine bestehende
Datei würde überschrieben: `tools/plant-prop-lab/README.md`, Blob `2730de64b5e1…`.

---

## S18.1 · 2026-09-19 · Sichtungsbefunde: Erde, Stammachse, Augenmass, Cartoon-Raster

Fünf Befunde aus der Bildsichtung, alle an derselben Stelle behoben, an der sie entstanden —
in der Messung, nicht in einem Korrekturfaktor.

**Erde statt Muster im Topfinneren.** Das Urteil fällt jetzt am Normalenvektor und am
gemessenen Innenradius, nicht an einer geratenen Höhe. Gemessen an `pot_D_large`: die
Erdkappe ist eine flache Fläche bei **v = 0,85** mit Radius ≤ 0,55, der Randkranz liegt bei
0,61…0,73 — eine Höhenschwelle trennt die beiden nicht, ein Radius schon. `innerRadius()`
nimmt den grössten Radius mit nach innen zeigender Normale (0,53) plus 12 % Luft = **0,597**.
Jede Palette trägt dafür eine eigene fünfte Farbe (`soil`), zweifach gekörnt, mit flacherer
Lichtantwort — sonst säuft die Topföffnung im Eigenschatten zu Schwarz ab. Das Kantenband
liegt jetzt nur noch an der Wand, nicht auf der Deckfläche.

**Stamm mittig, nicht im Rand.** `baseFootprint()` misst den Schwerpunkt der untersten 12 %
der Vertices und gleicht ihn aus. Zweiter, grösserer Fehler im selben Block gefunden: die
Fussebene wurde mit `-crownY / s` gesetzt statt mit `-crownY - raw.min.y·s`. Bei s = 1
identisch, bei skalierten Quaternius-Spendern um ein Vielfaches der Topfhöhe daneben. Der
Versatz mehrerer Pflanzen ist zusätzlich gegen den gemessenen Fussradius gedeckelt.

**Augen ins Netz eingepasst.** Der Wirt bekam seinen Radius aus der Boxbreite — auf einem
konischen Topf ist das der Kranz, also sassen die Augen davor. `wallRadiusAt()` misst
stattdessen das 90-%-Quantil der Radien im Höhenband (108 Vertices bei `pot_D_large`).
Ankerwerte je Sitz statt einer Tier-Voreinstellung: `ring` 0,22 → **0,50** (Augenradius
0,10 → **0,23**), `inset` 0,02 → 0,10.

**Cartoon-Raster.** Zellzahl gedeckelt (`CELL_LIMITS` nu ≤ 14, nv ≤ 8), Voreinstellung
Dichte 10 → 7. Jedes Motiv hat eine **Kontur** aus derselben Feldrechnung (ein Skalarfeld,
eine Schwelle → Füllung und Linie), der Akzent ist eine eigene runde Form statt Haarlinien.

**Harmonische Paletten.** 6 → **12**, jede mit benannter Harmonie (analog / komplementär /
Triade / Split / monochrom) und Rollen statt Indizes: `roles(id)` → Körper, Motiv, Akzent,
Rahmen, Erde.

**Offen:** Revisionsbindung weiterhin `main` statt Commit-SHA; L3–L5 nicht gebaut.

---

## S18 · 2026-09-18 · Crazy-Cat Plant Prop Lab: Requisite als Rezept, Muster als Projektion

**Neu:** `KFB_Plant_Prop_Lab_S18.html` + `lib/plant-inventory.js` · `lib/plant-recipe.js` ·
`lib/plant-pattern.js` · `lib/plant-rig.js` · `lib/plant-eyes.js` · `lib/plant-light.js`.
Zwei Packs aufgenommen (`tt_plants`, `qu_env` in `lib/kit-lab.js`), **72 von 72 Teilen** geladen
und vermessen, 0 Fehlschläge. Dateiwahrheit wieder über die **Contents-API**
(`tools/probe-plant-packs.html`) — die Tree-API listet `.gltf` in diesem Repo nicht, dieselbe
Falle wie bei den Bits-Packs in S14.

**Das Muster entscheidet eine Messung, nicht der Geschmack.** `probeUV()` korreliert je Topf die
UV-Achsen mit Umfangswinkel und Höhe. Ein zylindrisch abgewickelter Topf hätte |r| ≈ 1; gemessen
sind **r(Umfang) = 0,010 / 0,024** und r(Höhe) = 0,386, die u-Spanne ist ein Atlasfenster
(0,076–0,670). Die Quaternius-Spender sind **entartet**: `Plant_1` hat v 0,105…0,106 — ein
einziger Texel. Also trägt die analytische Zylinderprojektion (`u = atan2(z,x)/2π`, `v` = Höhe,
Shader hinter `#include <map_fragment>`, Atlas-**Luminanz** bleibt als Schattierung stehen), und
der UV-Canvas-Weg bleibt als sichtbar verzogenes Vergleichsbild umschaltbar. Acht Musterfamilien
über ein float-Uniform statt über Defines — ein Familienwechsel baut den Shader nicht neu
(`customProgramCacheKey` konstant).

**Nicht-destruktiv heisst Material klonen.** `instance()` klont die Szene, aber three teilt die
MATERIALIEN zwischen Klonen: ohne `material.clone()` färbt ein Topfmuster jeden Topf jeder Szene
mit — inklusive der Quelle im Cache.

**Die Einsetztiefe stand im Pack.** `monstera_plant_large_potted` (3,680) minus
`monstera_plant_large` (Oberkante 2,880) = **0,800** Ursprung in einem 1,00 hohen Topf, also
0,20 der Topfhöhe. Als Anteil statt absolut, damit ein `small`-Topf (0,40) dieselbe Pflanze 0,08
tief einsetzt. Der Bau prüft das gegen die eigene Rechnung nach.

**Zwei Lagen, die nur eine Strahlprobe liefert.** Der Topf steht auf dem **Innenboden** des
Untersetzers, nicht auf seiner Oberkante — und der Innenboden ist Rückseite: ein Strahl von oben
trifft ihn nur, wenn die Materialien für die Dauer der Messung zweiseitig geschaltet werden
(danach zurück; der Bericht nennt je Teil den Weg). Die **Topfkante** ist der grösste Radius, auf
dem ein Strahl noch nahe der Oberkante auftrifft: `pot_C_large` 0,625 gegen `pot_A_large` 0,687
bei fast gleicher Aussenbreite (1,50 / 1,498).

**Maßstabsbrücke statt Faktor aus der Luft.** Tiny Treats ist Tischmaßstab, Quaternius sind
Weltbäume (`Tree_Spiral_2` 6,187 hoch, `Tree_Swirl_1` 7,349). Der Alienmaßstab folgt dem
gemessenen Kantenradius: Kronenbreite = 2,1 × Kantendurchmesser (`Tree_Spiral_2` → Faktor 0,345).

**Kein zweites Augensystem.** `pet-eye-rig.v6.js` wird über jsDelivr geladen und bekommt einen
gemessenen FaceHost (`lib/plant-eyes.js`). Drei Befunde, die nur aus dem Lesen des Spenders
folgen: der Wirt darf **nicht** `visible = false` sein (das Rig hängt IN ihm — unsichtbar über
`opacity 0` + `depthWrite false`), er braucht `castShadow = false` (sonst wirft ein unsichtbares
Ellipsoid Schatten — derselbe Fehler wie die Kontaktschatten-Scheibe in S13.3), und vor `build()`
muss die Weltmatrix stehen. Dazu einer aus dem Bild: **`PotRoot` trägt die Zufallsdrehung des
Rezepts**, ein Gesicht darin blickte bei Vorgabe E 137,5° vom Betrachter weg — der Wirt hängt
jetzt in der unverdrehten Container-Elternschaft, bei `crown` mit Gierausgleich. Die sechs Mienen
werden aus `contracts/kfb-pet-graft-driver.v4.json` **gelesen**, nicht erfunden.

**Laub ist keine Gesichtsfläche.** Ein Wirt in der Kronenmitte liefert Augen zwischen den
Blättern (Monstera, Pothos, ZZ sind offene Strukturen). Lesbarkeit gemessen am Bild:
`pot` > `float` > `crown`; `pot` ist Standard, alle drei bleiben wählbar.

**Landmarke ist ein Verhältnis, keine Zahl.** Eine einzelne Riesenpflanze im Bild ist von einer
normalen nicht zu unterscheiden. Beweis 4 zeigt deshalb Faktor 12 **und** dieselbe Komposition in
Tischmaßstab daneben.

**Zwei Fehler der Werkbank selbst, beide behoben.** (1) *Leere Bühne mit Horizont*: ein Klick
während eines laufenden Baus räumte die Szene, während der ältere Bau weiterlief und danach auf
eine nicht mehr existierende Box rahmte — Bauten sind jetzt gereiht, ein `gen`-Zähler verwirft
überholte. (2) *Stehende Bewegung, Etiketten oben links*: die eigene Bildschleife hatte den
**rAF-Notnagel** nicht, den `makeViewer` hat — diese Vorschau parkt `requestAnimationFrame` auch
im sichtbaren Zustand (Lehre aus S7.1/S11, hier wieder fällig).

**Farbtafel:** vier Kalibrierungen (TinySkies Tag · Abend · Interieur warm · Nacht kühl) als
Pixelabzug derselben Aufstellung. `installLight()` räumt die Lichter des Viewers EINMAL ab, statt
zwei Lichtsätze zu addieren. Wieder gebraucht: `readRenderTargetPixels` liefert lineare Werte,
solange `rt.texture.colorSpace` nicht auf sRGB steht, und Zeilen von unten (S13.2). Die Kachel
rahmt über den **horizontalen** Öffnungswinkel — die Orbitkamera zu leihen und nur `aspect` zu
ändern schnitt die Reihe ab.

**Geprüft:** 12 Pflichtprüfungen, 11 PASS, 1 TEILWEISE (schmales Fenster: geerbte CSS-Regel,
nicht neu gemessen). 10 Regenerationen über 10 Seeds, 1–5 Töpfe, alle fünf Maßstabsklassen:
0 Fehlschläge, 0 fehlende Teile. Seed 5150 zweimal: Rezepttext und gebautes Weltmaß identisch.
Rezept-Rundlauf Export→Import→Export byte-identisch (2132 Zeichen).

**Export:** `export/KFB_Plant_Prop_Lab_v0_EXPORT_2026-09-18/` — 22 Dateien, 216 KiB, ohne ein
einziges kopiertes Modell. GITHUB PUSH: NOT PERFORMED.

**Bewusst nicht gebaut:** Knochen-Rigs, L3–L5 (EXPRESSIVE/INTERACTIVE/CHARACTER),
Kollisions-/Supportphysik, Konsumenten-Integration, Katalogeintrag in Game Dev Studio.

---

## S17 · 2026-09-18 · Restaurant neu aufgesetzt: Platzierungsgrammatik statt Flächenlogik

**Befund an S16.** Der Löser war messbar kollisionsfrei (0 Durchdringungen von 2016) und trotzdem
falsch: Teller auf dem Boden, zwei Spülbecken im Durchgang, ein Bilderrahmen in der
Fensteröffnung, ein einzelner Stuhl mitten im Raum. Kein einzelner Fehler, sondern ein fehlendes
Modell — Requisiten wurden nach **Grundfläche** verteilt, und Fläche sagt nichts darüber, worauf
ein Teil gehört, wozu es dient und wo es im Weg steht.

**Drei Begriffe statt einer Fläche** (`lib/restaurant-plan.js`, `KayKit_Restaurant_S17.html`).
*Anker*: `ground` · `backed` (Rücken auf der Wandfläche) · `hung` · `carried`. Ein Teil ohne Anker
wird nicht platziert. *Träger*: `hob` · `worktop` · `dinertop` · `setting` · `passtop` ·
`shelftop`; die Zuordnung ist **gerichtet** — `pan_A` nennt `hob` als einzigen Träger, also kann
eine Pfanne auf keinem Gasttisch landen, nicht weil ein Wurf es verhindert, sondern weil die
Verbindung fehlt. *Zone*: Raum plus ein Zirkulationsnetz auf dem Quadrantenraster (2,00).
Requisiten entstehen **zuletzt**, aus angemeldeten Trägerflächen — auf dem Boden gibt es keine.

**Gang zuerst, nicht als Restfläche.** Vor jeder Öffnung sind beidseitig die anliegenden
Quadranten Gang (hart, kein Wurf), dazu der Weg Eingang → Küchentür. Auf **Zellebene**: eine
Quadrantenkette läuft treppenförmig und schnitt fünf Zellen an, von neun Gastzellen blieb eine
übrig. Unter sechs Gastzellen dagegen ist die feine Kette richtig — vier Gangzellen sind in einem
2 × 2-Gastraum der ganze Raum (gemessen 18 von 24 Quadranten, null Tische).

**Räumung statt weiterer Sperrregeln.** Regeln allein reichen nicht: ein Stuhl, der legal in die
Nachbarzelle ragt, kann der letzte Engpass sein. Also wird die Begehbarkeit **gemessen** (zweite
Breitensuche über die unbelegten Quadranten) und im Konfliktfall der billigste Weg freigeräumt —
Arbeitsstationen ausgenommen, die sind gebaut, nicht gestellt. Der Überstand einer tiefen Station
sperrt seither den **Stellplatz**, nicht den **Weg**: 0,35 versperren keinen 2,00 breiten Gang,
und diese fehlende Unterscheidung hielt eine Küchentür für unerreichbar.

**Gemessen statt geschätzt, vier neue Stellen.** Die Wandfläche einer Trennwand liegt `2 · face`
weiter als die Fuge (die Wand rückt in den Gastraum) — an ihr steckten Bilder und Kommoden eine
halbe Wanddicke im Mauerwerk. Die Naturausrichtung eines Wandteils ist seine **dünnste
waagerechte Achse**, ihr Minimum die Rückseite; damit liegt ein Rahmen auf der Wand statt flach
daran. Die **Ablage der Durchreiche** wird per Strahl von oben gemessen (z −0,45 · y 1,15) — lokal
+z zeigt bei einer x-laufenden Fuge nach global +z, bei einer z-laufenden nach global +x, und
dieser Unterschied entscheidet, auf welcher Seite der Wand der Teller landet. Der Stuhl wird
**vor** dem Tisch gewählt, weil er die Tischgrösse bestimmt: ein 3,00-Tisch mit Stuhlkreis
braucht 2,40 je Seite und damit mehr als eine 4,00-Zelle — ohne diese Reihenfolge standen acht
Tische mit fünf Stühlen im Raum.

**Vier neue Gates.** Öffnungsfreiheit (Strahlen quer durch jede Lichtung gegen alles ausser Wand,
Boden, Türblatt und Durchreichenablage) · Begehbarkeit · Trägerprobe (unter jeder Requisite wird
die Fläche gemessen, auf der sie liegt) · Möbelrücken an der Wand, inklusive Test „hängt aussen“
(ein Strahl nach unten muss einen Boden finden — Rückenabstand allein ist auf beiden Wandseiten
gleich). Stand: 5 × 4 bei voller Dichte, alle Gates grün, **0 Durchdringungen von 6441**,
8 Tische · 16 Stühle · 23 Requisiten; Dungeon-Hülle ebenso grün.

**`lib/kit-lab.js`:** `repairTextures` hat jetzt einen packweiten Spenderpool (jedes intakt
geladene Material wird registriert). Eine Szene mit genau einem Bilderrahmen blieb sonst schwarz,
weil ein Teil nur von einem anderen Teil **derselben** Szene geheilt werden konnte.

---

## S14–S16 · 2026-09-18 · Drei Bits-Packs aufgenommen: Space Base, Restaurant, Furniture

**Inventar gelesen statt geraten.** Die `.gltf`-Dateien tauchen in keiner Verzeichnisliste des
Werkzeugs auf (gleiches Muster wie Hex und Dungeon) — diesmal aber nicht mit Kandidatennamen
erprobt: die **GitHub-Contents-API** listet sie (`tools/list-pack-files.html`). Ergebnis in
`lib/bits-inventory.js`: **57 / 144 / 53 = 254 Teile**, alle geladen und vermessen
(`tools/measure-bits-packs.html`, 0 Fehlschläge). Das ersetzt für diese drei Packs die
Ladeversuch-Runden aus S3b/S11/S13 vollständig.

**`KayKit_Bits_Model_S14.html` · ein Modell je Pack, plus die Kompatibilitätsfrage.**
Space Base: Zelle trägt das Modul, **Fuge trägt die Röhre**, dritte Achse trägt das Gelände.
Restaurant: **dieselbe Fugenarchitektur wie der Dungeon** plus ein zweites, feineres Band an der
Wandfläche. Furniture: **kein Raster** — die Ankerklasse steckt im **Pivot** und ist damit
messbar (Wand: z = 0 einseitig verankert, y um 0 zentriert; Boden: Unterkante y 0).

**Gemessene Kompatibilität (`tools/probe-corners-compat.html`).** Dungeon und Restaurant teilen
Zelle (4,00), Wandlänge (4,00) und Wandhöhe (4,00) — eine Küche passt ohne Skalierung in einen
Dungeon. Verschieden sind genau zwei Zahlen: **Wanddicke** 0,50 gegen 1,00 und **Boden-Oberkante**
0,50 gegen 0,05. Space Base und City Builder liegen auf 2,00; wegen 4 = 2 × 2 zerfällt eine
Innenzelle in vier Aussenzellen ohne Rest. Gleicher Maßstab in allen Packs, belegt an den
Requisiten (chair_A 0,75 in Restaurant UND Furniture, table 2 × 2 in Furniture UND Dungeon).

**`KayKit_Space_Base_S15.html` + `lib/space-grid.js`.** Höhenkarte in zwei Ebenen (das Pack hat
genau zwei Platten), Plateaurand als Kantenlöser: eine Seite tief → Rampe, zwei anliegende Seiten
→ Aussenecke, tiefe Diagonale → Innenecke, **zwei gegenüberliegende Seiten → kein Bauteil**, also
wird die Zelle abgetragen. Module wachsen auf einem **Untergitter mit Schrittweite 2** (nur dort
passt die gerade Röhre mit gemessenen 2,00 exakt in die Lücke); Diagonalröhren gibt es für
anliegende (0,83) und zwei Zellen entfernte (3,66) Module. Röhren sind **waagerecht** — zwei
Ebenen lassen sich nicht verbinden, und der Generator bietet es nicht an.

**`KayKit_Restaurant_S16.html` + `lib/restaurant-grid.js`.** Hülle als Fugengraph (Tür,
Durchreiche, Fenster), darüber das **Arbeitsband** auf halbem Modul: je geschlossener Küchenfuge
zwei Plätze à 2,00, Rückwand nach −z, Innenecke in Raumecken, Hängeschrank/Abzug auf der
Wandfläche. Gastraum und Möblierung aus **Furniture Bits** über die drei Ankerklassen.
**Hüllen-Umschalter Restaurant ⇄ Dungeon** — dieselbe Raumlogik, vier gemessene Zahlen anders.

### Was dabei gelernt wurde, und zwar am Fehler

- **Die Zelle ist der Ankerpunkt, nicht die Ausdehnung.** `landingpad_large` misst 2,50,
  `cargodepot_A` 2,07, `basemodule_D` 2,44 — alle grösser als ihre 2,00-Zelle. Mit gemessenen
  Fussabdrücken im Generator fielen die Durchdringungen von **51 auf 0** (13.000 Paare).
- **Bodenkontakt ist messbar, nicht rechenbar** (von Georg am Bild erkannt: schwebende Räder).
  `spacetruck` reicht 0,08 unter seinen Pivot, `roofmodule_*` 0,10, `lander_A` sitzt auf einem
  Landeplatz. Eine Auflagefläche **je Rolle** plus Strahl von oben: 42/42 Kontakte, 4 nachgesetzt.
  Dächer stehen bewusst NICHT in dieser Tabelle — ihr Sockel taucht konstruktiv 0,10 ein.
- **Wo die Wand steht, entscheidet die Sample-Szene des Packs, nicht der Geschmack.** Die
  Unterschränke stossen dort flächenbündig an die Wand UND tilen im 2er-Takt inklusive Ecke.
  Beides gleichzeitig geht nur, wenn die **Innenfläche** der Wand auf der Fuge liegt. Mit der Wand
  mittig auf der Fuge (S13.2-Konvention) überlappt die Innenecke ihre Nachbarn um eine halbe
  Wanddicke.
- **Wandfläche ≠ halbe Nenndicke.** Die Dungeon-Wand ist 1,00 dick, ihre Mauersteine ragen heraus:
  gemessene Fläche 0,50, Relief **0,25**. Anschlag ist die vorderste Fläche (sonst stecken die
  Steine im Schrank), und die Bandprobe lässt genau das Relief als Spalt zu — bei der glatten
  Restaurantwand also 0,00. Dieselbe Lehre wie der Fackelabstand in S13.3.
- **`wall_doorway` bringt im Dungeon sein Türblatt mit** (`wall_doorway_door`, S13.2). Es wird
  ENTFERNT, nicht versteckt: three.js raycastet auch unsichtbare Objekte — die Strahlprobe meldete
  sonst 0 von 3 Türen begehbar bei sichtbar offener Tür.
- **Vorzeichen der Trennwand.** Sie rückt von der Küche in den Gastraum; vom Gastraum aus liegt
  die Fuge also in Richtung −out. Mit `out === v` verglichen greift die Freiraumregel nie, und ein
  3,00-Tisch landet in einer 1,00 dicken Wand (gemessen 0,50).
- **Stuhlabstand aus der Messung:** Platte 3,00 (halb 1,50) + Stuhl 0,85 tief (halb 0,43) + 0,03 →
  1,96. Bei 1,75 schneidet die Rückenlehne die Platte um 0,13. Ein Stuhl darf in die Nachbarzelle
  ragen — dann wird sie mitbeansprucht, sonst greifen zwei Stuhlkreise ineinander (0,74).
- **Eine Röhre belegt ihren Weg.** Ohne diese Buchführung kreuzten sich zwei lange Diagonalen
  (0,60 Röhre gegen Röhre) und Felsen standen in den Röhren. Die kurze Diagonale belegt dagegen
  KEINE Zelle — sie sitzt im Eckspalt (0,83); die konservative Rechteckregel hat jeden Anbau von
  seiner Röhre getrennt und drei Inseln erzeugt.
- **Verbindbarkeit ist eine Eigenschaft des Verfahrens, keine Prüfung danach.** Unabhängig
  gewürfelte Module ergaben verlässlich vier Inselgruppen; seit die Module über
  Untergitter-Nachbarn WACHSEN, ist es eine.

### Nachtrag aus der Abnahme · Furniture war unsichtbar

Die Gates waren grün, und trotzdem fehlte das dritte Pack: **0 Möbelteile im Standardzustand**,
3 bei Schieber 100 — aus 53 Teilen. Ursache war nicht das Gate, sondern der Löser: Wandmöbel nur
an Aussenfugen OHNE Öffnung (von 17 Fugen qualifizierte genau **eine**, weil 9 Öffnungen tragen)
und Bodenmöbel nur in ganz freien Gastraumzellen (von 6 waren 2 frei, davon oft die gesperrte
Eingangszelle). Erwartungswert unter 1. Jetzt: Wandmöbel auch an Fenster- und Trennwandfugen,
mehrere je Fuge an gemessenen Positionen, und Bodenmöbel nach derselben **Quadrantenlogik** wie in
der Küche — nach Grundfläche getrennt (Quadrant ≤ 1,80: Kaktus, Stehlampe, kleiner Schrank; ganze
Zelle: Sofa 3,00, table_low 2,40). Ergebnis 11–12 Teile je Szene statt 0.

Drei Folgefehler, alle gemessen statt geschätzt:
- **Rahmen greifen um die Ecke.** Ein Wandmöbel steht 0,20 aus der Wand; zwei auf rechtwinkligen
  Fugen trafen sich am Fugenende (0,10). Grenze jetzt 1,60 − halbe Breite statt 1,90.
- **Überstände treffen sich in zwei Lagen**, nicht in einer: gegenüber über die Zelle (2,35 + 2,35
  gegen 4,00) und **diagonal auf rechtwinkligen Wänden**, wo die Vorderkanten in der Zellmitte
  zusammenstossen (gemessen 0,32, „oven ∩ oven"). Gesperrt wird nur der echte Überstand (> 0,10);
  die 0,04-Lippe eines gewöhnlichen Unterschranks bleibt erlaubt.
- **Erste Fassung dieser Sperre war zu scharf** und liess für jeden Gegenplatz kein Bauteil übrig
  — `R.weighted` auf einer leeren Liste warf, und die Seite blieb bei „rechne Grundriss…" stehen.
  Toleranz des Audits steht jetzt auf 0,09 (zwei Lippen à 0,042), mit Grund in der Zeile.

### Gates (10 × 10 Space Base, Saat MARS-1 · 4 × 3 Restaurant, Saat DINER-1)

S15: Höhenprobe 79/79 · Rampenprobe 14/14 · Bodenkontakt 42/42 · Röhren angedockt 14/14 ·
Durchdringungen 0 von 13.041 · Inseln 1 · unbaubare Randzellen 0.
S16 Restaurant-Hülle: Türen 2/2 · geschlossene Fugen 15/15 · Arbeitsband 9/9 · Bodenkontakt 25/25
· Durchdringungen 0 von 2016 · 12 Möbelteile.
S16 Dungeon-Hülle (Crossover): Türen 3/3 · Türblätter entfernt 3 · Fugen 14/14 · Arbeitsband 12/12
· Wandrelief 0,25 · Bodenkontakt 26/26 · Durchdringungen 0 von 2080 · 11 Möbelteile.

---

## S13.4 · 2026-09-17 · Requisiten am Rand, Bedeutung je Raum

Neu in `lib/dungeon-grid.js`: `PROP_CANDIDATES` (38 Kandidatennamen, 13 existieren), `PROP_ROLES`
(sechs Raumbedeutungen), Platzierung in `assemble()` und Setzen in `layout()`. Neu in der Seite:
Schieber **Requisiten** (0–60 %, Standard 30), Requisiten im Draufsicht-Plan, zwei neue Prüfzeilen.
Ausführlich: `docs/PROPS_S13_4.md`.

**Die Regel ist strukturell, nicht kosmetisch.** Eine Requisite sitzt in einer **Zelle**, nie auf
einer **Fuge** — der Wegegraph ist damit unberührt, nicht bloss geprüft. Der Rest sind gemessene
Abstände: Wandfläche (nicht Nenndicke) + 0,15, freie Mitte ≥ 0,30, Breite ≤ Modul − 1,2 (der
`wall_corner`-Schenkel belegt 0,5 um die Wandlinie).

**Drei Zahlen kamen erst aus den Prüfungen, nicht aus dem Entwurf.**
`keyring_hanging` verschwand mit dem Pivot auf der Wandmittellinie **ganz in** der 1,00 dicken
Platte (0,38 Durchdringung, seine volle Tiefe) — die S13.3-Regel „Asymmetrie ≈ 0 → Mittellinie"
ist für dicke Wände falsch, richtig ist Wandfläche + halbe Eigentiefe. · `shelves` auf dem Boden
und `shelves` an der Wand an derselben Wandseite überlappen um 0,44 — der Sperrschlüssel ist Fuge
**plus Seite**, gegenüber im anderen Raum ist dieselbe Wand frei. · Ein `table_small` mit 0,55
freier Mitte hat an Fuge 3.5,6 die **Tür** versperrt: die Strahlprobe tastet ±0,6 quer ab, also
braucht eine Zelle **mit** Durchgang 0,70 freie Mitte statt 0,30.

**Die Prüfungen sind schärfer geworden, nicht zahlreicher.** Requisiten liegen jetzt mit im
Türstrahl, sind Teil des Fussabdruck-Audits und werden in der Draufsicht wie Wand eingefärbt — eine
Kiste über einer Zellmitte fällt damit im Bild auf. Die Strahlzeile nennt seit S13.4 **das
versperrende Teil beim Namen**; genau das hat den 0,70-Wert gefunden. Neu als eigene Gates:
Zellmitte frei (gerendert) und Bodenkontakt ± 0,03.

**Bestand ist dünner als die Bedeutungen versprechen.** 13 von 38 Namen existieren im FREE-Tier;
`beinhaus` hat kein einziges eigenes Teil. Die Fehlliste steht als Prüfzeile in der Seite.
Alle Gates grün bei A1 und Q7, 8×8 und 16×16. Bloom weiter zurückgestellt.

---

## S13.3b · 2026-09-17 · Split-Screen-Schale als Referenz herausgelöst

`ref/ui-split-shell.html` — der Rahmen aus S13.2 (ein 38-px-Kopf, der scrollt statt umzubrechen;
Leiste zu als Normalfall; unter 1100 px legt sie sich über die Szene) als eigene, lauffähige Seite
zum Kopieren. Einbauanleitung für den Resident Atlas S6:
`docs/UI_SPLIT_EINBAU_RESIDENT_ATLAS.md`.
...
## 2026-09-19 · S20 · Sample-Atlas & Pack-Mapping (Reverse Engineering)
- Register gelesen statt Namen geraten: `registry/assets/v1/packs/kaykit-dungeon-pack-1-1-free-2.json`
  → **207 gltf-Modelle**, Namen gesichert in `ref/dungeon_pack_names.json`.
- 13 Promobilder als `ref/samples/s01..s13*.png` abgelegt und einzeln kartiert:
  Hülle · Boden · Requisiten nach Bedeutung · Licht · schaltbare Gruppen · Lücken.
  Daten: `lib/sample-map.js`, lesbare Fassung: `KayKit_Sample_Atlas_S20.html`.
- Drei Befunde für DG2-A: (1) rund zwanzig von 39 Namen in `PROP_CANDIDATES` treffen keinen Pfad
  und fallen still heraus; (2) `wall_endcap`/`wall_Tsplit`/`wall_crossing` existieren in 1.1,
  der Kommentar in `dungeon-grid.js` behauptet das Gegenteil; (3) `floor_tile_big_spikes` und
  drei Rost-Paare (zu/offen) decken die vom Brief geforderte Gefahren-/Fallenrolle ab.
- Fehlende Assets: `docs/PACK_GAPS_DUNGEON_1_1.md` — blockierend nur das Skeleton-Charakterpack
  (DG2-D), eine ganze Bühne fehlt für S11 (Mine) und S13 (Taverne, EXTRA-Stufe).
- Noch nicht gebaut: kein Raum nachgebaut, keine Messung geladen. Die Zuordnungen sind gelesen,
  nicht bestätigt — jede Zeile mit `unsicher` wartet auf den Ladeversuch.

## 2026-09-19 · S21 · Raumstudie: zwei Promoräume nachgebaut
- **Neu: `KayKit_Room_Study_S21.html` + `lib/room-recipes.js` + `lib/room-fx.js`.**
  R02 Schatz-/Esskammer (Bild S02) und R07 Grosse Halle (Bild S07), gebaut aus registrierten
  Teilen, Anker- und Drehregeln aus `dungeon-grid.js` übernommen. Kein zweiter Generator:
  Rezepte, keine Grundrisslogik.
- **Ecken werden abgeleitet, nicht aufgeschrieben.** Aus der Wandliste entsteht die Armtopologie
  je Gitterpunkt; zwei rechtwinklige Arme sind ein Eckteil, und daraus folgt die Länge:
  eine Fuge mit einer Ecke trägt `wall_half`, mit zwei Ecken nichts.
- **Vier Messungen haben das Rezept korrigiert, nicht der Geschmack:**
  `floor_tile_big_grate` misst 4,00 × 4,00 — trotz „big" **eine** Zelle (ein Block über vier
  Zellen liess drei offen) · `rubble_large` 8,13 × 3,18 und `rubble_half` 4,00 × 3,00 sind
  Felsrücken, kein Streugut — der feine Schutt der Promobilder kommt aus `floor_tile_large_rocks` ·
  Banner sind 3,20 hoch bei 4,00 Wandhöhe, also Mitte 0,55 statt 0,72 · `floor_tile_small_weeds_A`
  ist 2,00 und füllt keine Zelle.
- **Bug mit Ansage:** `buildScene` liest `s` als **Skalierung**. Die Gruppenzustände hiessen
  zuerst `s` — `setScalar('zu')` ergibt NaN, 20 Requisiten verschwanden spurlos bei sonst
  korrekter Position (Box3 leer, Kamera-Fit NaN, schwarzes Bild). Feld heisst jetzt `zs`.
- **Semantische Gruppen** über Schalter **und** Klick ins Objekt: Truhe zu ⇄ offen + Beute,
  Tafel gedeckt ⇄ abgeräumt, Hort voll ⇄ geplündert, Falltür zu ⇄ offen, Stachelplatte.
  Ton ist **synthetisch** (WebAudio, `lib/room-fx.js`) — kein KayKit-Pack enthält Audio.
  Effekt hängt am **Ereignis**, nicht am Dauerzustand: Funken, Glut, Schockwelle.
- **`lib/dungeon-grid.js` repariert (Freigabe Georg):** `PROP_CANDIDATES` gegen das Register
  getauscht — vorher fielen rund zwanzig Namen still heraus, jetzt meldet S13.2
  „alle Kandidaten existieren". Der Kommentar „das Pack hat kein wall_end, kein T, kein Kreuz"
  ist als **falsch für 1.1** markiert; `wall_endcap`, `wall_Tsplit`, `wall_crossing`,
  `ceiling_tile` und die modularen Treppen sind auf der S21-Werkbank geladen und vermessen.
  Gegenprobe S13.2 nach dem Eingriff: 0 freie Enden, 0 Durchdringungen, 5/5 lesbare Räume.

## 2026-09-19 · S21.2 · Nachbau statt Streuung — Post mortem und Neuaufbau R02
Georgs Befund am gebauten Raum: „konzeptlos zusammengebastelt … du hast NICHTS vom Set verstanden".
Berechtigt. Der Fehler ist dokumentiert, nicht still korrigiert — **Post mortem pm1–pm5 im
Sample-Atlas S20** (`lib/sample-map.js`, Abschnitt POSTMORTEM, additiv angehängt).

- **Modell zuerst:** `docs/MENTAL_MODEL_DIORAMA.md` — sieben Regeln, aus S02 gelesen und an
  S03/S05/S08/S12 gegengeprüft: Bühne statt Grundriss · zwei Wandzüge à zwei volle Module ·
  ein Ereignis je Wandmodul · drei Bodenzonen (Ecke = Ziel, Mitte = Leben, Rand = Echo) ·
  ein Schlüssellicht, keine Fackel · Deckungsprobe gegen die Vorlage · Abweichungen benannt.
- **R02 neu in Weltkoordinaten** statt Zellversatz — ein Nachbau ist eine Abmessung, keine
  Streuregel. Bodenplatte 3×3, Wände über die hinteren zwei Module je Seite (im Vorbild läuft
  der Boden vor den Wandenden weiter, die vordere Hälfte ist leer).
- **`wall_corner_small` gefunden und gesetzt.** Mit `wall_corner` (Schenkel 2,0) wird jedes
  Nachbarmodul zu `wall_half` — im Promobild ist kein halbes Teil. Die Eckenwahl entscheidet
  jetzt die Wandlänge, gemessen statt gesetzt.
- **Türblatt bleibt.** Die Generatorregel „Blatt entfernen" gilt für begehbare Fugen, nicht für
  ein Diorama; der Durchgang war ein schwarzes Loch. Auf/Zu ist jetzt eine Gruppe.
- **Seiten gespiegelt:** bei Kamera aus +x/+z zeigt +z nach rechts, also ist die W-Wand die
  bildlinke. Erste Fassung hatte beide Züge vertauscht; alle Requisiten mitgespiegelt
  (Diagonale x↔z, Drehung θ → 90−θ).
- **Zwei Bauschritte gegen die alten Fehler:** klemmen gegen die **gemessene** Wandfläche
  (Rechnung aus `assemble()`, vorher nicht übernommen) und auseinanderschieben bei Überlappung.
  Die Prüfung zählt jetzt **Requisite gegen Wand** mit — vorher war der Fehler unsichtbar.
- **Vorlage im Bild:** Knopf „Vorlage" → daneben / darüber (halbdurchsichtig) für die
  Deckungsprobe. Kamera aus dem Rezept (Richtung, Brennweite, Ziel), nicht automatisch gerahmt.
- **R07 zurückgezogen**, bis R02 abgenommen ist. Die Halle war der schlimmste Teil des Befunds
  (frei stehende Wand, Findlinge, Stachelplatte in der Ecke ohne Anlass).
- **S13 ist in fünf Aussagen überholt** (pm5): die dortige Namensprobe hat geratene Namen
  (`wall_end`, `wall_T`, `wall_cross`, `spikes`, `coin_stack` …) zu einer Aussage über das Pack
  gemacht. Ein Ladeversuch beantwortet nur, ob ein NAME existiert, nicht ob eine FUNKTION fehlt.

## 2026-09-20 · S21.3 · Nische richtig herum + Mini-Editor
- **`wall_window_closed_scaffold` statt zwei `shelves`**: die Vorlage zeigt eine Wandnische mit
  Holzrahmen — ein eigenes Bauteil. Zwei übereinandergesetzte Regale waren das
  Bastelteil-Antipattern (und die Streben landeten in der Ecke). Die Tür trägt jetzt denselben
  Rahmen (`wall_doorway_scaffold`).
- **Fensterbank gemessen** statt geschätzt: waagerechte Dreiecke im mittleren Drittel des Teils,
  höchste unter halber Wandhöhe → y 1,30. Steht als Zeile auf der Werkbank.
- **Teil war seitenverkehrt** (Georg am Bild): das Bauteil ist einseitig, Bretterrücken aussen,
  Einschub innen. Neues Rezeptfeld `drehen: 180` — die Seite folgt nicht aus dem Überstand,
  also gehört sie ins Rezept, nicht in eine Heuristik.
- **Türblatt war beim Teiletausch wieder verschwunden** (pm2, einen Umbau später): der Blattname
  stand als fester String im Code. Jetzt `<teil>_door` aus dem Teil abgeleitet.
- **Mini-Editor** (Georgs Vorschlag): Knopf „Editor" → Objekt oder ganze Bedeutungsgruppe
  anfassen, verschieben/drehen (TransformControls, Raster 0,1 / 15°), „Absetzen" legt die
  Unterkante auf den Boden. Der Editor schreibt **keine Datei**, sondern einen Rezept-Ausschnitt
  zum Kopieren — das Rezept bleibt die Quelle, nicht der Zufall der letzten Maus.
- **`lib/kit-lab.js`**: Grössen-Notnagel in der Renderschleife (Puffer gegen Layout), plus
  `onResize`-Haken. Betraf alle 20 Seiten — die Seite lud sonst blank mit 2×2-Puffer.

## 2026-09-20 · S21.4 · Grundfläche korrigiert · Truhendeckel · Editor-Schicht
- **Georgs Hypothese bestätigt und gemessen:** die Bodenplatte war zu klein. Mit drei Modulen je
  Wandzug (12 × 12 statt 8 × 8) fällt die Silhouette von 0,60 auf **0,576** gegen die Vorlage
  0,57, die Kamerahöhe löst sich auf 0,294 statt auf einen erzwungen flachen Winkel, und der
  Blickfang ist **96 %** sichtbar. Nebenbei verschwindet der Balken aus der Ecke (das Eckmodul
  ist jetzt eine glatte Wand) und die offene Truhe bekommt Platz. → Post mortem **pm7**.
- **Der Deckel ist ein eigenes Mesh** (`chest_gold_lid`, Scharnier im Pivot) — dieselbe Bauart
  wie das Türblatt. Aufklappen ist eine Drehung am Packteil, keine Eigengeometrie. → **pm6**,
  `docs/PACK_GAPS_DUNGEON_1_1.md` Abschnitt „Geprüft und KEINE Lücke".
- **Bericht war tot:** `deckel` wurde in `bauen()` gezählt und in `bericht()` gelesen — der
  ReferenceError riss Prüfblock und Rezept-Export mit. Als Parameter durchgereicht.
- **Herzschlag an zwei Quellen:** rAF UND Intervall (40 ms), gemeinsame Uhr. Diese Vorschau
  drosselt rAF auf null Bilder; vorher standen Punktlichter und Effekte still.
- **Editor-Schicht** (`docs/EDITOR_LAYER.md`): Mini-Menü **am Objekt** (verschieben, drehen,
  absetzen, Reichweite, schliessen) statt Wege in die Kopfzeile, Handarbeit überlebt den Reload
  (localStorage je Raum und Platzierungsindex), Ergebnis bleibt ein **Rezept-Ausschnitt** —
  der Editor schreibt keine Datei. Vertrag für E2 (Figuren posieren: sitzen, greifen, blicken
  über echte Clips) steht im Dokument.
