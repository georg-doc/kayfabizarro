...
date: 2026-09-19T19:40:00Z

### Updated in this project

- **S21 · zwei Promoräume nachgebaut** (`KayKit_Room_Study_S21.html`, `lib/room-recipes.js`,
  `lib/room-fx.js`): R02 Schatz-/Esskammer nach Bild S02, R07 Grosse Halle nach Bild S07 —
  aus registrierten Teilen, Anker- und Drehregeln aus `dungeon-grid.js`. Kein zweiter Generator.
- **Messungen, die das Rezept korrigiert haben:** `floor_tile_big_grate` 4,00 × 4,00 (eine Zelle,
  trotz „big“) · `rubble_large` 8,13 × 3,18 und `rubble_half` 4,00 × 3,00 sind Felsrücken, kein
  Streugut · Banner 3,20 hoch bei Wandhöhe 4,00 → Mitte 0,55 · `floor_tile_small_weeds_A` 2,00
  füllt keine Zelle.
- **Fallstrick für jede künftige Seite:** `buildScene` liest `s` als **Skalierung**. Ein
  Gruppenzustand in `s` ergibt `setScalar('zu')` → NaN; 20 Requisiten verschwanden bei korrekter
  Position (leere Box3, Kamera-Fit NaN). Feld heisst jetzt `zs`.
- **`lib/dungeon-grid.js` repariert:** `PROP_CANDIDATES` gegen das Register getauscht (vorher
  fielen ~20 Namen still heraus); S13.2 meldet jetzt „alle Kandidaten existieren“, Gates nach dem
  Eingriff grün (0 freie Enden, 0 Durchdringungen, 5/5 lesbare Räume). Der Kommentar
  „kein wall_end, kein T, kein Kreuz“ ist als falsch für 1.1 markiert.
- Ton ist **synthetisch** (WebAudio) — kein KayKit-Pack enthält Audio.

### Vorherige Synchronisation

date: 2026-09-19T18:58:48Z

### Updated in this project

- **S20 · Sample-Atlas.** `registry/assets/v1/packs/kaykit-dungeon-pack-1-1-free-2.json` gelesen
  (gelesen, nicht geraten): **207 gltf-Modelle**, Namen gesichert in `ref/dungeon_pack_names.json`.
  Die 13 Promobilder liegen als `ref/samples/s01..s13*.png` und sind je Bild kartiert —
  Hülle · Boden · Requisiten nach Bedeutung · Licht · schaltbare Gruppen · Lücken
  (`lib/sample-map.js`, lesbar in `KayKit_Sample_Atlas_S20.html`).
- **Korrektur an S13.2, aus dem Register belegt:** rund zwanzig der 39 Namen in
  `PROP_CANDIDATES` treffen keinen Pfad (`barrel`, `crate`, `bed`, `bookcase`, `chest_open`, …)
  und fallen still heraus. `wall_endcap`, `wall_Tsplit`, `wall_crossing` **existieren** in 1.1 —
  der Kommentar in `dungeon-grid.js` behauptet das Gegenteil. `floor_tile_big_spikes` und drei
  Rost-Paare (zu/offen) decken die vom Brief geforderte Gefahrenrolle ab.
- **Fehlende Assets:** `docs/PACK_GAPS_DUNGEON_1_1.md`. Blockierend ist nur das
  Skeleton-Charakterpack (DG2-D); eine ganze Bühne fehlt für S11 (Mine) und S13 (Taverne, EXTRA).
- Noch **nichts gebaut**: kein Raum nachgebaut, kein Modell geladen. Jede Zuordnung ist gelesen,
  nicht bestätigt.
...
- **S18.2 · Übergabe an WSA Lead.** Drei Dokumente: `docs/HANDOFF_WSA_S18.md` (Prüfvorlage:
  Vertrag, die vier tragenden Entscheidungen, Clean-Run in sechs Schritten, Zahlen mit ihrer
  Sonde, vier Risiken), `docs/BACKLOG_PLANT_PROP.md` (P0–P3, ungeschönt),
  `docs/SPRINT_19_CARTOON_DEFORMER.md` (Vertrag vor Code nach
  `skills/kfb-cartoon-animation_v2.md`, gelesen aus dem Repo).
- **Inhaltswünsche gegen den gemessenen Bestand geprüft.** Kakteen (`cactus_A…D`) und
  Sukkulenten (`succulent_A…D`) sind vermessen vorhanden und fehlen nur in der Grammatik.
  Bäume ebenso (Quaternius 2,08–5,88; KayKit Forest 3,5–10,8) — brauchen eine eigene
  Maßstabsklasse. Früchte und Hamburger-Bauteile liegen in den Restaurant Bits
  (`food_burger`, `food_ingredient_*`): ein Hamburger-Baum ist **Komposition, kein Asset**.
  **Fleischfresser, Pilze, Blumen, Süsswaren fehlen** in allen geprüften Packs (Forest Pack:
  105 Teile, kein Pilz) — als P2-Q mit Sondenauftrag notiert, nicht mit Ersatzgeometrie
  überspielt.
- **Export r2** `export/KFB_Plant_Prop_Lab_v1_EXPORT_2026-09-19/`, Revision `2026-09-19-r1`,
  25 Dateien, 252 KiB, weiterhin ohne eine kopierte Modelldatei.
- **GITHUB PUSH: BLOCKED (403).** Der Zugang ist lesend — Branch-Anlage *und* Schreiben auf
  `main` antworten mit `Resource not accessible by integration`. Der Commit liegt push-fertig
  in `export/github-checkin-2026-09-19/` (Repo-Baum `tools/plant-prop-lab/`, der Slot stand
  auf „NO RUNTIME YET"), mit `COMMIT_MESSAGE.txt` und `PUSH_ANLEITUNG.md`. Genau eine
  bestehende Datei würde überschrieben: `tools/plant-prop-lab/README.md`, Blob
  `2730de64b5e1420cdfb71f38b1ac4014d383d3de`. Für einen echten Push braucht die App
  `contents: write` auf `georg-doc/kayfabizarro`.

### Vorherige Synchronisation

date: 2026-09-18T17:40:00Z

### Updated in this project

- **S18 · Crazy-Cat Plant Prop Lab** (`KFB_Plant_Prop_Lab_S18.html` + sechs `lib/plant-*.js`):
  Pflanzen-Requisite als **Rezept** über unveränderten Quellen. Zwei Packs aufgenommen
  (`tt_plants`, `qu_env`), **72 von 72 Teilen** geladen und vermessen, 0 Fehlschläge. Dateiwahrheit
  wieder über die **Contents-API** — die Tree-API listet die `.gltf` dieser Packs nicht (wie S14).
- **Der Musterweg ist gemessen, nicht gewählt.** Korrelation der Topf-UVs mit dem Umfangswinkel:
  **r = 0,010 / 0,024** (zylindrisch abgewickelt wäre ≈ 1), u-Spanne 0,076–0,670 = Atlasfenster.
  Die Quaternius-Spender sind **entartet** (`Plant_1`: v 0,105…0,106, ein Texel). Also analytische
  Zylinderprojektion im Shader, UV-Canvas nur als verzogenes Vergleichsbild.
- **Die Einsetztiefe stand im Pack:** `monstera_plant_large_potted` 3,680 − `monstera_plant_large`
  2,880 = **0,800** Ursprung in einem 1,00 hohen Topf → 0,20 der Topfhöhe, als Anteil gespeichert.
- **Topf steht auf dem Innenboden des Untersetzers**, und der ist Rückseite: messbar nur mit für
  die Messung zweiseitig geschalteten Materialien. Topfkante radial abgetastet: `pot_C_large`
  0,625 gegen `pot_A_large` 0,687 bei Aussenbreite 1,50 / 1,498.
- **Kein zweites Augensystem:** `pet-eye-rig.v6.js` per jsDelivr geladen, dazu ein gemessener
  FaceHost. Der Wirt darf **nicht** `visible = false` sein (das Rig hängt in ihm) und braucht
  `castShadow = false`. **`PotRoot` trägt die Zufallsdrehung des Rezepts** — ein Gesicht darin
  blickte 137,5° vom Betrachter weg; der Wirt hängt jetzt in der unverdrehten Elternschaft.
  Mienen aus `kfb-pet-graft-driver.v4.json` gelesen. Laub ist keine Gesichtsfläche: `pot` liest
  am besten, `crown` nur mit kleinem vorgerücktem Wirt.
- **Material klonen ist Pflicht:** `instance()` klont die Szene, three teilt die Materialien —
  ohne `clone()` färbt ein Topfmuster jeden Topf jeder Szene, inklusive Cache.
- **Zwei Werkbank-Fehler behoben:** gleichzeitige Bauten (Klick während eines Baus → leere Bühne;
  jetzt gereiht mit `gen`-Zähler) und die fehlende **rAF-Notnagel**-Schleife (diese Vorschau parkt
  `requestAnimationFrame` auch sichtbar — Lehre aus S7.1/S11, hier wieder fällig).
- **Serie:** 10 Regenerationen über 10 Seeds, 1–5 Töpfe, alle fünf Maßstabsklassen — 0
  Fehlschläge, 0 fehlende Teile. Rezept-Rundlauf byte-identisch, Seed 5150 zweimal deckungsgleich.
...
date: 2026-09-18T02:05:00Z

### Updated in this project

- **Drei Packs aufgenommen: Space Base Bits, Restaurant Bits, Furniture Bits** (S14–S16). Die
  `.gltf`-Dateien listet die Tree-API wieder nicht — aber die **Contents-API** tut es
  (`tools/list-pack-files.html`). `lib/bits-inventory.js` hält damit das vollständige Inventar:
  **57 / 144 / 53 = 254 Teile**, alle geladen und vermessen, 0 Fehlschläge. Das korrigiert
  `docs/PACK_GAPS.md` §2: diese drei Packs sind im Repo.
- **Kompatibilität ist gemessen, nicht behauptet.** Dungeon und Restaurant teilen Zelle 4,00,
  Wandlänge 4,00 und Wandhöhe 4,00 — verschieden sind nur Wanddicke (0,50 / 1,00) und
  Boden-Oberkante (0,50 / 0,05). Space Base und City Builder liegen auf 2,00, und 4 = 2 × 2.
  Gleicher Maßstab quer über alle Packs, belegt an den Requisiten (chair_A 0,75 in zwei Packs).
- **S15 Space Base:** zwei Ebenen (mehr hat das Pack nicht), Plateaurand als Kantenlöser,
  Module auf einem Untergitter mit Schrittweite 2 (nur dort passt die 2,00-Röhre exakt),
  Diagonalröhren für anliegende (0,83) und zwei Zellen entfernte (3,66) Module. Röhren sind
  waagerecht — zwei Ebenen sind nicht verbindbar, und das wird nicht angeboten.
- **S16 Restaurant + Furniture:** Hülle als Fugengraph, darüber das Arbeitsband auf halbem Modul
  (zwei Plätze à 2,00 je Küchenfuge), Rückwand −z, Innenecke in Raumecken, Hängeschränke auf der
  Wandfläche. **Hüllen-Umschalter Restaurant ⇄ Dungeon** als Crossover-Probe: dieselbe Raumlogik,
  vier andere Messwerte, alle Gates grün.
- **Die Zelle ist der Ankerpunkt, nicht die Ausdehnung.** landingpad_large 2,50 · cargodepot_A
  2,07 · basemodule_D 2,44 gegen 2,00 Zelle. Mit gemessenen Fussabdrücken im Generator:
  Durchdringungen 51 → 0.
- **Bodenkontakt gemessen statt gerechnet** (Georg am Bild: schwebende Räder). spacetruck reicht
  0,08 unter seinen Pivot, roofmodule 0,10. Auflagefläche je Rolle + Strahl von oben; Dächer
  bleiben ausgenommen, ihr Sockel taucht konstruktiv ein.
- **Wandfläche ≠ halbe Nenndicke:** Dungeon-Wand 1,00 dick, gemessene Fläche 0,50, Relief 0,25.
  Anschlag ist die vorderste Fläche, und genau das Relief darf als Spalt stehen bleiben.
- **wall_doorway (Dungeon) bringt sein Türblatt mit** — entfernen, nicht verstecken: three.js
  raycastet auch unsichtbare Objekte (0 von 3 Türen begehbar bei sichtbar offener Tür).
...
| KayKit_Bits_Model_S14.html, lib/bits-inventory.js, tools/list-pack-files.html, tools/measure-bits-packs.html, tools/heightmap-bits.html, tools/probe-corners-compat.html, tools/probe-space-links.html | media/3D_Assets/KayKit_Space_Base_Bits_1.0_FREE/Assets/gltf/*.gltf (57), KayKit_Restaurant_Bits_1.0_FREE/Assets/gltf/*.gltf (144), KayKit_Furniture_Bits_1.0_FREE/Assets/gltf/*.gltf (53) — Inventar über die Contents-API gelesen, alle 254 vermessen |
| KayKit_Space_Base_S15.html, lib/space-grid.js | media/3D_Assets/KayKit_Space_Base_Bits_1.0_FREE/Assets/gltf/{terrain_*, basemodule_*, roofmodule_*, tunnel_*, landingpad_*, lander_*, cargodepot_*, drill_structure, structure_*, windturbine_*, solarpanel, lights, cargo_*, containers_*, rock*, spacetruck*}.gltf · sample.png + contents.png als Vorlage |
| KayKit_Restaurant_S16.html, lib/restaurant-grid.js | media/3D_Assets/KayKit_Restaurant_Bits_1.0_FREE/Assets/gltf/{wall*, floor_kitchen*, door_*, pillar_*, kitchencounter_*, kitchencabinet*, stove_*, oven, fridge_*, extractorhood, dishrack*, table_round_*, kitchentable_*, chair_*, crate*, pot_*, pan_*, bowl, plate, jar_*, ketchup, mustard, cuttingboard}.gltf · KayKit_Furniture_Bits_1.0_FREE/Assets/gltf/*.gltf (Möblierung) · KayKit_Dungeon_Pack_1.1_FREE 2 (Hülle im Crossover) · sample.png als Beleg für den Wandanschlag |
| KFB_Plant_Prop_Lab_S18.html, lib/plant-{inventory,recipe,pattern,rig,eyes,light}.js, tools/probe-plant-packs.html, tools/measure-plant-parts.html | media/3D_Assets/Tiny_Treats_House_Plants_1.0_FREE 2/Assets/gltf/*.gltf (46 vermessen: 12 Töpfe, 6 Untersetzer, 23 Pflanzen, 2 getopfte Zwillinge als Referenz, 3 Zubehör) · SciFI_Ultimate Space Kit_Quaternius/Environment/GLTF/*.gltf (26 vermessen) · tools/KFB-ToolBox/kfb-rigs-embed-v3/petstudio-v9/studio-v12/pet-eye-rig.v6.js + contracts/kfb-pet-graft-driver.v4.json (jsDelivr, nicht kopiert) · skills/chat/workflows/PLANT_PROP_LAB_2026-09-18/{FLOATE_DESIGN_BRIEF,MENTAL_MODEL}.md |
| KayKit_Room_Study_S21.html, lib/room-recipes.js, lib/room-fx.js | media/3D_Assets/KayKit_Dungeon_Pack_1.1_FREE 2/Assets/gltf/{floor_tile_large, floor_dirt_large, floor_tile_large_rocks, floor_tile_big_grate(_open), floor_tile_big_spikes, wall, wall_half, wall_cracked, wall_broken, wall_doorway, wall_gated, wall_shelves, wall_scaffold, wall_pillar, wall_corner, wall_endcap, wall_half_endcap, wall_Tsplit, wall_crossing, wall_sloped, ceiling_tile, stairs_modular_center, stairs_long, chest, chest_gold, trunk_large_B, coin*, table_*, chair, stool, plate_*, bottle_A_green, barrel_*, box_*, banner_yellow, banner_shield_red, keyring_hanging, candle_*, torch_mounted, rubble_half}.gltf · Vorlage: Samples/Dungeon_sample2.png + Dungeon_sample7.png |
| KayKit_Sample_Atlas_S20.html, lib/sample-map.js, ref/dungeon_pack_names.json, ref/samples/s01..s13*.png, docs/PACK_GAPS_DUNGEON_1_1.md | registry/assets/v1/packs/kaykit-dungeon-pack-1-1-free-2.json (207 gltf, Namensquelle) · media/3D_Assets/KayKit_Dungeon_Pack_1.1_FREE 2/Samples/*.png (13 Promobilder) · skills/chat/workflows/KFB_MODULAR_MINIGAME_HUB_2026-09-19/DUNGEON_GENERATOR_V2_BRIEF.md |
...
| docs/{HANDOFF_WSA_S18,BACKLOG_PLANT_PROP,SPRINT_19_CARTOON_DEFORMER}.md, export/github-checkin-2026-09-19/ | Zielslot `tools/plant-prop-lab/` (README-Blob `2730de64b5e1…`, Status „NO RUNTIME YET") · `skills/kfb-cartoon-animation_v2.md` als Vertragsgrundlage für S19 |
