...
Stand: 2026-09-19, nach S21 (Raumstudie R02/R07 + Generator-Reparatur; zuvor S20 Sample-Atlas, S19 Register-Grammatik).
...
| `KayKit_Room_Study_S21.html` | **AKTIV** | Raumstudie **R02** (Bild S02) als 1:1-Nachbau mit Vorlagen-Deckungsprobe. **R07 zurückgezogen** nach Georgs Befund — Post mortem pm1–pm5 im Atlas S20. Semantische Gruppen über Schalter **und** Klick ins Objekt, WebAudio-Ton, Funken/Glut/Schockwelle. Werkbank misst `wall_endcap`/`wall_Tsplit`/`wall_crossing`/`ceiling_tile`/modulare Treppen |
...
|---|---|---|
| `lib/room-recipes.js` | **AKTIV** | S21 — Raumrezepte + `buildRoom()`. Ecken werden aus der Wandliste abgeleitet, nicht aufgeschrieben. **Zustandsfeld heisst `zs`, nie `s`** (`buildScene` liest `s` als Skalierung → NaN) |
...
4. **R07** → Hort in der Mitte, Falltür zu ⇄ offen, Stachelplatte. Freie Wandenden: 2 (Stummel) —
   das ist gewollt und im Bericht benannt, dort gehört `wall_endcap` hin.
5. `KayKit_Dungeon_Generator_S13_2.html` nach der Reparatur: Zeile „Katalog · nicht im FREE-Tier"
