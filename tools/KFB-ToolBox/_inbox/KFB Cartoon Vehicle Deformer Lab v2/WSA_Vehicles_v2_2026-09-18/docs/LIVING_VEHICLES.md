...
```
Zuletzt passiert   18.09.2026 · sechste Runde: 15 fehlende Bodenfixtures aus Georgs neuem
                   Handoff aufgenommen (46 → 61), 10 Flug-Fixtures deklariert, Oberfläche v2 im
                   Schnitt des Resident Atlas S6. Details unten und in `CHANGELOG.md`.
                   VORHER   18.09.2026 · Session-Cut. Die dritte Runde (Georgs Sichtprüfung) ist
...
Aktuelle Oberfläche: `KFB Cartoon Vehicle Deformer Lab v2.dc.html` (Atlas-Schnitt, zwei Tabs).
Vorgängerin: `KFB Cartoon Vehicle Deformer Lab.dc.html` — unverändert liegen gelassen.
...
## Sechste Runde · 18.09. · fehlende Fahrzeuge, Atlas-Schnitt, Flug-Tab

Georgs Auftrag: die fehlenden Vehikel aufnehmen (Beispiel: das `car.glb` aus dem Driver-Paket),
die Oberfläche analog zum **Resident Atlas S6** umsetzen — »cleanere Views mit Fokus auf die
Models« —, und den Flight Deformer schon einmal planen.

**V18 · Eine Liste wird gegen ihre Quelle GERECHNET, nicht durchgesehen.** Der neue Handoff
(`consumer: animation-lab`, `29aac1061bdd`, 141 Assets) gegen `fixture-adapters.v2.js`: 112
Assets nicht referenziert. Ohne die Rechnung wäre das Driver-Auto einzeln nachgetragen worden und
die anderen neunzehn Fahrzeuge hätten weiter gefehlt. Die Rechnung ist der Befund, nicht das Auge.

**V19 · Die neue Liste hängt an, sie ersetzt nicht.** `fixture-adapters.v3.js` importiert v2 und
konkateniert. Grund: v2 trägt drei Runden gemessener Zahlen und abgelesener Orientierungen je
Zeile. Eine neu generierte Gesamtliste hätte sie stillschweigend überschrieben. Die
Speicherschlüssel für Flip und Orient bleiben dieselben — eine Korrektur, die Georg einmal gemacht
hat, bleibt gemacht. Die beiden Funktionen mussten dafür in v3 neu geschrieben werden: die
v2-Fassungen suchen über die v2-Liste und kennen die neuen Zeilen nicht.

**V20 · Ein Fixture gilt erst als aufgenommen, wenn es im Browser geladen und vermessen ist.**
Alle 25 neuen Zeilen geladen. Ergebnis in `CHANGELOG.md`; die Kurzfassung: vier Fahrzeuge finden
vier Räder, drei finden zwei, eines findet eins, der Rest null. **Die Nullen sind der Beweis, dass
die Paar-Regel arbeitet** — Traktor (kleine Vorder-, große Hinterräder), Schubkarre (ein Rad),
Rollstuhl (zwei große, zwei kleine). Neu offen: `rover-round` meldet **1** Rad, und eine Eins
verletzt die Paar-Regel. Das ist ein Verdacht auf eine Lücke im Insel-Weg, kein Messwert.

**V21 · Luft bekommt keine Bodenfamilie.** Zehn Flug-Fixtures, alle mit 0 Rädern — Schlingern,
Zwei-Rad und Manöver ankern ausnahmslos an Radmaßen. Der Flug-Tab hängt darum beim Laden keine
Familie an, statt sie mit Nullen laufen zu lassen. Plan: `PLAN_flight_deformer.md`.

**V22 · Spulen heißt neu rechnen.** Eine Sequenz mit Springs lässt sich nicht rückwärts abspielen;
der Regler startet sie von null und tickt in 1/60-Schritten zur Zielzeit. Damit ist ein Standbild
reproduzierbar (V8) und der Regler hält die Fixture wirklich an.

**Was am Atlas-Schnitt übernommen ist** (gelesen: `tools/resident_atlas_s6/KFB_Resident_Atlas_S6.html`):
Raster aus Bühne plus 380-px-Leiste, warm-dunkle Palette, QA-Marke oben links, Auswahlfelder
im Kopf statt Knopfreihen, Dock unten mit Zeitleiste und Stempel, einklappbare Leiste.
Nicht übernommen: Anfasser, Bone-Posing und der Patch-Export — die gehören zum Atlas-Auftrag,
nicht zu dieser Werkbank.
