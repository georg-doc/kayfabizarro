...
import { GENERATORS } from './props.js';
...
/* TOLERANTE ZUORDNUNG statt immer weiterer Suffix-Muster. Die Intake-Packs haben die
   starre Variante über Sonde 15 dreimal widerlegt, und jedes Mal war MEIN Muster das
   Problem, nicht die Datei:
     · `character_skeleton_archer_armLeft_broken` — das Suffix steht HINTER dem Teilnamen,
       also trifft `/ArmLeft$/i` nicht. Ich hatte daraus „die Broken-Fassungen haben keinen
       Teilebaum“ geschlossen. Falsch: sie haben einen, nur anders benannt.
     · `character_mageArnLeft` — KAYS TIPPFEHLER, „Arn“ statt „Arm“. Keine Konvention,
       ein Verschreiber, und deshalb mit einer Konvention nicht zu treffen.
     · `character_jack` / `character_witch` / `character_template553` — Rumpf ohne „Body“.
   Normalisiert wird deshalb erst (Suffix `_broken` weg, Kleinschreibung, Arn→Arm), und der
   Rumpf fällt auf den obersten character-Knoten zurück. Welche Regel gegriffen hat, steht
   im Ergebnis — eine tolerante Zuordnung, die nicht sagt WARUM sie passte, ist geraten. */
function legacyMatch(parts) {
  const norm = (s) => (s || '').toLowerCase().replace(/_broken$/, '').replace(/arn(left|right)$/, 'arm$1');
  const regeln = [
    ['armLeft', (s) => /arm_?left$/.test(s), 'Endung armLeft'],
    ['armRight', (s) => /arm_?right$/.test(s), 'Endung armRight'],
    ['Head', (s) => /_?head$/.test(s), 'Endung head'],
    ['Body', (s) => /_?body$/.test(s), 'Endung body']
  ];
  const found = {}, warum = {};
  const kandidaten = [];
  parts.traverse((o) => { if (o.name) kandidaten.push(o); });
  for (const [bone, test, label] of regeln) {
    for (const o of kandidaten) {
      if (found[bone]) break;
      if (test(norm(o.name))) { found[bone] = { node: o, world: o.matrixWorld.clone() }; warum[bone] = o.name + ' (' + label + ')'; }
    }
  }
  if (!found.Body) {
    const belegt = new Set(Object.values(found).map((f) => f.node));
    const o = kandidaten.find((x) => /^character/i.test(x.name) && !belegt.has(x) &&
      !/(hat|helmet|visor|hood|cowl|mask|jaw|eyes|hair|beard|horn|cape|cloak|backpack|quiver|belt)/i.test(x.name));
    if (o) { found.Body = { node: o, world: o.matrixWorld.clone() }; warum.Body = o.name + ' (Rückfall: oberster character-Knoten)'; }
  }
  return { found, warum };
}
/* ZUSATZTEILE — und der Grund, warum es sie braucht, ist ein DEFEKT, den der Orc Warband
   nie gezeigt hat. `legacyAssemble` hängt genau vier Teile um; alles andere im Teilebaum
   wird beim Umhängen zur Waise und verschwindet. Der Orc Warband hat genau vier Teile,
   deshalb fiel es seit S28 nicht auf. Gemessen an den Intake-Packs (Sonde 15): Barbarian
   2,049 → 1,759 posiert, weil `character_barbarianHat` wegfällt; Dungeon-Mage 2,437 →
   1,671; die Skelette bringen zusätzlich `_hood`, `_mask`, `_jaw`, `_eyes`, Jack einen
   `_Hat`. Eine Höhe, die beim Zusammensetzen um 30 % fällt, ist kein Posen-Effekt.
   Zusatzteile hängen deshalb an dem Bone, zu dem sie gehören, und was NICHT zugeordnet
   werden konnte, wird gemeldet statt still verloren. */
export const LEGACY_EXTRAS = [
  [/(Hat|Helmet|HelmetVisor|Visor|Hood|Cowl|Mask|Jaw|Eyes|Hair|Beard|Horn)(_broken)?$/i, 'Head'],
  [/(Cape|Cloak|Backpack|Quiver|Belt)(_broken)?$/i, 'Body']
];

export async function legacyAssemble(partsPath, rigPath = LEGACY_RIG, commit = PIN.legacy, rigCommit = null, partsMap = null) {
  /* ZWEI COMMITS, weil es zwei Packs sind. Der S5-Legacy-Pin kennt die 2026 eingecheckten
     Intake-Packs nicht (404 auf alle 14 Figuren), die Bewegungsquelle liegt aber nur dort.
     Vorher war der Pin für Rig UND Teile derselbe — das ging gut, solange beide im selben
     Revisionsstand lagen, und ist genau die Annahme, die der Intake bricht.
     `partsMap` überschreibt die Namensmuster je Pack: Spooktober nennt den Rumpf nach der
     Figur (`character_jack`), nicht `…Body`, und ein Suffix-Muster kann das nicht treffen. */
  const map = partsMap || LEGACY_PARTS;
  const root = await instance(rigPath, rigCommit || commit);  const parts = await instance(partsPath, commit);
  let skel = null;
  ...
  const m = partsMap ? (() => {
    const f = {}, w = {};
    parts.traverse((o) => {
      for (const [bn, rx] of Object.entries(map)) {
        if (!f[bn] && rx.test(o.name)) { f[bn] = { node: o, world: o.matrixWorld.clone() }; w[bn] = o.name + ' (Rezept-Muster)'; }
      }
    });
    return { found: f, warum: w };
  })() : legacyMatch(parts);
  const found = m.found;
  /* Zusatzteile sammeln: benannte Gruppen, die kein Hauptteil sind und auch nicht UNTER
     einem Hauptteil hängen (die fahren beim Umhängen mit). Vor dem Umhängen sammeln, weil
     danach die Elternkette eine andere ist. */
  const hauptteile = new Set(Object.values(found).map((f) => f.node));
  const unterHauptteil = (o) => { for (let p = o.parent; p; p = p.parent) if (hauptteile.has(p)) return true; return false; };
  const extras = [], extraOffen = [];
  parts.traverse((o) => {
    if (hauptteile.has(o) || unterHauptteil(o)) return;
    if (!o.name || !/^character/i.test(o.name)) return;
    if (!o.children.length && !o.isMesh) return;
    const hit = LEGACY_EXTRAS.find(([rx]) => rx.test(o.name));
    if (hit) extras.push({ node: o, world: o.matrixWorld.clone(), bone: hit[1], name: o.name });
    else extraOffen.push(o.name);
  });

  for (const boneName of ['Body', 'Head', 'armLeft', 'armRight']) {
  ...
  for (const e of extras) {
    const bone = skel && skel.bones.find((b) => b.name === e.bone);
    if (!bone) { extraOffen.push(e.name); continue; }
    const inv = skel.boneInverses[skel.bones.indexOf(bone)];
    bone.add(e.node);
    e.node.matrixAutoUpdate = false;
    e.node.matrix.copy(inv).multiply(e.world);
    e.node.matrixWorldNeedsUpdate = true;
    extraPlaced.push(`${e.name} → ${e.bone}`);
  }
  root.userData.legacy = { placed, missing, extraPlaced, extraOffen, warum: m.warum, rig: rigPath, parts: partsPath };
  return root;
}
...
/* ---------- placement helpers ---------- */
/* Weltlage eines SkinnedMesh-Vertex, von Hand gerechnet — und zwar aus einem Grund, der zwei
   falsche Korrekturen gekostet hat: `applyBoneTransform` liefert das Ergebnis in MESH-LOKALEN
   Koordinaten via `bindMatrixInverse`, und diese Matrix ist die Umkehrung der Weltmatrix ZUM
   BINDEZEITPUNKT. Verschiebt man den Figurenknoten danach, wandern die Bones mit (steckt in
   bone.matrixWorld), und ein anschließendes `applyMatrix4(mesh.matrixWorld)` addiert dieselbe
   Verschiebung ein zweites Mal. Gemessen: eine Korrektur von 0,087 landete 0,087 auf der
   anderen Seite, und eine Iteration über vier Runden änderte den Restfehler um null.
   Die Standard-Skinning-Formel hat das Problem nicht:
     world = Σ wᵢ · boneᵢ.matrixWorld · boneInverseᵢ · bindMatrix · v
   Sie hängt an den AKTUELLEN Bone-Weltmatrizen und an bindMatrix (konstant), an keiner
   veralteten Umkehrung. */
const _skM = new THREE.Matrix4(), _skV = new THREE.Vector3(), _skA = new THREE.Vector3();
...
/* Unterkante auf der POSIERTEN Geometrie. Box3 sieht die Skinning-Verformung nicht (S9), und
   bei flachen Clips ist der Unterschied nicht kosmetisch: die Kriechpose des Marksman liegt
   0,125 unter ihrer Bind-Pose-Box — nach Box3-Grounding steckt die Figur genau so tief im
   Boden. `applyBoneTransform` ist der einzige Weg, der die angewandte Pose sieht; für
   Nicht-Skinned-Knoten fällt die Funktion auf Box3 zurück, weil es dort dasselbe ist. */
function lowestY(node, step = 3) {
  ...
  return skinned && Number.isFinite(min) ? min : box(node).min.y;
}
...
/* ---------- ATLAS-EIGENE GEOMETRIE ----------
   Ein `gen`-Eintrag erzeugt seine Geometrie statt sie zu laden. Die Grenze ist bewusst
   markiert: kein Pack liefert ein Smartphone, und alles, was der Atlas selbst authored,
   muss im Recipe-JSON und im Panel als solches lesbar bleiben. Deshalb bekommt es einen
   eigenen Pfad-Namensraum (`atlas://generated/...`) und wird GENAUSO gemessen wie ein
   geladenes Asset — dieselbe Tabelle, dieselben Prüfungen, keine Sonderbehandlung. */
function generated(it) {
  const make = GENERATORS[it.gen];
  if (!make) throw new Error(`kein Generator „${it.gen}“ — vorhanden: ${Object.keys(GENERATORS).join(', ')}`);
  const node = make(it.genOpts || {});
  const b = new THREE.Box3().setFromObject(node);
  const size = b.getSize(new THREE.Vector3());
  measured.set(it.a, { name: node.name || it.gen, path: it.a, generated: true,
    size: size.toArray(), min: b.min.toArray(), max: b.max.toArray(), skin: false,
    facts: node.userData.generated });
  return node;
}
    ...
    try {
      ...
      if (it.pose) {
        ...
        if (hit) {
          ...
          /* boneTweak: eine gemessene Nachdrehung EINZELNER Bones auf der fertigen Pose.
             Additiv auf die lokale Quaternion, die der Clip gesetzt hat — also derselbe
             Ansatz wie `strumClip`, nur statisch. Gebraucht für Dinge, die kein Clip der
             Bibliothek hergibt: eine Kopfneigung zum Handy. Grad, nicht Radiant, und je
             Bone protokolliert; ein nicht gefundener Bone ist ein OPEN-Punkt, kein stilles
             Nichts. Vorbedingung ist `poseFreeze` — ein laufender Mixer würde die Drehung
             beim nächsten Update zurücksetzen, deshalb wird das geprüft und gemeldet. */
          if (it.boneTweak) {
            if (it.poseFreeze === false) {
              open.push(`boneTweak an ${it.id} bei laufendem Clip (poseFreeze:false) — der Mixer setzt die Drehung bei jedem Update zurück. Nicht angewandt.`);
            } else {
              const done = [], miss = [];
              for (const [bn, deg] of Object.entries(it.boneTweak)) {
                const f = findBone(node, bn);
                if (!f) { miss.push(bn); continue; }
                const e = new THREE.Euler(
                  THREE.MathUtils.degToRad(deg[0] || 0),
                  THREE.MathUtils.degToRad(deg[1] || 0),
                  THREE.MathUtils.degToRad(deg[2] || 0));
                f.bone.quaternion.multiply(new THREE.Quaternion().setFromEuler(e));
                done.push(`${f.matched} [${deg.join(', ')}]°`);
              }
              node.updateWorldMatrix(true, true);
              if (done.length) notes.push(`${it.id}: ${done.length} Bone-Nachdrehung(en) auf der fertigen Pose — ${done.join(' · ')}. Additiv auf die lokale Drehung des Clips, nicht statt ihr; die Bibliothek hat für diese Haltung keinen Clip.`);
              if (miss.length) open.push(`boneTweak an ${it.id}: Bone(s) ${miss.join(', ')} nicht gefunden — Nachdrehung nicht angewandt.`);
            }
          }
      ...
      /* paletteShift: eine Farbvariante, die KEINE zweite Datei ist. Gemessen am Marksman:
         sein 1024²-Körperatlas wird von allen Meshes nur in v 0,039…0,497 gesampelt — die
         obere Hälfte ist vollflächig belegt (65536/65536 abgetastete Pixel opak) und in
         sechs von sechs Stichproben ACHROMATISCH, wo die genutzte Hälfte farbig ist
         (107,102,67 gegen 210,210,209 usw.). Das ist die Schneevariante der Promo, im
         selben Blatt gestapelt. Ein Versatz von map.offset um [du, dv] holt sie hervor.

         Anders als `faceTile` ist das keine Kachel-Rechnung und nicht auf ein Material mit
         "face" im Namen beschränkt: der Versatz wird PRO MATERIALNAME angegeben, weil die
         Paletten nicht in allen Blättern eines Packs an derselben Stelle liegen — die
         Laubtextur (1024×336) hat in ihrem freien Viertel gar keine zweite Palette,
         sondern nur Transparenz (Anteil opak 0,00). Wer dort blind dasselbe dv anwendet,
         macht das Laub unsichtbar statt weiß.

         Material und Textur werden geklont, sonst wandert jede Instanz mit (faceTile-Lehre). */
      if (it.paletteShift) {
        ...
        node.traverse((o) => {
          ...
          mats.forEach((m, i) => {
            if (!m || !m.map) return;
            const shift = it.paletteShift[m.name];
            if (!shift) { missed.push(m.name); return; }
            const mm = m.clone();
            mm.map = m.map.clone();
            mm.map.offset.set(m.map.offset.x + shift[0], m.map.offset.y + shift[1]);
            mm.map.needsUpdate = true;
            if (Array.isArray(o.material)) o.material[i] = mm; else o.material = mm;
            applied.push(`${o.name}/${m.name} +[${shift.join(', ')}]`);
          });
        });
        if (applied.length) {
          notes.push(`${it.id}: Farbvariante über Atlas-Versatz statt zweiter Datei — ${applied.join(' · ')}. Die zweite Palette liegt im ungenutzten Bereich desselben Blatts; Versatz pro Materialname, weil er nicht in jedem Blatt gleich ist.`);
          if (missed.length) notes.push(`${it.id}: ohne Versatz geblieben (kein Eintrag in paletteShift): ${[...new Set(missed)].join(', ')} — bewusst, nicht vergessen: dort gibt es keine zweite Palette.`);
        } else open.push(`paletteShift an ${it.id} traf kein Material — Schlüssel [${Object.keys(it.paletteShift).join(', ')}] passen auf keinen Materialnamen.`);
      }
      ...
      /* Nachkorrektur für posierte Figuren: das Box-Grounding oben arbeitet auf der
         Bind-Pose. Wenn die posierte Unterkante daneben liegt, wird die Differenz
         nachgeschoben und BERICHTET — kein stiller Versatz. Schwelle 0,01, damit
         Rundungsrauschen keine Notiz produziert. */
      if (it.pose && !it.sitOn) {
        /* ITERATIV mit Selbstprüfung: die erste Fassung schob den gemessenen Versatz einmal
           und landete um genau denselben Betrag auf der anderen Seite (Figur 0,087 ÜBER dem
           Boden statt 0,087 darunter). Ursache nicht sauber isoliert — deshalb wird jetzt
           nachgemessen statt gerechnet, und der Restfehler steht in der Notiz. Eine
           Korrektur, die sich nicht selbst nachprüft, ist eine Behauptung. */
        const target = it.on ? topOf(nodes.get(it.on) || node) : (it.y || 0);
        const before = lowestY(node);
        let rest = before - target;
        for (let k = 0; k < 4 && Math.abs(rest) > 0.004; k++) {
          node.position.y -= rest;
          node.updateWorldMatrix(true, true);
          rest = lowestY(node) - target;
        }
        if (Math.abs(before - target) > 0.01) {
          notes.push(`${it.id}: posierte Unterkante lag ${(before - target).toFixed(3)} neben dem Boden — nachgesetzt, Restfehler ${rest.toFixed(3)}. Ursache ist Box3: sie sieht die Skinning-Verformung nicht, also gründet sie die Bind-Pose (S9). Bei flachen Clips ist das sichtbar, bei Standposen nicht.`);
        }
      }
      nodes.set(it.id, node);
    } catch (e) {
