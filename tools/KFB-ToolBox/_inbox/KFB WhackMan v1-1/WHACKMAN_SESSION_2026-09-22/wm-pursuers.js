...
import { clampToMaze, Bounce } from './wm-collide.js';
...
export class Pursuer {
  ...
  update(dt, world) {
    ...
    const clamped = clampToMaze(rawX, rawZ, this.graph, this.motor.node, MOD, this.MOD * 0.22, this.motor.to);
    /* Ausschlag aus Akteur-Akteur-Trennung (siehe wm-gate-c.js) ist ein additiver Versatz auf
       die schon legale Motor-Position — nie ein Ersatz dafür (kfb-cartoon-animation_v2 §8.3). */
    this.bounce.update(dt, { k: 140, damp: 15 });
    const withBounce = clampToMaze(clamped.x + this.bounce.x, clamped.z + this.bounce.z, this.graph, this.motor.node, MOD, this.MOD * 0.22, this.motor.to);
    const wx = withBounce.x, wz = withBounce.z;
    ...
    /* Heimkehr abgeschlossen? Bewusst länger im Pferch als ein frischer Start (1,2–3,6s): direkt
       nach dem Whack sofort wieder rauszulaufen erzeugt genau das Ineinanderlaufen, das Georg an
       den Keksen beschrieben hat — kein Ausweichen, weil es dafür keine eigene Kollisionsprüfung
       zwischen Akteuren gibt. Das ist hier nur eine Linderung (länger liegen bleiben), keine
       echte Lösung — Akteur-Akteur/Akteur-Sammelgut-Kollision bleibt offen für später. */
