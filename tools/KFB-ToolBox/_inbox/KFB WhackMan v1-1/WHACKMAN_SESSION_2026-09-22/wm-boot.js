...
import { MODE_COLOR } from './wm-pursuers.js';
...
/* Neben dem Titel statt oben rechts — rechts oben steht jetzt die Minimap. */
metaBtn.style.cssText = 'position:fixed;left:158px;top:14px;z-index:34;width:34px;height:34px;padding:0;' +
...
/* Erst NACH dem ersten Layout messen — beim Skriptlauf selbst hat der Titel oft noch keine
   Box (0 Breite), und der Knopf landete dann links auf dem Text statt daneben. */
function placeMetaBtn() {
  const t = document.querySelector('[data-wm-title]');
  if (!t) return;
  const r = t.getBoundingClientRect();
  if (r.width > 0) metaBtn.style.left = (r.right + 8) + 'px';
}
requestAnimationFrame(() => requestAnimationFrame(placeMetaBtn));

/* ---------- Minimap · Draufsicht auf den MazeGraph ----------
   Referenz butchler/Pacman-3D@gh-pages `renderHud`: eine zweite Kamera in eine kleine Ecke des
   Bildes. Hier als eigene 2D-Canvas statt eines zweiten WebGL-Scissor-Passes — der Viewer
   (kit-lab makeViewer) besitzt den einzigen Renderer und seinen eigenen rAF-Takt; ein zweiter
   Scissor-Pass müsste dort eingreifen. Der Graph kennt ohnehin schon jede Zelle, Spieler- und
   Verfolgerposition — eine flache Zeichnung daraus ist Millisekunden billiger. */
const mmCanvas = document.createElement('canvas');
mmCanvas.width = 128; mmCanvas.height = 128;
mmCanvas.style.cssText = 'position:fixed;right:14px;top:14px;z-index:30;width:128px;height:128px;' +
  'background:rgba(21,16,31,.88);border:1px solid ' + LINE + ';display:none';
document.body.appendChild(mmCanvas);
const mmCtx = mmCanvas.getContext('2d');
function drawMinimap() {
  if (!built || !built.graph || !built.pursuers) { mmCanvas.style.display = 'none'; return; }
  mmCanvas.style.display = 'block';
  const g = built.graph, W = mmCanvas.width, H = mmCanvas.height;
  const sx = W / g.w, sy = H / g.h;
  mmCtx.clearRect(0, 0, W, H);
  mmCtx.fillStyle = 'rgba(231,223,208,.42)';
  for (const n of g.nodes.values()) mmCtx.fillRect(n.x * sx, n.y * sy, Math.ceil(sx), Math.ceil(sy));
  const p = built.motor.pose();
  mmCtx.fillStyle = GOLD;
  mmCtx.beginPath(); mmCtx.arc((p.x + 0.5) * sx, (p.y + 0.5) * sy, Math.max(2, sx * 0.4), 0, 7); mmCtx.fill();
  if (built.pursuers) {
    for (const gh of built.pursuers) {
      const gx = gh.holder.position.x / built.MOD, gz = gh.holder.position.z / built.MOD;
      const hex = MODE_COLOR[gh.mode] ?? 0xffffff;
      mmCtx.fillStyle = '#' + hex.toString(16).padStart(6, '0');
      mmCtx.beginPath(); mmCtx.arc((gx + 0.5) * sx, (gz + 0.5) * sy, Math.max(2, sx * 0.32), 0, 7); mmCtx.fill();
    }
  }
}
...
const hudEl = panel('left:14px;bottom:14px;letter-spacing:.06em;pointer-events:none;display:none;opacity:.62');
const mkBtn = () => {
...
/* ---------- Fog of War · stufenloser Regler ----------
   Default 0 = reguläre Dämmerung, unverändert. Der Regler existiert schon jetzt für später
   feinere Abstimmung, auch wenn der Startwert bewusst "aus" ist. */
const nvRow = document.createElement('div');
nvRow.style.cssText = 'display:none;flex-direction:column;gap:3px';
const nvLabel = document.createElement('div');
nvLabel.textContent = 'Fog of War: aus';
nvLabel.style.cssText = 'font:' + MONO + ';color:' + INK + ';font-size:10px';
const nvInput = document.createElement('input');
nvInput.type = 'range'; nvInput.min = '0'; nvInput.max = '1'; nvInput.step = '0.01'; nvInput.value = '0';
nvInput.style.cssText = 'width:100%;accent-color:' + GOLD;
nvRow.appendChild(nvLabel);
nvRow.appendChild(nvInput);
grpSpiel.appendChild(nvRow);
nvInput.oninput = () => {
  const v = parseFloat(nvInput.value);
  built && built.setNightVision(v);
  nvLabel.textContent = 'Fog of War: ' + (v <= 0.01 ? 'aus' : Math.round(v * 100) + '%');
};
...
async function mount(which) {
  ...
  try {
    if (which === 'C') {
      ...
      nvRow.style.display = 'flex';
      nvInput.value = '0';
      nvLabel.textContent = 'Fog of War: aus';
      built.setNightVision(0);
      const setMode = (m) => {
        built.setMode(m);
