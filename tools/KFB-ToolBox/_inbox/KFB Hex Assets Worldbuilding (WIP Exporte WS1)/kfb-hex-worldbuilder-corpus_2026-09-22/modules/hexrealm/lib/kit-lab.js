...
/* Eine Atlas-Textur, nicht zwölf.
   Jede gltf verweist relativ auf ihre Textur, also löst `tiles/base/…` eine andere URL auf als
   `buildings/blue/…` — obwohl beide dieselbe Datei meinen. Gemessen: 12 Anfragen für 12-mal
   `hexagons_medieval.png`. Das ist der Kaltstart-Posten (12× Download + 12× Dekodieren) und
   zugleich die Ressource, deren gleichzeitiges Laden die Texturen zerlegte.
   Der URL-Modifier faltet sie auf die zuerst gesehene URL zusammen: Schlüssel ist Pack-Ordner +
   Dateiname, nicht der Dateiname allein — `colormap.png` heißt in mehreren Kenney-Kits gleich und
   zeigt auf verschiedene Bilder. Danach greifen Cache und Dekodierung genau einmal. */
export const textureAliases = new Map();   // 'pack|datei' -> kanonische URL
...
manager.setURLModifier((url) => {
  ...
  const key = m[1] + '|' + m[2];
  textureFolds.seen++;
  if (!textureAliases.has(key)) { textureAliases.set(key, url); textureFolds.unique++; }
  return textureAliases.get(key);
});
...
export function makeViewer(canvas, opts = {}) {
  ...
  /* Pro-Frame-Haken: der Hub braucht Mixer-Updates und Figurenbewegung VOR dem Render.
     Ein zweiter rAF-Loop daneben würde in unbestimmter Reihenfolge laufen. */
  const frameHooks = [];
  ...
  function draw() {
    ...
    lastFrame = now;
    for (const h of frameHooks) { try { h(dt); } catch (e) { console.warn('frame hook', e); } }
    controls.update();
