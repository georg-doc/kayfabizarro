...
function groundTexture(zone, col, stats, renderer) {
  const R = zone.rectW, W = R.maxX - R.minX, D = R.maxZ - R.minZ;
  const cw = 4096, ch = Math.round(cw * D / W), s = cw / W;
  ...
  function chains(list, width) {
    ...
    const sig = (r) => r.cls + '|' + width(r).toFixed(1);
...
function conformDetail(EG, S, d, yBase, P_, I_) {
  const { P, a, b, len } = S, dx = b.x - a.x, dz = b.z - a.z, NU = 4, NV = 6;
  ...
  const at = (uu, tt, depth) => {
    ...
    let nx = -ez / l, nz = ex / l; const cc = EG.deformElasticXZ(P.c, t, P);
    if (nx * ((da.x + db.x) / 2 - cc.x) + nz * ((da.z + db.z) / 2 - cc.z) < 0) { nx = -nx; nz = -nz; }
  ...
  for (const depth of [d.off + d.d / 2, d.off - d.d / 2 - 0.04]) {      // Front, Rückseite (in der Wand)
    for (let j = 0; j <= NV; j++) for (let i = 0; i <= NU; i++) P_.push(...at(d.u + (i / NU - .5) * du, d.t + (j / NV - .5) * dt, depth));
  }
...
export function buildCityLayer(zone, { mode = 'elastic', style, CC, EG, ghosts = true, renderer = null, extraBase = new Set() }) {
  const group = new THREE.Group(); group.name = 'city:' + mode;
  const Ly = layersFrom(style), clean = mode === 'clean', E = ELASTIC_PALETTE;
  const stats = { buildings: 0, base: 0, base2: 0, ghosts: 0, flatRoofRouted: 0, details: 0, roadParts: 0, tunnelsSkipped: 0, roadsByTier: {} };
  ...
  /* Gebäude */
  const lmFp = zone.landmark.footprint;
  ...
  const anchor = zone.landmark.centroid;                 // blockPull zur Landmarke (Viewer: Blockmittel) — Host-Setzung
  for (let b of zone.buildings) {
    ...
    const role = partOf(r, lmFp) ? 'base' : extraBase.has(b.id) ? 'base2' : (!clean && zone.conflicts.has(b.id)) ? 'ghost' : 'blocks';
    if (clean || role === 'ghost') {
      let g; try { g = new THREE.ExtrudeGeometry(new THREE.Shape(r.map((p) => new THREE.Vector2(p.x, -p.z))), { depth: h, steps: 1, bevelEnabled: false }); } catch (e) { continue; }
      ...
      let shell; try { shell = EG.buildElasticShell(src, anchor); } catch (e) { continue; }
      const sg = tint(shell.geometry, colorSlot(E.walls, b.id, 'wall'));
      ...
      if (role === 'base' || role === 'base2') { buckets[role].push(sg, rg); }
        ...
        const SP = detailSpecs(EG, CC, src, shell);
        for (const d of SP.list) { const map = details[d.kind === 'window' ? 'window' : 'door'], key = d.kind === 'window' ? wc : dc; if (!map.has(key)) map.set(key, { P: [], I: [] }); const acc = map.get(key); conformDetail(EG, SP, d, b.minH || 0, acc.P, acc.I); stats.details++; }
      }
    }
  ...
  out.base = add(buckets.base, mat(0.95), 'landmark-base:procedural (OSM parts inside ' + zone.landmark.id + ')');
