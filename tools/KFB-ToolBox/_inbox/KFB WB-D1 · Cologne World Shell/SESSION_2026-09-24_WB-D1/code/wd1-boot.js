...
import { buildCityLayer, layersFrom } from './wd1-city.js';
...
const URL = {
  ...
  fixture: './fixtures/cologne-dom-crop-v0.json'
};
...
/* ---------- Props (DC-Tweaks) ---------- */
const P = Object.assign({ view: 'SHELL', bend: 1, torsion: 1.2, chrome: 'QUIET', ghosts: false }, window.__wd1Props || {});
...
/* ---------- Zustand ---------- */
const S = { ready: false, view: P.view, drawer: P.chrome === 'FULL', ink: false, wind: false, ghosts: P.ghosts === true, tw: null, bend: +P.bend, torsion: +P.torsion };
...
async function boot() {
  ...
  Object.assign(rig.sun.shadow.camera, { left: -560, right: 560, top: 560, bottom: -560, near: 600, far: 2800 });
  rig.sun.shadow.bias = -0.0005; rig.sun.shadow.normalBias = 0.9;
  scene.add(rig.sun.target);
  log('TinySkies sky-presets.js · DAY · buildLightRig · paintRadialSky');

  /* Zone über die Seam */
  const zone = X.zone = await loadZone({ kind: 'frozen-fixture', url: URL.fixture });
  log('zone · ' + zone.id + ' · ' + zone.counts.buildings + ' buildings · ' + zone.counts.roadParts + ' road parts · ' + zone.counts.water + ' water · OSM ' + zone.provenance.osmBaseTimestamp);
  ...
  X.city = buildCityLayer(zone, { mode: 'elastic', style: cityStyle, CC, EG, FL, ghosts: S.ghosts, renderer, extraBase: new Set(hbfIds) });
...
function setView(v, instant) {
  ...
  if (v === 'FIXTURE' && !X.fixture) {
    ...
    log('01 fixture · clean extrusion (no massing) · ' + (X.fixture.stats.buildings + X.fixture.stats.base) + ' OSM footprints · landmark footprint outlined red');
  }
  const on = { FIXTURE: ['fixture'], LANDMARK: ['iso'], SHELL: ['terrain', 'city', 'edit', 'socket'] }[v];
  for (const k of ['terrain', 'city', 'edit', 'socket', 'iso', 'fixture']) if (G[k]) G[k].visible = on.includes(k);
  if (EDIT) {
    EDIT.setOn(v === 'SHELL');
...
function paint() {
  ...
  let g = group(barL);
  for (const [v, l] of [['FIXTURE', '01 FIXTURE'], ['LANDMARK', '02 LANDMARK'], ['SHELL', '03 SHELL']]) chip(g, l, S.view === v, () => setView(v));
  if (S.view === 'SHELL') {
    g = group(barL);
    for (const c of ['REGION', 'LANDMARK', 'STREET', 'TOP']) chip(g, c.toLowerCase(), S.cam === c, () => camTo(c));
    g = group(barL);
...
function frame() {
  ...
  if (S.ready) {
    X.water.update(t);
    X.dom.lm.update(t, S.wind ? 1 : 0); X.iso.el.update(t, S.wind ? 1 : 0); X.iso.donor.update(t, S.wind ? 1 : 0);
    ...
    X.sun.target.position.copy(focus); X.sun.position.copy(focus).addScaledVector(sd, 1600); X.sun.target.updateMatrixWorld();
