...
export class LullShow {
  ...
  /* `moving` = der Akteur fährt gerade. Eine laufende Nummer wird sofort abgebrochen, wenn
     jemand eine Taste drückt — eine Animation darf die Steuerung nicht geiseln. */
  update(dt, moving) {
    if (moving) {
    ...
    if (this.active) {
      this.active.t += dt;
      if (this.active.t >= this.active.dauer) this.stop();
      return this.active;
    }
    this.idleT += dt;
    if (this.idleT < this.wait) return null;
    this.idleT = -Math.random() * 2.2;          // Streuung, damit es kein Metronom wird
    const e = this.pick();
    const a = this.actions.get(e.name);
    if (!a) return null;
    a.reset();
    a.setEffectiveWeight(1);
    a.fadeIn(0.18).play();
    this.active = { ...e, action: a, t: 0 };
    return this.active;
  }
