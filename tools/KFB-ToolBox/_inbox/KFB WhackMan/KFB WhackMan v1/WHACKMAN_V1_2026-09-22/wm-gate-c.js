...
export async function buildGateC({ scene, camera, controls, onProgress = () => {}, label = () => {},
  ...
  const sparkMat = new THREE.PointsMaterial({ size: MOD * 0.14, transparent: true, opacity: 0, depthWrite: false });
  ...
  for (const [k, e] of byNode) {
    ...
    e.phase = (h / 997) * 6.283;
    Object.assign(e.g.userData, { st: 0, vx: 0, vy: 0, vz: 0, sq: 0, sqv: 0, spin: 0, tilt: 0, kalt: 0, weg: 0 });
  }
  ...
  const api = {
    root, report, graph, MOD, motor, state, pickups: B.pickups,
    ...
    update(dt, t) {
      ...
      player.position.set(wx, FLOOR_Y + jumpY, wz);
      ...
      if (state.view === 'freiflug') {
        ...
        const bx = Math.sin(camYawSmooth), bz = Math.cos(camYawSmooth);
        ...
        camWant.set(wx - bx * dist, CHASE.height, wz - bz * dist);
        camera.position.lerp(camWant, firstFrame ? 1 : Math.min(1, dt * CHASE.smooth));
        firstFrame = false;
        lookAt.set(wx + bx * CHASE.lookAhead, CHASE.lookY, wz + bz * CHASE.lookAhead);
        camera.lookAt(lookAt);
        controls.target.copy(lookAt);
      }
      ...
      /* ---------- Verfolger ---------- */
      const playing = state.phase === 'PLAYING' || state.phase === 'POWERED';
      for (const g of pursuers) {
        if (playing || state.phase === 'HIT') g.update(dt, world);
        if (!playing) continue;
      ...
      if (sparkT < 0.6) {
        ...
        sparkGeo.attributes.position.needsUpdate = true;
        sparkMat.opacity = Math.max(0, 1 - sparkT / 0.6);
      ...
      for (const [k2, e] of byNode) {
        ...
        if (d <= PLAYER_R + e.r) { here = k2; hit = e; break; }
      }
      ...
      for (const [, e] of byNode) {
        ...
        if (ud.st) {                                   // 1 = getreten, 2 = eingesammelt
          ...
          /* Ruhe: Puls an Ort und Stelle, und zurück an den Platz kriechen, falls ein Tritt
             das Stück versetzt hat. */
          const w = Math.sin(t * 2.1 - e.phase);
          e.g.position.y = e.baseY + w * MOD * 0.03;
          e.g.position.x += (e.baseX - e.g.position.x) * Math.min(1, pdt * 3.5);
          e.g.position.z += (e.baseZ - e.g.position.z) * Math.min(1, pdt * 3.5);
          e.g.rotation.z *= 1 - pdt * 4;
        }

        if (!e.g.visible) continue;

        /* Kontakt — nur wenn das Stück unten liegt und der Tritt nicht gerade erst war. */
        ud.kalt = Math.max(0, (ud.kalt || 0) - pdt);
        if (!ud.st && !ud.kalt) {
          for (const a of koepfe) {
            if (!a.jagt) continue;                     // der Spieler sammelt, er tritt nicht
            if (Math.hypot(a.x - e.g.position.x, a.z - e.g.position.z) > a.r + e.r) continue;
            werfen(e, a.x, a.z, MOD * 4.6, 1);
            break;
          }
        }
