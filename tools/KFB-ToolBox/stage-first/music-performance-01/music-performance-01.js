...
const state={ready:false,mode:'source',playing:false,loop:true,recipe:null,sourceMeta:null,root:null,mixer:null,actions:new Map(),audioOwnerCount:1,errors:[],lastBeat:null};
...
function setBeat(b){
  ...
  audio.currentTime=Math.max(0,timeForBeat(b)); syncActions(); renderTimeline();
}
...
function renderRuler(){
  ...
  for(let b=m.startBeat;b<m.finishBeat;b++){
    const x=document.createElement('button');x.className='beat-cell'+(b%r.timing.beatsPerBar===0?' downbeat':'');x.dataset.beat=String(b);x.innerHTML='<span>'+(b%r.timing.beatsPerBar+1)+'</span>';x.onclick=()=>setBeat(b);h.append(x);
  }
...
$('#sourceMode').onclick=()=>setMode('source');$('#performanceMode').onclick=()=>setMode('performance');$('#play').onclick=()=>togglePlay().catch(e=>fail('audio transport',e));$('#restart').onclick=()=>setBeat(0);$('#loop').onchange=e=>{state.loop=e.target.checked;renderTransport();};$('#scrub').oninput=e=>{audio.pause();setBeat(e.target.value);};$('#download').onclick=downloadRecipe;
