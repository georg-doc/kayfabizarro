 ...
 */
/**
 * pad-base.v2 — Fork von `studio-v9/pad-base.v1.js` für das FrizzleBob-Studio v13, EINE Änderung drin.
 *
 * GEORGS BEFUND (11.09.): ein schwarz umrandetes Rechteck liegt auf der Bühne, »das sollte mal eine
 * Karte sein« — es soll raus. GEMESSEN in seinem Fenster: `planeOn false` (die Platte ist unsichtbar,
 * Studio-Standard) und trotzdem `inkVis true`; `footR 0.25` und `scale 1` sind die ANFANGSWERTE, also
 * hat `update()` nie gerechnet — auch nicht nach einem Aufruf von Hand.
 *
 * Die v1 kennt den Fehler und benennt ihn wörtlich (»ein Rand ohne Fläche ist eine Behauptung«) —
 * ihre Abhilfe steht aber INNERHALB von `update()`. Eine Regel, die nur gilt, wenn die Rechnung läuft,
 * hilft genau dann nicht, wenn sie nicht läuft. Deshalb hier: **beide Flächen kommen unsichtbar zur
 * Welt.** Sichtbar macht sie ausschließlich `update()`, und das ist dieselbe Stelle, die auch ihre
 * Größe setzt — Sichtbarkeit und Maß haben damit EINEN Eigentümer.
 *
 * Wirkung anderswo: keine. In v9–v12 läuft `update()`, dort war der Anfangszustand nie zu sehen —
 * diese Fassung liegt trotzdem neben v13, damit die eingefrorenen Studios unberührt bleiben.
 */
export function createPadBase(o) {
  ...
  /* v2 · DIE EINE ÄNDERUNG: unsichtbar geboren. `update()` ist der einzige Weg auf die Bühne. */
