...
| Sphere / planet terrain approach (MIT) | `https://github.com/ZyFou/ProceduralTerrains` @`f58a8ddb81d1fbb526a41282a9a7e9c05c2d2070` — Planet mode is real (`planetBundle.js` + `PlanetWorld` cube-sphere, WSA-checked): engine split from UI, seeded parameters, chunk/LOD. Take the architecture and MIT code where it fits; not its React shell. |
| Sky, clouds, weather, day/night, moods | `https://cdn.jsdelivr.net/gh/georg-doc/kayfabizarro@main/travel/wip/travel_globe_wsa/globe-v13/` → `sky-presets.js`, `sky-atmosphere.js`, `day-night.js`, `weltstimmungen.js`, `rain-overlay.js`, `starfield.js`, `sun-shadow.js`, `light-budget.js`. Rule: world mood shifts **hue**, not saturation/lightness. Take only sky/weather/light — nothing else from Travel. These modules expect a host global `THREE` and relative `sky-presets.js`: wrap them explicitly. |
