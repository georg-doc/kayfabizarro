...
import { CollageEngine } from './collage-engine.js';
...
// B2b-P1 seam: one additional CanvasTexture owner fed by the copied Gate-1 collage donor.
let collageEngine = null;
let collageTimer = null;
let collageSeed = 18472;
let collageTreatment = 'LIVING_SCREEN';
let collageTickCount = 0;
...
const report = {
  ...
  collage: { seed: 18472, treatment: 'LIVING_SCREEN', running: false, ticks: 0, sequence: [], current: null },
  errors
};
window.__B2A_REPORT__ = report;
window.__B2B_REPORT__ = report;
...
function setDiag(extra = '') {
  ...
  el.textContent = [
    ...
    report.mode === 'collage' ? ('collage: seed ' + report.collage.seed + ' · ' + report.collage.treatment + ' · ' + (report.collage.current?.recipe || 'loading…')) : '',
    report.mode === 'collage' ? ('sequence: ' + (report.collage.sequence || []).slice(-4).map((x) => x.recipe + '/' + x.hero).join(' · ')) : '',
...
function updateTreatmentButton() {
  const b = $('b2b-treatment');
  if (!b) return;
  b.textContent = collageTreatment === 'LIVING_SCREEN' ? 'SCREEN: LIVING' : 'SCREEN: FLAT';
}

function stopCollage() {
  if (collageTimer) clearInterval(collageTimer);
  collageTimer = null;
  if (report.collage) report.collage.running = false;
}
...
function syncCollageReport() {
  ...
  report.collage = {
    ...
    running: !!collageTimer,
    ticks: collageTickCount
  };
}

async function startCollage(token) {
  ...
  collageEngine = new CollageEngine({
    manifestUrl: './collage-provenance.json',
    seed: collageSeed,
    treatment: collageTreatment
  });
  await withTimeout(collageEngine.load(), 15000, 'collage local asset pool did not load in 15s');
  if (token !== modeToken) return;
  collageEngine.setCard(cardResult);
  collageEngine.render();
  collageTickCount = 0;

  showCanvas(
    collageEngine.canvas,
    collageEngine.canvas.width / collageEngine.canvas.height,
    'COLLAGE · Gate-1 donor + 6 local CC0 stills · ' + collageTreatment
  );
  syncCollageReport();
  updateTreatmentButton();

  collageTimer = setInterval(() => {
    if (currentMode !== 'collage' || !collageEngine) return;
    collageEngine.update(1 / 12);
    collageTickCount++;
    if (tex) tex.needsUpdate = true;
    syncCollageReport();
    if (collageTickCount % 12 === 0) setDiag();
  }, 1000 / 12);
  report.collage.running = true;
}

function setTreatment(next) {
  if (!['FLAT_COLLAGE', 'LIVING_SCREEN'].includes(next)) return;
  collageTreatment = next;
  if (collageEngine) {
    collageEngine.setTreatment(next);
    collageEngine.render();
    if (tex) tex.needsUpdate = true;
    syncCollageReport();
  } else {
    report.collage.treatment = next;
  }
  updateTreatmentButton();
  setDiag();
}

async function setCollageSeed(seed) {
  const n = Number(seed);
  collageSeed = Number.isFinite(n) ? (n >>> 0) : 18472;
  report.collage.seed = collageSeed;
  if (currentMode === 'collage') await setMode('collage');
}

function setActive(mode) {
  document.querySelectorAll('[data-mode]').forEach((b) => b.classList.toggle('active', b.dataset.mode === mode));
  $('b2a-video-hint').style.display = mode === 'video' ? 'block' : 'none';
  const treatmentButton = $('b2b-treatment');
  if (treatmentButton) treatmentButton.style.display = mode === 'collage' ? 'inline-block' : 'none';
  updateTreatmentButton();
}
...
$('b2b-treatment')?.addEventListener('click', () => {
  setTreatment(collageTreatment === 'LIVING_SCREEN' ? 'FLAT_COLLAGE' : 'LIVING_SCREEN');
});
...
function snapshot() {
  ...
  return {
    ...
    dom: {
      ...
      collageTimerActive: !!collageTimer
    }
  };
}
...
window.__b2b = {
  ...
  previewSequence(seed = collageSeed, count = 8) {
    return collageEngine ? collageEngine.previewSequence(seed, count) : [];
  },
  collageSnapshot() { return collageEngine ? collageEngine.snapshot() : structuredClone(report.collage); }
};
