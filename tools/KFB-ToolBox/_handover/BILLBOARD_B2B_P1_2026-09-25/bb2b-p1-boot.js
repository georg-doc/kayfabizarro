// KFB Billboard / Media Residency · B2b-P1 COLLAGE · 2026-09-25
// Forks accepted B2a without changing CARD/COVER/VIDEO INLINE/SLOGAN or CSS3D rear culling.
// New seam: one deterministic CanvasTexture collage compositor derived from Gate-1 drawCollageFace()/BillboardContent.
...
import { createCollageEngine, loadCc0Pool } from './collage-engine.js';
...
let collageEngine = null;
let collageSeed = new URLSearchParams(location.search).get('seed') || 'kfb-b2b-p1';
let collageTreatment = (new URLSearchParams(location.search).get('treatment') || 'flat').toLowerCase() === 'living'
  ? 'LIVING_SCREEN' : 'FLAT_COLLAGE';
...
const report = {
  slice: 'BILLBOARD_B2B_P1_2026-09-25',
  ...
  b2b: {
    ...
    visualLanguage: 'mixed hypernormalisation',
    donor: 'Gate-1 drawCollageFace()/BillboardContent',
    seed: collageSeed,
...
function setDiag(extra = '') {
  ...
  el.textContent = [
    ...
    report.mode === 'collage'
      ? 'collage: ' + (report.b2b.recipe || '—') + ' · ' + (report.b2b.grade || '—') + ' · ' + report.b2b.treatment
      : '',
    report.mode === 'collage'
      ? 'seed/cut: ' + report.b2b.seed + ' / ' + report.b2b.compositionIndex + ' · hero ' + (report.b2b.hero || '—')
      : '',
...
function syncCollageReport() {
  if (!collageEngine) return;
  const s = collageEngine.snapshot();
  Object.assign(report.b2b, s);
  report.b2b.seed = collageSeed;
  report.b2b.treatment = collageTreatment;
}

function updateTreatmentUi() {
  document.querySelectorAll('[data-treatment]').forEach((b) => {
    b.classList.toggle('active', b.dataset.treatment === collageTreatment);
  });
}

async function ensureCollageEngine() {
  if (collageEngine) return collageEngine;
  const cc0 = await withTimeout(loadCc0Pool('./cc0-pool.json'), 15000, 'CC0 pool did not resolve in 15s');
  const cards = [];
  for (let i = 0; i < Math.min(3, CARD_POOL.length); i++) {
    const r = await withTimeout(renderCardQuarter(CARD_POOL[i]), 15000, 'KFB collage card did not resolve in 15s');
    cards.push({
      id: 'kfb:' + r.packId + ':' + r.cardNumber,
      kind: 'KFB',
      label: r.title + ' #' + r.cardNumber,
      image: r.canvas,
      tags: ['kfb', r.packId]
    });
  }
  collageEngine = createCollageEngine({
    canvas: cv,
    seed: collageSeed,
    treatment: collageTreatment,
    beats: BEATS,
    assets: cards.concat(cc0),
    onFrame(kind) {
      if (tex) tex.needsUpdate = true;
      syncCollageReport();
      if (kind === 'structural') setDiag();
    }
  });
  syncCollageReport();
...
function setCollageTreatment(treatment) {
  collageTreatment = treatment === 'LIVING_SCREEN' ? 'LIVING_SCREEN' : 'FLAT_COLLAGE';
  updateTreatmentUi();
  if (collageEngine) {
    collageEngine.setTreatment(collageTreatment);
    syncCollageReport();
    if (currentMode === 'collage') setDiag();
  }
}

function nextCollage() {
  if (!collageEngine || currentMode !== 'collage') return;
  collageEngine.next();
  syncCollageReport();
  setDiag();
}

function setCollageSeed(seed) {
  collageSeed = String(seed || 'kfb-b2b-p1');
  report.b2b.seed = collageSeed;
  if (collageEngine) {
    collageEngine.setSeed(collageSeed);
    syncCollageReport();
  }
  if (currentMode === 'collage') setDiag();
}
...
function setActive(mode) {
  ...
  const cc = $('b2b-collage-controls');
  if (cc) cc.style.display = mode === 'collage' ? 'flex' : 'none';
}

async function setMode(mode) {
  if (!['quarter', 'cover', 'video', 'slogan', 'collage'].includes(mode)) mode = 'quarter';
  ...
  try {
    if (mode === 'quarter') {
      ...
      const engine = await ensureCollageEngine();
      if (token !== modeToken) return;
      const ar = 16 / 9;
      ensureAspect(ar);
      resizeCanvas(ar);
      engine.attachCanvas(cv);
      engine.setSeed(collageSeed, false);
      engine.setTreatment(collageTreatment);
      engine.start();
      engine.render(true);
      setPanelTexture(cv);
      syncCollageReport();
      report.content = 'COLLAGE · ' + report.b2b.recipe + ' · ' + report.b2b.grade;
      report.ready = true;
      document.documentElement.dataset.b2aReady = '1';
      document.documentElement.dataset.b2bReady = '1';
      updateTreatmentUi();
      setDiag();
    }
  } catch (e) {
...
document.querySelectorAll('[data-treatment]').forEach((b) => b.addEventListener('click', () => {
  setCollageTreatment(b.dataset.treatment);
}));
$('b2b-next-cut')?.addEventListener('click', nextCollage);
...
function tick() {
  ...
  applyCamera();
  if (currentMode === 'collage' && collageEngine) collageEngine.advance(lastTick);
  syncInlineVideoTransform();
...
// B2a API remains intact above; B2b adds only collage-specific controls/evidence.
window.__b2b = {
  setMode,
  setView(name) { const p = CAMS[name]; if (p) { Object.assign(S, p); tick(); } },
  setTreatment: setCollageTreatment,
  nextCollage,
  setSeed: setCollageSeed,
  sequence(n = 12, seed = collageSeed) {
    if (!collageEngine) return [];
    return collageEngine.sequence(n, seed);
  },
  snapshot,
  report,
  VIDEO
