...
if (!id || !/^[a-z0-9-]+$/.test(id)) throw new Error('Usage: node fetch-city.mjs <city-id>');

const DATA = new URL(`../data/${id}/`, import.meta.url);
const spec = JSON.parse(await fs.readFile(new URL('SOURCE_SPEC.json', DATA), 'utf8'));
if (spec.id !== id) throw new Error(`SOURCE_SPEC id mismatch: expected ${id}, got ${spec.id}`);
  ...
  try {
    ...
    const res = await fetch(endpoint, {
      ...
      headers:{
        ...
        'user-agent':'KFB-OSM-City-Lab/0.2 (https://github.com/georg-doc/kayfabizarro)'
      },
      body,
...
await fs.writeFile(new URL('PROVENANCE.json', DATA), JSON.stringify(provenance,null,2)+'\n');
console.log(JSON.stringify({ok:true,id,endpoint:used,elements:raw.elements.length,sha256},null,2));
