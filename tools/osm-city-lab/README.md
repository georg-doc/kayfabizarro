...
**S2 IMPLEMENTATION / RECEIVER TESTED RESULT:** per-city consumer exports exist. The tested 11.384 km corridor source + normalized data + consumer scene are now on City Lab `main` via PR #86 / merge `3813d24e3db04a43117676890e48f7b6baaf6cd9`; post-merge consumer run `35415680112` and S2 export-contract run `35415680163` are PASS. Main refreshed only the generated consumer evidence timestamp at `25c2c616720e18d97028bf06fdd03d10c16b3504`; normalized and scene hashes stayed exact. The separate Race / Free-Roam receiver also passed: run `35414946641`, job `105821630552`, Hürth C1 26/26 plus continuous corridor 17/17, with a 6.807 m short drive, four C0 wheel contacts and no new recovery/run. This is still DRIVE-only and is not Travel Walk↔Drive acceptance.

**PUBLIC DEPLOYMENT · TESTED RESULT:** public S1 city navigator/viewer proof passed on `kayfabizarro.pages.dev`: workflow run `35390530619`, attempt 2, job `105748631023`, **27 checks / 2 cities PASS**. This proves the public S1 browser consumers, provenance, source counts, no separate roof caps, and no City movement owner.

**PUBLIC STUNT WORLD PACKAGE:** current main also contains the thin Stage mirror at `/kfb-hub/stage/stunt-world/` from commit `8607fb512726af81848b6545eda25ad451491f36`. Its 9 runtime files are source-identical to tested Race runtime `d2529e634952f25d691d68873801b2651125ac0c`; Race remains the implementation SSOT. Package status is still `PUBLIC_STAGE_PACKAGE_TESTED_SOURCE_PENDING_MIRROR`, so this is not yet a public/mobile acceptance claim.
