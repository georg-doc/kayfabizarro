...
export async function compileWorldZone({repoRoot=process.cwd(),cityId='dom-zentrum-v0',zoneId='cologne-dom-zentrum-v0',revision='2026-09-24.1',outRoot=null,sourceCommit=process.env.GITHUB_SHA||'UNPINNED'}={}){
  ...
  const locks=JSON.parse(await fs.readFile(path.join(lab,'src/world-zone/source-locks.json'),'utf8'));
  const lock=locks.datasets?.[cityId];
  if(!lock)throw new Error(`No World Zone source lock for ${cityId}`);
  const rawSha=sha256(rawBuf),normSha=sha256(normalizedBuf),querySha=sha256(queryBuf);
  if(rawSha!==lock.cachedFileSha256)throw new Error(`cached source file hash mismatch: ${rawSha} != ${lock.cachedFileSha256}`);
  if(prov.sourceSha256!==lock.reportedSourceSha256)throw new Error(`reported source hash mismatch: ${prov.sourceSha256} != ${lock.reportedSourceSha256}`);
  if(normSha!==lock.cachedNormalizedFileSha256)throw new Error(`cached normalized file hash mismatch: ${normSha} != ${lock.cachedNormalizedFileSha256}`);
