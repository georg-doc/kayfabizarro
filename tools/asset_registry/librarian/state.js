export const CANONICAL_REGISTRY_BASE = '../../../registry/assets/v1';
export const LIVE_REGISTRY_BASE = 'https://raw.githubusercontent.com/georg-doc/kayfabizarro/bot/asset-registry-update/registry/assets/v1';
...
function storedSelection() {
  try { const value = JSON.parse(localStorage.getItem(STORAGE_SELECTION) || '[]'); return Array.isArray(value) ? value.filter((v) => typeof v === 'string') : []; }
  catch { return []; }
}
...
export function setRegistryMode(mode){
  ...
  state.registryMode=next; state.registryBase=next === 'live' ? LIVE_REGISTRY_BASE : CANONICAL_REGISTRY_BASE;
  localStorage.setItem(STORAGE_REGISTRY_MODE,next); resetRegistryCaches();
}
export async function fetchJSON(url,optional=false){const response=await fetch(url,{cache:'no-store'});if(!response.ok){if(optional)return null;throw new Error(`${response.status} ${response.statusText}: ${url}`);}return response.json();}
export async function fetchJSONL(url){const response=await fetch(url,{cache:'no-store'});if(!response.ok)throw new Error(`${response.status} ${response.statusText}: ${url}`);const text=await response.text();return text.split(/\r?\n/).filter(Boolean).map((line)=>JSON.parse(line));}
export function option(value,text){const node=document.createElement('option');node.value=value;node.textContent=text;return node;}
export function badge(text,cls=''){const node=document.createElement('span');node.className=`badge ${cls}`.trim();node.textContent=text;return node;}
export function formatBytes(value){const bytes=Number(value||0);if(!Number.isFinite(bytes)||bytes<=0)return'0 B';const units=['B','KB','MB','GB'],i=Math.min(3,Math.floor(Math.log(bytes)/Math.log(1024))),number=bytes/(1024**i);return`${number>=10||i===0?number.toFixed(0):number.toFixed(1)} ${units[i]}`;}
export function formatDuration(seconds){if(!Number.isFinite(seconds))return'duration unknown';return`${Math.floor(seconds/60)}:${Math.round(seconds%60).toString().padStart(2,'0')}`;}
