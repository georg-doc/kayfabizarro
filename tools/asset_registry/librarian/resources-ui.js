...
async function ensureManifest() {
  if (!cache.manifest) cache.manifest = await json(`${RESOURCE_BASE}/manifest.json`);
  return cache.manifest;
}
async function ensureActors() {
  if (!cache.actors) cache.actors = await jsonl(`${RESOURCE_BASE}/actors.jsonl`);
  return cache.actors;
}
async function ensureRows(tab) {
  ...
  if (tab === 'rigs') {
    if (!cache.configs) cache.configs = await jsonl(`${RESOURCE_BASE}/configs.jsonl`);
    return cache.configs;
  }
  if (tab === 'motions') {
    if (!cache.motions) cache.motions = await jsonl(`${RESOURCE_BASE}/motions.jsonl`);
    return cache.motions;
  }
  if (tab === 'fx') {
    if (!cache.fx) cache.fx = await jsonl(`${RESOURCE_BASE}/fx.jsonl`);
    return cache.fx;
  }
  return [];
}
...
function badge(text, cls='') {
  const span=document.createElement('span'); span.className=`badge ${cls}`.trim(); span.textContent=text; return span;
}
...
function resourceSummary(row) {
  if (row.kind === 'actor') {
    ...
    return `${row.actorKind || 'actor'} · ${row.motionCount || 0} linked motions${asset ? ` · ${asset.format || 'asset'} body` : ''}`;
  }
  if (row.kind === 'motion') {
    return `${row.motionType || 'motion'}${row.animationSet ? ` · ${row.animationSet}` : ''}${row.trigger ? ` · trigger ${row.trigger}` : ''}`;
  }
...
function actionButton(label, handler, primary=true) {
  const button=document.createElement('button'); button.type='button'; button.className=primary?'primary':'quiet'; button.textContent=label; button.onclick=handler; return button;
}
function linkButton(label, href) {
  const a=document.createElement('a'); a.className='button-link quiet'; a.textContent=label; a.href=href; a.target='_blank'; a.rel='noreferrer'; return a;
}
...
async function showResource(row) {
  ...
  $('resourceDetailBadges').replaceChildren(badge(row.status || 'UNKNOWN', statusClass(row.status)), ...(row.actorRefs || []).map((id)=>badge(id)), row.scopeRef ? badge(row.scopeRef) : document.createDocumentFragment());
  $('resourceDetailJson').textContent = JSON.stringify(row, null, 2);
  const actions=$('resourcePrimaryAction'); actions.replaceChildren();

  if (row.kind === 'actor' && row.assets?.[0]?.exists && showAssetCallback) {
    actions.append(actionButton('Open body asset', () => showAssetCallback(row.assets[0].assetId)));
  }
  if (row.kind === 'motion' && row.motionType === 'embedded-clip' && row.sourceAssetId && showAssetCallback) {
    actions.append(actionButton(`Play ${row.clipName || 'clip'}`, () => showAssetCallback(row.sourceAssetId, row.clipName)));
  ...
  const raw=row.source?.rawPinned || row.source?.rawLatest;
  if (raw) actions.append(linkButton('Open source', raw));
}
...
function matchesScope(row, actor) {
  ...
  if (row.actorId === actor || row.compositionId === actor) return true;
  return false;
}
function filteredRows() {
  const q=$('resourceSearch').value.trim().toLowerCase();
  const actor=$('resourceActorFilter').value;
...
function renderResources() {
  const rows=filteredRows(); const list=$('resourceList'), tpl=$('resourceTemplate'); list.replaceChildren();
...
async function fillActorFilter() {
  const actors=await ensureActors(); const select=$('resourceActorFilter');
  const previous=select.value; select.replaceChildren(option('','All actors / scopes'), option('kfb-pets','KFB Pets · custom motion/FX scope'), ...actors.map((a)=>option(a.actorId,a.displayName || a.actorId))); select.value=previous;
}
...
export async function activateProductionTab(tab) {
  ...
  if (assets) return;
  try {
    const manifest=await ensureManifest(); $('resourceRevision').textContent=manifest.resourceRegistryRevision || 'R0'; await fillActorFilter(); currentRows=await ensureRows(tab);
    $('resourceEyebrow').textContent=tab; renderResources();
...
export function initProductionResources({ showAsset }) {
  ...
  $('resourceSearch').oninput=renderResources; $('resourceActorFilter').onchange=renderResources; $('resourceStatusFilter').onchange=renderResources;
  $('resourceDetailClose').onclick=closeResourceDetail;
