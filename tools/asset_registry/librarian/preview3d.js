...
export function playClip(index){if(!root||!clips[index])return;if(!mixer)mixer=new THREE.AnimationMixer(root);mixer.stopAllAction();mixer.clipAction(clips[index]).reset().play();$('previewStatus').textContent=`${clips.length} clip(s) · playing ${clips[index].name||`#${index+1}`}`;}
