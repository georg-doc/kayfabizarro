...
> **Zwei Bahnen — zuerst klären, welche gilt.** Dieses Blatt entstand für **Deck-in-Welt** (Kartenbilder
> im Gelände). Baust du ein **eigenständiges 3D-Spiel/App** (Voxel, Mech, Boxel Blitz), gilt die
> **Spiel-Bahn**: Referenz = der **Genre-Standard** des Spiels, **nicht** die Kartenbilder; **PBR und
> Game-Industry-Basics sind erlaubt und meist richtig; kein Ink-Outline, keine Tusche-Feder, keine
> Spielkarten-Motive**, außer Georg fordert sie an. KFB-Aroma ist opt-in NACH sauberen Basics. Die
> Zeilen unten mit „Kartenbilder als Referenz / PBR = Verstoß" gelten nur für die **Deck-Bahn**.

| Vorbild | Bei KFB |
|---|---|
| **Referenz: Cities Skylines II** | **Deck-Bahn:** Referenz sind die **Kartenbilder** — „gehört diese Welt in dasselbe Universum wie ihr Deck". **Spiel-Bahn:** Referenz ist der **Genre-Standard**; **PBR ist kein Kanon-Verstoß**, sondern oft richtig. Aroma opt-in nach den Basics |
...
| **P-08** | **Referenz je Bahn** | **Deck-in-Welt:** die eigenen Kartenbilder (Stimmigkeit, nicht Aufwand). **Eigenständiges 3D-Spiel:** der Genre-Standard, den es umsetzt — nicht die Kartenbilder |
...
Reihenfolge gemischt. Referenz nach Bahn: Deck-in-Welt gegen ein Deck-Kartenbild,
eigenständiges 3D-Spiel gegen den Genre-Standard. Keine Zehnerskala.

# Regeln

- Nutzen, was laeuft. Vorhandene Registry und Module werden importiert, nicht
  nachgebaut. Der Ink-Kanon gilt der Deck-Bahn; 3D-Spiele importieren ihn nur,
  wenn Georg das Aroma ausdruecklich will. Ein zweiter Nachbau ist ein Fehler.
- Keine Zahlenwerte im Bild (Deck-Bahn). Karten sind Beweißtuecke, keine Powers.
