...
- `workflows/KFB_COLOGNE_ROUTE_01_2026-09-25/START_HERE.md` — **CURRENT ROUTE TARGET · DOWNSTREAM OF TRACK CORE**: Hürth → Dom → Rhein → Mülheimer Brücke → SAE remains Route 01; M1–M4 are piece semantics on the Track Core, and Playable Track R0 / OSM composition start only after the Track-Core gate sequence
- `workflows/KFB_TRACK_CORE_SLICE_2026-09-26/START_HERE.md` — **CURRENT TRACK / FLEXIBLE-CONNECTOR GATE**: one base track, pieces as data; Perplexity research contributes true Clothoid/Euler easing, connector boundary state and staggered parameter transitions; agent order = Web census/contract → Georg language gate → Blender MCP proof → Web runtime parity → Claude Design visual grammar; current next gate TRACK-CORE-0
