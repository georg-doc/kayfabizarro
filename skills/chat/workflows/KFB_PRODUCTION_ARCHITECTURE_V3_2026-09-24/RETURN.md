...
## 2026-09-25 · Claude Design standard Session-Cut export skill

Status: **PERSISTED · ROUTED · 8/8 WIRING CHECK PASS**
...
Purpose:
- one standard /claude-export command for a full Claude Design Session Cut + ZIP;
- closed runnable current candidate codebase, not only changed files;
- Handover + active Changelog + Housekeeping + Source/Donor locks + Test Report + checksums + NEXT_CHAT;
- honest external/missing dependency inventory;
- voice-input ambiguity handling through VOICE_INPUT_UNCERTAIN instead of guessing names/paths;
- no redesign or owner replacement during export.

Relationship:
- skills/session-export_v1.md remains the slim session-delta export;
- the new Claude-specific skill is the complete handoff/recovery Session Cut;
- after two failed repair passes on the same gate, CLAUDE_DESIGN_FAILURE_RECOVERY_EXPORT.md remains the escalation path.

Discoverability updated:
- skills/chat/REGISTRY.json;
- skills/chat/START_HERE.md;
- skills/chat/adapters/claude-design.md;
- skills/session-export_v1.md.

Validation:
**8/8 PASS** for skill existence, trigger, runnable-code requirement, voice rule, generic-skill routing, Claude adapter routing, registry entry and router exposure.

No Claude Design ZIP was generated by this architecture checkpoint; this is skill/router validation only.

Exactly one next practical gate:
**use /claude-export on the next completed WorldBuilder or ToolBox/Animation Claude Design session and intake that real ZIP.**


## 2026-09-25 evening · Session ZIP proof + ToolBox/World intake + Ear Rig ownership

Status: **INTAKE CLASSIFIED · RECOVERY UPDATED · HUMAN GATES NEXT**

### Export workflow

Georg's current full Claude Design export entry is:
`skills/session_ZIP_v1.md` with trigger `/session-zip`.

The two current uploads prove the fallback contract is productive: both Session Cuts include source closure, Handover, active Changelog/Housekeeping, SOURCE, manifest, checksums, tests/evidence and NEXT_CHAT.

Recovery caveat:
the GitHub file is a short entry/fallback and reports the complete canonical skill + `zipcheck.py` at Claude project Skills / `claude/session-zip-SKILL.md`. That exact full body is not in this repo yet and should be mirrored when available.
...
Classification:
**KEEP · FUNCTIONAL CANDIDATE · HUMAN REVIEW PENDING.**

Reported candidate evidence:
- Studio + Animation Lab share one runtime/actor;
- direct IK points reuse edit-layer;
- Motion audition/scrub/roles/contact correction;
- Legacy builder;
- Actor/Pose/Motion persistence;
- Claude-preview self-test **13/13 PASS**.

No architecture claim upgrades this to Georg PASS.

Exactly one ToolBox gate:
...
Classification:
**KEEP · CORRECT INTEGRATION DIRECTION.**

The candidate mounts the accepted WB2 terrain/object/persistence owners into Hürth instead of creating another editor.

Reported evidence:
- World self-test **26/26 PASS** in Claude preview;
- WB2 sandbox boot PASS;
- WB2 34/34 regression NOT_RUN;
- Georg human world gate NOT_RUN.

Exactly one World gate:
**WEB REHOME 1:1 → 26/26 + WB2 34/34 → GEORG HUMAN REVIEW.**

### Ear Rig

PR #214 remains candidate-only.

Owner decision:
- **Animation Lab / ToolBox Motion** = single secondary-motion runtime owner;
- **FrankenStein Studio** = ear geometry/placement/rest+acted pose/profile authoring consumer;
- only one runtime copy: `tools/KFB-ToolBox/ear-rig/ear-dangle.v1.js`.

ToolBox manifest/README and central Registry record this split.

First gate:
**EAR-DANGLE-01** after ToolBox Production-01 review:
FrizzleBob + canonical KayKit Idle/Walk/Run/Jump Start-Air-Land + one accepted Motion Library dance.

No game runtime, Stage, merge or Live promotion is claimed.
