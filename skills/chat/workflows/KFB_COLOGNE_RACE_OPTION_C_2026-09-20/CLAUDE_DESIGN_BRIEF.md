...
# 7 · Dom/Zentrum OSM gate

Current repo OSM City Lab contains the tested Ehrenfeld↔Hürth corridor, but not the Dom/Zentrum pilot area.
...
The final playable Dom Loop requires a raw export at:

`source/osm/dom-zentrum.overpass.json`

The raw file must be:
- actual OSM / Overpass output;
- preserved unchanged;
- accompanied by endpoint, timestamp and query pin in `SOURCE.json`;
- attributed as ODbL data.

Before the export exists, Claude may work only on non-geographic donor isolation and Track/material prototypes.

If no export can be obtained:

`MISSING_OSM_EXPORT: DOM_ZENTRUM`

Do not label authored fallback blocks as OSM.
...
4. Obtain and save real Dom/Zentrum OSM export from checked-in Overpass query.
5. Isolate Dom donor.
