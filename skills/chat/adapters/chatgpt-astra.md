# Adapter · ChatGPT / Astra Work

Apply after `skills/chat/PRODUCTION_SOP.md`.

- Use the connected GitHub state directly when current status matters.
- Astra may manage branches, PRs, tests, merges and deployment when explicitly within the project contract.
- Request browser/local-server approval in the active Work chat when needed. Do not treat a policy failure in another session as a project failure.
- Keep the implementation SSOT in the named project repository. Publishing mirrors remain mirrors.
- When code conflicts with the lead plan, STOP with the concrete code evidence rather than inventing a broader architecture.
- Return compactly: what changed, what was tested, what remains open, exact branch/PR/commit/public URL.
