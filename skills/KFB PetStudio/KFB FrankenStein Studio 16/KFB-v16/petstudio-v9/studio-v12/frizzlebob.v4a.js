...
export default class FrizzleBob {
  ...
  /* ══ ZONEN ═══════════════════════════════════════════════════════════════════════════════════
     ⚠ DIE ZONEN GAB ES SCHON, SIE WAREN NUR STILL. Referenz: `KFB Combat Arena v5b`
     (`combat-arena-v1/frizzlebob.v1.js` + `combat-arena-v4/gun-look.v4a.js#matchBellyZone`) — dort
     teilen sich **Gesicht (Schnauze), Zier und Hände** einen Ton und der Körper trägt das FB-Gelb.
     Es braucht keinen Schnitt: der Spender liefert die Zonen als MATERIALNAMEN.
     GEMESSEN im Asset: `Main` (Körper) · `Main_Light` (Schnauze/Hände) · `Main2` (Zier).
     Warum das im Studio nicht zu sehen war: das Gelb kommt hier aus der gepatchten Datei, also ist
     `tintOn` aus — und mit ihm lag die Zonenlogik brach. Diese Methode wirkt UNABHÄNGIG davon.

     `sat` sättigt das Körpergelb (Georg 12.09.: »das FB-Gelb könnte noch etwas satter sein«) —
     Sättigung im HSL-Raum, Helligkeit bleibt, damit der Ton nicht ins Braune kippt. */
  applyZones(o = {}) {
    if (!this.figure) return null;
    const T = this.THREE, S = SPEC.tint;
    /* Die Zonen können auch AUSSERHALB von `figure` liegen — beim Graft sitzt der verpflanzte Kopf
       in einer eigenen Gruppe. Deshalb eine Liste von Wurzeln, nicht eine. */
    const roots = [].concat(o.roots || [], this.figure).filter(Boolean);
    const lightMix = o.lightMix != null ? o.lightMix : S.lightMix;
    const sat = o.sat != null ? o.sat : 1;
    const seen = new Set(), rows = [];
    let body = null, face = null;
    const hsl = {};
    const walk = (fn) => roots.forEach((r) => r.traverse((mesh) => {
      if (!mesh.isMesh && !mesh.isSkinnedMesh) return;
      [].concat(mesh.material).forEach((mat) => { if (mat && mat.color) fn(mat); });
    }));
    walk((mat) => {
      if (seen.has(mat.uuid)) return;
      seen.add(mat.uuid);
      if (mat.userData.kfbZoneOrig == null) mat.userData.kfbZoneOrig = mat.color.getHex();
      if (mat.name !== 'Main') return;
      mat.color.setHex(o.bodyTone != null ? o.bodyTone : mat.userData.kfbZoneOrig);
      if (sat !== 1) { mat.color.getHSL(hsl); mat.color.setHSL(hsl.h, Math.min(1, hsl.s * sat), hsl.l); }
      mat.needsUpdate = true;
      body = mat.color.clone();
      rows.push('Main → #' + mat.color.getHexString() + (sat !== 1 ? ' (sat ×' + sat.toFixed(2) + ')' : ''));
    });
    /* Zweiter Durchgang: das Gesicht braucht den fertigen Körperton als Bezug. */
    walk((mat) => {
      if (mat.name !== 'Main_Light') return;
      if (mat.userData.kfbZoneOrig == null) mat.userData.kfbZoneOrig = mat.color.getHex();
      if (o.faceTone != null) mat.color.setHex(o.faceTone);
      else if (body) mat.color.setRGB(body.r + (1 - body.r) * lightMix, body.g + (1 - body.g) * lightMix, body.b + (1 - body.b) * lightMix);
      else mat.color.setHex(mat.userData.kfbZoneOrig);
      face = mat.color.clone(); mat.needsUpdate = true;
      rows.push('Main_Light → #' + mat.color.getHexString());
    });
    /* Zier auf denselben Ton wie das Gesicht — genau das tut `matchBellyZone` in der Arena. */
    if (face) {
      seen.clear();
      walk((mat) => {
        if (mat.name !== 'Main2' || seen.has(mat.uuid)) return;
        seen.add(mat.uuid); mat.color.copy(face); mat.needsUpdate = true;
        rows.push('Main2 → #' + mat.color.getHexString() + ' (wie Gesicht)');
      });
    }
    this.zoneReport = { rows, body: body ? '#' + body.getHexString() : null, face: face ? '#' + face.getHexString() : null, lightMix, sat };
    this.log('Zonen · ' + rows.join(' · '));
