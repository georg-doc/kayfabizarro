...
export const DEFAULTS = Object.freeze({enabled:true, mask:.35, thickness:1, length:.6,
  ...
  /* ⚠ GEORG 12.09.: »die sind zu weit vorne vor dem Kopf«. `lift` WAR der Regler dafür, hieß aber
     »Float in front of the face« und stand auf **0,92 Augenradien** — fast einen ganzen Radius vor
     der Stirn. Vorgabe jetzt **0,30**, und die Spanne reicht ins Negative, damit die Braue auch in
     den Kopf sinken kann. Ein Pet mit eigenem gespeicherten Wert behält ihn. */
  bendLeft:0, bendRight:0, follow:.85, lift:.30,
  /* ⚠ GEORG: »die sind unterschiedlich groß« — und ja, das liegt an der Linienkonstruktion: die
     Federbreite kommt aus einer GESEEDETEN Tuschesimulation, die links und rechts verschieden
     ausfällt (handgezeichneter Eindruck, so gewollt). `symmetric` spiegelt die linke Hälfte auf die
     rechte — dann sind beide Brauen auf den Punkt gleich breit. */
  symmetric:true,
  /* v14 · Georg: »die Braue ist von der Seite kaum lesbar — geht das als 3D-Tube?« Ja, mit
     demselben Verlauf. `round` staucht den Querschnitt in der Tiefe (1 = Kreis, 0 = wieder flach).
     Vorgabe 1: 0,8 hätte 20 % der möglichen Tiefe verschenkt, ohne Grund — und die Tiefe IST hier
     der Zweck. Am Pixel gemessen (strenge Seitenansicht, dunkle Tinte gezählt): bei Federbreite
     2,42 flach 1065 → rund 1307 Bildpunkte, +23 %. */
  solid:true, round:1,
  /* ⚠ GEORG 12.09. AM BILD: die Tube erbt die UNREGELMÄSSIGE Tuschebreite der Feder — auf einer
     Fläche liest sich das organisch, auf einem Körper werden daraus eingeschnittene Ecken.
     `even` nimmt statt der Feder EINEN Radius (den Mittelwert der Feder über die ganze Kurve —
     gemessen, nicht gewählt, damit der Dicke-Regler weiter dasselbe bedeutet), `cap` rundet die
     Enden wie bei der Nase. Drei Stände: Strich · Tube (Feder) · Balken (gleichmäßig). */
  even:true, cap:0.8,
  color:'#17130f'});
...
export function validate(patch) {
  const limits = {mask:[0,1],thickness:[0,3],length:[0,1],taper:[0,1],height:[0,1],x:[-1,1],y:[-1,1],
    tiltLeft:[-1,1],tiltRight:[-1,1],bendLeft:[-1,1],bendRight:[-1,1],follow:[0,1],lift:[-1,3],round:[0,2],cap:[0,1]};
  for (const [k,v] of Object.entries(patch)) {
    ...
    if (k==='symmetric' && typeof v!=='boolean') return {status:'UNSUPPORTED',field:k,reason:'Expected boolean'};
    if (k==='solid' && typeof v!=='boolean') return {status:'UNSUPPORTED',field:k,reason:'Expected boolean'};
    if (k==='even' && typeof v!=='boolean') return {status:'UNSUPPORTED',field:k,reason:'Expected boolean'};
...
export class BrowRig {
  ...
  rebuild() {
    ...
    /* Spiegelung: die rechte Hälfte bekommt die Federbreite ihres linken Gegenübers. Eine Zeile —
       und die beiden Brauen sind messbar gleich breit. */
    const N=samples.length;
    const nibW = p.symmetric === false ? nib : (i => nib(i < N/2 ? i : N-1-i));
    /* Gleichmäßiger Balken: EIN Radius statt der Federkurve. Der Mittelwert der Feder — damit
       bleibt der Dicke-Regler derselbe Regler, nur die Unruhe fällt weg. */
    ...
    for(let i=0;i<samples.length;i++){
      ...
      /* Im Balken-Stand trägt die Hüllkurve nicht mit — sonst wäre er wieder ein Pinselstrich.
         Statt dessen runden die Enden über `cap` ab (Viertelkreis auf den letzten Abtastungen). */
      ...
      const half=widthAt(i)*width/512*capF*(p.even!==false?1:(1+.18*p.taper*Math.sin(Math.PI*t)));
      const [x,y]=geoAt(q);
      ...
      /* ⚠ VON DER SEITE WAR DIE BRAUE EIN STRICH OHNE KÖRPER (Georg 12.09.). Sie war ein BÄNDCHEN:
         zwei Punkte je Abtastung, also eine Fläche ohne Dicke — im Profil verschwindet sie.
         `solid` legt DENSELBEN Verlauf als Schlauch an: je Abtastung ein Ring aus `RAD` Punkten mit
         dem Radius, den die Tuschebreite ohnehin liefert. Gleiches Konzept, gleiche Kurve, gleiche
         Spiegelung — nur rund statt flach. `solid:false` gibt das Bändchen zurück. */
      if(p.solid===false){
        for(const sg of [-1,1]){positions.push(x-tangent.y*half*sg,y+tangent.x*half*sg,z);uv.push(t,sg*.5+.5);}
        if(i<samples.length-1){const a=i*2;indices.push(a,a+1,a+2,a+1,a+3,a+2);}
      }else{
        const RAD=6, r=Math.max(.0015,half);
        for(let k2=0;k2<RAD;k2++){
          const a2=k2/RAD*Math.PI*2, cs2=Math.cos(a2), sn2=Math.sin(a2);
          /* Querschnitt: in der Bildebene senkrecht zur Tangente, dazu die Tiefe (z). */
          positions.push(x-tangent.y*r*cs2, y+tangent.x*r*cs2, z+r*sn2*(p.round??1));
          uv.push(t,k2/RAD);
        }
        if(i<samples.length-1){const a=i*RAD,b2=(i+1)*RAD;
          for(let k2=0;k2<RAD;k2++){const n2=(k2+1)%RAD;indices.push(a+k2,b2+k2,a+n2, a+n2,b2+k2,b2+n2);}}
      }
    }
    ...
    /* Ein Schlauch braucht Tiefenschreiben, sonst zeichnet er sich selbst durcheinander; das
       Bändchen brauchte es nicht (flach, immer vorn). */
    if(this.material.depthWrite !== (p.solid!==false)){this.material.depthWrite=(p.solid!==false);this.material.needsUpdate=true;}
