...
export const DEFAULTS = Object.freeze({ enabled: true, size: 0.9, height: 0.55, depth: 0.10, x: 0, bulb: 0.85, droop: 0.45, gloss: 0.55, color: '#e96049',
  /* ⚠ GEORG 12.09.: »für die Nase die Möglichkeit, Breite und Höhe getrennt einzustellen«. Vorher
     war `size` EIN Faktor für alle drei Achsen. `wide` und `tall` liegen als Faktoren OBEN DRAUF,
     Vorgabe 1 — gespeicherte Pets sehen damit unverändert aus. */
  wide: 1, tall: 1 });
...
export function validate(patch) {
  const limits = { size: [0.2, 2.5], height: [-1, 3], depth: [-1, 1], x: [-1, 1], bulb: [0, 1], droop: [0, 1], gloss: [0, 1], radius: [0, 0.5], wide: [0.3, 3], tall: [0.3, 3] };
...
export class NoseRig {
  ...
  rebuild() {
    ...
    const w = p.size * 1.35 * R * (p.wide ?? 1), h = p.size * 1.15 * R * (p.tall ?? 1), d = p.size * 1.30 * R;
