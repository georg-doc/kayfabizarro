 ...
 */
/* ═══ v2 (Pet Studio v12, 09.09.2026) — EINE Änderung, additiv ═══════════════════════════════════
   Die Messkiste (Pflanzen UND Stempelgröße) überspringt Netze mit `userData.noMeasure` (Hausregel 7:
   wer eine Fläche ergänzt, aber nicht besitzt, trägt `noMeasure`). Anlass: FrizzleBob mit Waffe —
   die Waffe ragt 0,27 unter die Füße; `setFromObject(target)` pflanzte die MÜNDUNG auf den Boden und
   der Hase schwebte über seinem Stempel (gemessen vom Prüfer, 09.09.). Der Stempel selbst zeichnet die
   Waffe weiter mit — sie wirft ja Schatten; nur das MASS kommt vom Körper. v1 bleibt unverändert, weil
   Rolli und alle Cube-Pets daran hängen. */

export const version = 'gnd-v2.0';
...
export function createGround(o) {
  ...
  let last = { lift: 0, w: 1 };
  /* v2: Kiste ohne `noMeasure`-Netze. Ohne solche Netze exakt `setFromObject(target)`. */
  ...
  function measureBox() {
    ...
    target.traverse((o) => {
      ...
      let p = o, skip = false; while (p && p !== target) { if (p.userData && p.userData.noMeasure) { skip = true; break; } p = p.parent; }
      if (skip || !o.geometry) return;
      if (o.isSkinnedMesh) { o.computeBoundingBox(); if (o.boundingBox) box.union(_b.copy(o.boundingBox).applyMatrix4(o.matrixWorld)); }
      else { if (!o.geometry.boundingBox) o.geometry.computeBoundingBox(); box.union(_b.copy(o.geometry.boundingBox).applyMatrix4(o.matrixWorld)); }
    });
    return box;
  }
