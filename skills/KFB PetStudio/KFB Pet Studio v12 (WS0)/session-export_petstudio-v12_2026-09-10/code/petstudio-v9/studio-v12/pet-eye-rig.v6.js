/* pet-eye-rig.v6.js — FORK von studio-v3/pet-eye-rig.v5.js (Pet Studio v12, 09.09.2026). ZWEI Eingriffe, additiv:
   (v6a, 09.09. abends: der Splay-Vorschub ist jetzt ABGETASTET statt geschätzt — siehe Kommentar in build().)
   5) EYE FRAME — `eyeFrame()` ist der ÖFFENTLICHE Augen-Anker für Bauteile, die AM Auge hängen (Brauen, Nase):
      Lage beider Augen, Radius, Elternknoten und eine Bau-Nummer (`gen`), die bei jedem `build()` zählt.
      Vorher las Astras Brauen-Prototyp `rig.eyes`/`_R` privat — die WSA-Liste nennt den öffentlichen Anker als
      Vorbedingung. Kein Feld wird umbenannt, nichts Privates wird veröffentlicht.
   4) SPLAY — `splay` (0…1, Standard 0 = v5-Bild): beide Augen drehen SPIEGELGLEICH um die Hochachse nach
      außen und rücken entlang der neuen Blickrichtung ein Stück nach vorn, so dass sie auf einem RUNDEN Kopf
      (FrizzleBob, Ellipsoid-Host) außen liegen statt beide geradeaus zu starren. Georg 09.09.: »die beiden
      Augen gemeinsam nach außen drehen … erstmal über EINEN Regler«. Oben/unten später (Backlog). Die
      Abtastung (fitAt) bleibt von vorn; nur die Augengruppe dreht, Pupille und Lider hängen darin. Winkel =
      splay × 45°. v5 bleibt unangetastet (Ladeweg von v9/v10/v11, Combat Arena, SpinballCast). */
...
export class EyeRig {
  constructor(ch, opts = {}) {
    ...
    this.splay = opts.splay != null ? opts.splay : 0;   // v6
  ...
  build() {
    ...
    const mkEye = (sx) => {
      ...
      /* v6 SPLAY: Drehung um die Hochachse nach außen (links negativ, rechts positiv). Die Lage kommt aus einer
         ABTASTUNG entlang der GEDREHTEN Blickrichtung (v6a, 09.09.: der feste Vorschub 0,35 R / 0,6 R war eine
         Schätzung — gemessen am Gun-Hasen bei splay 0,5 lag das Auge bei z 0,50 auf einem Host mit Halbtiefe 0,78:
         Augen unter der Haut, nur die Braue ragte heraus). Jetzt gilt dieselbe Formel wie geradeaus — Treffer minus
         R·(0,24 + inset·1,15) — nur entlang der neuen Achse; ohne Treffer bleibt der alte Schätzwert als Rückweg. */
      if (this.splay) {
        const ang = sx * clamp(this.splay, 0, 1) * Math.PI / 4;
        e.rotation.y = ang;
        const dirL = new THREE.Vector3(Math.sin(ang), 0, Math.cos(ang));                 // Blickrichtung des gedrehten Auges (lokal)
        const oL = new THREE.Vector3(ex, ey, lc.z).addScaledVector(dirL, U * 3.5);       // weit außen auf der gedrehten Achse
        const oW = body.localToWorld(oL.clone());
        const dW = dirL.clone().negate().transformDirection(body.matrixWorld).normalize();
        ray.set(oW, dW);
        const hit = ray.intersectObject(body, false)[0];
        if (hit) {
          const hL = body.worldToLocal(hit.point.clone());
          e.position.copy(hL).addScaledVector(dirL, -R * (0.24 + this.inset * 1.15));
        } else {
          e.position.x += Math.sin(ang) * R * 0.35;
          e.position.z += (1 - Math.cos(ang)) * R * 0.6;
        }
      }
      e._uv = fit.uv;
    ...
    this._max = U * A.track;
    this.gen = (this.gen || 0) + 1;   // v6: jede Neubau-Runde zählt — Brauen/Nase binden sich daran neu
  }
  /* v6 · ÖFFENTLICHER ANKER. Koordinaten im System des ELTERN-Knotens (`parent`), also dort, wo Braue und Nase
     als Geschwister der Augen hängen. `null`, solange kein Rig steht — ein Bauteil meldet dann UNSUPPORTED. */
  eyeFrame() {
    if (!this.rig || !this.eyes || !this.eyes.length) return null;
    const L = this.eyes[0], R = this.eyes[1] || this.eyes[0];
    return { left: L.position.clone(), right: R.position.clone(), radius: this._R, parent: this.rig.parent, rig: this.rig, gen: this.gen || 0, unit: this._R / (this.anchor.ring || 0.25) };
  }
