...
## Stand 2026-09-10 · aktiver Scope: **Pet Studio v12 S7–S9** — Braue/Nase erweitert · Schnurrbart · Waffen-Mods · **SESSION-EXPORT**

| Artefakt | Status | Anmerkung |
|---|---|---|
| `petstudio-v9/KFB Pet Studio v12.dc.html` | **AKTIV** | Deliverable. Diese Session: Bart-Abschnitt im Face-Tab, Waffen-Wähler im Body-Tab, drei neue Bedienwege, Belichtung/Ladeweg unverändert |
| `petstudio-v9/studio-v12/brow-rig.v2.js` | **AKTIV · neu (S7)** | Fork von v1. Kopfrundung (13 Strahlen) · Dicke bis 3 (`hbAt`, bei 1 auf die Ziffer wie v1) · Biegung je Seite · echter Balken bei `taper` 0. Importiert `../kfb-ink-canon.js`. **GETEILT:** `pet-moustache.v1.js` importiert `hbAt` von hier |
| `petstudio-v9/studio-v12/brow-rig.v1.js` | **SUPERSEDED · Rückweg** | eine Zeile im Wirt (Z. 1042). Nicht löschen, solange v2 nicht abgenommen ist |
| `petstudio-v9/studio-v12/pet-nose.v2.js` | **AKTIV · neu (S7)** | Knolle als Superellipsoid (`bulb`, `droop`), 1,35 × 1,15 × 1,30 R. Nimmt v1-Feld `radius` an und übergeht es, damit gespeicherte Pets nicht abgewiesen werden |
| `petstudio-v9/studio-v12/pet-nose.v1.js` | **SUPERSEDED · Rückweg** | eine Zeile im Wirt (Z. 1044) |
| `petstudio-v9/studio-v12/pet-moustache.v1.js` | **AKTIV · neu (S8)** | Acht Formen, frei wählbar (Georg 10.09.). Anker `eyeFrame()` **plus** `nose.frame`. Importiert `../kfb-ink-canon.js` **und** `./brow-rig.v2.js` — bei einem Umzug müssen beide mit |
| `petstudio-v9/studio-v12/weapon-mods.v1.js` | **AKTIV · neu (S9)** | Kenney-Blaster statt Standardgewehr; misst `getObjectByName('Gun')` am Knochen `FistR`. Keine Importe. Lädt GLB per RAW-URL, je Sitzung einmal |
| `media/3D_Assets/kenney_blaster-kit_2.1/` (Repo) | **ASSET · extern** | 40 Vorschaubilder gelesen, GLB gemessen erreichbar (206). Nicht ins Projekt kopieren |
| `export/KFB Pet Studio v12 standalone.html` | **AKTIV** | 1,3 MB, am 10.09. neu gebündelt (S7/S8/S9 drin, im Bündel am Material geprüft). Anker zeigt noch auf die Projekt-Adresse |
| `export/src/KFB Pet Studio v12 standalone-src.dc.html` | **AKTIV · Quelle** | aus dem DC erzeugt, drei Eingriffe (Anker · keine lokale `@font-face` · Vorschaubild). Nie den compilierten Export bearbeiten |
| `docs/petstudio-v12/UPLOAD_v12_github.md` | **AKTIV · neu** | die zwölf Dateien für den Repo-Upload samt der drei relativen Fallen |
| `export/session-export_petstudio-v12_2026-09-10/` | **AKTIV · Lieferung** | Manifest-Export: 34 Code-Dateien (2,3 MB) + Standalone + Docs + 20 Captures. Keine Datei über 2 MB |
| `petstudio-v9/fonts/` (17 Dateien) | **ASSET · nicht im Export** | 5,2 MB, steht in KEINEM Verweis des Ladewegs; der Standalone holt die Schriften über Google Fonts (gemessen 0 `@font-face`) |
| `captures/v12s7` · `v12s8` · `v12s9` | **AKTIV · Belege** | 20 Bilder, 818 kB. Abnahme von S7/S8/S9 |

**Clean-Run-Checkliste v12 (10.09.):** Ladeweg maschinell aufgelöst — **25 von 25 Verweisen vorhanden**,
34 Dateien, 2,3 MB, keine doppelte Fassung im Lauf, kein toter Import. ⚠ Das Prüfwerkzeug meldete zuerst
vier Fehlende (`pet-LIBRARY.js`, `kfb-pets.js`, `bubble-shapes.js`, `kfb-pinball-sfx.js`) — es sind
**`.json`-Bänke**, mein Suchmuster hat den Namen mitten im Wort abgeschnitten. Alle vier liegen an ihrem Platz.

**Aufräum-Kandidaten (benannt, NICHT ausgeführt — Georgs Freigabe je Schritt):**

1. `petstudio-v9/studio-v12/frizzlebob.v1.js` · `v2*.js` · `v3*.js` — drei alte Fassungen, Ladeweg ist `v4a`. Seit 09.09. offen.
2. `brow-rig.v1.js` + `pet-nose.v1.js` — erst löschen, wenn Georg v2 abgenommen hat. **Empfehlung: behalten.**
3. `petstudio-v9/fonts/` (5,2 MB) — ins Repo, dann per URL. Nichts im Ladeweg hängt daran.
4. Doppelte EMBED-Kopien im Repo + veraltetes Embed-Bündel (seit 09.09. offen).
5. Verarbeitete Feedback-Bilder in `uploads/` — Hygiene-Regel des Fahrplans.

---

## Stand 2026-09-09 (abends) · aktiver Scope: **Pet Studio v12 S4–S6** — Braue · Body-Tab Biped · Nase · **CHECK-IN v12**

| Artefakt | Status | Anmerkung |
|---|---|---|
| `petstudio-v9/KFB Pet Studio v12.dc.html` | **AKTIV** | Deliverable. S4 Braue · S5 Body-Tab Zweibeiner (`V2CFG_BIPED`) · S6 Nase · S6b Entwürfe bereinigt. Ladeweg `studio-v12/` + geteilte `studio-v3…v10/` |
| `KFB Pet Studio v12.dc.html` (Wurzel) | **AKTIV · Starter** | nur Weiterleitung nach `petstudio-v9/` — damit v12 im Hauptmenü steht. Kein Ladeweg |
| `petstudio-v9/studio-v12/brow-rig.v1.js` | **AKTIV · neu** | Port von Astras `Studies/Acting-Lab-v0.2/brow-rig.js`; liest nur `rig.eyeFrame()`; importiert `../kfb-ink-canon.js` |
| `petstudio-v9/studio-v12/pet-nose.v1.js` | **AKTIV · neu** | Knubbelnase, KFB-Rot, Glanzregler, derselbe Anker |
| `petstudio-v9/studio-v12/pet-eye-rig.v6.js` | **AKTIV** | + `eyeFrame()`/`gen` (S3b) · Splay abgetastet (v6a). Import mit `?v6a` |
| `petstudio-v9/studio-v12/frizzlebob.v1.js` | **SUPERSEDED · Rückweg** | ⚠ oben zweimal gelistet (einmal AKTIV, einmal SUPERSEDED) — gilt: **Rückweg**, Ladeweg ist `v4a` |
| `petstudio-v9/kfb-ink-canon.js` | **AKTIV · GETEILT** | jetzt auch Ladeweg der Braue (neben Shaper/Bubbles). Nicht als tot einstufen |
| `docs/petstudio-v12/LIVING_petstudio_v12.md` | **AKTIV · SSOT** | S3b…S6b diese Sitzung |
| `docs/petstudio-v12/HANDOVER_v12_WSA.md` | **AKTIV · neu** | Einbau-Anleitung Hase, Sprintplan S7–S10, offene Fragen, Repo-Verweise mit Prüfstand |
| `docs/petstudio-v12/EMBED_CUBE_PET_FULL_v2.3_NACHTRAG.md` | **AKTIV · neu** | §17–§21 additiv zur Repo-v2.2; Georg lädt hoch (drei Kopien im Repo nachziehen) |
| `github.md` (Wurzel) | **AKTIV · neu** | Repo-Anker für den Skill-Abgleich |
| `captures/petstudio-v12/s4-brow-bunny.jpg` | **Abnahme-Capture** | einziges Bild dieser Sitzung auf Platte (übrige Messungen als Zahlen im LIVING) |
| `export/KFB Pet Studio v12 standalone.html` | **AKTIV · auslieferbar (mit Anker)** | **1,3 MB** (vorher 5,0: 17 lokale Schriften raus, Georg: »kann weg«). Quelle `export/src/…standalone-src.dc.html` + `support.js`. **Gemessen (S6d):** Bunny mit Papier (`Paper004` 1024², `uHasTex 1`) + Braue, Hase gelb (`yellow-asset`), Braue/Nase OK, Münder `male` aus dem Repo, 27 Pets, 6 Emotes. ⚠ Bausteine, Bibliothek und Vertrags-Rückweg laden über `window.__KFB_MODBASE` — heute die Projekt-Serve-URL, nach dem Repo-Upload jsdelivr (eine Zeile). Ohne Netz zur Basis: leere Bühne |
| `export/src/` | **AKTIV · Quelle des Standalone** | Kopie des DC ohne @font-face + Anker-Zeile; `support.js` daneben. Neu erzeugen, nie von Hand editieren |

### Aufräum-Kandidaten (benannt, NICHT ausgeführt — Freigabe je Zeile)

1. `petstudio-v9/studio-v12/frizzlebob.v1.js` — Rückweg der S1; seit S2 ungenutzt. **Empfehlung: behalten bis S9 (Export) grün, dann löschen.**
2. Doppelte Zeile `frizzlebob.v1.js` im Block vom 09.09. oben — **Empfehlung: die AKTIV-Zeile streichen** (Text-Fix, kein Datei-Löschen).
3. `uploads/WUNSCHLISTE_PetStudio_v12-v13_Arena.md` — verarbeitet in LIVING §7 und HANDOVER §5. **Empfehlung: behalten** (Quelle für S8/S10).
4. Im Repo (nicht hier): drei byteidentische `EMBED_CUBE_PET_FULL_v2.2.md`, zwei `kfb-pets.js`, altes `kfb-embed-bundle/` neben `v3`. **Empfehlung: Georg entscheidet beim Upload des Nachtrags.**
5. Schwer, gehört per URL: `petstudio-v9/assets/models/FrizzleBob_Yellow*.gltf` (1,28 MB zusammen) — **Upload-Kandidat ins Repo**, dann Modulpfad auf RAW.

### Clean-Run-Checkliste (v12, Ergänzung)

1. Hase laden → Konsole `[studio-v12] FrizzleBob gebaut · Kenney-Augen ausgeblendet 3 Netze`, kein Fehler.
2. Face-Tab: Braue `anchored at eyeFrame() · rig gen N`, Nase `skin sampled`. Splay ändern → `gen` zählt hoch, Braue/Nase folgen.
3. Body-Tab beim Hasen zeigt drei Abschnitte (Biped · Light · Ground), beim Bunny die alten sieben.
4. **Standbild nur nach 30× Hand-Tick** (Lider!). Messen am Modul, nicht über Regler-Handler (S6b).

---

## Stand 2026-09-09 · Re-Home aus WS1: **Pet Studio v11 + Combat Arena v1**

| Artefakt | Status | Anmerkung |
|---|---|---|
| `petstudio-v9/KFB Pet Studio v12.dc.html` | **AKTIV · neu** | Fork von v11, V12-S1: FrizzleBob als Bewohner. SSOT `docs/petstudio-v12/LIVING_petstudio_v12.md` |
| `petstudio-v9/studio-v12/pet-eye-rig.v6.js` | **AKTIV · Fork** | von `studio-v3/pet-eye-rig.v5.js`, ein Eingriff (`splay`). v5 bleibt Ladeweg aller anderen |
| `petstudio-v9/studio-v12/frizzlebob.v4a.js` · `gun-look.v4a.js` | **AKTIV · aus Astras 4A** | Ladeweg des Hasen in v12; zwei Pfadzeilen geändert. Farben aus gepatchten Modellen, Waffe Würfel-Palette |
| `petstudio-v9/assets/models/FrizzleBob_Yellow.gltf` · `_Gun.gltf` | **ASSET · aus 4A** | 608 + 676 kB, Puffer eingebettet. Körper #f2c83c, Gesicht/Hand/Fuß/Bauch #e6b671 |
| `petstudio-v9/studio-v12/frizzlebob.v1.js` | **SUPERSEDED · Rückweg** | Arena-v1-Kopie, eine Zeile im DC zurück |
| `petstudio-v9/studio-v12/ground-plane.v2.js` | **AKTIV · Fork** | von `studio-v7/ground-plane.v1.js`, eine Änderung: `noMeasure`-Netze fallen aus der Messkiste (Waffe). v1 bleibt Ladeweg von v9/v10/v11 |
| `petstudio-v9/studio-v12/frizzlebob.v1.js` | **AKTIV · Kopie** | byteidentisch zu `combat-arena-v1/frizzlebob.v1.js`; eigener Ladeweg für v12, Arena bleibt eingefroren |
| `petstudio-v9/KFB Pet Studio v11.dc.html` | **AKTIV · neu** | WS1-Fork von v10, zwei Zeilen Unterschied. Erbt den Ladeweg des Ordners (20 Module byteidentisch gemessen) |
| `petstudio-v9/studio-v3/kfb-pets.v1.2.6-verlust.json` | **BELEG · neu** | 33 kB, Datenverlust-Beleg aus WS1; wird nicht geladen |
| `KFB Combat Arena v1.dc.html` · `combat-arena-v1/` (8) | **AKTIV · neu** | Wurzel. Teilt `studio-v3/pet-eye-rig.v5.js` + `pet-mouth.v1.js` (byteidentisch), `podcast-v1/stage.v1.js`, `cardbuilder/`, `media/kfb/` |
| `docs/combat-arena-v1/` · `docs/petstudio-v11/` | **DOKU · neu** | LIVING + 3 Belegbilder (JPG, 3 Dateien) · Re-Home-Bericht, WS1-Dokumente, Brauen-Briefing, Monster-Look-Handover |
| `studio-v3/` (Wurzel) | **AKTIV · GETEILT** | ⚠ jetzt Ladeweg von Pet Studio v4 UND Combat Arena. Drei Dateien weichen von `petstudio-v9/studio-v3/` ab — **nicht angleichen ohne Messung** |
| `KFB Pet Studio v4.dc.html` u. a. ältere Studios/Editoren (Wurzel) | unverändert | |

Nichts überschrieben, nichts gelöscht. Staging-Ordner `_staging_ws1/` nach dem Vergleich entfernt.

---

## Stand 2026-09-08 · aktiver Scope: **KFB Boxel Blitz v5** — VFX/SFX, Blüten, Zeitlupe · **CHECK-IN v5**

| Artefakt | Status | Anmerkung |
|---|---|---|
| `KFB Boxel Blitz v5.dc.html` | **AKTIV** | Deliverable, 122 kB. three **0.160**. Diese Sitzung: stufenlose Zeitlupe (`_setLupe`, exponentiell), Mystery-Auswahl im Zahnrad, Ladeweg auf `flowers.v2`/`speedlines.v2`, Striche am Messgriff |
| `boxelblitz-v5/boxel.v4.js` | **AKTIV** | additiv erweitert: `sichtMasse` (ein Eigentümer der sichtbaren Masse) + `topAt(x,y)`. ⚠ `surfaceAt` bleibt die Fläche der PHYSIK — die zwei nicht verwechseln |
| `boxelblitz-v5/flowers.v2.js` | **AKTIV · neu** | Fork von v1, zwei Änderungen: Blüte reitet auf ihrer Zelle · Grenze ist das FELD, nicht das Blatt |
| `boxelblitz-v5/speedlines.v2.js` | **AKTIV · neu** | **Neubau**, nicht Fork. Vorbild `terrain-v13/speed-lines.js`. Strich im Raum samt Höhe, 3D-Achse, kamerazugewandtes Band, 1 Zeichenaufruf |
| `boxelblitz-v5/smoke.v3.js` | **AKTIV** | additiv: fünf `mystery0N`-Einträge + Liste `MYSTERY`. ⚠ **ohne `gluehen`** — mit Glut sind es weiße Klumpen (V5-S19b) |
| `boxelblitz-v5/dice.v5.js` · `faces.v4.js` · `audio.v2.js` · `bumpercap.v1.js` | **AKTIV** | unverändert aus früheren Scheiben dieser Reihe |
| `boxelblitz-v5/fx/mystery0N_5x6.png` (5) | **ASSET · in Prüfung** | 1,10 MB zusammen. Georgs Urteil steht aus; nach der Wahl bleibt **eine**, der Rest wird gelöscht |
| `boxelblitz-v5/fx/mystery/` (20 Rohdateien) | **⚠ AUFRÄUM-KANDIDAT** | **6,70 MB.** Die Herkunftsdateien (a/b/c-Farben). Nach der Farbwahl entbehrlich — **hier liegt das Gewicht, nicht im Blüten-Atlas** |
| `boxelblitz-v5/fx/blumen_10x7.png` | **ASSET** | 1,19 MB, 1280×896, 10 Formen × 7 Farbreihen. Georgs Verschlankungs-Idee ist gemessen: siehe `LIVING` §V5-S19c |
| `boxelblitz-v3/speedlines.v1.js` | **SUPERSEDED · Rückweg** | drei gemessene Ursachen im Kopf der v2. **Bleibt liegen** — die Begründung ist die Landkarte des Fehlers |
| `boxelblitz-v5/flowers.v1.js` | **SUPERSEDED · Rückweg** | Elter des Forks |
| `boxelblitz-v5/faces.v3.js` · `smoke.v1/v2.js` | **SUPERSEDED · Rückweg** | Eltern früherer Forks dieser Reihe |
| `boxelball-v1/*` · `boxelblitz-v2/*` · `boxelblitz-v3/*` · `boxelblitz-v4/decks.v1.js` | **AKTIV · GETEILT** | Ladeweg der eingefrorenen v1–v4 **und** der v5. **Nicht als tot einstufen** |
| `cardbuilder/kfb-card-builder.js` | **AKTIV · GETEILT** | ⚠ Podcast und SpinballCast lesen dieselbe Datei |
| `docs/boxelblitz-v5/LIVING_boxelblitz_v5.md` | **AKTIV · SSOT** | additiv, neueste Zeile oben. V5-S18 / S19 / S19b / S19c ist diese Sitzung |
| `docs/boxelblitz-v5/ONBOARDING_v6.md` | **AKTIV · neu** | Einstieg für einen frischen Chat: Ladeweg, die drei tragenden Zahlen, Clean-Run, offene Punkte in Reihenfolge, Fehlerklassen |
| `export/KFB Boxel Blitz v5 standalone.html` | **⚠ FIX-KANDIDAT · nicht auslieferbar** | 832 kB, gebaut. Kopfzeile und Bedienleiste rendern, **die 3D-Szene bleibt leer** (»wird geladen …«). Konsole: *SCRIPT failed to load*. **Dieselbe Wand wie beim v4-Export** — die ES-Modulkette mit Importkarte (~25 relative Modulimporte) übersteht die Einbettung nicht |
| `captures/stufe5/mystery_formen.png` · `mystery_mapeffects.png` · `mystery03_bewegt.png` | **Abnahme-Captures** | drei Bilder, keine Doppel |

### Clean-Run-Checkliste (v5)

Vollständig in `docs/boxelblitz-v5/ONBOARDING_v6.md` §4 (zehn Punkte). Die drei, die am häufigsten
Zeit kosten:

1. **`document.hidden` parkt die Bildschleife** — in dieser Reihe dreimal bezahlt. Ein Standbild
   misst dann das FENSTER, nicht den Bau.
2. **Ein Bild braucht einen Renderer, der gerade gezeichnet hat.** Zwei Aufnahmen 2,3 s auseinander
   waren bildpunktgleich — nicht »nichts passiert«, sondern »niemand hat gezeichnet«.
3. **Versionen prüfen** (`flowers` = `boxelblitz-flowers-v2.0`, `speedlines` =
   `boxelblitz-speedlines-v2.0`) — eine Versionsnummer ist ein Messwert.

### Aufräum-Kandidaten (nur benannt, **nichts ausgeführt** — Freigabe je Schritt)

| Kandidat | Größe | Empfehlung |
|---|---|---|
| `boxelblitz-v5/fx/mystery/` (20 Rohdateien) | **6,70 MB** | **Nach Georgs Formwahl löschen.** Bis dahin liegen bleiben — sie sind die Farbwahl (a/b/c). Rückholbar aus dem Repo |
| nicht gewählte `mystery0N_5x6.png` | je ~0,22 MB | mit derselben Freigabe |
| `export/…v5 standalone.html` | 832 kB | **erst reparieren, dann behalten** — aktuell nicht auslieferbar |
| Blüten-Atlas auf eine Reihe verschlanken | spart **~1,0 MB** | **nicht empfohlen**, siehe `LIVING` §V5-S19c — der Preis ist die Farbnuance, und drei der sieben Reihen taugen nicht als Vorlage |

### Pfad-Hygiene

Geprüft. Alle Modulimporte sind **relativ und projektlokal** (gewollt: sie sind der Ladeweg, nicht
Assets). Über URL laufen: three + cannon (Importkarte, unpkg), Schriften (Google), die
Kartenrückseite und die Tonaufnahmen (raw.githubusercontent). **Die `fx/`-Sprites liegen lokal** —
sie sind der einzige Kandidat für eine Umstellung auf RAW-URL, sobald sie im Repo liegen; solange
die Auswahl offen ist, wäre das verfrüht.
...
| `captures/boxelblitz-v4/` (21 Bilder) | **Abnahme-Captures** | ein Bild je Scheibe. Die 70 Doppel aus mehrstufigen Aufnahmen sind am 06.09. **nach Georgs Freigabe gelöscht** |
